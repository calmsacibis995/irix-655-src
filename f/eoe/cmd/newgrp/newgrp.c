...
char	PW[] = DEF_MSG(_MSG_NEWGRP_PW);
char	NG[] = DEF_MSG(_MSG_NEWGRP_NG);
char	PD[] = DEF_MSG(_MSG_NEWGRP_PD);
char	UG[] = DEF_MSG(_MSG_NEWGRP_UG); 
char	NS[] = DEF_MSG(_MSG_NEWGRP_NS);
	...
	if(argc > 1 && *argv[1] == '-'){
		switch(argv[1][1]) {
			...
			case '-':
				...
				break;
			default:
				error(GETTXT(_MSG_NEWGRP_BAD_OPT));
		}
	}
