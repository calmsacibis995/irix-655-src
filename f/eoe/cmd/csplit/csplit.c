...
char *strrchr();
char linbuf[LINE_MAX];		/* Input line buffer */
char *expbuf;
...
regex_t pReg;
int nRegMode;			/* Regular Expressions used, call regfree() before exit */

/*
*	These defines are needed for regexp handling (see regexp(7))
*/
#define PERROR(x)	fatal(MM_ERROR,PFMTTXT(_MSG_ILLEGAL_REGEXP),targ,NULL,UXDFM);
...
static off64_t findline(off64_t oset);
static FILE *getfile(void);

static	const	char	outofrangee[] = MSGNUM_DEFMSG(_MSG_OUT_OF_RANGE);
...
char **argv;
{
	...
	while((ch=getopt(argc,argv,"skf:n:")) != EOF) {
		switch(ch) {
			case 'f':
				if((int)strlen(ptr=optarg) > 
					(int)(sizeof(file) - 1))
				{
					fatal(MM_ERROR,PFMTTXT(_MSG_PREFIX_TOOLONG),ptr,NULL,UXDFM);
				}
				strcpy(file,optarg);
			...
			case 'n':
				...
				if(errno) {
					...
					_sgi_nl_error(SGINL_NOSYSERR,
						cmd_label,
				GETTXT(_MSG_MMX_xpg_csplit_number_invalid));
					setcat("uxdfm");
	...
	if(argc < 1 || errflg) {
		...
		_sgi_nl_usage(SGINL_USAGE, cmd_label, 
			GETTXT(_MSG_MMX_xpg_csplit_usage));
		setcat("uxdfm");
		fatal(MM_ACTION, "dummy", NULL, NULL, UXSGICORE);
	}
	if ((strlen(file) + inumber) > PATH_MAX) {
		setcat("uxsgicore");
		_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
			GETTXT(_MSG_MMX_xpg_csplit_filenametoolong));
		setcat("uxdfm");
		fatal(MM_ACTION, "dummy", NULL, NULL, UXSGICORE);
	}
	if(strcmp(*argv, "-") == 0) {
			...
			if(fwrite(tmpbuf, 1, n, infile) == 0)
				if(errno == ENOSPC) {
					pfmt(stderr, MM_ERROR,
						PFMTTXT(_MSG_NOSPACE_LEFT));
					if(nRegMode)
						regfree(&pReg);
					exit(1);
				}else{
					pfmt(stderr, MM_ERROR,
						PFMTTXT(_MSG_CANT_WRITE_TEMP),
						strerror (errno));
					if(nRegMode)
						regfree(&pReg);
					exit(1);
	...
	/*	clear the buffer to get correct size when writing buffer	*/
		...
		fatal(MM_ERROR,PFMTTXT(_MSG_CANNOT_OPEN),
...
*	Its arguments are the offset (oset).
*	The variable lncnt is used to count the number
...
findline(off64_t oset)
{
	...
	while(getline(FALSE) != NULL) {
		...
		if((sptr=strrchr(linbuf,'\n')) != NULL)
			*sptr = '\0';
		if(!regexec(&pReg, linbuf, (size_t) 0, NULL, 0)) {
...
static FILE *getfile(void)
{
	...
	if(create) {
		...
		if(ctr > MAXFLS)
			fatal(MM_ERROR,PFMTTXT(_MSG_FILE_LIMIT),targ, NULL, UXDFM);
		sprintf(fptr,tbuf,ctr++);
		if((opfil = fopen(file,"w")) == NULL) {
			strcpy(tfile,file);
			sprintf(fptr,tbuf,(ctr-2));
			fatal(MM_ERROR,PFMTTXT(_MSG_CANNOT_CREATE),tfile, strerror (errno), UXDFM);
		}
		return(opfil);
	}
	return(NULL);
...
*	The line is put into linbuf and may not be larger than LINE_MAX.
...
static char *getline(int bumpcur)
{
	...
	if(bumpcur)
		curline++;
	ret=fgets(linbuf,LINE_MAX,infile);
	return(ret);
}
...
line_arg(char *line)
{
	...
	if(my_atoll(line,&to) == ERR)
		fatal(MM_ERROR,PFMTTXT(_MSG_BAD_LINENUMBER),line, NULL, UXDFM);
	to_line(to);
}
...
num_arg(register char *arg,int md)
{
	...
	for(++arg; *arg != '}'; arg++) {
		if(ptr == &rep[11])
			fatal(MM_ERROR,PFMTTXT(_MSG_RPTCNT_TOOLARGE),targ, NULL, UXDFM);
		if(*arg == NULL)
			fatal(MM_ERROR,PFMTTXT(_MSG_MISSING_BRACKET),targ, NULL, UXDFM);
		*ptr++ = *arg;
	}
	*ptr = NULL;
	if((my_atol(rep,&repeat) == ERR) || repeat < 0L)
		fatal(MM_ERROR,PFMTTXT(_MSG_ILLEGAL_RPTCNT),targ, NULL, UXDFM);
	if(md == LINMODE) {
		...
		for(;repeat > 0L; repeat--) {
			...
			for(;repeat > 0L; repeat--)
				to_line(findline(offset));
		else
			fatal(MM_ERROR,PFMTTXT(_MSG_NO_OPERATION),targ, NULL, UXDFM);
}
...
re_arg(char *string)
{
	...
	register char ch;
	int errCode;
	int i = 1;/* i holds the return value of mblen() for invalid mbchar check*/
	...
	while((*(ptr+=i)) != ch) {
		if((*ptr == '\\') && (*(ptr+1) == '/'))
			++ptr;
		if(*ptr == NULL)
			fatal(MM_ERROR,PFMTTXT(_MSG_MISSING_DELIM),targ, NULL, UXDFM);
		i = mblen(ptr, MB_CUR_MAX);
		if (i == -1)
			PERROR(i);
	}
	...
	*ptr = NULL;
	if(my_atoll(++ptr,&offset) == ERR)
		fatal(MM_ERROR,PFMTTXT(_MSG_ILLEGAL_OFFSET),string, NULL, UXDFM);

	string++;
	errCode = regcomp(&pReg, string, REG_NOSUB);
	if (errCode)
		PERROR(errCode);
	to_line(findline(offset));
}
...
{
	signal(SIGINT,sig);
	fatal(MM_ERROR,PFMTTXT(_MSG_INTERRUPT_ABORT),targ, NULL, UXDFM);
}
