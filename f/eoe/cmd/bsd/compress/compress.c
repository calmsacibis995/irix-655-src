...
#		define min(a,b) ((a>b) ? b : a)
	...
	*plen = r;
    ...
    fprintf (stderr, GETTXT(_MSG_MMX_compress_ReadFail), fd, readbuf, sizereadbuf); 
    perror (filename);
    return -1;
}
...
void compress_filedesc (int ifd, int ofd, off64_t fsize, const char *filename)
{
	...
	case 0:
	    ...
	    break;
	default:
	    fprintf (stderr, GETTXT(_MSG_MMX_compress_unknownerr), 
		    filename);
	    break;
    }
...
void uncompress_filedesc (int ifd, int ofd, off64_t fsize, const char *filename)
{
	...
	case 1:
	    fprintf (stderr,
		    GETTXT(_MSG_MMX_compress_lessbits), 
		    filename, MAXBITS, MAXBITS);
	    break;
	case 2:
	    fprintf (stderr, GETTXT(_MSG_MMX_compress_corruptdata), 
		    filename);
	    break;
	case 3:
	    fprintf (stderr, GETTXT(_MSG_MMX_compress_compressfmt), 
		    filename);
	    break;
	default:
	    fprintf (stderr, GETTXT(_MSG_MMX_compress_uncompresserr),
		    filename, ret);
	    break;
    }
    if (ret) {
...
overwrite_check (const char *ofname)
{
    ...
    response[0] = 'n';
    fprintf (stderr, GETTXT(_MSG_MMX_compress_alreadyexit), ofname);
    if (!isatty(0)) {
	fprintf (stderr, GETTXT(_MSG_MMX_compress_cannotoverwr)); 
	exit(1);
    }
    if (foreground()) {
	fprintf (stderr,
	    GETTXT(_MSG_MMX_overwrite_yorn), 
	    ofname);
	fflush (stderr);
	read (2, response, 2);
	while (response[1] != '\n') {
    ...
    if (response[0] != 'y') {
	fprintf(stderr, GETTXT(_MSG_MMX_notoverwritten)); 
	return -1;
    }
...
errmsg(char *cmd, char c)
{
	...
	GETTXT(_MSG_MMX_uncompress_usage));
    else if (strcmp (cmd, "zcat") == 0)
	_sgi_nl_usage(SGINL_USAGE, cmd_label, 
	GETTXT(_MSG_MMX_zcat_usage));
    else
	_sgi_nl_usage(SGINL_USAGE, cmd_label, 
	GETTXT(_MSG_MMX_compress_usage));
...
register int argc; char **argv;
{
	...
	if (strcmp (ifname, "-") == 0) {
		...
		if (strcmp (ifname + strlen(ifname) - 2, ".Z") == 0) {
		    fprintf (stderr, GETTXT(_MSG_MMX_compress_zsuffix),
			ifname);
		    perm_stat = 1;
		    goto done1;
		}
...
copystat(const char *ifname, char *ofname)
{
	...
	if(quiet)
	    	fprintf(stderr, "%s: ", ifname);
	fprintf(stderr, GETTXT(_MSG_MMX_compress_regularfile)); 
	perfile_stat = 1;
	perm_stat = 1;
    } else if (statbuf.st_nlink > 1) {
	if(quiet)
	    	fprintf(stderr, "%s: ", ifname);
	fprintf(stderr, GETTXT(_MSG_MMX_compress_links), 
		statbuf.st_nlink - 1);
	perfile_stat = 1;
	perm_stat = 1;
    } else if (perfile_stat == 2 && (!force)) { /* No compression: remove file.Z */
	if(!quiet)
		fprintf(stderr, GETTXT(_MSG_MMX_compress_unchanged));
	...
	if (unlink(ifname))	/* Remove input file */
	    perror(ifname);
	if(!quiet)
		fprintf(stderr, GETTXT(_MSG_MMX_compress_replaced), ofname); 
...
{
	fprintf(stderr, "%s\n", rcs_ident);
	fprintf(stderr, GETTXT(_MSG_MMX_Options));
#ifdef vax
	fprintf(stderr, "vax, ");
#endif
#ifdef NO_UCHAR
