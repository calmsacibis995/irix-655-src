...
#define badopen PFMTTXT(_MSG_FILE_CANNOT_OPEN) 
...
prf(char *x)
{
	if (pfmt(stdout, MM_NOSTD, PFMTTXTCAT(_MSG_FILE_NAME), x) < 9) 
		printf("\t");
}
...
char **argv;
{
	...
	if(!cflg && !fflg && (eflg || optind == argc)) {
		if (!eflg)
			pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_FILE_INCORRECT_USAGE_));
use:
                pfmt(stderr, MM_INFO, 
			PFMTTXT(_MSG_FILE_USAGE));
		exit(2);
	}
...
char	*file;
{
	...
	case S_IFCHR:
		...
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_CHARACTER_SPECIAL_DD_),
	...
	case S_IFDIR:
		if (mbuf.st_mode&S_ISVTX)
		    pfmt(stdout, MM_NOSTD, PFMTTXTCAT(_MSG_APPENDONLY_));
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_DIRECTORY_));
		return(0);
	case S_IFSOCK:
		pfmt(stdout, MM_NOSTD, PFMTTXTCAT(_MSG_SOCKET_LF));
		return(0);

	case S_IFIFO:
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_FIFO_));
		return(0);

 	case S_IFNAM:
			switch (mbuf.st_rdev) {
			case S_INSEM:
		    		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_XENIX_SEMAPHORE_));
       	                 	return(0);
			case S_INSHD:
		    		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_XENIX_SHARED_MEMORY_HANDLE_));
       	                 	return(0);
			default:
		    		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_UNKNOWN_XENIX_NAME_SPECIAL_FILE_));
       	               	  	return(0);
			}

	case S_IFLNK:
		if ((cc = readlink(file, buf, BUFSIZ)) < 0) {
		    pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_FILE_CANNOT_READ_SYMBOLIC_LINK_S_S_),file,strerror(errno));
			return(1);
		}
		buf[cc] = '\0';
		if (stat64(file, &statb) != 0) {
		    pfmt(stdout, MM_NOSTD, PFMTTXTCAT(_MSG_DANGLING_SYMBLINK));
		    return(1);
		}
		    pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_SYMBOLIC_LINK_TO_S_),buf);
		return(0); 

	case S_IFBLK:
					/* major and minor, see sys/mkdev.h */
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_BLOCK_SPECIAL_DD_),
			major(mbuf.st_rdev), minor(mbuf.st_rdev));
	...
	if (strncmp(mbuf.st_fstype, "proc", 4) ==0) { 
		pfmt(stdout, MM_NOSTD, 
			PFMTTXTCAT(_MSG_MMX_file_procfile));
		return(0);
	}
	...
	if(ifd < 0) {
		...
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_FILE_CANNOT_READ_S_S_),
			file, strerror(errno));
                return(1);
        }
        if(fbsz == 0) {
    	    pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_EMPTY_FILE_));
            goto out;
	}
	if(fbsz >= TBLOCK) {
	    ...
	    if ( tar = (strncmp(tarbuf->tbuf.t_magic, "ustar", 5)) == 0) {
		    pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_TAR__));
		    goto out;
	    }
	}
	if(is_bru())
	...
	if(sccs()) {	/* look for "1hddddd" where d is a digit */
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_SCCS_));
		goto out;
	}
	...
	for(i=0; i < Max; )
		if(fbuf[i]&0200) {
			...
			if (fbuf[0]=='\100' && fbuf[1]=='\357') {
		    		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_TROFF_OUTPUT_));
				goto out;
			}
		/* start modification for multibyte env */
			if ((length=mbtowc(&wchar, &fbuf[i],MB_LEN_MAX)) <= 0
			    || !wisprint(wchar)){
		    		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_DATA_));
				goto out; 
			}
			i += length;
		}
...
outa:
	...
	while(i < fbsz){
		if (isascii(fbuf[i])){
			...
			if ((length=mbtowc(&wchar, &fbuf[i],MB_LEN_MAX)) <= 0
		        ...
		        case C_TEXT:
		    		pfmt(stdout, MM_NOSTD,
		    		PFMTTXT(_MSG_FILE_C_PROGTEXT_W_GARBAGE_));
		        	break;
		        case FORTRAN_TEXT:
		    		pfmt(stdout, MM_NOSTD,
		    		PFMTTXT(_MSG_FILE_FORT_PROGTEXT_W_GARBAGE_));
				break;
			case AS_TEXT:
		    		pfmt(stdout, MM_NOSTD,
		    		PFMTTXT(_MSG_FILE_ASSEM_PROGTEXT_W_GARBAGE_));
				break;
			case NROFF_TEXT:
		    		pfmt(stdout, MM_NOSTD,
		    		PFMTTXT(_MSG_FILE_NTROFF_TEXT_W_GARBAGE_));
				break;
			case CMD_TEXT:
		    		pfmt(stdout, MM_NOSTD,
		    		PFMTTXT(_MSG_FILE_COM_TEXT_W_GARBAGE_));
				break;
			case ENGLISH_TEXT:
		    		pfmt(stdout, MM_NOSTD,
		    		PFMTTXT(_MSG_FILE_ENGTEXT_W_GARBAGE_));
				break;
			case ASCII_TEXT:
		    		pfmt(stdout, MM_NOSTD,
		    		PFMTTXT(_MSG_FILE_ASCII_TEXT_W_GARBAGE_));
				break;
			case TEXT:
		    		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_TEXT_W_GARBAGE_));
				break;
			}
			goto out;
			}
	...
	switch(cur_type){
	case C_TEXT:
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_C_PROGTEXT_));
		break;
	case FORTRAN_TEXT:
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_FORT_PROGTEXT_));
        	break;
	case AS_TEXT:
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_ASSEM_PROGTEXT_));
		break;
	case NROFF_TEXT:
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_NTROFF_TEXT_));
		break;
	case CMD_TEXT:
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_COM_TEXT_));
		break;
	case ENGLISH_TEXT:
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_ENGTEXT_));
		break;
	case ASCII_TEXT:
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_ASCII_TEXT_));
		break;
	case TEXT:
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_TEXT_));
		break;
	}
out:
	return(0);
...
script() 	/* Also prints script type */
{
	...
	if (fbuf[0] == '#' && fbuf[1] == '!') {
		...
		if(i >= fbsz)
			return 0;
		pfmt(stdout, MM_NOSTD, PFMTTXTCAT(_MSG_FILE_SCRIPTXT), i - 2,
			&fbuf[2]);
		return 1;
	}
	return 0;
...
{
        CATNUM_DEFMSG(_MSG_FILE_UNKNOWNTYPE),
        CATNUM_DEFMSG(_MSG_FILE_RELOCATABLE),
        CATNUM_DEFMSG(_MSG_FILE_EXECUTABLE),
        CATNUM_DEFMSG(_MSG_FILE_DYNAMICLIB),
        CATNUM_DEFMSG(_MSG_FILE_COREFILE)
};
...
mips_elfnone(int fd)
{
	...
	if (elfhdr->e_ident[EI_DATA] == ELFDATA2LSB)
	  	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_LSB));
	if (elfhdr->e_ident[EI_DATA] == ELFDATA2MSB)
	  	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_MSB));
	if (check_e_type(elfhdr->e_type))
		return 0;
	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_MIPS));
	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_VERSION_LD),elfhdr->e_version);
...
mips_elf32(int fd)
{
	...
	if(elf_version(EV_CURRENT) == EV_NONE) {
	  	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_MMX_file_elfversion));
		return (1);
	}

	if((elf = elf_begin(fd, cmd, (Elf *)0)) == NULL) {
	  	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_MMX_file_elfbegin));
		elf_end(elf);
		return (1);
	}

	if ((elfhdr = elf32_getehdr(elf)) == NULL) {
	  	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_MMX_file_elfehdr));
		elf_end(elf);
		return (1);
	}
	...
	if (elfhdr->e_flags & EF_MIPS_ABI2)
	  	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_MMX_file_N32));
	else
		pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_32BIT));
	if (elfhdr->e_ident[EI_DATA] == ELFDATA2LSB)
	  	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_LSB));
	if (elfhdr->e_ident[EI_DATA] == ELFDATA2MSB)
	  	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_MSB));
	...
	if (arch=elfhdr->e_flags & EF_MIPS_ARCH) {
		if (arch == EF_MIPS_ARCH_2)
	  		pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_MIPS2_));
		else if (arch == EF_MIPS_ARCH_3)
	  		pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_MIPS3_));
		else if (arch == EF_MIPS_ARCH_4)
	  		pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_MMX_file_mips4));
	}
	if (dynamic && elfhdr->e_type==ET_EXEC)
	  	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_DYNAMIC_));
	if (check_e_type(elfhdr->e_type)) {
		elf_end(elf);
		return 0;
	}
	if (!stripped && (elfhdr->e_type==ET_REL || elfhdr->e_type==ET_EXEC))
	  	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_NOT_STRIPPED));
	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_MIPS));
	pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_VERSION_LD),elfhdr->e_version);
	elf_end(elf);
...
mips_elf64(int fd)
{
	...
	if(elfhdr->e_type == ET_IR) {
		if (elfhdr->e_flags & EF_MIPS_64BIT_WHIRL) 
	  		pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_FILE_64BIT_));
		else
	  		pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_MMX_file_N32));
	} else {
		if (elfhdr->e_flags & EF_MIPS_ABI2)
...
check_e_type(ushort elf_e_type)
{
	...
	switch (elf_e_type) {
		...
		case ET_CORE :
			...
			break;
		case ET_IR :
	  		pfmt(stdout, MM_NOSTD,PFMTTXTCAT(_MSG_MMX_file_object));
			break;
		/* 
	...
	if (!strncmp(hdr->ar_name, ELF_AR64_SYMTAB_NAME,
	    ...
	    !strncmp(&hdr->ar_name[ELF_AR64_SYMTAB_NAME_LEN], BLANKS,
			16 - ELF_AR64_SYMTAB_NAME_LEN))
                printf( GETTXTCAT(_MSG_FILE_ARCHIVE64) );
	    ...
	    !strncmp(&hdr->ar_name[ELF_AR_SYMTAB_NAME_LEN], BLANKS,
			16 - ELF_AR_SYMTAB_NAME_LEN))
                printf( GETTXTCAT(_MSG_FILE_ARCHIVE32) );
...
is_bru(void)
{
	...
	/* protect against somebody dropping FBSZ */
        if(FBSZ < 2228) printf( GETTXTCAT(_MSG_FILE_FILE_FBSZ) );
	if(fbsz>=2228 && strncmp("1234", &fbuf[176], 4) == 0  && 
		...
		if(strncmp("SGI Release", &fbuf[400], 11) == 0)
			...
			notsgi = "non-SGI ";
		pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_FILE_SBRU_ARCHIVE_), notsgi);
		return 1;
	}
	return 0;
}
