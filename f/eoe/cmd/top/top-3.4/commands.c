...

{
    ...
    fputs(CATGETS(catd, _MSG_TOP_HELP1), stdout);

    /* not all commands are availalbe with overstrike terminals */
    if (overstrike)
    {
	fputs(CATGETS(catd, _MSG_TOP_OVERSTRIKE_HELP), stdout);
    }
    else
    {
	fputs(CATGETS(catd, _MSG_TOP_HELP2),  stdout);
#ifdef ORDER
	fputs(CATGETS(catd, _MSG_TOP_HELP3), stdout);
#endif
	fputs(CATGETS(catd, _MSG_TOP_HELP4), stdout);
    }
}
...

{
	...
	return(NULL);
     ...
     err_listem = CATGETS(catd, _MSG_DISPLAY_ERR_LIST);
    /* sort the errors */
...

{
	...
	{
	    return(CATGETS(catd, _MSG_NO_PROCESS_KILL));
	}
	...
	if (isdigit(str[1]))
	{
	    ...
	    {
		return(CATGETS(catd, _MSG_INVALID_SIGNAL_NUM));
	    }
	}
	else 
	{
	    ...
	    if (sigp->name == NULL)
	    {
		return(CATGETS(catd, _MSG_BAD_SIGNAL_NAME));
	    }
	}
...

{
    ...
    {
	return(CATGETS(catd, _MSG_BAD_PRI_VAL));
    }
#endif
    ...
    {
	return(CATGETS(catd, _MSG_NO_PROCESS_SPECIFIED));
    }
