...
usage ()
{
  fprintf (stderr, CATGETS(cfp, _MSG_DIFF_USAGE1));
  fprintf (stderr, CATGETS(cfp, _MSG_DIFF_USAGE2));
  exit (2);
} 
     ...
     enum output_style style;
{
  if (output_style != OUTPUT_NORMAL
      && output_style != style)
    error (CATGETS(cfp, _MSG_DIFF_CONFLICTING_SPECIFICATIONS_OF_OUTPUT_STYLES));
  output_style = style;
}
     ...
     char *argv[];
{
  ...
  while ((c = getopt (argc, argv,
	...
	case '0':	gnudiffopts++;
	  ...
	  /* If a context length has already been specified,
	    ...
	    fatal (CATGETS(cfp, _MSG_DIFF_CONTEXT_LENGTH_SPECIFIED_TWICE));
	...
	case 'C':	gnudiffopts++;
	  if (optarg)
		...
		for (p = optarg; *p; p++)
		  if (!isdigit(*p))
		    fatal (CATGETS(cfp, _MSG_DIFF_INVALID_CONTEXT_LENGTH));
	      }
	      context = atoi (optarg);
	    }
	...
	case 'L':	gnudiffopts++;
	  ...
	  if (!file_label[0])
	    ...
	    fatal (CATGETS(cfp,_MSG_DIFF_TOO_MANY_LABELS));
	  break;
	...
	case 'v':	gnudiffopts++;
	  printf (CATGETS(cfp,_MSG_DIFF_GNU_VERSION), version_string);
	  break;
  ...
  if (I18N_SBCS_CODE )
    {
      ...
      if (val != 0)
	error (CATGETS(cfp,_MSG_DIFF_FORMAT_STR_STR), ignore_regexp, val);
      ignore_regexp_compiled.fastmap = (char *) xmalloc (256);
    }
  else
    {
      val = mbRe_compile_pattern (ignore_regexp, strlen (ignore_regexp),
                                &ignore_regexp_compiled);
      if (val != 0)
        error (CATGETS(cfp,_MSG_DIFF_FORMAT_STR_STR), ignore_regexp, val);
    }
  ...
  if (I18N_SBCS_CODE )
    {
      ...
      if (val != 0)
	error (CATGETS(cfp,_MSG_DIFF_FORMAT_STR_STR), function_regexp, val);
      function_regexp_compiled.fastmap = (char *) xmalloc (256);
    }
  else
    {
      val = mbRe_compile_pattern (function_regexp, strlen (function_regexp), &function_regexp_compiled);
      if (val != 0)
        error (CATGETS(cfp,_MSG_DIFF_FORMAT_STR_STR), function_regexp, val);
    }
   }
  ...
  if (ferror (stdout) || fclose (stdout) != 0)
    fatal (CATGETS(cfp,_MSG_DIFF_WRITE_ERROR));
  exit (val);
}
...
{
	if(S_ISFIFO(mode))
		return(CATGETS(cfp, _MSG_DIFF_FIFO));
	else if(S_ISCHR(mode))
		return(CATGETS(cfp, _MSG_DIFF_CHAR_SPECIAL));
	else if(S_ISBLK(mode))
		return(CATGETS(cfp, _MSG_DIFF_BLOCK_SPECIAL));
	else if(S_ISREG(mode))
		return(CATGETS(cfp, _MSG_DIFF_REGULAR));
	else if(S_ISLNK(mode))
		return(CATGETS(cfp, _MSG_DIFF_SYMBOLIC_LINK));
	else if(S_ISSOCK(mode))
		return(CATGETS(cfp, _MSG_DIFF_SOCKET));
	else
		return(CATGETS(cfp,_MSG_DIFF_UNKNOWN));
}

     ...
     int depth;
{
  ...
  if (! entire_new_file_flag && (name0 == 0 || name1 == 0))
    {
      ...
      message (CATGETS(cfp, _MSG_DIFF_ONLY_IN), dir, name);
  ...
  if (errorcount)
    {
       if(errorcount & 2)
       {
	 fprintf(stdout,CATGETS(cfp, _MSG_DIFF_FILE_AND_NOT_FILE),
    ...
    {
      if (output_style == OUTPUT_IFDEF)
	fatal (CATGETS(cfp, _MSG_DIFF_DOPTION_NOT_SUPPORTED));
      ...
      if (depth > 0 && !recursive)
	{
	  ...
	  message (CATGETS(cfp,_MSG_DIFF_COMMON_SUBDIRS),
	...
	fatal (CATGETS(cfp,_MSG_DIFF_CANNOT_COMPARE_HASH_TO_DIRECTORY));
      ...
      else
	{
	  ...
	  if (inf[dir_arg].dir_p)
	    {
	      error (CATGETS(cfp,_MSG_DIFF_OF_DIR_AND_NOT_DIR1),
      ...
      if (inf[0].desc == -1 || inf[1].desc == -1)
	{
	  if (entire_new_file_flag && recursive)
	  ...
	  else
	    {
	      ...
	      message (CATGETS(cfp,_MSG_DIFF_ONLY_IN), dir, name0);
	      val = 1;
	    }
	}
      else
	{
	  ...
	  if (inf[0].dir_p)
	    message (CATGETS(cfp,_MSG_DIFF_OF_DIR_AND_NOT_DIR2),
		     inf[0].name, inf[1].name);
	  else
	    message (CATGETS(cfp,_MSG_DIFF_OF_DIR_AND_NOT_DIR2),
  ...
  if (val == 0 && !inf[0].dir_p)
	...
	message (CATGETS(cfp,_MSG_DIFF_IDENTICAL_FILES),
		 inf[0].name, inf[1].name);
    }
  else
    fflush (stdout);
