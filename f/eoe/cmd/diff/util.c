     ...
     struct line_def *line;
{
  ...
  if (tab_expand_flag)
    {
      ...
      for (i = 0; i < line->length; i+=nBytes)
	{
	  ...
	  nBytes=1;
	  	...
	  	if (line->text[i]=='\t') {
			putc (c, out);
	  		...
	  		MbToWc(&wcChar, &line->text[i], &nBytes);
	  		putwc (wcChar, out);
	  	}
	  }
	}
  ...
  if ((line_flag == NULL || line_flag[0] != 0) && text[length - 1] != '\n'
      && line_end_char == '\n')
    fprintf (out, CATGETS(cfp, _MSG_DIFF_NO_NEWLINE_AT_THE_END1));
}
...
for (next = hunk; next; next = next->link)
    {
      ...
      for (i = next->line0; i <= l0 && ! nontrivial; i++)
        if ((!ignore_blank_lines_flag || files[0].linbuf[i].length > 1)
            && (!ignore_regexp
                || 0 > mbRe_search (&ignore_regexp_compiled,
                  files[0].linbuf[i].text,
     ...
     unsigned size;
{
  ...
  if (!value)
    fatal (CATGETS(cfp, _MSG_DIFF_VIRTUAL_MEM_EXHAUST));
  return value;
}
...
/*********************************************************************
     ...
     Purpose : Function to convert one character in mb string to wide char.

      Parameters :  wchar_t * wcPtr, char * mbString, int * nBytes
			nBytes: no. of bytes in mb string which form a wc.

      Return Values : none

*********************************************************************/
void 
MbToWc(wchar_t * wcPtr, char * mbString, int * nBytes)
{
static int invalid_locale_char_seen = 0;
*nBytes=mbtowc(wcPtr, mbString, MB_CUR_MAX);
if (*nBytes==0) {
 *wcPtr=(wchar_t) mbString[0];
 *nBytes=1;
}
if (*nBytes == -1) {
  if (! invalid_locale_char_seen ) {
	fprintf(stderr, CATGETS(cfp,_MSG_DIFF_FORMAT_STR_STR), program,
		CATGETS(cfp, _MSG_DIFF_INVALID_LOCALE_CHAR));
	invalid_locale_char_seen = 1;
	}
 *wcPtr=(wchar_t) mbString[0]; 
 *nBytes=1;
}
