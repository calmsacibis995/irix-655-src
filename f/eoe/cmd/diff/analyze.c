...
tryodiff (char *cmd, int argc, char **argv)
{
	...
	if ((pid = fork()) == 0) {
		...
		write (fileno (stderr), CATGETS(cfp, _MSG_DIFF_SIGACTION_ERROR), strlen(CATGETS(cfp, _MSG_DIFF_SIGACTION_ERROR)));
     ...
     int depth;
{
  ...
  if (binary || no_details_flag)
    {
      ...
      if (differs) 
	message (binary ? CATGETS(cfp, _MSG_DIFF_DIFFERENT_BINARY_FILES)
		 : CATGETS(cfp, _MSG_DIFF_DIFFERENT_FILES),
  ...
  if (! ROBUST_OUTPUT_STYLE (output_style)
	...
	{
	  error (CATGETS(cfp, _MSG_DIFF_NO_NEWLINE_AT_THE_END2), filevec[i].name, "");
	  changes = 2;
	}
