     ...
     struct re_pattern_buffer *bufp;
{
 ...
 invalid_pattern:
  return (CATGETS(cfp, _MSG_DIFF_INVALID_REGEXPR));

 unmatched_open:
  return (CATGETS(cfp, _MSG_DIFF_UNMATCHED_RIGHT));

 unmatched_close:
  return (CATGETS(cfp, _MSG_DIFF_UNMATCHED_LEFT));

 end_of_pattern:
  return (CATGETS(cfp, _MSG_DIFF_PREMATURE_END));

 nesting_too_deep:
  return (CATGETS(cfp, _MSG_DIFF_NESTING_TOO_DEEP));

 too_big:
  return (CATGETS(cfp, _MSG_DIFF_REGEXPR_TOO_BIG));

 memory_exhausted:
  return (CATGETS(cfp, _MSG_DIFF_MEM_EXHAUST));
}
...
/* Entry points compatible with 4.2 BSD regex library.  */
	...
	/** This portion has been commented out because
	   the declarations of re_comp(), and re_exec()
	   in regex.h were interfering with definitions
	   in re_comp.h(standard definitions).
 	   This re_comp.h is includedin unistd.h
	   which is further included in mkmsgs.h
	   which itself is included in uxeoe.h
	**/
#ifndef emacs
...
*********************************************************************/
char *
mbRe_compile_pattern (pattern, nSize, bufp)
     char *pattern;
     int nSize;
     struct re_pattern_buffer *bufp;
{
...
int
mbRe_search (pbufp, string, nSize)  /**  nSize = no. of wc's less null 
							   -1 if no match found
							   +1 if match found 
							   0  if error  **/
     struct re_pattern_buffer *pbufp;
...
int ret=0;
	...
	if (pbufp->err)
		return 0;
	String = (char *)xmalloc(nSize+1);
	memcpy(String, string, nSize);
	String[nSize]='\0';
