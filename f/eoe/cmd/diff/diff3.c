...
nl_catd cfp;
static char Binary_as_Text[3];   /* 0,1,2: - file0, file1, file2.
					Binary_as_Text[0]=1 if file0 is binary and always_text=1
					Binary_as_Text[1]=1 if file1 is binary and always_text=1
					Binary_as_Text[2]=1 if file2 is binary and always_text=1
				*/
     ...
     char **argv;
{
  ...
  while ((c = getopt (argc, argv, "aeimvx3AEL:TX")) != EOF)
...
#if 0
	case 'v':
	  printf (CATGETS(cfp, _MSG_DIFF3_GNU_VERSION), version_string);
	  exit (0);
#endif
	case 129:
	  usage ();
	...
	case 'L':
	  ...
	  try_help (CATGETS(cfp,_MSG_DIFF3_TOO_MANY_LABELS));
	default:
	  try_help (0);
	}
    }
  ...
  if (incompat > 1  /* Ensure at most one of -AeExX3.  */
    ...
    try_help (CATGETS(cfp, _MSG_DIFF3_INCOMPATIBLE_OPTIONS));

  if (argc - optind != 3)
    try_help (argc - optind < 3 ? CATGETS(cfp, _MSG_DIFF3_MISSING_OPERAND) : CATGETS(cfp, _MSG_DIFF3_EXTRA_OPERAND));
  ...
  if (strcmp (file[2], "-") == 0)
	...
	fatal (CATGETS(cfp, _MSG_DIFF3_HASH_SPECIFIED_FOR_MORE_FILES));
      mapping[0] = 0;
      mapping[1] = 2;
      mapping[2] = 1;
    }
  ...
  for (i = 0; i < 3; i++)
	...
	if (stat (file[i], &statb) < 0)
	  ...
	  {
	    fprintf (stderr, CATGETS(cfp, _MSG_DIFF3_IS_A_DIR),
		     program_name, file[i]);
	    exit (2);
	  }
      }
...
/** Checking for binary files
    This SLURP function actually performs the checking
    Sets the corressponding Binary_as_Text array item as 1 if binary file.
    index 0 for file0, 1 for file1, 2 for file2. Even if 
    any of the first two files is binary and always_text falg is
    set then the 3rd array item is deliberately set to 1.
    So if even one file is binary the block below expects
    others files also to be binary **/

if (always_text )
    {
        Slurp (file[rev_mapping[FILE0]]);
	if (Binary_as_Text[0]) {
		Binary_as_Text[2]=1;
		goto Skip;
	}
        Slurp (file[rev_mapping[FILE1]]);
	if (Binary_as_Text[1]) {
		Binary_as_Text[2]=1;
		goto Skip;
	}
        Slurp (file[rev_mapping[FILE2]]);
    }
...
Skip:
  ...
  if (edscript)
	...
	= output_diff3_merge (stdin, stdout, diff3, mapping, rev_mapping,
			      tag_strings[0], tag_strings[1], tag_strings[2]);
      if (ferror (stdin))
	fatal (CATGETS(cfp, _MSG_DIFF3_READ_ERROR));
    }
  else
    {
     ...
     char const *reason;
{
  if (reason)
    fprintf (stderr, "%s: %s\n", program_name, reason);
  fprintf (stderr, CATGETS(cfp, _MSG_DIFF3_TRY_HELP),
...
check_stdout ()
{
  if (ferror (stdout) || fclose (stdout) != 0)
    fatal (CATGETS(cfp, _MSG_DIFF3_WRITE_ERROR));
}
...
usage ()
{
  printf (CATGETS(cfp, _MSG_DIFF3_USAGE_MYFILE), program_name);

  printf ("%s",CATGETS(cfp, _MSG_DIFF3_USAGE2)); 
  printf ("%s",CATGETS(cfp, _MSG_DIFF3_USAGE3));
  printf ("%s",CATGETS(cfp, _MSG_DIFF3_USAGE4)); 
  printf (CATGETS(cfp, _MSG_DIFF3_USAGE5));
}

/*
  ...
  while (current[0] || current[1])
	     ...
	     && D_LOWLINE (other_diff, FC) <= high_water_mark + 1)
	{
	 ...
	 included in this diff3_block.  Create it.  */
      ...
      if (!tmpblock)
	fatal (CATGETS(cfp, _MSG_DIFF3_INTERNAL_ERROR_SCREWUP_IN_FORMAT));
     ...
     struct diff_block **last_block;
{
  ...
  while (scan_diff < diff_limit)
	...
	{
	  fprintf (stderr, CATGETS(cfp, _MSG_DIFF3_DIFF_ERROR), program_name);
        if (I18N_SBCS_CODE)
          {
	  do
	    {
	  ...
	  exit (2);
	...
	else
  	  {
                              wchar_t wChar;
                                do
                                  {
                                    mbtowc (&wChar, scan_diff, MB_CUR_MAX);
                                    scan_diff+=mblen(scan_diff, MB_CUR_MAX);
                                    putwc (wChar, stderr);
                                  }
                                while (*scan_diff != '\n');
                                putc ('\n', stderr);
	...
	case CHANGE:
	  break;
	default:
	  fatal (CATGETS(cfp, _MSG_DIFF3_INTERNAL_ERROR_INVALID_DIFF_TYPE));
	  break;
	}
      ...
      if (dt == CHANGE)
	{
	  if (strncmp (scan_diff, "---\n", 4))
	    fatal (CATGETS(cfp, _MSG_DIFF3_INVALID_CHANGE_SEPERATOR));
	  scan_diff += 4;
	}
...
#define INT_STRLEN_BOUND(type) ((sizeof(type)*CHAR_BIT - 1) * 302 / 1000 + 2)
  ...
  *ap = 0;
  ...
  if (pipe (fds) != 0)
    perror_with_exit (CATGETS(cfp, _MSG_DIFF3_PIPE));
  ...
  if (pid == 0)
      ...
      if (fds[1] != STDOUT_FILENO)
	{
      ...
      write (STDERR_FILENO, CATGETS(cfp, _MSG_DIFF3_NOT_FOUND), 12);
      _exit (2);
    }

  if (pid == -1)
    perror_with_exit (CATGETS(cfp, _MSG_DIFF3_FORK_FAILED));
  ...
  *p = '\0';
  ...
  do {
	...
	if (current_chunk_size < 2 * current_chunk_size)
	  ...
	  fatal (CATGETS(cfp ,_MSG_DIFF3_FILES_ARE_LARGE));
	diff_result = xrealloc (diff_result, (current_chunk_size *= 2));
      }
  } while (bytes);

  if (total != 0 && diff_result[total-1] != '\n')
    fatal (CATGETS(cfp, _MSG_DIFF3_INVALID_INCOMPLETE_LAST_LINE));
  ...
  *output_placement = diff_result;
  ...
  if (close (fd) != 0)
    perror_with_exit (CATGETS(cfp, _MSG_DIFF3_PIPE_CLOSE));
  if (waitpid (pid, &wstatus, 0) < 0)
    perror_with_exit (CATGETS(cfp, _MSG_DIFF3_WAITPID_FAILED));

#endif /* HAVE_FORK */

  if (! (WIFEXITED (wstatus) && WEXITSTATUS (wstatus) < 2))
    fatal (CATGETS(cfp, _MSG_DIFF3_SUBSIDIARY_DIFF_FAILED));
     ...
     int leadingchar;
{
  ...
  if (!(scan_ptr[0] == leadingchar
	&& scan_ptr[1] == ' '))
    fatal (CATGETS(cfp, _MSG_DIFF3_INVALID_INCORRECT_LEADING_LINE_CHARS));
                        ...
                        do
                          {
                            if (edscript)
                                {
                                mbtowc(&wChar, line_ptr, MB_CUR_MAX);
                                putwc (wChar, stderr);
                                }
                            line_ptr+=mblen(line_ptr, MB_CUR_MAX);
                          }
                        while (*line_ptr != '\n');
                        if (edscript)
                        putc ('\n', stderr);
     ...
     int const mapping[3], rev_mapping[3];
{
  ...
  for (ptr = diff; ptr; ptr = D_NEXT (ptr))
	...
	case DIFF_3RD:
	  ...
	  break;
	default:
	  fatal (CATGETS(cfp, _MSG_DIFF3_INVALID_DIFF_TYPE_PASSED));
	}
	   ...
	   i = (oddoneout == 1 ? skew_increment[i] : i + 1))
	{
	  ...
	  if (lowt <= hight)
	      ...
	      do
		{
	      ...
	      if (cp[length - 1] != '\n')
		fprintf (outputfile, CATGETS(cfp, _MSG_DIFF3_NO_NEWLINE));
	    }
	}
    }
}
     ...
     char const *file0, *file1, *file2;
{
  ...
  for (b = diff; b; b = b->next)
	...
	case DIFF_ALL: if (simple_only) continue; conflict = flagging;
      ...
      linesread += i;
      while (0 <= --i)
	if (I18N_SBCS_CODE || Binary_as_Text[2])   /* Check binary_as_text only for commonfile */
	{
	do
	  {
	    c = getc (infile);
	    if (c == EOF)
	      if (ferror (infile))
		perror_with_exit (CATGETS(cfp, _MSG_DIFF3_INPUT_FILE));
	      else if (feof (infile))
		fatal (CATGETS(cfp, _MSG_DIFF3_INPUT_FILE_SHRANK));
	    putc (c, outputfile);
	  }
	while (c != '\n');
	}
...
{
  wint_t wInt;
  while ((wInt = getwc (infile)) != WEOF || !(ferror (infile) | feof (infile)))
    putwc (wInt, outputfile);
}
  return conflicts_found;
}
     ...
     size_t size;
{
  ...
  if (result == -1)
    perror_with_exit (CATGETS(cfp, _MSG_DIFF3_READ_FAILED));
  return result;
}

static VOID *
     ...
     size_t size;
{
  ...
  if (!result)
    fatal (CATGETS(cfp, _MSG_DIFF3_MEMORY_EXHAUSTED));
  return result;
}
...
/*********************************************************************
Purpose               : It is used to check whether a file is binary
			or not. Called by Slurp().
 Parameters 	: Buffer (which has contents of the file),
		  size   (no. of bytes to check)
 Return Values 	: 1 if binary, 0 otherwise
*********************************************************************/
#include <fcntl.h>
static int
...
/*********************************************************************
Purpose               : Used to set corresponding array items in
			the array Binary_as_Text[] if the corresponding
			file is binary. Makes use of Binary_file_p().
 Parameters 	: filename
 Return Values 	: None
*********************************************************************/
void
Slurp (char *filename)
{
  /* Used to read whole of a file into a buffer
   * If it's a regular file, we can just get the size out of the stat
   * block and I18N_slurp it in all at once.
  ...
  if ((desc = open(filename,O_RDONLY))==-1)
    {
        fprintf (stderr, CATGETS(cfp, _MSG_CANNOT_OPEN_FILE), filename);
        exit(1);
    }
  ...
  if ((StatBuf.st_mode & S_IFMT) == S_IFREG)
    {
      ...
      buffered_chars
        = read (desc, buffer, bufsize);

      if (buffered_chars < 0)
        perror_with_exit (filename);
    }
  else
    {
      int cc;
      ...
      /* Not a regular file; read it in a little at a time, growing the
         buffer as necessary. */
      while ((cc = read (desc,
                         buffer + buffered_chars,
                         bufsize - buffered_chars))
             > 0)
        {
          buffered_chars += cc;
          if (buffered_chars == bufsize)
            {
              bufsize = bufsize * 2;
  ...
  if (Binary_file_p (buffer,
                min (buffered_chars, binary_file_threshold)))
     Binary_as_Text[Count] = 1;
   Count++;
   free (buffer);
}
