...
char	*months[12] = {
	"   ", "   ", "   ", "   ", "   ", "   ",
	"   ", "   ", "   ", "   ", "   ", "   "};
wchar_t  wMonths[12][4];

struct	tm	ct = {
	0, 0, 1, 0, 86, 0, 0};

static const char USE[] = MSGNUM_DEFMSG(_MSG_INCORRECT_USAGE);
static const char nonl[] = MSGNUM_DEFMSG(_MSG_SORT_NEWLINE_ADDED_END_FILE);
static const char badcreate[] = MSGNUM_DEFMSG(_MSG_CANNOT_CREATE);
static const char badchmod[] = MSGNUM_DEFMSG(_MSG_SORT_CANNOT_CHANGE_MODE);
...
more:	    switch(*++arg) {
		  ...
		  if(--argc > 0)
		    outfil = *++argv;
		      ...
		      pfmt(stderr, MM_ERROR,
			   PFMTTXT(_MSG_SORT_CANNOT_IDENTIFY));
		      exit(1);
		  }
		break;
		...
		if (--argc > 0) {
			...
			pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SORT_LONG_PATH),
			     *argv);
			exit(1);
		    }
		    else dirtry[0] = *argv;
		...
		break;
	      ...
	      case 't':
		if(*++arg == '\0' && argc < 2){
		    pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SORT_FIELDSEP));
		    exit(1);
		}
		if(*arg == '\0'){
		    argc--;
		...
		if(++nfields >= nf){
			...
			pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SORT_MANY_KEYS));
			exit(1);
		    }
		    nf += NF;
		}
		...
		if(*++arg == '\0'){
			...
			pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SORT_KEYDEF));
			exit(3);
		    }
		    else{
			argc--;
	...
	if(eargc == 0)
		eargv[eargc++] = "-";
	if(cflg && eargc>1) {
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SORT_CAN_CHECK_ONE));
		exit(1);
	}
	...
	if (!mflg && !cflg)
		if ((alloc=grow_core(tryfor,(unsigned) 0)) == 0) {
			pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SORT_NOMEM_BEFORE_SORT),
				strerror(errno));
			exit(1);
		}
	...
	if(a < 0) {
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SORT_CANNOT_LOCATE_TEMP_FILE),
			strerror(errno));
		exit(1);
	}
	(void) close(a);
	...
	if (nway < 2 || brk((char *)lspace + alloc) != 0) {
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SORT_NOMEM_BEFORE_MERGE),
			strerror(errno));
		term();
	}
	...
	do {
		...
		*lp-- = cp;
		...
		while ((char *)lp > cp + 1 ) {
			...
			if (fgets(cp,n,iop) == NULL)
				...
				if (iflag)
					...
					pfmt(stderr, MM_ERROR, PFMTTXTCAT(_MSG_SORT_NULLPRESENT));
					term();
				    }
				}
#else
			...
			if ( cp + 2 < (char *) lp ) {
				...
				if(f == NULL)
					...
					pfmt(stderr, MM_WARNING, PFMTTXT(_MSG_SORT_NEWLINE_ADDED_INPUT_FILE), f);
				*++cp = '\n';
				*++cp = '\0';
				*lp-- = ++cp;
				keep = 0;
		...
		if (keep != 0 && *(lp+1) == (char *) lspace) {
			pfmt(stderr, MM_WARNING, PFMTTXT(_MSG_SORT_LARGE_RECORD));
			term();
		}
		first = 0;
		lp += 2;
...
register char *s;
{
	...
	if ( n < maxrec - 1) {
		...
		*++s = '\n';
		...
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SORT_LONG_LINE));
		term();
		/*NOTREACHED*/
	}
	return 0;
...
void checksort()
{
	...
	while ( !rline(iop, s[j]) )  {
		r = (*compare)(s[i], s[j]);
		if (r < 0)
			disorder(CATNUM_DEFMSG(_MSG_SORT_DISORDER), s[j]);
		if (r == 0 && uflg)
			disorder(CATNUM_DEFMSG(_MSG_SORT_NON_UNIQUE), s[j]);
		r = i;  i = j; j = r;
	}
	if (ferror(iop))
		rderror(f);
...
char *f;
{
	pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_CANNOT_OPEN), f, strerror(errno));
	term();
}
...
char *i, *j;
{
	...
	unsigned char ca[2], cb[2];
		...
		/* Function Prototypes */
	int nIsIgnore( char *mbPtr, int *nLen ); 
	int nIsCode( char *mbPtr, wchar_t *wPtr );
...
loop_SB:
		...
		if ( __libc_attr._collate_res._coll_as_cmp )
			...
			int nLena, nLenb;
			wchar_t wca[2], wcb[2];
loop_MB:		while(*pa && nIsIgnore((char *)pa, &nLena))
				pa += nLena;
			while(*pb && nIsIgnore((char *)pb, &nLenb))
				pb += nLenb;
			if(pa>=la || *pa=='\n')
				if(pb<lb && *pb!='\n')
					return(fp->rflg);
			...
			if(pb>=lb || *pb=='\n')
				return(-fp->rflg);
			pa += nIsCode((char *) pa, wca );
			pb += nIsCode((char *) pb, wcb );
			if((sa = wcscoll(wcb, wca )) == 0)
				goto loop_MB;
		}
		return(sa*fp->rflg);
	}
 ...
 Purpose       : The function determines whether the first char of the 
		 data area passed is to be ignored or not depending on the
		 commmand-line options.
 Parameters    : The pointers to the data area, char *mbPtr and
		 the pointer to an int variable, nLen which stores the
		 value of the first char in the data area on return.
 Return Values : An int nRetVal,  indicating whether the char is to be
		 ignored or not for comparison.
		 A value is also put in int *nLen, one of the parameters 
		 passed indicating the length of the MB char.
...
int nIsIgnore( char *mbPtr, int *nLen ) 
{
	...
	*nLen = mbtowc( &wTemp, mbPtr, MB_CUR_MAX );
	if (nflgDict)
		nRetVal = !iswalnum(wTemp);
	else if (nflgPrint)
		nRetVal = !iswprint(wTemp);
 ...
 /**************************I18N Function Header***************************
 Purpose       : The function puts one MB char from a char buffer passed
		 into a wide-char buffer after changing it into a wide-char
 Parameters    : The pointer to the char buffer, char *mbPtr and
		 the pointer to the wide-char buffer.
 Return Values : An int containing length of the MB char.
 **********************End of I18N Function Header*************************/
...
int nIsCode( char *mbPtr, wchar_t *wPtr )
{
 ...
 /**************************I18N Function Header***************************
 Purpose       : The function compares two data areas according to the 
		 specified sort keys.                                                      
 Parameters    : The pointers to the data area which are to be sorted.        
 Return Values : An int indicating result of comparing the data areas.     
 **********************End of I18N Function Header*************************/
...
{
	int nTemp;
	wchar_t wTemp1[2],wTemp2[2];

	wTemp2[1] = wTemp1[1] = L'\0';
	pa += mbtowc(wTemp1, pa, MB_CUR_MAX);
	pb += mbtowc(wTemp2, pb, MB_CUR_MAX);
	while( (nTemp = wcscoll(wTemp2, wTemp1)) == 0 )  {
		if(wTemp1[0] == L'\n') return(0);
		pa += mbtowc(wTemp1, pa, MB_CUR_MAX);
		pb += mbtowc(wTemp2, pb, MB_CUR_MAX);
	}
...
register unsigned char *p;
{
			...
			do {
				...
				if (I18N_SBCS_CODE)     {
					ew = 1;
                                        ...
                                        ew = mblen((const char *)p, MB_CUR_MAX);
				if (*p != *tbc)
					if(*p != '\n') {
						p += ew;
						contflag = 1;
	...
	while((i > 0) && (*p != '\n')) {
		if (I18N_SBCS_CODE)     {
			...
			p += mblen((const char *)p, MB_CUR_MAX);
			i--;
		}
	}
ret:
...
char *s;
{
	...
	for(i=0; i<12; i++) {
		if (I18N_SBCS_CODE)	{
			...
			for(j = 0, wPtr = wMonths[i]; towupper(wTemp[j++])==towupper(*wPtr++); )
				if (*wPtr == L'\0')
					return(i);
		}
	}
...
{
	if (s)
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_READ_ERROR), s,
			strerror(errno));
	else
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SORT_STDIN_READ_ERROR),
			strerror(errno));
	term();
}
...
int s;
{
	switch(s){
	case SORT:
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SORT_SORTING_WRITE_ERROR),
			strerror(errno));
		break;
	case MERGE:
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SORT_MERGING_WRITE_ERROR),
			strerror(errno));
		break;
	}
	term();
