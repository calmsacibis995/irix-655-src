...
usage(int complain)
{
	...
	pfmt(stderr, MM_ACTION, PFMTTXT(_MSG_CMP_USAGE));
	exit(2);
}
...
subset(char *name)
{
    fprintf(stderr, GETTXT(_MSG_CMP_EOF), name);
    exit(1);
}
...
{
	if (lflg)
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_CANNOT_OPEN), name,
			strerror(errno));
	exit(2);
}
...
{
	(void)setlabel("cmp");
	pfmt(stderr, MM_INFO, PFMTTXT(_MSG_EOF_FOUND), name);
	exit(1);
}
