...
#ident	"$Header: /proj/irix6.5f/isms/eoe/cmd/sed/RCS/sed1.c,v 1.15 1998/09/18 19:47:22 sherwood Exp $"
...
char	rub[] = {"\\177"};
extern nl_catd ncCatd;
extern int errno;

static int match(regex_t *);
static void cpWstrtrns(wchar_t *wpOrg,wchar_t *wpTr,char *cpStr);
...
char *file;
{
	...
	if (file) {
		if ((f = open(file, 0)) < 0) {
			fprintf(stderr, CATGETS(ncCatd,_MSG_SED_CANT_OPEN_FORMAT_S), file); 
			return(1);
		}
		if (!eolflg)
			eolflg = 1;
...
static void check_matchbuf(regex_t *expbuf)
{
	if (expbuf->re_nsub >= MATCHBUF_MAX) {
		fprintf(stderr,CATGETS(ncCatd,_MSG_SED_ERROR_TOO_MANY_SUBEXPRESSIONS)); 
		exit(1);
	}
}
...
substitute(union reptr	*ipc)
{
	...
	do {
		...
		while(c = *rp++) {
			...
			if((c == '\\' && (rp-1)!=rhsbuf && mblen(rp-2,MB_CUR_MAX)<=1) ||
			   (c == '\\' && (rp-1)==rhsbuf)) {
				c = *rp++;
				if (isdigit(c) &&
				    c != '0' &&
		...
		if(memccpy(sp, lp, '\0', &genbuf[LBSIZE-1] - sp) == NULL) {
			fprintf(stderr, CATGETS(ncCatd,_MSG_SED_OUTPUT_LINE_TOO_LONG)); 
toolong:
...
union reptr	*ipc;
{
		...
		case ACOM:
			*aptr++ = ipc;
			if(aptr >= &abuf[ABUFSIZE]) {
				fprintf(stderr, CATGETS(ncCatd,_MSG_SED_TOO_MANY_APPENDS_AFTER), 
					lnum);
			}
			*aptr = 0;
			break;
		...
		case LCOM:
			...
			while (*p1) {
				if((unsigned char)*p1 < 040) {
				  ...
				  else if ( I18N_SBCS_CODE && !isprint(*p1 & 0377)) {
                                        p2 = append_genbuf(p2,"\\");
                                        *p2++ = (*p1 >> 6) + '0';
                                        p2 = check_genbuf(p2,1);
                                        *p2++ = ((*p1 >> 3) & 07) + '0';
				 ...
				 else if (!( I18N_SBCS_CODE ) && (nLen=mbtowc(&wC,p1,MB_CUR_MAX)) &&
				           ( nLen != -1 ) && !iswprint(wC)) {
						    while(nLen--) {
							p2 = append_genbuf(p2,"\\");
							*p2++ = (*p1 >> 6) + '0';
	...
	cpcom:
		...
		case RCOM:
			...
			*aptr++ = ipc;
			if(aptr >= &abuf[ABUFSIZE])
				fprintf(stderr, CATGETS(ncCatd,_MSG_SED_TOO_MANY_READS_AFTER), 
					lnum);
		...
		case YCOM: 
			...
			cpWstrtrns(wCporg,wCptr,p1);
			strcpy(linebuf,p1);
			spend=linebuf+strlen(p1); 
		    }
			break;
	}
...
/****************************************************************************
     ...
     Purpose : Transforms 'cpStr'.  Any character that appears in 'wpOrg' is replaced
	 with the character in the same position in 'wpTr'.  

      Parameters : cpStr: contains string to be transformed.         
		   wpTr: contains new positional characters.
		   wpOrg: contains old positional characters.

      Return Values : Transformed 'cpStr' 

****************************************************************************/

void cpWstrtrns(wchar_t *wpOrg,wchar_t *wpTr,char *cpStr)
{
	int nI=-1,nLen=0,nChars=0,nBytes=0;
	wchar_t *wpOldstr, *wpStr;
	nChars=mbstowcs(0,cpStr,0);
        if(errno == EILSEQ) {
                perror("data contains ");
                exit(1);
        }
	wpStr=(wchar_t *)malloc((nChars+1)*sizeof(wchar_t));
	if (wpStr == NULL) {
		fprintf(stderr,CATGETS(ncCatd,_MSG_SED_CANNOT_ALLOCATE_FORMAT),nChars + 1);
		exit(1);
	}
	mbstowcs(wpStr,cpStr,nChars+1);
	wpOldstr=wpStr;
	nLen=wcslen(wpOrg);
	while(((*wpStr++) != 0)) { 
	     while(nI++ < (nLen-1))
	...
	wcstombs(cpStr,wpOldstr,wcstombs(0,wpOldstr,0)+1);
}
