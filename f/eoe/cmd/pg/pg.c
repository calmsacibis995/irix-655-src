...
/*	Regular expression search using ( regcomp() ,regerror()	*/
/*	   and regexec() used instead of compile() and step() )	*/                      
/*	re_error() has been modified as compile() is no longer	*/
/*	   being used						*/
...
USAGE()
{
	(void)pfmt(stderr, MM_ERROR,PFMTTXT(_MSG_INCORRECT_USAGE));
	(void)pfmt(stderr, MM_ACTION,
		PFMTTXT(_MSG_PG_USAGE));
	exit(1);
}
					...
					if (p < initbuf + sizeof(initbuf))
						*p++ = *s++;
						...
						(void)pfmt(stderr, MM_ERROR,
							PFMTTXT(_MSG_PG_PATTERN_TOO_LONG));
						exit(1);
					}
				*p = '\0';
			}
	...
	while (fnum < nfiles) {
		if (strcmp(fnames[fnum],"") == 0)
			...
			if (out_is_tty)
				...
				if (prnames) {
					(void) fputs(GETTXT(_MSG_PG_DOTS_TO_FILE_ST),stdout);
					(void) fputs(fnames[fnum],stdout);
					(void) fputs(GETTXT(_MSG_PG_DOTS_TO_FILE_EN),stdout);
				}
...
register char *s;
{
	...
	(void) pfmt(stderr, MM_ERROR,PFMTTXT(_MSG_PG_PROMPT_TOO_LONG));
	exit(1);
}
	...
	for (;;) {
		...
		if (clropt) {
			...
			if (start < old_ss.first_line)
				sopr(MSG_NUM(_MSG_PG_SKIPPING_BACKWARD),DEF_MSG(_MSG_PG_SKIPPING_BACKWARD),0);
			else
			if (start > old_ss.last_line + 1)
				sopr(MSG_NUM(_MSG_PG_SKIPPING_FORWARD),DEF_MSG(_MSG_PG_SKIPPING_FORWARD),0);
		}
		...
		if (new_ss.is_eof) {
			if (!eof_pause || eofl_no == 1)
				return(1);
			hadchance++;
			error(MSG_NUM(_MSG_PG_EOF),DEF_MSG(_MSG_PG_EOF));
		}
		old_ss = new_ss;
	...
	for (;;) {
			...
			*/
			...
			if (setjmp(restore) != 0) {
				...
				if ((sf=fopen(cmdptr,"w")) == NULL) {
					error(MSG_NUM(_MSG_PG_CANNOT_OPEN),DEF_MSG(_MSG_PG_CANNOT_OPEN),
						cmdptr,	strerror(errno));
					break;
				}
				kill_line();
				sopr(MSG_NUM(_MSG_PG_SAVE_FILE),DEF_MSG(_MSG_PG_SAVE_FILE),1);
				sopr(NULL, cmdptr,1);
				(void) fflush(stdout);
				save_input(sf);
				error(MSG_NUM(_MSG_PG_SAVED),DEF_MSG(_MSG_PG_SAVED));
			}
			(void) fclose(sf);
			break;
		case 'q':
		...
		case '!':       /* shell escape */
			if (rmode) {	/* restricted mode */
				(void) pfmt(stderr, MM_ERROR,PFMTTXT(_MSG_PG_RESTRICTED_MODE));
				break;
			}
			...
			if ((id = fork ()) < 0) {
				error(MSG_NUM(_MSG_PG_FORK_FAILED),DEF_MSG(_MSG_PG_FORK_FAILED));
				break;
			}
			if (id == (pid_t)0) {
				/*
...
help()
{
	...
	(void) fputs(GETTXT(_MSG_PG_DASHES), stdout);
	(void) pfmt(stdout, MM_NOSTD,PFMTTXT(_MSG_PG_HELP_HELP));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_QUIT));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_NXT_PAGE));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_NXT_LINE));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_HALF_PAGE));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_REDISPLAY));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_SKIP_PAGE));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_NEXT_FILE));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_PREVIOUS_FILE));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_LAST_PAGE));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_WIN_SIZE));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_SAVE_FILE));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_PATTERN1));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_PATTERN2));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_PATTERN3));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_COMMAND));
	(void) putc('\n', stdout);
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_STATEMENT1));
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_STATEMENT2));
	(void) putc('\n', stdout);
	(void) pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_PG_HELP_STATEMENT3));
	(void) fputs(GETTXT(_MSG_PG_DASHES), stdout);
}
...
register int nskip;
{
	if (fnum + nskip < 0) {
		nskip = -fnum;
		if (nskip == 0)
			error(MSG_NUM(_MSG_NO_PREVIOUS_FILE),DEF_MSG(_MSG_NO_PREVIOUS_FILE));
	}
	else
	if (fnum + nskip > nfiles - 1) {
		nskip = (nfiles - 1) - fnum;
		if (nskip == 0)
			error(MSG_NUM(_MSG_NO_NEXT_FILE),DEF_MSG(_MSG_NO_NEXT_FILE));
	}
	return(nskip);
}
...
register char *fs;
{
	...
	if (strcmp(fs,"-") == 0) {
		...
		if ((f=fopen(fs, "r")) == (FILE *)NULL) {
			(void) fflush(stdout);
			pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_CANNOT_OPEN),fs,
				strerror(errno));
		     	return ((FILE *)NULL); 
		}
		f_was_opened = 1;
	...
	if (fstat(fileno(f), &stbuf) == -1) {
		if(f_was_opened)
			(void) fclose(f);
		(void) fflush(stdout);
		pfmt(stderr, MM_ERROR,PFMTTXT(_MSG_CANNOT_ACCESS), fs,
			strerror(errno));
	     	return ((FILE *)NULL); 
	}
	if ((stbuf.st_mode & S_IFMT) == S_IFDIR) {
		if (f_was_opened)
			(void) fclose(f);
		(void) pfmt(stderr, MM_ERROR,PFMTTXT(_MSG_IS_DIR),fs);
		return ((FILE *)NULL);
	}
	if ((stbuf.st_mode & S_IFMT) == S_IFREG) {
		...
		if (f != stdin) {
			if (f_was_opened)
				(void) fclose(f);
			(void) pfmt(stderr, MM_ERROR,PFMTTXT(_MSG_SPL_FILE));
			return((FILE *)NULL);
		}
		else {
			(void) mktemp(tmp_name);
			if ((fd=creat(tmp_name,(S_IRUSR|S_IWUSR))) < 0) {
			    (void) pfmt(stderr, MM_ERROR,PFMTTXT(_MSG_CANNOT_CREATE_TMP),
			    	strerror(errno));
			    return((FILE *)NULL);
			}			
			(void) close(fd);
			if ((tmp_fou = fopen(tmp_name, "w")) == NULL) {
				(void) pfmt(stderr, MM_ERROR,PFMTTXT(_MSG_CANNOT_WRITE_TMP),
					strerror(errno));
				return((FILE *)NULL);
			}
			if ((tmp_fin = fopen(tmp_name, "r")) == NULL) {
				(void) pfmt(stderr, MM_ERROR,PFMTTXT(_MSG_CANNOT_READ_TMP),
					strerror(errno));
				return((FILE *)NULL);
			}
			pipe_in = 1;
...
re_error()
{
	error(MSG_NUM(_MSG_PATTRN_NOT_FND),DEF_MSG(_MSG_PATTRN_NOT_FND));
...
register int n;
{
	...
	int errCode=0;			/* Holds regcomp() error status */
   	
	if (setjmp(restore) == 0) {
		errCode=regcomp(&pReg,buf,REG_EXTENDED |REG_NOSUB);
		if (errCode)	{ 

			/* regcomp() error handling */
		...
		while (find(1,direction) != END_COND){
			if (brk_hit)
				break;
		if (!regexec(&pReg,Line,(size_t) 0,NULL,0))	/* Regular expression check */
				if ((n -= direction) == 0) {
					switch(leave_search) {
					case 't':
...
save_pipe()
{
	if (!doliseof)
		while (fgetputc(stdin) != EOF)
			if (brk_hit) {
				brk_hit = 0;
				error(MSG_NUM(_MSG_PIPED_INPUT),DEF_MSG(_MSG_PIPED_INPUT));
				break;
			}
	(void) fclose(tmp_fou);
}
...
compact()
{
	(void) pfmt(stderr, MM_ERROR,PFMTTXT(_MSG_PG_OUT_MEM),dol->l_no);
	end_it();
}
...
terminit()	/* set up terminal dependencies from termlib */
{
	...
	if ((freopen("/dev/tty","r+",stdout)) == NULL) {
		(void) pfmt(stderr, MM_ERROR,PFMTTXT(_MSG_CANNOT_STDOUT),
			strerror(errno));
		exit(1);
	}
...
char *filename;
{
	...
	int pagenum;
	if (filename != NULL) {
		sopr(MSG_NUM(_MSG_PG_NXT_FILE),DEF_MSG(_MSG_PG_NXT_FILE),1);
