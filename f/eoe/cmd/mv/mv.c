...
static	char cmd_label[25] = "UX:??";  

static	char xmsgbuf[PATH_MAX+80];	/* buffer for message */

/* some often used strings */

/* static	char x_directory[] = DEF_MSG(_MSG_MMX_directory); */
/* static	char x_file[] = DEF_MSG(_MSG_MMX_file); 	*/
 ...
 */
static char opt_cp[]   = DEF_MSG(_MSG_MV_CP_USAGE1); 
static char opt_mv[]   = DEF_MSG(_MSG_MV_MV_FI); 
static char opt_ln[]   = DEF_MSG(_MSG_MV_LN_FIS); 
static char xx_usage[] = DEF_MSG(_MSG_MMX_usagespc);

static struct xxtext xxx_txt[] = {
	{ MSG_NUM(_MSG_MMX_Usage), DEF_MSG(_MSG_MMX_Usage) },  
	{ opt_mv,		0		},
	{ MSG_NUM(_MSG_MMX_file1), DEF_MSG(_MSG_MMX_file1) },  
 	{ MSG_NUM(_MSG_MMX_file2), DEF_MSG(_MSG_MMX_file2) },  
	{ 0,			0		}
};
static struct xxtext xxx_txt2[] = {
	{ MSG_NUM(_MSG_MMX_usagespc), xx_usage	},
	{ opt_mv,		0		},
	{ MSG_NUM(_MSG_MMX_file1toN), DEF_MSG(_MSG_MMX_file1toN)},  
	{ MSG_NUM(_MSG_MMX_dir),      DEF_MSG(_MSG_MMX_dir)	},  
	{ 0,			0		}
};
static struct xxtext mv_txt3[] = {
	{ MSG_NUM(_MSG_MMX_usagespc), xx_usage	},
	{ opt_mv,		0		},
	{ MSG_NUM(_MSG_MMX_dir1), DEF_MSG(_MSG_MMX_dir1)	},  
	{ MSG_NUM(_MSG_MMX_dir2), DEF_MSG(_MSG_MMX_dir2)	},  
	{ 0,			0		}
};
...
register char *argv[];
{
	...
	if (EQ(cmd, cmd_mv))
		...
		_sgi_ffmtmsg(stderr, 0, cmd_label, MM_ERROR, "%s",
			GETTXT(_MSG_MMX_whoami));  
		_sgi_ffmtmsg(stderr, 0, cmd_label, MM_FIX, "%s",
			GETTXT(_MSG_MMX_cpmvln)); 
		return(1);
	}
	...
	if (argc > 2) {
		...
		if (!ISDIR(s2)) {
		    if (!rflag && !Rflag) {
			_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
			    GETTXT(_MSG_MMX_Targetmusbe),  
			    argv[argc-1],
			    GETTXT(_MSG_MMX_directory));
			usage(1);
		    }
		}
	}
...
int yn;			/* yes or no default */
{
	...
	do {
		...
		MSG_NUM(_MSG_MMX_query),MSG_NUM(_MSG_MMX_querydef),
		MSG_NUM(_MSG_MMX_yes),MSG_NUM(_MSG_MMX_no));
...
char *source, *target;
{
	...
	if (!symlnk && ISDIR(s1) && !mve && !rflag && !Rflag) {
		_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
		    cpy ?
			GETTXT(_MSG_MMX_Cannotcopy) 
		    :
			GETTXT(_MSG_MMX_Cannotlink), 
		    GETTXT(_MSG_MMX_directory),
		    source);
		return(1);
	}
	...
	if (cpy && do_rtime) {
		...
		if ((rtextsize = xfsrtextsize( ptr )) <= 0 ) {
			Error1(GETTXT(_MSG_MV_NO_XFS_REAL_TIME_PARTITION)); 
			return(1);
		}

		if ((rt_size != -1) && (rt_size % rtextsize)) {
			_sgi_nl_error(SGINL_NOSYSERR, cmd_label, 
			    GETTXT(_MSG_MMX_badextsize),
			    rt_size, rtextsize);
			return(1);
		}
	}
	...
	if (targetexists) {
		...
		if (stat64(target, &s2) >= 0
			...
			if(mve && ISDIR(s1) && !ISDIR(s2)) {
				_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
				    GETTXT(_MSG_MMX_cannot_owr), 
				    GETTXT(_MSG_MMX_file),
				    target);
				return(1);
			}
...
skip:
	...
	if (lnk) {
		...
		if (errno == EXDEV) {
			_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
			    GETTXT(_MSG_MMX_diff_filsys), 
			    source, target);
			return (1);
		}
		_sgi_nl_error(SGINL_SYSERR2, cmd_label,
		    GETTXT(_MSG_MMX_Cannotlinkto), 
		    target,
		    source);
		result = 1;
	    }
	...
	if (mve) {
		...
		if((ret = nftw64(source, nftw_cp, OPEN_MAX, 
			...
			_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
			    GETTXT(_MSG_MMX_xpg_mv_partdest),target); 
			mve = 1;
			cpy = pflag = Rflag = 0;
			umask(mask);
			return(1);
		...
		if((ret = nftw64(source, nftw_rm, OPEN_MAX, 
				FTW_DEPTH|FTW_PHYS|FTW_MOUNT)) != 0 ) {
			_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
			    GETTXT(_MSG_MMX_xpg_mv_partsrc),source); 
			return(1);
		}
	    ...
	    if(!ISREG(s1))
	    {
			...
			_sgi_nl_error(SGINL_SYSERR2, cmd_label,
				GETTXT(_MSG_MMX_CannotAccess), target); 
			return(1);
	    	}
	    	...
	    	if( rm_error ) {
	    		_sgi_nl_error(SGINL_SYSERR2, cmd_label,
				GETTXT(_MSG_MMX_Cannotunlink), target); 
	    		return (1);
	    	}
	    	if( create_error ) {
	    		_sgi_nl_error(SGINL_SYSERR2, cmd_label,
	    			GETTXT(_MSG_MMX_CannotCreat), target); 
	    		return (1);
	    	}
		if(utime(target, &times) < 0)
			_sgi_nl_error(SGINL_SYSERR2, cmd_label,
			    GETTXT(_MSG_MMX_xpg_mv_utime),target); 
		
		if(chown(target, FUSER(s1), FGROUP(s1)) < 0) {
			_sgi_nl_error(SGINL_SYSERR2, cmd_label,
			    GETTXT(_MSG_MMX_xpg_mv_chown), 
			    target, FUSER(s1), FGROUP(s1));
			if(chmod(target, FMODE(s1)&(~(S_ISUID|S_ISGID))) < 0)
				_sgi_nl_error(SGINL_SYSERR2, cmd_label,
				    GETTXT(_MSG_MMX_xpg_mv_chmod), 
				    target, FMODE(s1));
		}
	...
	if(!do_dio || !cpy) do_dio=0;
		...
		if( (fcntl(from,F_SETFL,0 | FDIRECT)) == EINVAL ) {
			if(verbose) {
				_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
				    GETTXT(_MSG_MMX_nodiosupp), source);
			}
			do_dio=0;
			close(from);
			from =  open(source, 0);
			...
			if( (fcntl(from,F_DIOINFO, &from_dio)) == -1 ) {
				if(verbose) {
					...
					GETTXT(_MSG_MMX_dioerror), source);
				}
				do_dio=0;
				close(from);
				from = open(source,0);
...
again:
	...
	if(do_dio) {
		if((fcntl(to,F_SETFL,FDIRECT)) == EINVAL) {
			if(verbose) {
				_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
				    GETTXT(_MSG_MMX_nodiosupp), target);
			}
			do_dio=0;
			close(from);
			from = open(source,0);
			...
			if( (fcntl(to,F_DIOINFO, &to_dio)) == -1 ) {
				if(verbose) {
					_sgi_nl_error(SGINL_SYSERR, cmd_label,
					GETTXT(_MSG_MMX_dioerror), target);
				}
				do_dio=0;
				close(from);
				from = open(source,0);
	...
	if( !do_dio) {
		...
		if(verbose>1) {
			_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
				GETTXT(_MSG_MMX_diosizes), dio_min,dio_max);
		}
		...
		if (do_rtime) {
			...
			if (fcntl(to, F_FSSETXATTR, &fsxattr) < 0) {
				_sgi_nl_error(SGINL_SYSERR2, cmd_label,
					GETTXT(_MSG_MMX_rtiofail), target);
			}
		}
	}
	...
	for (extent = 0; extent < nextents; extent++) {
		...
		if (cpy_holes && (outmap[extent].bmv_block == -1)) {
			...
			if (fcntl(to, F_UNRESVSP64, &space) < 0) {
				Error1(GETTXT(_MSG_MV_TRUNCATING_EXTENT)); 
			}
		...
		if (do_prealloc) {
			...
			if (fcntl(to, F_RESVSP64, &space) < 0) {
				Error1(GETTXT(_MSG_MV_RESERVING_FILE_SPACE));
			}
		}
#endif
		...
		for (cnt = outmap[extent].bmv_length; cnt > 0;
			...
			if (mmap_in) {
				...
				if (mmap_buf == MAP_FAILED) {
					perror(GETTXT(_MSG_MV_ON_MMAP_CALL)); 
					exit(1);
				}
		...
		if (dp == NULL) {
			...
			if (made_dir) {
				...
				if (chmod(to, FMODE(f_sbuf)&(~mask)) < 0) {
					...
					if(utime(to, timep) < 0)
						_sgi_nl_error(SGINL_SYSERR2, cmd_label,
						    GETTXT(_MSG_MMX_xpg_mv_utime),to); 
        ...
        switch(nftw_flag)
        {
			...
			*b = 0;
			...
			if(utime(dir_path, &times) < 0)
				_sgi_nl_error(SGINL_SYSERR2, cmd_label,
				    GETTXT(_MSG_MMX_xpg_mv_utime),dir_path); 
			...
			_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
			    GETTXT(_MSG_MMX_xpg_mv_dirnoread),path); 
                        return(1);
                case FTW_NS:    /* unknown type, stat failed */
			_sgi_nl_error(SGINL_SYSERR2, cmd_label,
			    GETTXT(_MSG_MMX_cannotstat),path); 
                        return(1);
        }
	return(1);
}
...
nftw_rm(const char *path, const struct stat64 *snftw, int nftw_flag, struct FTW *Snftw)
{
        switch(nftw_flag)
        {
			...
			if(unlink(path) < 0) {
				_sgi_nl_error(SGINL_SYSERR2, cmd_label,
				    GETTXT(_MSG_MMX_Cannotunlink), path); 
			}
			return(0);
			...
			if(rmdir(path) < 0) {
				if(errno == EEXIST)
					errno = ENOTEMPTY;
				_sgi_nl_error(SGINL_SYSERR2, cmd_label,
				    GETTXT(_MSG_MMX_xpg_mv_rmdir),path); 
			}
			return(0);
...
xdev_quit(sig)
{
	if(xdev_state == 1)
		_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
		    GETTXT(_MSG_MMX_xpg_mv_partdest),xdev_target); 
	else 
		_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
		    GETTXT(_MSG_MMX_xpg_mv_partsrc),xdev_source); 
	exit(1);
}
...
attr_replicate(int src_fd, int dst_fd, int count)
{
	...
	for (k = 0; k < count; k++) {
		if (attr_ops[k].am_error) {
			printf(GETTXT(_MSG_MV_GETERR_ATTRIBUTE), 
				attr_ops[k].am_error);
			break;
		}
	...
	if (attr_multif(dst_fd, attr_ops, k, 0) < 0) {
		perror(GETTXT(_MSG_MV_ATTR_MULTIF)); 
	}
	for (j = 0; j < k; j++) {
		if (attr_ops[j].am_error) {
			printf(GETTXT(_MSG_MV_SETERR_ATTRIBUTE), 
				attr_ops[j].am_error);
			return(-1);
		}
	}
                      ...
                      char *attr_buf, int buf_len, int flags)
{
	...
	char	*ptr;
        ...
        do {
			...
			perror(GETTXT(_MSG_MV_ATTR_LISTF)); 
                        return(1);
		}
...
#define	BUMP_CNT	\
	...
	do {
		if (fcntl(fd, F_GETBMAP, map) < 0) {
			Error1(GETTXT(_MSG_MV_XFS_EXTENTS)); 
			exit(1);
		}
