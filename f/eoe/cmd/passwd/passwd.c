...
static
const char 
	*sorry = MSGNUM_DEFMSG(_MSG_SORRY),
	*again = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_364), /* Try again later */
/* usage message of non-password administrator */
 	*usage  = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_362), /* Usage: passwd [-s] [name] */
	*MSG_NP = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_354), /* Permission denied */
	*MSG_NV = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_356), /* Invalid argument to option -%c*/
	*MSG_BS = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_357), /* Invalid combination of options */
	*MSG_FE = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_815), /* Unexpected failure. Password file(s) unchanged. */
	*MSG_FF = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_816), /* Unexpected failure. Password file(s) missing. */
	*MSG_FB = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_360), /* 360:Password file(s) busy. */
	*MSG_UF = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_817), /* Unexpected failure. */
	*MSG_UI = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_818), /* Unknown logname: %s */
	*MSG_AD = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_361), /* Password aging is disabled */
	*MSG_NO = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_819), /* Password may only be changed at login time */
	*badusage = MSGNUM_DEFMSG(_MSG_INCORRECT_USAGE), /* Incorrect usage */
	*badentry = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_365), /* Bad entry found in the password file. */
	*pwgenerr = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_820), /* Must select an offered value*/
	*pwgenmsg = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_821), /* Select a password from the following */
/* usage message of password administrator */
 	*sausage  = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_363); /* Usage:\n\tpasswd [name]\n\tpasswd  [-l|-d]  [-n min] [-f] [-x max] [-w warn] name\n\tpasswd -s [-a]\n\tpasswd -s [name] */
		...
		*/
		if (!flag)
			(void) pfmt(stderr, MM_INFO,
				PFMTTXT(_MSG_UXCORE_ABI_366), 
				uname); /* Changing password for %s */
	}
	else
		uname = argv[optind];
	...
	char	*uname;
{
	...
	*/
	if (!adm && *mastp->ia_pwdp) {
		if ((pswd = getpass(GETTXT(_MSG_UXCORE_ABI_378))) == NULL) { /* Old password */
	...
	*/
	...
	if (!adm && ((iop = get_pwvalidate()) != NULL))
	{
		...
		{
			fprintf(stderr,"%s", GETTXT(_MSG_PASSWD_NOT_ACCEPTABLE));
			fflush(stderr);
			return err;
		}
	}
...
static	const	char
	*tshort = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_898), /* Password is too short - must be at least %d characters */
	*nocirc = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_371), /* Password cannot be circular shift of logonid */
	*s_char = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_372), /* Password must contain at least two alphabetic characters\n\tand at least one numeric or special character. */
	*df3pos = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_373), /* Passwords must differ by at least 3 positions */
	*tomany = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_375), /* Too many tries */
	*nmatch = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_376), /* They don't match */
	*tagain = MSGNUM_DEFMSG(_MSG_UXCORE_ABI_377); /* Try again. */
		...
		*pwbuf;
{
	...
	while (!strlen(buf)) {
		if (insist >= 3) {       /* 3 chances to meet triviality */
			...
			cap_surrender(ocap);
			(void)pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_UXCORE_ABI_368));
			(void)pfmt(stderr, MM_ACTION, again);
			return NOPERM;
		}
		if ((pswd = getpass(GETTXT(_MSG_UXCORE_ABI_369))) == NULL) {
			(void) pfmt(stderr, MM_ERROR, sorry);
			return FMERR;
		}
		else {
		...
		*/
		if ((pswd = getpass(GETTXT(_MSG_UXCORE_ABI_822))) == NULL) {
			(void) pfmt(stderr, MM_ERROR, sorry);
			return FMERR;
		}
		else  {
	...
	*/
	if (((status >> 8) & 0377) == 0) {
		while (lcnt) {
			...
			(void) printf("\n");
			pswd = getpass(GETTXT(_MSG_UXCORE_ABI_369));
			if (!strlen(pswd)) {
...
char * pw;	/* clear passwd */
{
	...
	while ((linep = fgets(linep,sizeof(line),hfp)) != NULL)
	{
		...
		{
		    fclose(hfp);
		    fprintf(stderr,GETTXT(_MSG_PASSWD_REUSED_OLD));
		    return 1;
		}
		/* date it changed */
		if ( (pf2 = strtok(NULL,":\n")) == NULL)
