...
#ident	"$Header: /proj/irix6.5f/isms/eoe/cmd/mesg/RCS/mesg.c,v 1.5 1998/09/18 19:47:22 sherwood Exp $"
/*
...
char *argv[];
{
	...
	if (stat(tty, &sbuf) < 0)
		(void)error(MSGNUM_DEFMSG(_MSG_CANT_ACCESS_C_E),
		      tty, strerror(errno));
	if (argc < 2) {
...
{
	if (chmod(tty, m) < 0)
		(void)error(MSGNUM_DEFMSG(_MSG_CANT_CHNG_MODE_C_E),
		      tty, strerror(errno));
}
...
int complain;
{
	...
	pfmt(stderr, MM_ACTION, PFMTTXT(_MSG_MESG_USAGE));
	exit(2);
}
