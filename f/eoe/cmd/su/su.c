...
#define CHECK_MALLOC(_x) \
	if ((_x) == NULL) { \
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_MALLOC_FAILED)); \
		exit(1); \
	}
#endif /* sgi */
...
main(int argc, char *argv[])
{
	...
	while ((c = getopt (argc, argv, "mM:C:S")) != EOF)
	{
		switch (c)
		{
			...
			case '?':
				...
				{
					pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SU_USAGE_MESSAGE));
					exit(1);
				}
				optoffset = 2;
				break;
	...
	if (cap_envp(0, Sflag ? privs_size : privs_size - 1, privs) == -1) {
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_INSUFFICIENT_PRIVILEGE));
		exit(1);
	}
	...
	if (myname == NULL)
		myname = getlogin();
	if (myname == NULL) {
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_CANT_RECOGNIZE));
		exit(1);
	}
	...
	if (cap_enabled)
	{
		...
		if (newcap == (cap_t) NULL)
			err_out(errno, 1, GETTXT(_MSG_INVALID_CAPABILITY), Carg);
		...
		if (Carg != DEFCAP && !cap_user_cleared (nptr, newcap))
		{
			...
			satvwrite(SAT_AE_IDENTITY, SAT_FAILURE,
				GETTXT(_MSG_SU_USER_NOT_CLEARED),
				myname, nptr);
			cap_surrender(ocap);
			err_out(EPERM, 2, MSGNUM_DEFMSG(_MSG_SORRY), "");
		}
	}
	...
	if (mac_enabled)
	{
		...
		if (proclabel == (mac_t) NULL)
			err_out(errno, 1, GETTXT(_MSG_INVALID_MACLBL), "");
	...
	if (Sulog && *Sulog) {
		...
		if (fd == -1) {
			...
			free(p);
			if (fd == -1) {
				pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SU_STAT_FAILED));
				exit(1);
			}
			...
			if (setreuid((uid_t) -1, dirbuf.st_uid) == -1) {
				cap_surrender(ocap);
				pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SU_SETREUID_FAILED));
				exit(1);
			}
			cap_surrender(ocap);
			...
			if (setregid((gid_t) -1, dirbuf.st_gid) == -1) {
				cap_surrender(ocap);
				pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SU_SETREGID_FAILED));
				exit(1);
			}
			cap_surrender(ocap);
			...
			if (mac_enabled) {
				...
				omac = mac_get_proc();
				if (omac == NULL) {
					pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SU_MAC_GET_PROC_FAILED));
					exit(1);
				}
				...
				if (mac_set_proc(&lbl) == -1) {
					cap_surrender(ocap);
					pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SU_MAC_SET_PROC_FAILED));
					exit(1);
				}
				cap_surrender(ocap);
			}
			...
			cap_surrender(ocap);
			if (fd == -1) {
				pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SU_OPEN_SULOG_FAILED));
				exit(1);
			}
	...
	if (ia_openinfo(nptr, &uinfo) || (uinfo == NULL) || strlen(nptr) >= ENV_SIZE) {
		uid = -1;
		capv = CAP_AUDIT_WRITE, ocap = cap_acquire (1, &capv);
		ia_audit("SU", myname, 0, GETTXT(_MSG_INVALID_USERNAME));
		cap_surrender(ocap);
		err_out(0, 1, MSGNUM_DEFMSG(_MSG_UNKNOWN_USERID_S), nptr);
	}
	...
	if (ask_prompt(ia_pwdp, uid)) {
		password = getpass(GETTXT(_MSG_PASSWORD));
		if (password == (char *) NULL ||
		    strcmp(ia_pwdp, crypt(password, ia_pwdp)) != 0) {
			capv = CAP_AUDIT_WRITE, ocap = cap_acquire (1, &capv);
			satvwrite(SAT_AE_IDENTITY, SAT_FAILURE,
				GETTXT(_MSG_SU_INCORRECT_PASSWORD),
				  myname, nptr);
			cap_surrender(ocap);
			err_out(EPERM, 2, MSGNUM_DEFMSG(_MSG_SORRY), "");
		}
...
ok:
	...
	capv = CAP_SETGID, ocap = cap_acquire (1, &capv);
	if (setgid(gid) != 0)
		err_out(errno, 2, MSGNUM_DEFMSG(_MSG_INVALID_GID), "");
	cap_surrender (ocap);
	...
	if (gidcnt) {
		...
		if (setgroups(gidcnt, ia_sgidp) == -1) {
			err_out(errno, 2, MSGNUM_DEFMSG(_MSG_INVALID_SUP_GID), "");
		}
		cap_surrender (ocap);
	}
	...
	satvwrite(SAT_AE_IDENTITY, SAT_SUCCESS,
	    GETTXT(_MSG_SU_USER_SUCCEEDED), myname, nptr);
	cap_surrender(ocap);
	...
	capv = CAP_SETUID, ocap = cap_acquire (1, &capv);
	if (setuid(uid) != 0)
		err_out(errno, 2, MSGNUM_DEFMSG(_MSG_INVALID_UID), "");
	cap_surrender (ocap);
	/*
	...
	if (mac_enabled)
	{
		...
		if (mflag) {
			...
			if (newlabel == NULL)
				err_out(errno, 2, MSGNUM_DEFMSG(_MSG_SORRY), "");
		}
	...
	pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_NO_SHELL), "");
	exit(3);
...
{
	char	*suprmt = "PS1=# ";
	static const char nomem[] = CATNUM_DEFMSG(_MSG_OUT_OF_MEM);
...
log(FILE *fptr, char *towho, int how, int flag)
{
	...
	if (flag && (syslog_success || (syslog_fail && !how))) {
		...
		syslog(LOG_NOTICE|LOG_AUTH, GETTXT(_MSG_SU_USER_CHANGING), 
			how ? GETTXT(_MSG_SU_SUCCEEDED) :GETTXT(_MSG_SU_FAILED),
			(ttyn + sizeof("/dev/") -1),
			myname ? myname : "", towho);
		closelog();
	}
...
err_out(int err, int astatus, char *alt_msg, char *arg)
{
	switch (err) {
	case EPERM:
		(void) pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SORRY), "");
		break;
	case EACCES:
