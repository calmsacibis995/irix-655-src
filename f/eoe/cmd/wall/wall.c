...
char *s;
{
	_sgi_nl_error(SGINL_SYSERR, cmd_label,
	    GETTXT(_MSG_MMX_CannotOpen), s); /* Cannot open %s */
	exit(1);
}
...
char *s;
{
	_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
	    GETTXT(_MSG_MMX_CannotRead), s); /* Cannot read from %s */
}
...
{
	_sgi_nl_error(SGINL_SYSERR, cmd_label,
	    GETTXT(_MSG_MMX_cannotstat), s); /* cannot stat %s */
}
...
{
	_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
	    GETTXT(_MSG_MMX_outofmem)); /* Out of memory */
	exit(1);
}
...
char	*argv[];
{
	...
	if(uname(&utsn) == -1) {
		_sgi_nl_error(SGINL_SYSERR, cmd_label,
		    GETTXT(_MSG_MMX_cannot_uname)); /* Cannot uname() */
		exit(2);
	}
	systm = utsn.nodename;
...
sendmes(struct utmp *p)
{
	...
	if (strchr(p->ut_line, '/') != NULL) {
		...
		_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
		    GETTXT(_MSG_MMX_wall_illdev),
		    &p->ut_user[0], s); /* Illegal device for %.-8s on %s */
		exit(1);
	}
#ifdef DEBUG
	...
	if(f == NULL) {
		_sgi_nl_error(SGINL_SYSERR2, cmd_label,
		    GETTXT(_MSG_MMX_wall_cnsnd),
		    &p->ut_user[0], s); /* Cannot send to %.-8s on %s */
		exit(1);
	}
	_sgi_ffmtmsg(f, 0, cmd_label, MM_INFO,
	    GETTXT(_MSG_MMX_wall_mesg),
	        who, line, systm, timstr);
	        /* Broadcast Message from %s (%s) on %s %s */
	if(group)
	    _sgi_ffmtmsg(f, 0, cmd_label, MM_INFO,
		GETTXT(_MSG_MMX_wall_mesggrp), grpname);
		/* To group %s */
...
readargs(int ac, char **av)
{
	...
	for(i = 1; i < ac; i++) {
		if(equal(av[i], "-g")) {
			...
			if (i >= ac) {
			    _sgi_nl_error(SGINL_NOSYSERR, cmd_label,
				GETTXT(_MSG_MMX_wall_neddgrp));
				    /* -g needs a group name */
			    exit(1);
			}
			if((pgrp=getgrnam(grpname= av[i])) == NULL) {
			    _sgi_nl_error(SGINL_NOSYSERR, cmd_label,
				GETTXT(_MSG_MMX_unkngroupid), grpname);
				    /* unknown group id: %s */
