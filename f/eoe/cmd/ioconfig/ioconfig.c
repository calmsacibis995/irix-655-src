...
/*
 * List of directories to skip in walk_fn().  Directories can be regexp's
 * which are matched using fnmatch(3).
 */
	...
	struct 	FTW		*ftw_info) 
{
    ...
    int			num, i;
    int			inv_size = 10 * sizeof(inventory_t);
    char		*namep = (char *)canonical_name, **cp;
    device_process_t	*dpt;

    /*
     * If obj_type is a directory, and it is in the list of skip_dir's, 
     * inform nftw to not traverse the dir, and return. 
     */

    if (obj_type == FTW_D) {
	for (cp = skip_dirs; *cp != NULL; cp++) {
		if (fnmatch(*cp, canonical_name, FNM_PATHNAME) == 0) {
			ftw_info->quit |= FTW_PRUNE;
			return 0;
		}
	}
    }
