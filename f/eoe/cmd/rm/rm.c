...
/* Modified to support 18n msg catalog */
...
static const char badopen[] = MSGNUM_DEFMSG(_MSG_CANNOT_OPEN);
...
main(int argc, char *argv[])
{
	...
	if ((argc < 1 && !silent) || errflg) {
		if (!errflg)
			...
			PFMTTXT(_MSG_INCORRECT_USAGE));
		(void)pfmt(stderr, MM_ACTION,
			PFMTTXT(_MSG_RM_USAGE));
		exit(2);
	}
	...
	while (argc-- > 0) {
		if (!strcmp(*argv, "..")) {
			pfmt(stderr, MM_ERROR,
				PFMTTXTCAT(_MSG_CANT_RM_DOTDOT));
			argv++;
			errcode++;
			continue;
		}
		...
		if ((arglen >= 3) && !strncmp(*argv + arglen - 3, "/..", 3)) {
			*(*argv + arglen - 3) = '\0';
			pfmt(stderr, MM_ERROR,
				PFMTTXTCAT(_MSG_CANT_RM_FILES_ABOVE),
				*argv);
			argv++;
			errcode++;
			continue;
...
rm(char *path)
{
	...
	if (interactive) {
                sprintf(xmsgbuf,
			GETTXTCAT(_MSG_MMX_rm_filrm), path);
	...
	if (unlink(path) == FAIL) {
		if (!silent || interactive) 
			pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_NOT_RMVD),
				path, strerror(errno));
...
undir(char *path, dev_t dev, ino64_t ino)
{
	 ...
	 */
	if (!recursive) {
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_NOT_RMVD_DIR),
			path); 
		++errcode;
		return;
	}
	...
	if (interactive && !isinpath) {
		sprintf(xmsgbuf,
			GETTXTCAT(_MSG_MMX_rm_dirrm_con), path);
		/*
	...
	while ((direct = readdir64(name)) != NULL) {
		...
		if (strlen(path) + strlen(direct->d_name) + 2 > maxpathlen) {
			pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_PATH_TOO_LONG),
	...
	case 2:
		pfmt(stderr, MM_ERROR,
			PFMTTXTCAT(_MSG_CANT_RM_CWD),
			path);
		++errcode;
		return;
	case 1:
		pfmt(stderr, MM_ERROR,
			PFMTTXT(_MSG_CANT_RM_DIR_IN_PATH), path);
		return;
	case 0:
                /*
		 ...
		 */
                ...
                if ((interactive || !WRITEPERM) && !silent) {
                        sprintf(xmsgbuf,
				GETTXTCAT(_MSG_MMX_rm_dirrm), path);
                        /*
                         * If the answer is no, skip the directory.
                         */
                        if (!query(xmsgbuf, SGINL_DEFNO))
	...
	if (rmdir(path) == FAIL) {
		if (errno != EEXIST)
			pfmt(stderr, MM_ERROR,
				PFMTTXT(_MSG_CANT_RM_DIR),
				path, strerror(errno));
		else
			pfmt(stderr, MM_ERROR,
				PFMTTXTCAT(_MSG_CANT_RM_DIR_NOT_EMPTY),
				path);
		++errcode;
	}
}
...
query(char *omsg, int yn)
{
		...
		i = _sgi_nl_query_fd(stdin, ibuf, yn,
			CAT_MSG_NUM(_MSG_MMX_query),
			CAT_MSG_NUM(_MSG_MMX_querydef),
			CAT_MSG_NUM(_MSG_MMX_yes),
			CAT_MSG_NUM(_MSG_MMX_no), stderr);
