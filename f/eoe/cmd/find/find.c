...
#ident	"cmd/find/find.c: $Revision: 1.49 $"

/*	find	COMPILE:	cc -o find -s -O -i find.c -lS	*/

/* Modified to support EUC multibyte/Sjis-Big5/Full multibyte	*/
/* 1. Modified e3() & fglob() to handle MB characters.		*/
/* 2. Modified ok() to take the affirmative response from the catalogue */
...
main(int argc, char *argv[])
{
	...
	if(time(&Now) == (time_t) -1) {
	    err_sys(GETTXT(_MSG_MMX_canttime));
	    exit(2);
	}
	/* Is it XPG4 ? */
	...
	Argc = argc; Argv = argv;
	if(argc < 2) {
	    err_nosys(GETTXT(_MSG_MMX_insuffargs));
	    usage();
	}
	...
	if (Ai < argc) {
	    if(!(exlist = exp())) {
		err_nosys(GETTXT(_MSG_MMX_parserror));
		exit(1);
	    }
	}
	else {
	...
	if(Ai < argc) {
	    err_nosys(GETTXT(_MSG_MMX_missconj));
	    exit(1);
	}
...
{
	if(Randlast) {
		err_nosys(GETTXT(_MSG_MMX_opfollop));
		exit(1);
	}
	Randlast++;
	if(EQ(nxtarg(), "!"))
...
e3(void)			/* parse parens and predicates */
{
	...
	if(EQ(a, "-name"))
		...
		if((i=(i_t)getunum(UID, b)) == -1) {
		    ...
		    err_id(GETTXT(_MSG_MMX_IllegalUid),
			b);
		    exit(1);
		}
		...
		if((i=(i_t)getunum(GID, b)) == -1) {
		    ...
		    err_id(GETTXT(_MSG_MMX_IllegalGid),
			b);
		    exit(1);
		}
		...
		i = s=='d' ? S_IFDIR :
		    ...
		    0;
		if( !i) {
		    b[mblen(b, MB_CUR_MAX)] = 0;
			
		    _sgi_nl_error(SGINL_NOSYSERR, cmd_label,
			GETTXT(_MSG_MMX_illfiletype), b);
		    exit(1);
		}
...
nxtarg(void)			/* get next arg from command line */
{
	...
	if(strikes==3) {
		err_nosys(GETTXT(_MSG_MMX_IncomplStmt));
		exit(1);
	}
	if(Ai >= Argc) {
		strikes++;
...
fglob(anode_t *p)
{
	...
	if (xpg4) {
		...
		register char *pBuff;
		register int nLen;
		int nLBFoundFlag = 0, nRBFoundFlag = 0;

		pBuff = s;
		if (pBuff)  {
		   while(*pBuff)   {
			nLen = mblen(pBuff, MB_CUR_MAX);	
			if (nLen == 1)	{
				if (*pBuff == '[') {
					nLBFoundFlag = 1;
					if (nRBFoundFlag)
				...
				*t++ = '^';
				while (s && *s) {
					if (*s == '*') {
						*t++ = '.';
						*t++ = '*';
					} else if (*s == '!' && (s-1) && !nMBFlag && *(s-1) == '[')
						*t++ = '^';	/* Map ! to ^ */
					else
						*t++ = *s;
					nLen = mblen(s, MB_CUR_MAX);	/* nLen cannot be -1 here */
					nMBFlag = (nLen-- == 1) ? 0: 1;
					s++;
					while (nLen-- && s && *s)
						*t++ = *s++;
...
fstype(anode_t *p)
{
	...
	if (statvfs64(statname, &svb) == -1) {
		fprintf(stderr, GETTXT(_MSG_MMX_find_cantvfs), Pathname);
		perror("");
		return 0;
	}
...
ok(anode_t *p)
{
	...
	fflush(stdout); /* to flush possible `-print' */
	(void) fprintf(stderr, GETTXT(_MSG_MMX_find_query), Argv[p->L.I], Pathname);
	fflush(stderr);
	if( (gets(mbStr) != NULL) && !strcmp(mbStr, GETTXT(_MSG_MMX_find_y)) )
		yes = 1;
...
cpio(anode_t *p)
{
	...
	if (Statb.st_size > LONG_MAX) {
		fprintf(stderr, GETTXT(_MSG_MMX_find_skipfile), hdr.h_name);
		fflush(stderr);
		return 0;
	}
...
doex(int com)
{
	...
	if(pid = fork())
		if (pid < 0)
			err_sys(GETTXT(_MSG_MMX_CannotFork));
		else
...
chgreel(int x, int fl)
{
	...
	_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
	    x? GETTXT(_MSG_MMX_CannotWr)
	    : GETTXT(_MSG_MMX_CannotRd));
	fstat64(fl, &statb);
	if((statb.st_mode&S_IFMT) != S_IFCHR)
		exit(1);
again:
	close(fl);
	_sgi_ffmtmsg(stderr, 0, cmd_label, MM_FIX, GETTXT(_MSG_MMX_find_GoOn));
