...
#ident  "dvhtool/dvhtool.c: $Revision: 1.46 $"
...
int firstfileblk = 2;	/* first blk in volhdr for files; should match fx
	...
	*
	* As of 1/27/98, sector 1 will be used for XVM physical disk label,
	* and will be managed by XVM code.  dvhtool and fx should never
	* modify sector 1 of any XVM disk.  --  David Holst
	*/
	...
	for(ind=0; ind<argc && argv[ind][0] == '-' && (c=argv[ind][1]); ind++) {
		...
		case 'v': /* manipulate volume directory */
			...
			case 'l': /* list volume directory entries */
				...
				break;
			case 'p': /*  pack volume directory entries */
				vd_arg[numvd_arg].opt = VD_PACK;
				/* Store the new block size in the dvh_file. */
				vd_arg[numvd_arg++].dvh_file = argv[++ind];
				readonly = 0;
				break;
			default:
...
again:
	for (vd_argp=vd_arg, i=0; i < numvd_arg; i++, vd_argp++) {
		...
		case VD_GET:
			...
			break;
		case VD_PACK:
			/*  Do all packing after deletes, just like creates */
			/*  and adds. */
			if(pass == 0)
				adds++;
			else {
				/* The blocksize was stored in dvh_file */
				vd_pack(atoi(vd_argp->dvh_file));
				vd_argp->opt = VD_NOP;	/* just in case...*/
			}
			break;
		}
	}
...
/*
 * Make sure volume header can be compressed to a smaller size.  Also, 
 * see if we need to defrag the existing files in order to make them fit
 * into a smaller volume header.
 */
void
vd_pack(int newblocksize)
{
	...
	vhpt = getvhpart();
	if (!vhpt) {
		printf("vd_pack:  Cannot find volume header\n");
	  	return;
	}

	shrunk = vhpt->pt_nblks - newblocksize;	/* space trimmed */
	avail = roundblk(vh_availspace()) / dev_bsize;  /* space available  */
							/* in blocks 	*/

	/* If the amount shrunk is greater then the amount available,  */
	/* then we have a problem, and cannot continue.			*/
	if (shrunk > avail) {
		printf("The files in the volume header will not fit if the volume header is shrunk\nto %d blocks.  Delete files to free up %d bytes in order to continue.\n", newblocksize, (shrunk - avail) * dev_bsize);
		cleanup(1);
	}

	/*  At this point, we need to see if we have to defrag */

	/* For each file, figure out the ending block.  If the ending   */
	/* block is larger then the newblocksize, then we need to 	*/
	/* defrag the volume directory.					*/
	/* To do this,set vhdirty and needcopy.  vhdirty is necessary   */
	/* to get to the putdvh routine.				*/
	
        for(vd = vh.vh_vd; vd < &vh.vh_vd[NVDIR]; vd++) {
		endblk = vd->vd_lbn + roundblk(vd->vd_nbytes) / dev_bsize;
		if (endblk > newblocksize) {
			printf("Defragmenting volume header files\n");
			vhdirty = 1;	
			needcopy = 1;
			return;
		}
        }
        /* If we get here, then everything is fine, and we don't have 	*/
	/* to do any work */

 	printf("Volume header packed\n");
}
