...
#ident	"$Header: /proj/irix6.5f/isms/eoe/cmd/stty/RCS/stty.c,v 1.25 1998/09/18 19:47:22 sherwood Exp $"
...
char	*argv[];
{
	...
	switch(argv[1][1]) {
		...
		case 'g':
			...
			exit(0);
		default:
			fprintf(stderr, "%s", CATGETS(catd, _MSG_STTY_USAGE));
			exit(2);
	}
	while(--argc > 0) {		/* set terminal modes for supplied options */
		...
		if(!match)
			if(!encode()) {
				fprintf(stderr, "%s %s\n", 
				 CATGETS(catd, _MSG_STTY_UNKNOWN_MODE), arg);
				exit(2);
			}
	}
	if (term == ASYNC) {
