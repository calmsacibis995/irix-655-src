...
#ident	"$Header: /proj/irix6.5f/isms/eoe/cmd/rmdir/RCS/rmdir.c,v 1.7 1998/09/18 19:47:22 sherwood Exp $"
/*
...
/*************************I18N File Header***************************
 File:            rmdir.c
 Change History:  27 Feb. 1998  cnm
                  Changed existing MNLS cataloging calls to use new
                  macros
*********************************************************************/
...
char **argv;
{
	...
	argv = &argv[optind];
		...
		if( !(rc = rmdirp(path, remain))) {
			...
			_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
			    GETTXT(_MSG_MMX_pathremoved), ptr);
			...
			switch(rc) {
			    case -1:
				if(errno == EEXIST)
				    msg = GETTXT(_MSG_MMX_dirnotempty);
				    /*  "Directory not empty" */
				else
				...
				break;
			    case -2:
				msg = GETTXT(_MSG_MMX_cantrmdotdot);
				/* "Can not remove . or .." */
				break;
			    case -3:
				msg = GETTXT(_MSG_MMX_cantrmcwd);
				/* "Can not remove current directory" */
				break;
			}
			_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
			    GETTXT(_MSG_MMX_notremoved), ptr, remain);
			    /* "%s: %s not removed" */
		...
		switch(errno) {
			...
			break;
		    case ENOTDIR:
			msg = GETTXT(_MSG_MMX_patcompnotdir);
			/* "Path component not a directory" */
			break;
		    case ENOENT:
			msg = GETTXT(_MSG_MMX_dirnotexist);
			/* "Directory does not exist" */
			break;
		    case EACCES:
			msg = GETTXT(_MSG_MMX_nowrperm);
			/* "Search or write permission needed" */
			break;
		    case EBUSY:
			msg = GETTXT(_MSG_MMX_mountpoint);
			/* "Directory is a mount point or in use" */
			break;
		    case EINVAL:
			msg = GETTXT(_MSG_MMX_cantrmcwddot);
