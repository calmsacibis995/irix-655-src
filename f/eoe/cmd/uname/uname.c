...
/*	strftime() in version_info() function is used to get	*/
...
int argc;
{
	...
	if (Vflg) {
		...
		if (outstrlen > 0) {
			...
			(void) fprintf(stderr,
				GETTXT(_MSG_UNAME_INVALID_INST_VER),vnum_in);
			exit(1);
		}
	}
	if (Rflg) {
		if(syssgi(SGI_RELEASE_NAME, sizeof(releasename), releasename)) {
			perror(GETTXT(_MSG_UNAME_UNDET_EXT_RELEASE));
			*releasename = '\0';
		}
	}
#endif /* Vflg */
		...
		int curlen, i;
			    ...
			    PFMTTXT(_MSG_NAME_N_LETTERS),SYS_NMLN-1);
                        exit(1);
                }
		...
		if (sysinfo(SI_SET_HOSTNAME, nodename, SYS_NMLN) == -1) {
			pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SYSINFO_FAILED),
				strerror(errno));
			exit(1);
		}
#else /* !__sgi */
...
static char usage_str[] =	\
    DEF_MSG(_MSG_UNAME_USAGE_STRING);
#else
#if u3b2 || i386
static char usage_str[] = DEF_MSG(_MSG_UNAME_USAGE_I386_FORMAT);
...
int err;
{
	if (!err)
        ...
        (void) pfmt(stderr, MM_ACTION, PFMTTXT(_MSG_UNAME_USAGE_FORMAT), usage_str);
...
do_alphaversion(char *release_str)
{
	...
	if (!release_str) {
		fprintf(stderr, GETTXT(_MSG_UNAME_DO_ALPHAVERSION_NULL));
		exit(1);
	}
	if ((rstrlen = strlen(release_str)) <= 0) {
		fprintf(stderr, GETTXT(_MSG_UNAME_RELEASE_STR_LENGTH), rstrlen);
		exit(1);
	}
	...
	if (dflg && !aflg) {
		(void)fprintf(stderr,
		GETTXT(_MSG_UNAME_MISSING_VER_NO),
			release_str);
	}
	return(-1);
...
char *builders[] =	{ DEF_MSG(_MSG_UNAME_DEVELOPER), DEF_MSG(_MSG_UNAME_PROJECT), DEF_MSG(_MSG_UNAME_BUILD_GROUP) };
...
version_info(long version_num, char *outbuf, int max_outbuf_len)
{
    ...
    if (!outbuf) {
	fprintf(stderr, GETTXT(_MSG_UNAME_NULL_OUTBUF_PTR));
	fflush(stderr);
	exit(-3);
    }
    outbuf[0] = '\0';
	...
	ob_len = sprintf(obp, GETTXT(_MSG_UNAME_INVALID_VERS),
	...
	ob_len = sprintf(obp,
		GETTXT(_MSG_UNAME_MATCHED_FIELDS_NOT2),
		matchcnt);
	return(-ob_len);
    }
    if ((matchcnt = sscanf(version_str, "%3d%5d%1d%1d", &major, &alpha,
		&builder, &tree)) != 4) {
	ob_len = sprintf(obp,
		GETTXT(_MSG_UNAME_MATCHED_FIELDS_NOT4),
		matchcnt);
	return(-ob_len);
    }
...
#endif
	obp += sprintf(obp, GETTXTCAT(_MSG_UNAME_PATCH_BASED_RELEASE),
	...
	if (builder < 0 || builder >= 3)
	    sprintf(bname, GETTXT(_MSG_UNAME_INVALID_BUILDER), builder);
	else
	    sprintf(bname, "%s", builders[builder]);

	obp += sprintf(obp, GETTXT(_MSG_UNAME_RELEASE_BY), major, date_str, bname);
	if (tree)
	    obp += sprintf(obp, GETTXT(_MSG_UNAME_IN_TREE), tree, version_num);
	else
...
old_anums(long vnum, char *version_strp, char *bufp, int max_bstr_len)
{
    ...
    if (!bufp) {
	fprintf(stderr, GETTXT(_MSG_UNAME_NULL_BUFP), vnum);
	exit(1);
    }
    bufp[0] = '\0';
	...
	bstrlen = sprintf(bufp,
		GETTXT(_MSG_UNAME_MATCHED_FIELDS_NOT4_OLD_ANUMS),matchcnt);
	return(-bstrlen);
    }
#if ADEBUG
	...
	cp += sprintf(cp, GETTXT(_MSG_UNAME_MAJOR_AND_MINOR), major, minor);

	/* if 'maint' is nonzero this is a maintenance release */
	if (maint)
	    cp += sprintf(cp, GETTXT(_MSG_UNAME_MAINTENANCE_RELEASE), maint);

	cp += sprintf(cp, DEF_MSG(_MSG_UNAME_ALPHA_VERSION), alpha, vnum);
