...
*/
 ...
 /*                                        None needed since not changed.     */
 /* Change History : 17th July 1997    HCL                                    */
 /*                  Catalogs incorporated.                                   */
 /*                  Verified that no change is needed for MB chars support.  */
 /*                  6th October 1997  HCL                                    */
 /*                  Changes according to new coding and cataloging guidelines*/
...
char **argv;
{
	...
	case 2:
		...
		if( (input = fopen( argv[1], "r" )) == NULL )
			/* I18NCAT_PGM_MSG */
			errmsg(CATGETS(catd, _MSG_TSORT_NOT_OPEN), argv[1]);
		break;
	case 3:
		if( strcmp( argv[1], "--" ) ) 
			/* I18NCAT_PGM_MSG */
			errmsg(CATGETS(catd, _MSG_TSORT_USAGE), "");
		if( !strcmp( argv[2], "-" ) )
			break;
		if( (input = fopen( argv[2], "r" )) == NULL )
			/* I18NCAT_PGM_MSG */
			errmsg(CATGETS(catd, _MSG_TSORT_NOT_OPEN), "");
		break;
	default:
			/* I18NCAT_PGM_MSG */
		errmsg(CATGETS(catd, _MSG_TSORT_USAGE), "");
	...
	for(;;) {
		...
		if(x!=2)
			/* I18NCAT_PGM_MSG */
			errmsg(CATGETS(catd, _MSG_TSORT_ODD_DATA), "");
		i = index(precedes);
		j = index(follows);
		if(i==j||present(i,j)) 
			continue;
...
findloop()
{
	...
	for(i= &firstnode; i->nextnode!=NULL; i=i->nextnode)
		if(i->live==LIVE)
			break;
	/* I18NCAT_PGM_MSG */
	warn(CATGETS(catd, _MSG_TSORT_CYCLE_DATA), "");
	i = mark(i);
	if(i==NULL)
		/* I18NCAT_PGM_MSG */
		errmsg(CATGETS(catd, _MSG_TSORT_PROG_ERROR), "");
...
{
	/* I18NCAT_PGM_MSG */
	(void)fprintf(stderr,CATGETS(catd, _MSG_TSORT_WARNING), s, t);
	(void)fflush(stderr);
}
...
unsigned int	n;
{
	...
	if( (p = (char *)malloc(n)) == NULL ) {
		/* I18NCAT_PGM_MSG */
		(void)sprintf(tmp, CATGETS(catd, _MSG_TSORT_MEM_ERROR), n);
		errmsg(tmp, "");
	}
