...
#define MBISPRINT(c,n)   ( (n>1) && !ISASCII(c) || isprint(c)) /* Mutibyte equivalent of ISPRINT */
...
static const char nomem[] = MSGNUM_DEFMSG(_MSG_OUT_OF_MEM);
static const char usage[] = MSGNUM_DEFMSG(_MSG_LS_USAGE);
...
int mbScrLength(char *);	/* Screen Lenght of Multibyte String 	*/
int mbIsPrint(char *);	/* Multibyte Version io isprint() 	*/
...
main(int argc, char **argv)
{
	...
	for (i = 0; i < (amino ? amino : 1); i++) {
		if (SHOWLVL() || Cflg || mflg) {
             		width = mbScrLength(argv[optind]); /* (Screen Length of Multibyte String) */
			if (width > filewidth)
				filewidth = width;
		}
		...
		if (Mflg && (Cflg || mflg) && ep->label) {	
			width += mbScrLength(ep->label)+3; 
			if (width > filewidth)
				filewidth = width;
		}
		if (Pflg && (Cflg || mflg) && ep->cap) {	
			width += mbScrLength(ep->cap) + 3;
			if (width > filewidth)
				filewidth = width;
		}
		if (Dflg && (Cflg || mflg)) {
			if (ep->acl)
				width += mbScrLength(ep->acl) + 3;		
			if (ep->dacl)
				width += mbScrLength(ep->dacl) + 3;	
			if (ep->acl && ep->dacl)
				width -= 2;
			if (width > filewidth)
				filewidth = width;
...
pentry(lbuf_t *ap)  
{
	...
	if (lflg) {
		...
		if (timespeccmp(&p->lmtime, &year) < 0 ||
			...
			if (!id1)
				id1 = gettxt(MSG_NUM(_MSG_LS_DATE_FMT),DEF_MSG(_MSG_LS_DATE_FMT)); 
			cftime(time_buf, id1, &p->lmtime.tv_sec);
			curcol += printf("%s", time_buf);
		} else {
			if (!id2)
				id2 = gettxt(MSG_NUM(_MSG_LS_DATE_TIME_FMT),DEF_MSG(_MSG_LS_DATE_TIME_FMT)); 
	...
	if (qflg || bflg)
		pprintf(p->lnamep, midstr, dmark);
			...
			printf("%s%s%s", p->lnamep, midstr, dmark);
			curcol += mbScrLength(p->lnamep); 
			curcol += strlen(midstr);
			curcol += strlen(dmark);
	...
	if (Dflg) {
		...
		if (ap->acl) {
			printf("%s",ap->acl);
			curcol += mbScrLength(ap->acl);
			free (ap->acl);
			ap->acl = NULL;
		}
		if (ap->dacl) {
			printf("/%s", ap->dacl);
			curcol += mbScrLength(ap->dacl)+1;
			free (ap->dacl);
			ap->dacl = NULL;
		}
		curcol += printf("]");
...
rddir(char *dir)
{
	...
	if ((dirf = opendir(dir)) == NULL) {
		fflush(stdout);
		pfmt(stderr, MM_ERROR,PFMTTXT(_MSG_CANT_ACCESS_DIR),
			dir, strerror(errno));
		err = 2;
		return;
	}
	...
	while (dentry = readdir64(dirf)) {
		...
		if (aflg == 0 && dentry->d_name[0] == '.' &&
			...
			continue;
		if (SHOWLVL() || Cflg || mflg) {
             		width = mbScrLength(dentry->d_name);	
			if (width > filewidth)
				filewidth = width;
		}
...
pprintf(char *s1, char *s2, char *s3)
{
	...
	for (s = s1, i = 0; i < 3; i++) {
		while (c = (unsigned char)*s) {
			nChar = mblen(s,MB_CUR_MAX); /* Get Lenght of Multibyte Character */
			if ((i != 1) &&
				( !MBISPRINT(c,nChar) || (bflg && (c == ' ')))) {
				if (qflg)
					c = '?';
				else if (bflg) {
					curcol += 3;
			...
			s++;
				...
				nStatus = mbtowc(&wChar,s,MB_CUR_MAX);
				if (nStatus < 0) {
				pfmt(stderr, MM_ERROR,PFMTTXT(_MSG_MBTOWC_ERR));
				exit(1);
				}
				else {
				curcol+= wcwidth(wChar);
				putwchar(wChar);
				s += nChar;
				}
			}
...
int
mbScrLength(char *mbStr)
{
int     nChar=0,wcLen=0, wcScrWidth;
size_t  nVal;
wchar_t *wcStr,*wcStart;

        if (!mbStr)
                 return(0);

        /* Compute number of bytes in input string,
         * including terminating null
         */

        nChar = strlen(mbStr)+1;

        /* Allocate space for wchar array */
        wcStr = (wchar_t *) malloc ( nChar * sizeof(wchar_t));
        ...
        nVal = mbstowcs(wcStr,mbStr,nChar);

        /* In case of invalid character string using strlen as column width */
        if ( nVal == -1)
                return(strlen(mbStr));

        wcStart = wcStr;
        /* Calculate Screen Length of wide character */
        while(*wcStr) {
		wcScrWidth = wcwidth(*wcStr++);
		if (wcScrWidth > 0) wcLen += wcScrWidth;
	}
        free(wcStart);
        return(wcLen);
}
