...
char chars[256];  /* SPECIAL, APOS, DIGIT, or LETTER */
...
char **av;
{
	...
	if (errflg)
		fatal(CATGETS(catd, _MSG_DEROFF_USAGE), (char *) NULL );
	...
	if(line == NULL)
		fatal(CATGETS(catd, _MSG_CANNOT_ALLOC_LINEBUFMEM), (char *) NULL);
...
if( (fd = fopen(p, "r")) == NULL)
	fatal(CATGETS(catd, _MSG_CANNOT_OPEN_FILE), p);
...
for( ; ; )
	...
	if(lp > &line[linesz-2]) {
		...
		if(!line)
			fatal(CATGETS(catd, _MSG_CANNOT_REALLOC_LINEBUFMEM), NULL);
		lp = line + lpoff;
	}
	if(intable && c=='T')
		{
...
for(p1 = line ; ;)
	...
	if (!(I18N_SBCS_CODE))  
	 while (chars[*p1] == MB1)  {
		   p1 += mblen((const char *)p1, MB_CUR_MAX);
	  }
	if(*p1++ == '\0') return;
	else --p1;
	...
	/* skip initial specials ampersands and apostrophes */
	 while( chars[*p1] < DIGIT)
		if(*p1++ == '\0') return;
	nlet = 0;
	for(p = p1 ; ((i=chars[*p]) != SPECIAL) 
		&& ((I18N_SBCS_CODE) ? 1: (i != MB1)); ++p)
		if(i == LETTER) ++nlet;
...
if( (t0 = t = calloc( (unsigned)(strlen(s)+1), sizeof(*t) ) ) == NULL)
	fatal(CATGETS(catd, _MSG_CANNOT_ALLOC_MEM), (char *) NULL);
