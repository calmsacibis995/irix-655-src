...
id_usage(void)
{
	_sgi_nl_usage(SGINL_USAGE, cmd_label, 
		GETTXT(_MSG_MMX_id_usage1)); 
	_sgi_nl_usage(SGINL_USAGE, cmd_label, 
		GETTXT(_MSG_MMX_id_usage2)); 
	_sgi_nl_usage(SGINL_USAGE, cmd_label, 
		GETTXT(_MSG_MMX_id_usage3)); 
	_sgi_nl_usage(SGINL_USAGE, cmd_label, 
		GETTXT(_MSG_MMX_id_usage4)); 
	_sgi_nl_usage(SGINL_USAGE, cmd_label, 
		GETTXT(_MSG_MMX_id_usage5)); 
	exit(1);
}
...
plabel(void)
{
	...
	if ((mac = mac_get_proc()) == NULL) {
		perror(GETTXT(_MSG_ID_MAC_ERROR));
		return;
	}
	
	if ((lsp = mac_to_text(mac, NULL)) == NULL)
		printf(GETTXT(_MSG_ID_LABEL_ERROR));
	else
		printf(GETTXT(_MSG_ID_LABEL), lsp);
...
pcap(void)
{
	...
	if ((capset = cap_get_proc()) == NULL) {
		perror(GETTXT(_MSG_ID_CAPABILITY_ERROR));
		return;
	}
	
	if ((cp = cap_to_text(capset, NULL)) == NULL)
		printf(GETTXT(_MSG_ID_CAPABILITY_UNRECOGNIZED));
	else
		printf(GETTXT(_MSG_ID_CAPABILITY), cp);
