#ident "$Header: /proj/irix6.5f/isms/eoe/cmd/xfs/dump/inventory/RCS/inv_api.c,v 1.26 1998/06/02 20:23:13 cwf Exp $"
...
inv_getopt(int argc, char **argv, invt_pr_ctx_t *prctx) 
{
	...
	while ( ( c = getopt( argc, argv, GETOPT_CMDSTRING )) != EOF ) {
		...
		case GETOPT_INVPRINT:
			...
			if (rval & I_IFOUND) {
				mlog( MLOG_NORMAL | MLOG_INV,
				     "INV: invalid option \"%s\" "
				     "- only comma-separated suboptions allowed "
				     "with -I option\n",
				     argv[optind-1]);
				rval |= I_IERR;
			}
