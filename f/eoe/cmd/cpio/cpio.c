 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5f/isms/eoe/cmd/cpio/RCS/cpio.c,v 1.65 1998/11/18 16:18:14 tee Exp $"
...
#define MYMSG(msg) MSG_NUM ( msg ), DEF_MSG( msg )
#define MYCATMSG(msg) CAT_MSG_NUM ( msg ), DEF_MSG( msg )
...
char	Adir,			/* Flags object as a directory */
	...
	*Eom_p = DEF_MSG(_MSG_CHANGE_PART),
	*Eom_pid = MSG_NUM(_MSG_CHANGE_PART),
...
static const char
        mutex[] = DEF_MSG(_MSG_MUTEX),
        mutexid[] = MSG_NUM(_MSG_MUTEX),
        badaccess[] = DEF_MSG(_MSG_BAD_ACCESS),
        badaccessid[] = MSG_NUM(_MSG_BAD_ACCESS),
        badaccarch[] = DEF_MSG(_MSG_BAD_ACCESS_AR),
        badaccarchid[] = MSG_NUM(_MSG_BAD_ACCESS_AR),
        badcreate[] = DEF_MSG(_MSG_BAD_CREAT),
        badcreateid[] = MSG_NUM(_MSG_BAD_CREAT),
        badcreatdir[] = DEF_MSG(_MSG_BAD_CREAT_DIR),
        badcreatdirid[] = MSG_NUM(_MSG_BAD_CREAT_DIR),
        badfollow[] = DEF_MSG(_MSG_BAD_FOLLOW),
        badfollowid[] = MSG_NUM(_MSG_BAD_FOLLOW),
        badread[] = DEF_MSG(_MSG_READ_ERR),
        badreadid[] = MSG_NUM(_MSG_READ_ERR),
        badreadsym[] = DEF_MSG(_MSG_BAD_READ_SYM),
        badreadsymid[] = MSG_NUM(_MSG_BAD_READ_SYM),
        badreadtty[] = DEF_MSG(_MSG_BAD_READ_TTY),
        badreadttyid[] = MSG_NUM(_MSG_BAD_READ_TTY),
        badreminc[] = DEF_MSG(_MSG_BAD_RM_INC),
        badremincid[] = MSG_NUM(_MSG_BAD_RM_INC),
        badremtmp[] = DEF_MSG(_MSG_BAD_RM_TMP),
        badremtmpid[] = MSG_NUM(_MSG_BAD_RM_TMP),
        badwrite[] = DEF_MSG(_MSG_WRITE_ERR),
        badwriteid[] = MSG_NUM(_MSG_WRITE_ERR),
        badinit[] = DEF_MSG(_MSG_BAD_INIT),
        badinitid[] = MSG_NUM(_MSG_BAD_INIT),
        sameage[] = DEF_MSG(_MSG_SAME_AGE),
        sameageid[] = MSG_NUM(_MSG_SAME_AGE),
        badcase[] = DEF_MSG(_MSG_CANT_MKDIR_MKNOD),
        badcaseid[] = MSG_NUM(_MSG_CANT_MKDIR_MKNOD),
        badhdr[] = DEF_MSG(_MSG_BAD_HDR_TYPE),
        badhdrid[] = MSG_NUM(_MSG_BAD_HDR_TYPE),
        nomem[] = DEF_MSG(_MSG_MEM_ERR),
        nomemid[] = MSG_NUM(_MSG_MEM_ERR),
        badappend[] = DEF_MSG(_MSG_BAD_APPEND_AR),
        badappendid[] = MSG_NUM(_MSG_BAD_APPEND_AR),
        badchmod[] = DEF_MSG(_MSG_BAD_CHMOD),
        badchmodid[] = MSG_NUM(_MSG_BAD_CHMOD),
        badchown[] = DEF_MSG(_MSG_BAD_CHOWN),
        badchownid[] = MSG_NUM(_MSG_BAD_CHOWN),
        badpasswd[] = DEF_MSG(_MSG_BAD_PASSWD),
        badpasswdid[] = MSG_NUM(_MSG_BAD_PASSWD),
        badgroup[] = DEF_MSG(_MSG_BAD_GROUP),
        badgroupid[] = MSG_NUM(_MSG_BAD_GROUP),
        badorig[] = DEF_MSG(_MSG_BAD_ORIG),
        badorigid[] = MSG_NUM(_MSG_BAD_ORIG);
...
int argc;
{
	...
	case OCp: /* PASS */
		...
		break;
	default:
		msg(EXT, MYMSG(_MSG_OPT_ERR));
	}
	Blocks = (long long)(Blocks * (double)(Bufsize/BUFSZ));
	Blocks += (SBlocks + (BUFSZ-1)) / BUFSZ;
	if (oldgoodhdr == 0 && (Args & OCi))
		Blocks = 0;
        msg(EPOST, MYMSG(_MSG_BLOCK_FMT), Blocks);
        if (Error_cnt == 1)
                msg(EPOST, MYMSG(_MSG_1_ERR));
        else if (Error_cnt > 1)
                msg(EPOST, MYMSG(_MSG_ERR_FMT), Error_cnt);
	if (oldgoodhdr == 0 && (Args & OCi))
		Error_cnt++;
	return Error_cnt;
}
		...
		while ((Buffr.b_end_p - Buffr.b_in_p) >= Bufsize) {
			...
			if ((rv = g_read(Device, Archive, Buffr.b_in_p, Bufsize)) < 0) {
				...
				if (errno == ENOSPC) {
					...
					if (i++ > MX_SEEKS)
						msg(EXT, MYMSG(_MSG_RECOVER_ERR));
					if (rmtlseek(Archive, Bufsize, SEEK_REL) < 0)
						msg(EXTN, MYMSG(_MSG_LSEEK_ERR));
					Error_cnt++;
					Buf_error++;
					rv = 0;
					continue;
...
chgreel(int dir)
{
	...
	char str[APATH];
        ...
        if (dir)
                msg(EPOST, MYMSG(_MSG_END_MEDIUM_O));
        else
                msg(EPOST, MYMSG(_MSG_END_MEDIUM_I));
	...
	for (;;) {
		...
		do { /* tryagain */
			if (IOfil_p) {
				...
				msg(EPOST, MYMSG(_MSG_DEVICE_PRMT));
				if (fgets(str, sizeof(str), Rtty_p) == (char *)NULL)
					msg(EXT, badreadttyid, badreadtty);
			...
			if ((Archive = rmtopen(str, dir)) < 0) {
				msg(ERRN, MYMSG(_MSG_CANT_OPEN_STR), str);
				tryagain = 1;
			}
		} while (tryagain);
		(void)g_init(&tmpdev, &Archive);
		if (tmpdev != Device)
			msg(EXT, MYMSG(_MSG_CANT_CHNG_MED));
		...
		if (dir == INPUT)
			...
			if ((rv = g_write(Device, Archive, Buffr.b_out_p, Bufsize)) == Bufsize)
				...
				msg(ERR, MYMSG(_MSG_CANT_WRITE_MED));
		}
	} /* ;; */
	Eomflag = 0;
	return(rv);
...
ckname(void)
{
	...
	if (G_p->g_namesz > Max_namesz) {
		msg(ERR, MYMSG(_MSG_NAME_LEN_ERR));
		return(F_SKIP);
	}
	if (Pat_pp && !matched())
		return(F_SKIP);
	if ((Args & OCr) && !Adir) { /* rename interactively */
		(void)pfmt(Wtty_p, MM_NOSTD, PFMTTXT(_MSG_RENAME_FMT), G_p->g_nam_p);
		(void)fflush(Wtty_p);
		...
		if (*(Renam_p + lastchar) == '\n') /* remove trailing '\n' */
			*(Renam_p + lastchar) = '\0';
		if (*Renam_p == '\0') {
			msg(POST, MYMSG(_MSG_SKIP_FMT), G_p->g_nam_p);
			*G_p->g_nam_p = '\0';
			return(F_SKIP);
...
ckopts(long mask)
{
	...
	if ((mask & OCM) && !((mask & OCI) || (mask & OCO)))
		msg(ERR, MYMSG(_MSG_M_NEED_OPT));
	if ((mask & OCA) && !(mask & OCO))
		msg(ERR, MYMSG(_MSG_A_NEED_OPT));
	if (Kflag && !(mask & OCo))
		msg(ERR,  MYCATMSG(_MSG_CPIO_K_ONLY_O));
	if (Kflag && ((mask & OCc) || (mask & OCH)))
		msg(ERR, MYCATMSG(_MSG_CPIO_K_ONLY_BINARY));
	if (Bufsize <= 0)
		msg(ERR, MYMSG(_MSG_C_OPT_SIZE));
	if (mask & OCH) {
		t_p = Hdr_p;
		while (*t_p != NULL) {
			if (isupper(*t_p))
		...
		if (!strcmp("odc", Hdr_p)) {
			...
			msg(ERR, MYMSG(_MSG_HDR_ERR), Hdr_p);
	}
	if (mask & OCr) {
		Rtty_p = fopen(Ttyname, "r");
		Wtty_p = fopen(Ttyname, "w");
		if (Rtty_p == (FILE *)NULL || Wtty_p == (FILE *)NULL)
			msg(ERR, MYMSG(_MSG_CANT_RENAME), Ttyname);
	}
	if ((mask & OCE) && (Ef_p = fopen(Efil_p, "r")) == (FILE *)NULL)
		msg(ERR, MYMSG(_MSG_CANT_OPEN_PATT), Efil_p);
	if ((mask & OCI) && (Archive = rmtopen(IOfil_p, O_RDONLY)) < 0)
		msg(ERR, MYMSG(_MSG_CANT_OPEN_I), IOfil_p);
	if (mask & OCO) {
		if (mask & OCA) {
			if ((Archive = rmtopen(IOfil_p, O_RDWR)) < 0)
				msg(ERR, MYMSG(_MSG_CANT_OPEN_APP), IOfil_p);
		} else {
			oflag = (O_WRONLY | O_CREAT | O_TRUNC);
			if ((Archive = rmtopen(IOfil_p, oflag, 0666)) < 0)
				msg(ERR, MYMSG(_MSG_CANT_OPEN_O), IOfil_p);
		}
	}
	chkandfixaudio(Archive);
	if (mask & OCR) {
		if (Uid != 0)
			msg(ERR, MYMSG(_MSG_R_OPT_SU));
		if ((Rpw_p = getpwnam(Own_p)) == (struct passwd *)NULL)
			msg(ERR, MYMSG(_MSG_UNKNOWN_USER_ID), Own_p);
	}
...
cksum(char hdr, int byt_cnt)
{
	...
	case CRC:
		...
		if (lseek(Ifile, 0, SEEK_ABS) < 0)
			msg(ERRN, MYMSG(_MSG_CANT_RESET_CKSUM));
		break;
	...
	*/
	if (!Adir && !Aspec && !Alink) {
		if (access(Gen.g_nam_p, R_OK) < 0) {
			msg(ERR, MYMSG(_MSG_CANT_OPEN_STR), Gen.g_nam_p);
			return(0);
		}
		if(ftype != S_IFREG) {
			msg(ERR, MYMSG(_MSG_CPIO_BAD_FTYPE), Gen.g_nam_p);
			return(0);
		}
	}
	...
	case USTAR:
		...
		*/
		...
		if (len > sizeof(fullnam)) {
			msg(ERR, MYMSG(_MSG_FILE_TOO_LONG), Gen.g_nam_p);
			return(0);
		}
...
smore:
			...
			if ( cp == wholename ) { /* didn't find '/' in prefix side */
				msg(WARN, MYCATMSG(_MSG_MMX_cpio_prefix),Gen.g_nam_p);
				return (0);
			}
			/* Name will fit in 100 char array - Don't count "/" */
			if ( strlen(cp + 1) > NAMSIZ ) {
				msg(WARN, MYMSG(_MSG_FILE_LEN_ERR),(cp + 1),NAMSIZ);
				return (0);
			}
	...
	if (SrcSt.st_size >= BIGBLOCK && !Kflag && (Args & OCo)) {
		msg(POST, MYCATMSG(_MSG_CPIO_SKIP_USE_K), Gen.g_nam_p);
		return(0);
	}
	if (SrcSt.st_size >= BIGBLOCK && Kflag && (Args & OCo) && !portwarn) {
		portwarn = 1;
		msg(EPOST, MYCATMSG(_MSG_CPIO_PORT_WARN), Gen.g_nam_p);
	}
	...
	do {
		...
		if (!link(name1_p, name2_p)) {
			...
			if (!(Args & OCu) && G_p->g_mtime <= DesSt.st_mtime && (cnt == 0)) {
				...
				msg(ERRN, MYMSG(_MSG_CANT_UNLINK), name2_p);
		}
		cnt++;
	} while ((cnt < 2) && !missdir(name2_p));
	if (!cnt) {
		if (Args & OCv)
			(void)pfmt(Err_p, MM_NOSTD, PFMTTXT(_MSG_LINK_FMT), name1_p, name2_p);
		VERBOSE((Args & (OCv | OCV)), name2_p);
	} else if (cnt == 1)
		msg(ERRN, badcreatdirid, badcreatdir, name2_p);
	else if (cnt == 2)
		msg(ERRN, MYMSG(_MSG_CANT_LINK_STR), name1_p, name2_p);
	return(cnt);
}
...
creat_spec(void)
{
	...
	case 2:
		if (Adir)
			msg(ERRN, MYMSG(_MSG_CANT_CREAT_DIR_2), nam_p);
		else if (Aspec)
			msg(ERRN, MYMSG(_MSG_MKNOD_ERR), nam_p);
		if (*Over_p == '\0')
			rstfiles(U_KEEP);
		break;
	default:
...
creat_tmp(char *nam_p)
{
	...
	if (Uid && Aspec) {
		msg(ERR, MYMSG(_MSG_CANT_OVERWRITE), nam_p);
		return(-1);
	}
	...
	(void)mktemp(Over_p);
	if (*Over_p == '\0') {
		msg(ERR, MYMSG(_MSG_CANT_TMPNAME));
		return(-1);
	}
	if (rename(nam_p, Over_p) < 0) {
		msg(ERRN, MYMSG(_MSG_CANT_CREATE_TMP));
		return(-1);
	}
	return(1);
}
...
data_in(int proc_mode)
{
	...
	if (Args & (OCb | OCs | OCS)) { /* verfify that swapping is possible */
		...
		if (Args & (OCs | OCb) && G_p->g_filesz % 2) {
			msg(ERR, MYMSG(_MSG_CANT_SWAP_BYTES), nam_p);
			swapfile = 0;
		}
		if (Args & (OCS | OCb) && G_p->g_filesz % 4) {
			msg(ERR, MYMSG(_MSG_CANT_SWAP_HWORD), nam_p);
			swapfile = 0;
		}
	}
			...
			if(offs >= hm[chunki].offset && hm[chunki].count < 0) {
				if(proc_mode == P_PROC) {
					...
					if((offs=lseek(Ofile, -hm[chunki].count, SEEK_CUR)) == -1)
						msg(ERRN, MYMSG(_MSG_CPIO_SEEK_HOLE),
							nam_p);
				}
	...
	if (proc_mode == P_CMP) {
		...
		if (Hdr_type == CRC && G_p->g_cksum != cksumval) {
			msg(ERR, MYMSG(_MSG_CKSUM_ERR_FMT), nam_p);
			rstfiles(U_KEEP);
		} else
			rstfiles(U_OVER);
		VERBOSE(Args&(OCv|OCV), nam_p);
...
data_out(void)
{
	...
	if ((G_p->g_mode & Ftype) == S_IFLNK && (Hdr_type != USTAR && Hdr_type != TAR)) { /* symbolic link */
		...
		if ((lnksz=readlink(nam_p, T_lname, G_p->g_filesz)) < 0 || lnksz > NAMSIZ) {
			msg(ERRN, MYMSG(_MSG_FILE_LEN_ERR), T_lname, NAMSIZ);
			return;
		}
	...
	if ((Ifile = open(nam_p, O_RDWR)) < 0) {
		...
		if(errno == EBUSY && !stat(nam_p, &st) && S_ISREG(st.st_mode)) {
			msg(ERR, MYMSG(_MSG_CPIO_SKIP_SWAPF), nam_p);
			return;
		}
		else if ((Ifile = open(nam_p, O_RDONLY)) < 0) {
			msg(ERR, MYMSG(_MSG_Q_STR_FMT), nam_p);
			return;
		}
	}
	if (Hdr_type == CRC && ((G_p->g_cksum = cksum(CRC, 0)) == (ulong)-1)) {
		msg(POST, MYMSG(_MSG_SKIP_FMT_2), nam_p);
		(void)close(Ifile);
		return;
	}
	...
	if(Holeflag) {
		...
		if(chunks) {	/* has holes */
			if((sizeof(*bm)*(chunks+1)) > Buffr.b_size) {
				(void)close(Ifile);
				msg(ERR, MYMSG(_MSG_CPIO_HOLE_MAP), nam_p, sizeof(*bm)*(chunks+1));
				/*NOTREACHED*/
			}
			if(!portwarn) {
				portwarn = 1;
			...
			if (fcntl(Ifile, F_GETBMAP, bm) < 0) {
				msg(ERR, MYMSG(_MSG_CPIO_HOLE_ERR), nam_p);
...
data_pass(void)
{
	...
	if ((Ifile = open(Nam_p, 0)) < 0) {
		msg(ERRN, MYMSG(_MSG_OPEN_SKIP_FMT), Nam_p);
		(void)close(Ofile);
		rstfiles(U_KEEP);
		return;
	}
...
file_out(void)
{
	...
	if (!Aspec && !ArchSt.st_rdev && IDENT(SrcSt, ArchSt)) {
		...
		msg(POST, MYMSG(_MSG_SKIP_FMT_2),G_p->g_nam_p);
			...
			G_p = &tl_p->L_gen;
        		if (Hdr_type == CRC){
		        	if ((Ifile = open(G_p->g_nam_p, O_RDONLY)) < 0) {
                			msg(ERR, MYMSG(_MSG_Q_STR_FMT), G_p->g_nam_p);
                			return;
        			}
...
file_pass(void)
{
	...
	G_p = &Gen;
	if (Adir && !(Args & OCd)) {
		msg(ERR, MYMSG(_MSG_USE_D_COPY), G_p->g_nam_p);
		return;
	}
  ...
  for (i = 0; i < NLNKHASH; i++) {
	...
	while (l_p != &Lnk_hd[i]) {
		...
		if (stat64(Gen.g_nam_p, &SrcSt) == 0) { /* check if file exists */
			...
			msg(ERR, MYMSG(_MSG_DISAPPEAR_FMT), Gen.g_nam_p);
		tl_p = l_p;
		l_p = l_p->L_nxt_p;
		reclaim(tl_p);
	} /* l_p != &Lnk_hd */
	...
	do { /* hit == NONE && (Args & OCk) && Buffr.b_cnt > 0 */
		...
		if (hit != NONE) {
				...
				if (*Gen.g_nam_p == '\0') { /* tar trailer */
					...
					if (G_p->g_cksum != cksum(TARTYP, 0)) {
						...
						if (!strncmp(Buffr.b_out_p,"7070",4))
							msg(EXT, MYCATMSG(_MSG_MMX_cpio_bswap));
						msg(ERR, MYMSG(_MSG_BAD_HDR_CKSUM));
					}
				}
			...
			if (!goodhdr) {
				...
				if (!(Args & OCk))
					break;
				msg(ERR, MYMSG(_MSG_BAD_HDR_LOST));
			} else {
				FILL(hsize);
			}
		} /* hit != NONE */
		if (hit == NONE) {
			...
			if (!(Args & OCk))
				break;
			if (!cnt++)
				msg(ERR, MYMSG(_MSG_MAGIC_SEARCH));
		}
	} while (hit == NONE);
	if (hit == NONE) {
		if (Hdr_type == NONE)
			msg(EXT, MYMSG(_MSG_BAD_HDR_CPIO));
		else
			msg(EXT, MYMSG(_MSG_BAD_MAGIC));
	} else if (cnt > 0) {
		if(Args & OCv)
			fprintf(stderr, GETTXTCAT(_MSG_CPIO_D_BLOCK_FMT), cnt);
		msg(EPOST, MYMSG(_MSG_MAGIC_SYNC));
	}
	if (Hdr_type == NONE) {
		Hdr_type = hit;
		switch (Hdr_type) {
	...
	while (!goodfile) {
		...
		if (namelen > Max_namesz) {
			*(Gen.g_nam_p + namelen - 1) = '\0';
			msg(ERR, MYCATMSG(_MSG_MMX_cpio_maxlen),Gen.g_nam_p, Max_namesz);
			return(2);
		}
				...
				if ( BLKORCHR(SrcSt.st_mode) )
					if ( OVER_FLOW(SrcSt.st_rdev) ) {
						 ...
						 * uses binary mode, no expanded rdev */
							...
							msg(WARN, MYCATMSG(_MSG_MMX_cpio_expdev), Gen.g_nam_p, SrcSt.st_rdev >> NBITSMINOR, SrcSt.st_rdev & MAXMIN);
...
ioerror(int dir)
{
	...
	if ((archtype != S_IFCHR) && (archtype != S_IFBLK)) {
		if (dir) {
			if (errno == EFBIG)
				msg(EXT, MYMSG(_MSG_ULIMIT_ERR));
			else if (errno == ENOSPC)
				msg(EXT, MYMSG(_MSG_SPACE_ERR));
			else
				msg(EXTN, MYMSG(_MSG_IO_ERR));
		} else
                        msg(EXT, MYMSG(_MSG_EOF_ERR));
	} else {

		if (chknomedia(Archive)) {
			msg(EXT, MYCATMSG(_MSG_MMX_cpio_dooropen));
		}

		if (dir)
                	msg(EXTN, MYMSG(_MSG_IO_OUTPUT_ERR));
        	else
                	msg(EXTN, MYMSG(_MSG_IO_INPUT_ERR));
	}
}
...
msg(int severity,...)
{
	...
	if (severity == ERRN || severity == EXTN) {
                (void)pfmt(file_p, MM_NOSTD, PFMTTXT(_MSG_ERRNO_STR), Errno,
                        strerror(Errno));
	} else
		(void)fprintf(file_p, "\n");
	(void)fflush(file_p);
	va_end(v_Args);
        if (severity == EXT || severity == EXTN) {
                if (Error_cnt == 1)
                        (void)pfmt(file_p, MM_NOSTD, PFMTTXT(_MSG_1_ERR_NEWLINE));
                else
                        (void)pfmt(file_p, MM_NOSTD, PFMTTXT(_MSG_ERRS_FMT), Error_cnt);
		exit(Error_cnt);
	}
}
...
openout(void)
{
	...
	if (Max_filesz < G_p->g_filesz && G_p->g_filesz >= 0) { 
		msg(ERR, MYMSG(_MSG_ULIMIT_BYTE_FMT),
...
rstfiles(int over)
{
	...
	if (over == U_KEEP && *Over_p != '\0') {
		msg(POST, MYMSG(_MSG_EXIST_RESTORE), nam_p);
		(void)unlink(nam_p);
...
setup(int largc, char **largv)
{
	...
	register char	*opts_p = "abcdfiklmoprstuvABC:DE:H:I:KTWLM:O:R:SV6",
                        *dupl_p = DEF_MSG(_MSG_ONLY_1_ALLOW),
                        *dupl_pid = MSG_NUM(_MSG_ONLY_1_ALLOW);
	register int option;
	int blk_cnt;
	int i;
	struct rlimit64 rl;
	...
	if (Args & OCp) { /* get destination directory */
		...
		if ((DesSt.st_mode & Ftype) != S_IFDIR)
			msg(EXT, MYMSG(_MSG_NOT_DIR), Fullnam_p);
	}
...
set_tym(char *nam_p, time_t atime, time_t mtime)
{
	...
	if (utime(nam_p, &timev) < 0) {
		if (Args & OCa)
			if (errno == ENOSYS)
				msg(WARN, MYCATMSG(_MSG_MMX_cpio_settym), nam_p);
			else
				msg(WARN, MYMSG(_MSG_CANT_RESET_ATIME), nam_p);
		else
			msg(WARN, MYMSG(_MSG_CANT_RESET_MTIME), nam_p);
	}
}
...
usage(void)
{
	...
	(void)fflush(stdout);
	(void)pfmt(stderr, MM_ACTION, PFMTTXT(_MSG_CPIO_USAGE));
        (void)pfmt(stderr, MM_NOSTD, PFMTTXT(_MSG_CPIO_USAGE_2));
        (void)pfmt(stderr, MM_NOSTD, PFMTTXT(_MSG_CPIO_USAGE_3));
        (void)pfmt(stderr, MM_NOSTD, PFMTTXT(_MSG_CPIO_USAGE_4));
        (void)pfmt(stderr, MM_NOSTD, PFMTTXT(_MSG_CPIO_USAGE_5));
        (void)pfmt(stderr, MM_NOSTD, PFMTTXT(_MSG_CPIO_USAGE_6));
        (void)pfmt(stderr, MM_NOSTD, PFMTTXT(_MSG_CPIO_USAGE_7));
	(void)fflush(stderr);
	exit(Error_cnt);
}
...
verbose(char *nam_p)
{
	...
	if ((Args & OCt) && (Args & OCv)) {
		...
		case (S_IFLNK):
			...
			break;
		default:
			msg(ERR, MYMSG(_MSG_INVALID_FTYPE));
		}
		...
		if (Lastuid == (int)Gen.g_uid)
			(void)printf(GETTXTCAT(_MSG_CPIO_9STR_FMT), Curpw_p->pw_name);
		else {
			setpwent();
		...
		if (Lastgid == (int)Gen.g_gid)
			(void)printf(GETTXTCAT(_MSG_CPIO_9STR_FMT), Curgr_p->gr_name);
		else {
			setgrent();
		...
		(void)cftime(Time, GETTXT(_MSG_CFTIME_FMT), (time_t *)&Gen.g_mtime);
		(void)printf(GETTXTCAT(_MSG_CPIO_STR_STR_FMT), Time, nam_p);
		if ((Gen.g_mode & Ftype) == S_IFLNK) {
			...
			(void)printf(GETTXTCAT(_MSG_CPIO_ARROW_STR_FMT), Symlnk_p);
		}
		(void)printf("\n");
...
write_hdr(void)
{
	...
	case USTAR: /* USTAR */
		...
		if (T_lname[0] != '\0') {
			if (strlen(T_lname) > NAMSIZ) {
				msg(EPOST, MYMSG(_MSG_FILE_LEN_ERR),
					T_lname, NAMSIZ);
				/*
	...
	case TAR:
		...
		break;
	default:
		msg(EXT, MYMSG(_MSG_BAD_HDR_TYPE));
	} /* Hdr_type */

	if (uid_overflow_warning > 0) {
		msg(WARN, MYCATMSG(_MSG_MMX_cpio_uid_overflow), uid_overflow_warning);
...
chkandfixaudio(int mt)
{
	...
	if(status & CT_AUDIO) {
		msg(EPOST, MYMSG(_MSG_CPIO_AUDIO_DRIVE));
		if(rmtioctl(mt, MTIOCTOP, &mtop) < 0)
			msg(EPOST, MYMSG(_MSG_CPIO_TAPE_READ_ERR));
	}
}
