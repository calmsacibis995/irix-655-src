...
#ident "$Revision: 1.69 $"
/* Modified to support EUC multi-byte/sjis-Big5/Full multibyte  */
#include <bstring.h>
#include <dirent.h>
#include <errno.h>
#include <fcntl.h>
...
struct dferr {
	...
	{ SGINL_SYSERR,		CAT_MSG_NUM(_MSG_MMX_cannotstat),
		DEF_MSG(_MSG_MMX_cannotstat)},
	{ SGINL_NOSYSERR,	CAT_MSG_NUM(_MSG_MMX_df_notfnd),
		DEF_MSG(_MSG_MMX_df_notfnd)		},
	{ SGINL_NOSYSERR,	CAT_MSG_NUM(_MSG_MMX_df_NFSservnotresp),
		DEF_MSG(_MSG_MMX_df_NFSservnotresp)		},
	{ SGINL_NOSYSERR,	CAT_MSG_NUM(_MSG_MMX_df_supblk),
		DEF_MSG(_MSG_MMX_df_supblk)		},
	{ SGINL_NOSYSERR,	CAT_MSG_NUM(_MSG_MMX_df_freelist),
		DEF_MSG(_MSG_MMX_df_freelist)		},
	{ SGINL_NOSYSERR,	CAT_MSG_NUM(_MSG_MMX_df_badfreecnt),
		DEF_MSG(_MSG_MMX_df_badfreecnt)		},
	{ SGINL_NOSYSERR,	CAT_MSG_NUM(_MSG_MMX_df_badfreeblk),
		DEF_MSG(_MSG_MMX_df_badfreeblk)		},
	{ SGINL_NOSYSERR,	CAT_MSG_NUM(_MSG_MMX_df_rdfreeblk),
		DEF_MSG(_MSG_MMX_df_rdfreeblk)		},
	{ SGINL_SYSERR,		CAT_MSG_NUM(_MSG_MMX_CannotOpen),
		DEF_MSG(_MSG_MMX_CannotOpen)},
	{ SGINL_NOSYSERR,	CAT_MSG_NUM(_MSG_MMX_df_cantgetsts),
		DEF_MSG(_MSG_MMX_df_cantgetsts)		},
	{ SGINL_NOSYSERR,	CAT_MSG_NUM(_MSG_MMX_df_readsb),
		DEF_MSG(_MSG_MMX_df_readsb)		},
};
...
usage(void)
{
	_sgi_nl_usage(SGINL_USAGE, "UX:df",
	    GETTXT(_MSG_MMX_df_usage), 
	    "befhikmPlnqrtV");
	_sgi_nl_usage(SGINL_USAGE, "UX:devnm",
	    GETTXT(_MSG_MMX_devnm_usage));
	return(-1);
}
...
{
	switch (u) {
	    case halfk: return (pflag ? GETTXT(_MSG_DF_HALFBLK) : GETTXT(_MSG_DF_BLOCK));
	    case kb:	return (pflag ? GETTXT(_MSG_DF_ONEBLK) : GETTXT(_MSG_DF_KB));
	    case mb: 	return (GETTXT(_MSG_DF_MBBLK));
	    case human: return (GETTXT(_MSG_DF_SIZE));
	}
	/* NOTREACHED */
}
...
put_head(void)
{
	if(do_FStype || eflag || Vflag)
		return;
	if (Pflag)  {
		printf(GETTXT(_MSG_DF_FSAVAIL),
			mtptwidth, GETTXT(_MSG_DF_FILESYSTEM), unitname(units, 1));
	} else if (units == human) {
		printf(GETTXT(_MSG_DF_FSMETRICS1),
		     mtptwidth, GETTXT(_MSG_DF_FILESYSTEM), unitname(units, 0));
	} else if (units == mb) {
		printf(GETTXT(_MSG_DF_FSMETRICS2),
		     mtptwidth, GETTXT(_MSG_DF_FILESYSTEM), unitname(units, 0));
	} else {
		printf(GETTXT(_MSG_DF_FSMETRICS3),
		     mtptwidth, GETTXT(_MSG_DF_FILESYSTEM), unitname(units, 0));
	}
	if (do_inode_stats)
		printf(GETTXT(_MSG_DF_INODE_STATUS));
	else
		printf(GETTXT(_MSG_DF_MOUNTED_ON));
}
...
put_line(struct mntent *mntp)
{
	...
	if (eflag) {
		printf(GETTXT(_MSG_DF_FREE_INODES_FMT_STR), (df_filcnt_t)sfsb.f_ffree);
		return;
	}
	if (do_FStype)
		printf(GETTXT(_MSG_DF_FILESYSTEM_TYPE_FMT_STR), mntp->mnt_type);
	else if (Pflag) {
		if (units == human) {
			printf(GETTXT(_MSG_DF_P_OPTION_HUMAN_UNITS_FMT_STR),
			    number(total), number(used), 
			    number(sfsb.f_bfree),
			    (int)PERCENT(used, sfsb.f_blocks));
		} else {
			printf(GETTXT(_MSG_DF_P_OPTION_OTHER_UNITS_FMT_STR),
			    total, used,
		...
		if (units == human) {
			printf(GETTXT(_MSG_DF_HUMAN_UNITS_FMT_STR),
			    mntp->mnt_type,
			    number(total), number(used), 
			    number(sfsb.f_bfree),
			    (int)PERCENT(used, sfsb.f_blocks));
		} else if (units == mb) {
			printf(GETTXT(_MSG_DF_MB_UNITS_FMT_STR),
			    mntp->mnt_type, total, used,
			    (df_blkcnt_t)sfsb.f_bfree,
			    (int)PERCENT(used, sfsb.f_blocks));
		} else {
			printf(GETTXT(_MSG_DF_OTHER_UNITS_FMT_STR),
			    mntp->mnt_type, total, used,
			    (df_blkcnt_t)sfsb.f_bfree,
			    (int)PERCENT(used, sfsb.f_blocks));
		}
	...
	if (do_inode_stats) {
		...
		if (units == human) {
			printf(GETTXT(_MSG_DF_I_OPTION_HUMAN_UNITS_FMT_STR),
			    number(iused/1000), 
			    number((df_filcnt_t)sfsb.f_ffree / 1000),
			    (int)PERCENT(iused, sfsb.f_files));
		} else {
			printf(GETTXT(_MSG_DF_I_OPTION_OTHER_UNITS_FMT_STR),
	...
	if (!do_FStype) {
		if (mntp->mnt_dir != NULL)
			...
			printf(GETTXT(_MSG_DF_FILESYSTEM_NAME_FMT_STR), sfsb.f_fname);
	}
	putchar('\n');
}
...
{
	fprintf(stderr, GETTXT(_MSG_DF_STATFS_FMT_STR),
	    name,
	    sfbp->f_fstyp,
	    sfbp->f_bsize,
	    sfbp->f_frsize,
