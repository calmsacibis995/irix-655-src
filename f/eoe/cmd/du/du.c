...
char	opts[] = "aklmxrsL";
static const char errmsg[] = PFMTTXT(_MSG_UXCORE_37);
...
int entr;
{
	if (entr >= 1<<24) {
		pfmt(stderr, MM_ERROR,
			PFMTTXTCAT(_MSG_LINKTBL_OVERFL));
		exit(1);
	}
	...
	if (ml == NULL) {
		pfmt(stderr, MM_ERROR,
			PFMTTXTCAT(_MSG_CANT_ALLOCBYTES),
			entr * sizeof(*ml));
		exit(2);
	}
