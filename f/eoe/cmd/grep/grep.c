...
char *s;
{
	_sgi_nl_error(SGINL_SYSERR, cmd_label,
	    GETTXT(_MSG_MMX_CannotOpen), s);
}

#define REG_MSGLEN  (36 * MB_LEN_MAX)
static void
...
regex_t	*preg;
{
	...
	char msg[REG_MSGLEN];
	if( !re)
	    f = GETTXT(_MSG_MMX_grep_stdin);
	else
		f = re;
	(void)regerror(re_err, preg, msg, REG_MSGLEN);

	_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
	    GETTXT(_MSG_MMX_regexp_str), f);
...
/*
 * Purpose:	To solve the character boundary problem of strstr() because of
 *		"\" character in search string.
 * Parameters:	pStr1 : String in which to search .
 * 		pStr2 : String to search
 * Returns:	Byte position of first occurrence of pStr2 in pStr1. If pStr2
 *		is not found in pStr1 then NULL is returned.
 */

char *pStrStr(char *pStr1, char *pStr2)
{
	char *pStr=pStr1;
	int nLen=0;

	for(; *pStr!='\0' && !(pStr[0]==pStr2[0] && pStr[1]==pStr2[1]) ;)
		if((nLen=mblen(pStr,MB_CUR_MAX)) == -1) {
			fprintf(stderr,GETTXT(_MSG_MMX_grep_illpattern));
			pStr+=1;
		}
		else
			pStr+=nLen;
