/*
 *	$Id: awk.g.y,v 1.12 1998/09/18 19:47:22 sherwood Exp $
 */
...
Change History        : 13 Jan 1998        HCL
	Cataloging changes are done.
        * VSC BUG FIX : for failed assertion#(547)
...
program:
	...
	| error	{ yyclearin; bracecheck(); vyyerror(CATNUM_DEFMSG(_MSG_NAWK_BAILING_OUT)); } /* I18NCAT_PGM_MSG */
	;
...
simple_stmt:
	...
	| DELETE varname		{ yyclearin; vyyerror(CATNUM_DEFMSG(_MSG_NAWK_U_CAN_ONLY_DEL_ARR_ELEMENT)); $$ = stat1(DELETE, $2); } /* I18NCAT_PGM_MSG */
	| pattern			{ $$ = exptostat($1); }
...
stmt:
	...
	| NEXT st	{ if (infunc)
                                /* I18NCAT_PGM_MSG */
				vyyerror(CATNUM_DEFMSG(_MSG_NAWK_NEXT_IN_FUNC_IS_ILLEGAL));
			  $$ = stat1(NEXT, NIL); }
...
term:
	...
	| GETLINE			{ $$ = op3(GETLINE, NIL, NIL, NIL); }
        | CLOSE '(' patlist ')'         { $$ = op2(CLOSE, $3); }
	| INDEX '(' pattern comma pattern ')'
		{ $$ = op2(INDEX, $3, $5); }
	| INDEX '(' pattern comma reg_expr ')'
		{ vyyerror(CATNUM_DEFMSG(_MSG_NAWK_INDEX_NOT_PERMIT_REG_EXP)); /* I18NCAT_PGM_MSG */
		  $$ = op2(INDEX, $3, (Node*)$5); }
	| '(' pattern ')'		{ $$ = $2; }
	...
	Cell *p;
{
        /* I18NCAT_PGM_MSG */
	if (isarr(p))
		vyyerror(CATNUM_DEFMSG(_MSG_NAWK_STR_IS_AN_ARR_NOT_A_FUNC), p->nval);
	else if (isfunc(p))
		vyyerror(CATNUM_DEFMSG(_MSG_NAWK_CANNOT_DEFN_FUNC_MORE_THAN_ONCE), p->nval);
	curfname = p->nval;
}
