...
#define SSERR_SUCCESS            0            /* Success (Daemon started) */
...
static char szFNswcInitSwitcher[]          = "swcInitSwitcher";
static char szFNswcDoneSwitcher[]          = "swcDoneSwitcher";
static char szFNswcBeginProc[]             = "swcBeginProc";
static char szFNswcContProc[]              = "swcContProc";
static char szFNswcEndProc[]               = "swcEndProc";
...
char szServerVersionDate[128]                  = "11 May 1999";  /* Server version date */
...
{ struct timespec time_val;
  ...
  { /* Waiting for "Password ready" status (for inets mode need timeout) */
    ...
    { syslog_msg(LOG_ERR,SRVSEQN_CANTALLOCTCB,szCantAllocTcbEntry);
...
{ int i;
  ...
  szDelimitterLine,SSRV_VMAJOR,SSRV_VMINOR,__TIME__,__DATE__,szFixStartTime,debug_mode ? szDebug : szNormal,silenceMode);
#else
  i = sprintf(buf,"%s\nEventMon v%d.%d\nDaemon Statistics since: %s (%s mode), silence: %d\n",
  szDelimitterLine,SSRV_VMAJOR,SSRV_VMINOR,szFixStartTime, debug_mode ? szDebug : szNormal,silenceMode);
#endif
      ...
      { buf[l] = 0; est.statInTotalReadSocketSuccess++;
        ...
        { switch(buf[3]) {
          ...
          case SSRVCMD_STAT   : est.statInTotalInfoRequest++;
                                if(slen && (buflen = make_reportbuffer(&buf[4],sizeof(buf)-4)) > 0)
                                { buflen += 5; /* header + 0 */
...
{ /* init mutex resources */
  if(!init_dmutex(&start_listen_mutex))     { syslog_msg(LOG_ERR,SRVSEQN_INITMUTEXERR,szCantInitMutex,"start_listen"); exit(SSERR_INITDATA); }
  if(!init_dmutex(&start_killer_mutex))     { syslog_msg(LOG_ERR,SRVSEQN_INITMUTEXERR,szCantInitMutex,"start_killer"); exit(SSERR_INITDATA); }
  if(!init_dmutex(&start_info_mutex))       { syslog_msg(LOG_ERR,SRVSEQN_INITMUTEXERR,szCantInitMutex,"start_info"); exit(SSERR_INITDATA); }
  if(!init_dmutex(&start_confl_mutex))      { syslog_msg(LOG_ERR,SRVSEQN_INITMUTEXERR,szCantInitMutex,"start_confloader"); exit(SSERR_INITDATA); }
  if(!init_dmutex(&start_unloader_mutex))   { syslog_msg(LOG_ERR,SRVSEQN_INITMUTEXERR,szCantInitMutex,"start_unloader"); exit(SSERR_INITDATA); }
  if(!init_dmutex(&exit_phase_mutex))       { syslog_msg(LOG_ERR,SRVSEQN_INITMUTEXERR,szCantInitMutex,"exit"); exit(SSERR_INITDATA); }
  if(!init_dmutex(&reinit_mutex))           { syslog_msg(LOG_ERR,SRVSEQN_INITMUTEXERR,szCantInitMutex,"reinit"); exit(SSERR_INITDATA); }
  if(!init_dmutex(&ipaddrlist_mutex))       { syslog_msg(LOG_ERR,SRVSEQN_INITMUTEXERR,szCantInitMutex,"ipaddrlist"); exit(SSERR_INITDATA); }
  if(!init_dmutex(&tcblist_mutex))          { syslog_msg(LOG_ERR,SRVSEQN_INITMUTEXERR,szCantInitMutex,"tcblist"); exit(SSERR_INITDATA); }
  if(!init_dmutex(&switcher_mutex))         { syslog_msg(LOG_ERR,SRVSEQN_INITMUTEXERR,szCantInitMutex,"switcher"); exit(SSERR_INITDATA); }
  if(!init_dmutex(&password_mutex))         { syslog_msg(LOG_ERR,SRVSEQN_INITMUTEXERR,szCantInitMutex,"password"); exit(SSERR_INITDATA); }
  inited_mutex++;

  /* init condition variables */
  if(!init_dcond(&exit_condition))          { syslog_msg(LOG_ERR,SRVSEQN_INITCONDVARERR,szCantInitConVar,"exit"); exit(SSERR_INITDATA); }
  if(!init_dcond(&reinit_condition))        { syslog_msg(LOG_ERR,SRVSEQN_INITCONDVARERR,szCantInitConVar,"reinit"); exit(SSERR_INITDATA); }
  if(!init_dcond(&password_condition))      { syslog_msg(LOG_ERR,SRVSEQN_INITCONDVARERR,szCantInitConVar,"password"); exit(SSERR_INITDATA); }
  inited_cond++;
}
...
{ /* Init Listen thread */
  ...
  { exit_phase++; pthread_mutex_unlock(&start_listen_mutex.mutex);
    syslog_msg(LOG_ERR,SRVSEQN_INITTHREADERR,szCantInitThread,"listen"); exit(SSERR_INITTHREAD);
  }
  ...
  { exit_phase++; pthread_mutex_unlock(&start_info_mutex.mutex);
    syslog_msg(LOG_ERR,SRVSEQN_INITTHREADERR,szCantInitThread,"info"); exit(SSERR_INITTHREAD);
  }
  ...
  { exit_phase++; pthread_mutex_unlock(&start_confl_mutex.mutex);
    syslog_msg(LOG_ERR,SRVSEQN_INITTHREADERR,szCantInitThread,"confloader"); exit(SSERR_INITTHREAD);
  }
  ...
  { /* Init Unloader thread */
    ...
    { exit_phase++; pthread_mutex_unlock(&start_unloader_mutex.mutex);
      syslog_msg(LOG_ERR,SRVSEQN_INITTHREADERR,szCantInitThread,"unloader"); exit(SSERR_INITTHREAD);
    }
  }
  ...
  { exit_phase++; pthread_mutex_unlock(&start_killer_mutex.mutex);
    syslog_msg(LOG_ERR,SRVSEQN_INITTHREADERR,szCantInitThread,"killer"); exit(SSERR_INITTHREAD);
  }
}
/* -------------------------- send_test_request ---------------------------- */
...
{ struct stat statbuf;
  ...
  { syslog_msg(LOG_WARNING,SRVSEQN_CANFINDRESPATH,szCantFindResourcePath,szResourcePath);
...
#ifdef INCDEBUG
#ifdef SSRV_ENABLE_CHANGE_RESPATH
    "%s [-d] [-q] [-r] [-a<listen_port>][-p<resource_path>] [-l<switcher_library>] [-n<dbname>] [-t<addr_tablename>] [-i] [-v] [-c] [-h]\n" : "%s [-i] [-v] [-c] [-h]\n",szDaemonName);
#else
    "%s [-d] [-q] [-r] [-a<listen_port>] [-l<switcher_library>] [-n<dbname>] [-t<addr_tablename>] [-i] [-v] [-c] [-h]\n" : "%s [-i] [-v] [-c] [-h]\n",szDaemonName);
#endif
#else
#ifdef SSRV_ENABLE_CHANGE_RESPATH
    "%s [-q] [-r] [-a<listen_port>] [-p<resource_path>] [-l<switcher_library>] [-n<dbname>] [-t<addr_tablename>] [-i] [-v] [-c] [-h]\n" : "%s [-i] [-v] [-c] [-h]\n",szDaemonName);
#else
    "%s [-q] [-r] [-a<listen_port>] [-l<switcher_library>] [-n<dbname>] [-t<addr_tablename>] [-i] [-v] [-c] [-h]\n" : "%s [-i] [-v] [-c] [-h]\n",szDaemonName);
#endif
#endif
    if(privModeFlag)
    {
...
{ if(!silenceMode) fprintf(stderr,"%s, %s\n",szDaemonName,szCopyright); exit(SSERR_SUCCESS);
}
/* ------------------------ parse_parameters ------------------------------- */
...
{ char tmp[1024];
  int c,i,err = 0;
#ifdef INCDEBUG
  char *cmdkey_string = privModeFlag ? "hHcCvViIqQrRu:U:f:F:n:N:t:T:p:P:l:L:dDs:S:k:K:a:A:" : "hHcCvViIs:S:";
#else
  char *cmdkey_string = privModeFlag ? "hHcCvViIqQrRu:U:f:F:n:N:t:T:p:P:l:L:s:S:k:K:a:A:"   : "hHcCvViIs:S:";
#endif
