...
jseg:		KWD_SEGMENT		{
		...
		jattrs			{ if (!parse_jseg_stop()) YYERROR; }
	;
...
jattr:		KWD_STATIC		{
			yyjseg->ms_flags &=
				~MISER_SEG_DESCR_FLAG_NORMAL;
			yyjseg->ms_flags |=
				MISER_SEG_DESCR_FLAG_STATIC;
		}
	|	KWD_MULTIPLE TOK_INT	{ yyjseg->ms_multiple = $2; }
	|	KWD_PRIORITY TOK_INT	{ yyjseg->ms_priority = $2; }
	|	KWD_EXCEPT KWD_KILL	{
			yyjseg->ms_flags |=
				MISER_EXCEPTION_TERMINATE;
		}
	|	KWD_EXCEPT KWD_WTLESS	{
			yyjseg->ms_flags |=
				MISER_EXCEPTION_WEIGHTLESS;
		}
	|	KWD_TIME TOK_STR	{
...
qdef:		KWD_QUEUE TOK_INT TOK_STR	{
			if (!(yyqhdr = parse_qdef_start())) YYERROR;
			yyqhdr->name = $2;
			yyqhdr->fname = $3;
		}
		qhdrs			{ if (!parse_qdef_hdr()) YYERROR; }
		qsegs			{ if (!parse_qdef_stop()) YYERROR; }
	;
...
qseg:		KWD_SEGMENT		{
		...
		qattrs			{ if (!parse_qseg_stop()) YYERROR; }
	;
