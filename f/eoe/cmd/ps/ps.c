...
usage(void)
{
	...
	_sgi_nl_usage(SGINL_USAGE, cmd_label,
	    GETTXT(_MSG_MMX_ps_usage1));
#else
	_sgi_nl_usage(SGINL_USAGE, cmd_label,
	    GETTXT(_MSG_MMX_ps_usage3));
#endif /* _SHAREII */
	_sgi_nl_usage(SGINL_USAGESPC, cmd_label,
	    GETTXT(_MSG_MMX_ps_usage2));
	_sgi_nl_usage(SGINL_USAGESPC, cmd_label,
	    GETTXT(_MSG_MMX_ps_usage4));
	exit(1);
}
...
#define TSZ 30
	...
	if ((procdev == statb.st_dev) || (ustat(procdev, &ustatb) < 0)) {
		fprintf(stderr, GETTXT(_MSG_MMX_ps_not_mounted), _PATH_PROCFSPI);
		exit(1);
	}
	...
	*(cp + 1) = '\0';
	...
	if ((dirp = opendir(_PATH_PROCFSPI)) == NULL) {
		(void) fprintf(stderr, GETTXT(_MSG_MMX_ps_open_pinfo),
			strerror(errno));
		exit(1);
	}
...
void getdev(void)
{
	...
	case 1:
		_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
		    GETTXT(_MSG_MMX_ps_ftwproblem), strerror(errno));
		exit(1);
	case -1:
		_sgi_nl_error(SGINL_SYSERR, cmd_label,
		    GETTXT(_MSG_MMX_ps_ftwfailed), strerror(errno));
		exit(1);
	default:
		_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
		    GETTXT(_MSG_MMX_ps_ftwret), rcode, strerror(errno));
		exit(1);
	}
}
	 ...
	 int numb,struct FTW *f)
{
	...
	default:
		_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
		    GETTXT(_MSG_MMX_ps_gdeverr), numb);
		return 1;
	}
}
...
prstime(timespec_t st, int len)
{
	...
	if (tim - starttime > 24*60*60) {
		cftime(sttim, GETTXT(_MSG_MMX_b_d_format), &starttime);
		sttim[8] = '\0';
	} else {
		cftime(sttim, GETTXT(_MSG_MMX_time_format), &starttime);
		sttim[8] = '\0';
	}
...
prformattime(time_t tm, int len)
{
	...
	m = l % (60*60);
	if (tm > 24*60*60)
		sprintf(buf, GETTXT(_MSG_PS_DAYS_HR_MIN_SEC), tm / (24*60*60), l / (60*60),
		  m / 60, m % 60 );
	else if (tm > 60*60)
		sprintf(buf, GETTXT(_MSG_PS_HR_MIN_SEC), l / (60*60), m / 60, m % 60 );
	else
		sprintf(buf, GETTXT(_MSG_PS_MIN_SEC), m / 60, m % 60);
