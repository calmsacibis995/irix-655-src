...
char **argv;
{
	...
	if (*terminal == '\0') {
	    if (need_term) {
		fprintf(stderr, CATGETS(catd, _MSG_TABS_SPECIFY_TERM_TYPE));
		exit(1);
	    } else 
	...
	if ((terminal == NULL) || (*terminal == '\0')) {
		fprintf(stderr, CATGETS(catd, _MSG_TABS_NO_TERM_VALUE));
		exit(1);
	}
	...
	if (err <= 0) {
		fprintf(stderr,
			CATGETS(catd, _MSG_TABS_DB_NOT_FOUND),	
			terminal);
		exit(1);
	}
	if (set_tab == NULL) {
		fprintf(stderr,
			CATGETS(catd, _MSG_TABS_CANNOT_SET_TYPE),
			terminal);
		exit(1);
	}
	maxtab = columns;
...
int tabvect[NTABS], level;
{
	...
	if (*scan == '-')
		if ((c = *++scan) == '-')
			...
			error(CATGETS(catd, _MSG_TABS_UNKNOWN_TAB_CODE));
		else;
	else
...
int tabvect[NTABS];
{
	...
	for (i = 0; i<NTABS-1;) {
		if (*scan == '+') {
			...
			if (t = getnum(&scan))
				tabvect[i++] = last += t;
			else error(CATGETS(catd, _MSG_TABS_ILLEGAL_INCREMENT));
		}
		else {
			if ((t = getnum(&scan)) > last)
				tabvect[i++] = last = t;
			else error(CATGETS(catd, _MSG_TABS_ILLEGAL_TABS));
		}
		if (*scan++ != ',') break;
	}
	if (last > NCOLS)
		error(CATGETS(catd, _MSG_TABS_ILLEGAL_TABS));
	tabvect[i] = 0;
}
...
int tabvect[NTABS];
{
	...
	char *temp;
	if (level)
		error(CATGETS(catd, _MSG_TABS_FILE_INDIRECTION));
	if ((fildes = open(scan,0)) < 0)
		error(CATGETS(catd, _MSG_TABS_CANT_OPEN));
