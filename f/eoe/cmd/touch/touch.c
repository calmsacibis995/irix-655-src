...
char *argv[];
{
			...
			(rflg && stflg) )
	{
		if (!errflg)
			...
			/* ":8:Incorrect usage\n" */
		(void) pfmt(stderr, MM_ACTION, PFMTTXT(_MSG_TOUCH_USAGE_OB), argv[0],
				stflg ? GETTXT(_MSG_F_USAGE_ID) : tusage);
			/* :583:Usage: %s [%s] [mmddhhmm[yy]] file ...\n" */

		if (!stflg) {
			(void) pfmt(stderr, MM_STD, 
			        PFMTTXTCAT(_MSG_MMX_touch_usage),
	...
	if (rflg) {
		if (stat64(dashr_file, &prstbuf) == -1) {
			pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_FRMT_12),
	...
	if (fflg) {
		if (stat64(proto, &prstbuf) == -1) {
			pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_FRMT_12), proto,
				strerror(errno));
			exit(2);
		}
	} 
...
#endif
			(void) pfmt(stderr, MM_ERROR,
				PFMTTXT(_MSG_CANT_CHNG_TIME_ON),
				argv[c], strerror(errno));
			status++;
			continue;
		}
