...
/*	join F1 F2 on stuff */
/*      Modification to fix -j option on second field           */
/*      Check for invalid characters as field separator option  */
/*	Check for invalid characters in data file               */
...
char *argv[];
{
	...
	while ((c = getopt(argc, argv, "a:v:e:o:t:1:2:j:")) != EOF) {
		...
		case 't':
			msep1 = msep2 = optarg;
			if( (sepwidth = mblen(msep1,MB_CUR_MAX)) < 0 ) {
                    		pfmt(stderr,MM_ERROR,
					PFMTTXT(_MSG_JOIN_INVALID_SEPARATOR_CHARACTER));
                                exit(1); 
                        } 
			break;
		case 'o':
...
usage:
		...
		pfmt(stderr, MM_ACTION, PFMTTXT(_MSG_JOIN_USAGE));
		exit(1);
	}
	...
	do {
		...
		*pp++ = bp;	/* record beginning */
		...
		while ((c = *bp) != '\n' && c != '\0') {
			...
			if( (mltwidth = mblen(bp, MB_CUR_MAX)) < 0) {
				pfmt(stderr,MM_ERROR,
					PFMTTXT(_MSG_JOIN_INVALID_FIELD_CHARACTER));
				exit(1);
                        }
			bp += mltwidth;
		}
