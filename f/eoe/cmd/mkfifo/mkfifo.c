...
char       *argv[];
{
	...
	while ((c=getopt(argc, argv, ":m:")) != EOF) {
			...
			mflag++;
                        ms = optarg;
			if(strcmp(ms,"--")==0) {
				fprintf(stderr,CATGETS(catd,_MSG_MKFIFO_MKFIFO_MISSING_OPTION));  
				exit(1);
			}
			mode_check(ms);
                        break;
		case '?':
			fprintf(stderr,CATGETS(catd,_MSG_MKFIFO_MKFIFO_BAD_OPTION),optopt);  
			exit(1);
		case ':':
			fprintf(stderr,CATGETS(catd,_MSG_MKFIFO_MKFIFO_MISSING_OPTION));  
			exit(1);
	...
	if(mflag && optind >= argc) {
		fprintf(stderr,CATGETS(catd,_MSG_MKFIFO_MKFIFO_MISSING_ARGUMENTS));  
		exit(1);
	}
		...
		if(mflag)
			mode = newmode(mode);
		if(mkfifo (path,mode)) {
			fprintf(stderr,CATGETS(catd,_MSG_MKFIFO_MKFIFO), sys_errlist[errno]);  
			exitval = 1;
		}
		if(mflag && nowho == 0) {
			struct stat64 statbuf;
			if(stat64(path,&statbuf)) {
				fprintf(stderr,CATGETS(catd,_MSG_MKFIFO_MKFIFO_BAD_STAT_OF),path);  
				exitval = 1;
			} else if(chmod(path,newmode(statbuf.st_mode))) {
				fprintf(stderr,CATGETS(catd,_MSG_MKFIFO_MKFIFO_BAD_CHMOD_OF),path);  
				exitval = 1;
			}
		}
    	}
...
void usage()
{
	(void)fprintf(stderr, CATGETS(catd,_MSG_MKFIFO_USAGE_MKFIFO_M_MODE));  
	exit(1);
}
...
*/
	...
	if (*--msp) {
		fprintf(stderr,CATGETS(catd,_MSG_MKFIFO_MKFIFO_BAD_MODE));  
		exit(1);
	}
	return(nm);
}
...
char *modestr;
{
	...
	for(;*mstr;++mstr) {
		if(strchr(allow,*mstr) == NULL) {
			fprintf(stderr,CATGETS(catd,_MSG_MKFIFO_MKFIFO_BAD_MODE_CHAR),*mstr);  
			exit(1);
		}
	}
}
