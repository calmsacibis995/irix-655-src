...
main(int argc, char *argv[])
{
	...
	if(EQ(argv[0],"[")) {
		if(!EQ(argv[--ac],"]"))
		        pfmt(stderr,MM_ERROR,PFMTTXT(_MSG_TEST_SQ_BRACKET_MISSING));
	}
	argv[ac] = 0;
	if (ac<=1) exit(1);
	exit(exp()?0:1);
...
nxtarg(int mt)
{
	if (ap>=ac) {
		...
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_TEST_ARG_EXPECTED));
	}
	return(av[ap++]);
}
...
e3()
{
	...
	if(EQ(a, "(")) {
		p1 = exp();
		if(!EQ(nxtarg(0), ")")) 
		    pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_TEST_ROUND_BRC_EXPECTED));
		return(p1);
	}
	...
	pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_TEST_UNKNOWN_OP), p2);
	return(0);
}
