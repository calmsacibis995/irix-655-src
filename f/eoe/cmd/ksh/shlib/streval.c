...
{
	wchar_t		*nextchr; /* next char in current expression	*/
	wchar_t		*errchr;	 /* next char after error		*/
	struct lval	errmsg;	 /* error message text			*/
	wchar_t		*errstr;  /* error string			*/
#ifdef FLOAT
...
number	(*conv)();
{
	...
	convert = conv;
	if(level++ >= MAXLEVEL)
		(void)seterror(ksh_gettxt(_SGI_MMX_e_recursive,e_recursive));
	else
	{
		n = expr(0);
		if (peekchr() == L':') (void)seterror(ksh_gettxt(_SGI_MMX_e_badcolon,e_badcolon));
	}
	if (cur.errmsg.value)
	{
...
register int	precedence;
{
	...
	while ((c=getchr()) && is_space(c));
	switch (c)
	{
	case 0L:
		if(precedence>5)
			error(ksh_gettxt(_SGI_MMX_e_moretokens,e_moretokens));
		return(0);
...
#ifdef FLOAT
		if(cur.isfloat)
			error(ksh_gettxt(_SGI_MMX_e_incompatible,e_incompatible));
#endif /* FLOAT */
	...
	while(1)
	{
		...
		if(peekchr()== L'=' && !(op->precedence&NOASSIGN))
		{
			if(!noassign && (!lvalue.value || precedence > 2))
				error(ksh_gettxt(_SGI_MMX_e_notlvalue,e_notlvalue));
			assignop = lvalue;
			getchr();
			c = 3L;
		}
		...
		if(lvalue.value && (op->opcode!=ASSIGNMENT))
		{
			...
			if(cur.nextchr==0L)
				error(ksh_gettxt(_SGI_MMX_e_number,e_number));
			if(!(op->precedence&SEQPOINT))
				lvalue.value = 0;
			invalid = 0L;
		}
		if(invalid && op->opcode>ASSIGNMENT)
			error(ksh_gettxt(_SGI_MMX_e_synbad,e_synbad));
		if(precedence >= c)
			goto done;
		...
		{
		case RPAREN:
			error(ksh_gettxt(_SGI_MMX_e_paren,e_paren));
		...
		case COLON:
			(void)seterror(ksh_gettxt(_SGI_MMX_e_badcolon,e_badcolon));
			break;
#endif
		...
		case MOD:
			if(x!=0)
				...
				error(ksh_gettxt(_SGI_MMX_e_divzero,e_divzero));
			break;
