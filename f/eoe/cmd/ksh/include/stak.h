...
#define STAK_SMALL	1	/* argument to stakcreate */
	...
	extern wchar_t	*stakcopy __ARGS((const wchar_t*));
	extern char	*stakset __ARGS((char*, unsigned));
	extern char	*stakseek __ARGS((unsigned));
	extern int	stakputs __ARGS((const wchar_t*));
	extern char	*stakfreeze __ARGS((unsigned));
	extern char	*wc_stakfreeze __ARGS((unsigned));
...
#define stakputc(c)	(((_stak_cur.stakleft-=sizeof(wchar_t))<0? _stakgrow(sizeof(wchar_t)):0), \
					(*((wchar_t *)_stak_cur.staktop)=(c)),_stak_cur.staktop+=sizeof(wchar_t))
