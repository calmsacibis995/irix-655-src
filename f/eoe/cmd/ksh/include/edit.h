...
struct edit
{
	...
	wchar_t	*e_outbase;	/* pointer to start of output buffer */
	wchar_t	*e_outptr;	/* pointer to position in output buffer */
	wchar_t	*e_outlast;	/* pointer to end of output buffer */
	wchar_t	*e_inbuf;	/* pointer to input buffer */
	wchar_t	*e_prompt;	/* pointer to buffer containing the prompt */
	wchar_t	*e_ubuf;	/* pointer to the undo buffer */
	wchar_t	*e_killbuf;	/* pointer to delete buffer */
	wchar_t	e_search[SEARCHSIZE];	/* search string */
	wchar_t	*e_Ubuf;	/* temporary workspace buffer */
	wchar_t	*e_physbuf;	/* temporary workspace buffer */
	wchar_t	e_lbuf[LOOKAHEAD];/* pointer to look-ahead buffer */
	int	e_fd;		/* file descriptor */
	int	e_ttyspeed;	/* line speed, also indicates tty parms are valid */
	wint_t	*e_globals;	/* global variables */
	wchar_t	*e_window;	/* display window  image */
	char	e_inmacro;	/* processing macro expansion */
#ifndef KSHELL
	char	e_prbuff[PRSIZE]; /* prompt buffer */
#endif /* KSHELL */
	int	e_wide_cnt;	/* count of additional chars for double
				 * width (or more) screen chars
				 */
};
...
extern struct edit editb;
    ...
    extern void ed_putchar(wint_t);
    extern void ed_ringbell(void);
    extern void ed_setup(int);
    extern void ed_flush(void);
    extern int	ed_getchar(void);
    extern int	ed_virt_to_phys(wchar_t*,wchar_t*,int,int,int);
    extern int	ed_window(void);
    extern void ed_ungetchar(wint_t);
#   ifdef KSHELL
	extern int ed_macro(wint_t);
	extern int ed_expand(wchar_t[],int*,int*,int);
#   endif /* KSHELL */
#else
    extern void ed_crlf();
...
extern const wchar_t	wc_e_runvi[];
#ifndef KSHELL
