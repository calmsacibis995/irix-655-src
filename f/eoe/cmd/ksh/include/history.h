...
#define HISMAX		(sizeof(wchar_t)*IOBSIZE)
...
struct history
{
	...
	wchar_t		*fixname;	/* name of history file */
...
extern const wchar_t	hist_fname[];
...
/* these are the history interface routines */
    ...
    extern int		hist_copy(wchar_t*,int,int);
    extern void 	hist_eof(void);
    extern histloc	hist_find(wchar_t*,int,int,int);
    extern void 	hist_flush(void);
    extern void 	hist_list(off_t,int,wchar_t*);
    extern int		hist_match(off_t,wchar_t*,int);
    extern off_t	hist_position(int);
    extern void 	hist_subst(const wchar_t*,int,wchar_t*);
    extern wchar_t 	*hist_word(wchar_t*,int);
#   ifdef ESH
