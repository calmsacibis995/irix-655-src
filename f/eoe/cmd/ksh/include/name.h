...
struct Nodval
{
	...
	wchar_t		*namenv;	/* pointer to environment name */
...
struct namnod
{
	...
	wchar_t		*namid;		/* pointer to name of item */
...
#   ifdef PROTO
	extern wchar_t	*nam_fstrval(struct namnod*);
#   else
	extern wchar_t	*nam_fstrval();
#   endif /* PROTO */
#else
...
#define isnull(n)	((n)->value.namval.cp == 0L)  /* strings only */
...
#define array_elem(n)	array_ptr(n)->nelem
    ...
    extern wchar_t 		*array_subscript(struct namnod*,wchar_t*);
    ...
    extern wchar_t 		*array_subscript();
...
#define new_of(type,x)	((type*)malloc((unsigned)sizeof(type)+(x)))
    ...
    extern struct namnod	*nam_alloc(const wchar_t*);
    extern void 		nam_free(struct namnod*);
    extern void 		nam_fputval(struct namnod*,wchar_t*);
    extern int			nam_hash(const wchar_t*);
    ...
    extern void 		nam_putval(struct namnod*,wchar_t*);
    struct argnod;		/* struct not declared yet */
    extern void 		nam_scope(struct argnod*);
    extern struct namnod	*nam_search(const wchar_t*,struct Amemory*,int);
    extern wchar_t 		*nam_strval(struct namnod*);
