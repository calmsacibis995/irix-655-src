...
int 	xp;
{
	register wchar_t *argscan=argi;
	register struct namnod	*n;
	register wint_t sep = *argscan;
	wchar_t *sim;
	if(root==sh.alias_tree)
	{
		while((sep= *argscan) != L'=' && !expchar(sep) && sep!=L'/')
			argscan++;
	}
	else if(is_walpha(sep) || !expchar(sep))
	{
		do
		{
			sep = *++argscan;
	...
	if(argscan!=argi)
	{
		...
		*argscan = sep;
		...
		if(sep == L'=')
		{
			...
			{
				short namlen,vallen;
				namlen =wcslen(n->namid);
				vallen = wcslen(argscan);
			...
			else if(n==VISINOD || ((n==EDITNOD)&&isnull(VISINOD)))
			{
				...
				if(strmatch(argscan,L"*vi"))
				{
					off_option(EDITVI|EMACS|GMACS);
					on_option(EDITVI);
				}
				if(strmatch(argscan,L"*macs"))
				{
					off_option(EDITVI|EMACS|GMACS);
					if(*argscan==L'g')
						on_option(GMACS);
...
#   endif /* MULTIBYTE */
			...
			else if(n==CSWIDTHNOD)
			{
				if(ed_setwidth(argscan) && !initenv)
					sh_fail(n->namid,ksh_gettxt(_SGI_DMMX_e_format,e_format),ERROR);
			}
#endif /* MULTIBYTE */
		}
		return(n);
	...
	if(initenv)
		return(0);
failed:
	sh_fail (argi,(root==sh.alias_tree?ksh_gettxt(_SGI_DMMX_e_aliname,e_aliname): ksh_gettxt(_SGI_DMMX_e_ident,e_ident)),ERROR);
	/* NOTREACHED */
	return(0);
}
...
int fd;
{
	...
	if(seps==WCNULL)
		seps = (wchar_t *)wc_e_sptbnl;
	if(wcscmp(seps,wc_e_sptbnl)==0)
		mask =  ~(L' '|L'\t'|L'\n');
	else
		mask = 0L;
	fp = io_ftable[fd];
	if(!fp || fp == &io_stdout)
	{
		io_init(fd,(struct fileblk*)0,(wchar_t*)0);
		fp = io_ftable[fd];
	}
	if(fp->flag&IOWRT)
	{
	...
	while(1)
	{
		...
		if((issep && name) || is_eol)	/* from non-separator to separator */
		{
			if(name==WCNULL)
			{
				...
				{
					cc -= WC_SZ;
					if(wcschr(seps,*((wchar_t *)stakptr(cc))))
						continue;
					cc += WC_SZ;
					break;
				}
			...
			if(nam_istype(n, N_RDONLY) && (raw&C_FLAG)) {
				...
				p_prp(n->namid);
				p_str(wc_e_colon,0L);
				p_str(ksh_gettxt(_SGI_DMMX_e_readonly,e_readonly),NL);
				ret = 1;
			}
			else nam_putval(n,(wchar_t *)stakptr(rel));
...
register wint_t	flag;
{
	...
	{
		if(!flag)
			p_str(ksh_gettxt(_SGI_DMMX_e_bltfn,e_bltfn),0L);
		...
		{
			p_str(wc_e_fnhdr,0L);
			hist_list(n->value.namval.rp->hoffset,0L,L"\n");
		}
...
struct Amemory *root;
{
	...
	argv = argnam  = (wchar_t**)stakalloc((namec+1)*BYTESPERWORD);
	namec = gscan_some(pushnid,root,flag,flag);
	gsort(argv,namec,wcscmp);
	while(namec--)
	{
		if((np = nam_search(*argv++,root,N_NULL)) && np!=onp)
		{
			...
			if(nam_istype(np,N_ARRAY))
			{
				...
				do
				{
					...
					if(pflag) p_str((pflag==X_FLAG?L"export":L"readonly"),L' ');
...
static wchar_t *staknam(n,value)
wchar_t *	value;
register struct namnod	*n;
{
	register wchar_t *p,*q;
	q = (wchar_t *)stakalloc((wcslen(n->namid)*WC_SZ)+(value?(wcslen(value)*WC_SZ):0)+(2*WC_SZ));
	p=sh_copy(n->namid,q);
	if(value)
	{
		*p++ = L'=';
		        ...
		        if(val == N_INTGER && nam_istype(n,N_INTGER))
			{
				if(n->value.namsz != 10)
				{
					...
					if(nam_istype(n, N_DOUBLE))
						p_str(ksh_gettxt(_SGI_DMMX_e_precision,e_precision),SP);
					else
#endif /* FLOAT */
						p_str(ksh_gettxt(_SGI_DMMX_e_intbase,e_intbase),SP);
					p_num(n->value.namsz,SP);
				}
				break;
			}
...
{
	register wchar_t *cp;
	register wchar_t *wc_env = WCNULL;
	register struct namnod	*n;
	...
	if(e)
	{
		while(*e)
		{
			int env_len = strlen(*e);
			int m = 0;
			int w = 0;
			int ret;
			char *mb = *e++;
			wc_env = (wchar_t *)malloc((size_t)((env_len+1)*sizeof(wchar_t)));
			cp = wc_env;

			if(MB_CUR_MAX == 1)
			{
				for(m=0;m<env_len;++m)
					wc_env[m] = (unsigned char)mb[m];
				wc_env[m] = 0L;
			}
			else {
				while(m < env_len)
				{
					switch(ret=mbtowc(wc_env+w,mb+m,MB_CUR_MAX)) {
					case 0:
					case -1:
						/* Bad conversions passed as raw data */
						wc_env[w++] = (wchar_t)((unsigned char)mb[m++]);
						break;
					default:
						m += ret;
						++w;
				...
				cp[3]==L'z' && cp[4]==L'=')
			{
				...
				do
				{
					...
					if(cp[2])
					{
						...
						nam_newtype(n,(unsigned)(cp[0]-L' ')|N_IMPORT|N_EXPORT,cp[1]-L' ');
					}
				}
				while(cp=next);
			}
			else {
				if(n=env_namset(cp,sh.var_tree,(N_IMPORT|N_EXPORT))) {
					n->value.namenv = cp;
				}
			}
		}
...
register struct namnod	*n;
{
	...
	if(nam_istype(n,N_RDONLY|N_UTOL|N_LTOU|N_RJUST|N_LJUST|N_ZFILL|N_INTGER))
		attsize += (wcslen(n->namid)+4)*WC_SZ;
}
...
wchar_t **env_gen()
{
	...
	er = (wchar_t**)stakalloc((namec+3)*BYTESPERWORD);
	argnam = ++er;
	gscan_some (pushnam, sh.var_tree, N_EXPORT|N_IMPORT, N_EXPORT);
	*argnam = (wchar_t*)stakalloc(attsize);
	cp = attval = sh_copy(wc_e_envmarker,*argnam);
...
#endif	/* apollo */
	(ERRNO)->value.namval.cp = (wchar_t*)(&errno);
...
struct name_value *name_vals;
{
	...
	}
	{
		...
		{
			np->namid = (wchar_t*)nv->nv_name;
			np->value.namval.cp = (wchar_t*)nv->nv_value;
#ifdef apollo
			if(*nv->nv_value==0)
...
register int flag;
{
	register const wchar_t *arg,*path,*nam;
	register struct namnod *np;
	...
	while(arg = *++com)
	{
		...
		if(sh_lookup(arg,tab_reserved))
		{
			if(flag) {
				p_str(arg,0L);
				p_str(ksh_gettxt(_SGI_DMMX_is_reserved,is_reserved),NL);
			}
			else 	p_str(arg,NL);
		}
		/* non-tracked aliases */
			...
			&& (nam=nam_strval(np))) 
		{
			if(flag)
			{
				...
				if(nam_istype(np,N_EXPORT))
					p_str(ksh_gettxt(_SGI_DMMX_is_xalias,is_xalias),0L);
				else
					p_str(ksh_gettxt(_SGI_DMMX_is_alias,is_alias),0L);
			}
			p_qstr(nam,NL);
		}
			...
			if(flag)
			{
				...
				if(isnull(np))
				{
					if(!nam_istype(np,N_FUNCTION))
						goto search;
					p_str(ksh_gettxt(_SGI_DMMX_is_ufunction,is_ufunction),NL);
				}
				else if(is_abuiltin(np))
					p_str(ksh_gettxt(_SGI_DMMX_is_builtin,is_builtin),NL);
				else if(nam_istype(np,N_EXPORT))
					p_str(ksh_gettxt(_SGI_DMMX_is_xfunction,is_xfunction),NL);
				else
					p_str(ksh_gettxt(_SGI_DMMX_is_function,is_function),NL);
			}
			else 	p_str(arg,NL);
		}
		else
		...
		search:
			...
			if(path)
			{
				if(flag)
				{
					...
					if(!notrack && *path == L'/') {
						p_str(ksh_gettxt(_SGI_DMMX_is_talias,is_talias),0L);
						p_qstr(path,NL);
					}
					else {
						p_str(ksh_gettxt(_SGI_DMMX_is_,is_),0L);
						p_qstr(path,NL);
					}
				}
				else 	p_qstr(path,NL);
...
char *echo_mode()
{
	...
	if(echo_arg==0)
	{
		...
		{
			echo_arg = (*cp=='b'?(char*)e_echoflag:(wchar_t*)e_minus);
			return(echo_arg);
		}
#endif /* apollo */
		savopts = opt_flags;
