...
void
sh_fail_why(const wchar_t *s1, wchar_t *s2, int errval)
{
        static wchar_t ebuf[256];
        wcscpy(ebuf, s2);
        if(errno) {
		wchar_t err[256];
		int cnt = mbstowcs(err,strerror(errno),256);
		err[cnt] = 0L;
                wcscat(ebuf, L": ");
                wcsncat(ebuf, err, sizeof(ebuf)-(wcslen(s2)+3));
        }
