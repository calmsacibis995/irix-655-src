...
static const wchar_t wc_hist_stamp[2] = { H_UNDO, H_VERSION };
...
int  hist_open()
{
	...
	*fname = 0L;
	if(fp=wasopen)
	{
		...
		wasopen = 0;
		hist_ptr = fp;
		if(wcscmp(histname,fp->fixname)==0)
			return(1);
		else
			hist_free();
	}
retry:
	...
	if((fd=ksh_open(histname,O_APPEND|O_RDWR|O_CREAT,S_IREAD|S_IWRITE))>=0)
	{
	...
	if(!(fp=new_of(struct history,(--fixmask)*sizeof(off_t)*WC_SZ)))
	{
		io_fclose(fd);
		return(0);
	}
...
off_t size;
{
	...
	while((c=io_getc(fd))!=WEOF) switch(state)
	{
		case 1:
			if(c==H_CMDNO)
			{
...
void hist_eof()
{
	...
	{
		c = *(fp->fixfp->ptr)++;
		if(c==0L && (fp->fixfp->ptr > fp->fixfp->last))
		{
...
void hist_flush()
{
	...
	if((fp->fixcnt=lseek(fp->fixfd,(off_t)0,SEEK_END)) <0)
	{
		...
		p_setout(ERRIO);
		p_num(getpid(),L':');
		p_str(L"hist_flush: EOF seek failed errno",L'=');
		p_num(errno,NL);
		p_flush();
#endif /* DEBUG */
		hist_free();
	...
	/* KLUDGE - pad fixcnt with additional bytes for multibyte chars */
	wc = fd->base;
	while(wc < fd->ptr)
	{
		if((cnt=wctomb(mb,*wc)) > 1)
			pad += (cnt-1);
		++wc;
	}
	fp->fixcnt += pad;
...
wchar_t *nl;
{
	register wchar_t oldc=0L;
	register wchar_t c;
	register struct history *fp = hist_ptr;
	wchar_t  obuf[IOBSIZE+1];
	wchar_t *o = obuf;
	wchar_t *n = obuf;
	wchar_t *eob = obuf+IOBSIZE;
	wchar_t *x;
	int more = 1;

	if(offset<0 || !fp)
	{
		p_str(ksh_gettxt(_SGI_DMMX_e_unknown,e_unknown),L'\n');
		return;
	}
	...
	while (more)
	{
		...
		/* KLUDGE - remove added padding spaces before newline */
		while((n = wcschr(n,L'\n')) != (wchar_t *)0)
		{
			x = n;
			if(*(x-1) == L' ')
...
wchar_t *string;
{
	...
	do
	{
		...
		for(cp=string;*cp;cp++)
		{
			if((c=io_getc(fp->fixfd)) == WEOF || c ==0L)
				break;
			count++;
			if(c == L'\n')
				line++;
...
register wchar_t *s1;
{
	...
	register wchar_t *s1max = s1+MAXLINE;
	off_t offset;
	if(!fp)
		return(-1);
	...
	while ((c = io_getc(fp->fixfd)) && c!=WEOF)
	{
		if(c==L'\n')
		{
			/* Remove space padding at the end of command */
			while(s1 && (*(s1-1) == L' '))
				--s1;
			if(count++ ==line)
				break;
...
wchar_t *hist_word(s1,word)
wchar_t *s1;
{
	register wint_t c;
	register wchar_t *cp = s1;
	register int flag = 0;
	if(hist_ptr==0)
#ifdef KSHELL
		return(sh.lastarg);
...
wchar_t *replace;
{
	register wchar_t *new=replace;
	register wchar_t *sp;
	register wint_t c;
	struct fileblk fb;
	wchar_t inbuff[IOBSIZE+1];
	wchar_t *string;
	while(*++new != L'='); /* skip to L'=' */
	io_init(fd,&fb,inbuff);
	stakseek(0);
	while ((c=io_getc(fd)) != WEOF)
		stakputc(c);
	string = (wchar_t *)wc_stakfreeze(WC_SZ);
	io_fclose(fd);
	*new++ =  0L;
	if((sp=sh_substitute(string,replace,new))==0L)
		sh_fail(command,ksh_gettxt(_SGI_DMMX_e_subst,e_subst),ERROR);
	*(new-1) =  L'=';
