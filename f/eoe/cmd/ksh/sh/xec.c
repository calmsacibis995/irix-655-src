...
static int trim_eq();
static wchar_t *word_trim();
static wchar_t locbuf[TMPSIZ]; 	/* store last argument here if it fits */
		...
		switch(type&COMMSK)
		{
				...
				dofunction:
					if(np && !(st.states&NOFUNC))
					{
						...
						if(np->value.namval.ip==0)
						{
							...
							path_search(com0,0);
							if(np->value.namval.ip==0)
								sh_cfail(ksh_gettxt(_SGI_DMMX_e_found,e_found));
						}
			...
			case TFORK:
			{
					...
					2,4,8,16, and 32 seconds and then quits */
				{
					...
					if(!job_slot())	
						sh_fail(ksh_gettxt(_SGI_DMMX_e_jobs,e_jobs),NIL,ERROR);
					if(type&FTMP)
					{
					...
					{
						if(sh.cpid)
							sh_fail(ksh_gettxt(_SGI_DMMX_e_pexists,e_pexists),NIL,ERROR);
...
#endif	/* VFORK */
					{
						if((forkcnt *= 2) > FORKLIM)
						{
							...
							{
							case ENOMEM:
								msg = ksh_gettxt(_SGI_DMMX_e_swap,e_swap);
								break;
							default:
							case EAGAIN:
								msg = ksh_gettxt(_SGI_DMMX_e_fork,e_fork);
								break;
							}
...
#   endif /* SIGTSTP */
			...
			{
				register wchar_t **args;
				register int nargs;
				struct namnod *n;
				wchar_t **arglist;
				struct dolnod	*argsav=NULL;
				struct comnod	*tp;
				wchar_t *nullptr = WCNULL;
				int refresh = 1;
				wchar_t *cp;
...
#define c	type
						...
						if((val=nam_fstrval(REPLYNOD))==WCNULL)
							continue;
						else
						{
							if(*(cp=val) == 0L)
							...
							if(c!=0L)
								...
								c = wcstol(val,(wchar_t **)0,10)-1;
							if(c<0L || c >= nargs)
							{
								cp = (wchar_t*)wc_e_nullstr;
								args = &cp;
							}
							else
								args += c;
...
#undef c
					...
					if(t->tre.tretyp == TSELECT)
					{
						if((cp=nam_fstrval(REPLYNOD)) && *cp==0L)
							refresh++;
						args = arglist;
					}
					else
			...
			case TSW:
			{
				wchar_t *r = word_trim(t->sw.swarg,0);
...
#endif	/* _sys_timeb_ */
				p_setout(ERRIO);
				p_str(ksh_gettxt(_SGI_DMMX_e_real,e_real),L'\t');
				p_time(at,NL);
				p_str(ksh_gettxt(_SGI_DMMX_e_user,e_user),L'\t');
				at = after.tms_utime - before.tms_utime;
				at += after.tms_cutime - before.tms_cutime;
				p_time(at,NL);
				p_str(ksh_gettxt(_SGI_DMMX_e_sys,e_sys),L'\t');
			...
			{
				register struct namnod *np;
				register wchar_t *fname = ((struct procnod*)t)->procnam;
				register struct slnod *slp;
				if(!is_walpha(*fname))
					sh_fail(fname,ksh_gettxt(_SGI_DMMX_e_ident,e_ident),ERROR);
				np = env_namset(fname,sh.fun_tree,P_FLAG|V_FLAG);
				/* a function name cannot be a built-in yet */
				if(is_abuiltin(np))
				{
					p_setout(ERRIO);
					p_prp(ksh_gettxt(_SGI_DMMX_e_restricted,e_restricted));
					p_str(L":",L' ');
					p_str(fname,0L);
			...
			case TTST:
			{
				...
				static wchar_t *arg[6]=  {arg0};
				static wchar_t unop[3]= L"-?";
				if(type&TTEST)
					skipexitset++;
				if (type&TPAREN)
				{
				...
				else
				{
					...
					else if(type&TBINARY)
					{
						nargs = 4;
						arg[1] = left;
						arg[2] = (wchar_t*)(test_optable+(n&017)-1)->sysnam;
		...
		if(mainloop && com0)
		{
			if(sh.lastarg!= locbuf && sh.lastarg)
				free((char *)sh.lastarg);
			if(wcslen(com[argn-1]) < TMPSIZ)
			{
				nam_ontype(L_ARGNOD,N_FREE);
				sh.lastarg = wcscpy(locbuf,com[argn-1]);
			}
			else
			{
...
wchar_t **com;
{
	...
	{
		p_setout(ERRIO);
		p_str(mac_try(nam_fstrval(PS4NOD)),0L);
		if(com)
			echo_list(1,com);
		if(nl)
			newline();
...
static wchar_t *word_trim(arg,flag)
register struct argnod *arg;
{
	register wchar_t *r = arg->argval;
