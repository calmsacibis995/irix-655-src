...
pid_t pid;
{
	register wchar_t *sp = sh_copy(utos((ulong)pid,10),io_tmpname+7);
	*sp++ = L'.';
	temp_suffix = sp;
	serial = 0;
}
...
wchar_t *buf;
{
	register unsigned size = (IOBSIZE+1)*WC_SZ;
	if(!fp)
	{
		if(iofree)
		{
		...
		else
		{
			...
			if(fd==0 && ispipe(0))
				size = 2;
			fp = new_of(struct fileblk,size*WC_SZ);
			buf = (wchar_t*)(fp+1);
			fp->fstak = 0;
			fp->flag = IOFREE;
		}
...
register int f2;
{
	...
	if(f2>2 && flag==0)
		fs = 1;
	if(f1 >= NFILE)
		sh_fail(ksh_gettxt(_SGI_DMMX_e_flimit,e_flimit),NIL,ERROR);
	if(f1!=f2)
	{
		io_fclose(f2);
		f2 = fcntl(f1,F_DUPFD, f2);
		if(f2 < 0)
			sh_fail(ksh_gettxt(_SGI_DMMX_e_file,e_file),NIL,ERROR);
...
register wchar_t *fname;
{
	...
	register wchar_t *tmp_name = io_tmpname;
	register int fd;
	do
	{
		sh_copy(sh_itos(++serial),temp_suffix);
	}
	while((fd=ksh_open(tmp_name,O_CREAT|O_RDWR,RW_ALL))<0 && maxtry--);
	if(fname)
	{
		sh_copy(tmp_name,fname);
		if(fd<=0)
			sh_fail_why(tmp_name,ksh_gettxt(_SGI_DMMX_e_create,e_create),ERROR);
	}
	io_ftable[fd] = 0;
	return(fd);
}
...
#   endif	/* VSH */
	...
	skip:
		...
		if(fnobuff(fp) && fno==0)
		{
			char mb_read[MB_CUR_MAX];
			int i=0;
			n = 1;
			while(i<MB_CUR_MAX)
			{
				if((n = read(fno,mb_read+i,1)) == 1)
				{	
					size_t ret = mbtowc(fp->base,mb_read,i+1);
					if(ret == -1)
					{
						++i;
						continue;
		...
		else 	n = ksh_read(fno,fp->base, IOBSIZE);
	}
done:
#ifndef SIG_NORESTART
slowsig:
...
int			mode;
{
	...
	if (strmatch(path, L"/dev/@(tcp|udp)/*/*") && str2inet(path + 9, &addr))
	{
		if ((fd = socket(AF_INET, path[5] == L't' ? SOCK_STREAM : SOCK_DGRAM, 0)) >= 0 && connect(fd, &addr, sizeof(addr)))
		{
			close(fd);
			fd = -1;
		}
...
#endif /* SOCKET */
	fd = ksh_open(path, flags, mode);
	if (fd < 0)
		sh_fail_why(path, (flags & (O_WRONLY|O_RDWR|O_APPEND|O_CREAT)) ? ksh_gettxt(_SGI_DMMX_e_create,e_create) : ksh_gettxt(_SGI_DMMX_e_open,e_open),ERROR);
	return(fd);
}
...
register int	pv[];
{
	if(pipe(pv)<0 || pv[INPIPE]<0 || pv[OTPIPE]<0)
		sh_fail_why(ksh_gettxt(_SGI_DMMX_e_pipe,e_pipe),NIL,ERROR);
...
register struct ionod	*i;
{
	...
	while(i)
	{
		...
		while((ksh_link(i->ioname,io_tmpname))< 0 && maxtry--);
		if(maxtry<=0)
			sh_fail_why(io_tmpname,ksh_gettxt(_SGI_DMMX_e_create,e_create),ERROR);
		if(i->iolink)
			free(i->iolink);
...
struct ionod	*iop;
{
	register wchar_t *ion;
	register int 	iof;
	register int	fd;
	int o_mode;
	static wchar_t io_op[5];
	int fn;
	int mark = MARK;
	int indx = sh.topfd;
	int traceon;
	...
	for(;iop;iop=iop->ionxt)
	{
		...
		if(*ion)
		{
			...
			else if(iof&IORDW)
			{
				...
				sh_fail(ion,ksh_gettxt(_SGI_DMMX_e_restricted,e_restricted),ERROR);
			else
			{
				if(iof&IOAPP)
				{
				...
				else
				{
					...
					if(iof&IOCLOB)
						...
						sh_fail(ion,ksh_gettxt(_SGI_DMMX_e_fexists,e_fexists),ERROR);
				}
			openit:
...
fail:
	if(st.states&FUNCTION)		
		st.states &= ~FUNCTION;
	sh_fail(ion,ksh_gettxt(_SGI_DMMX_e_file,e_file),ERROR);
	/* NOTREACHED */
}
...
/* 				c = (fp->flast&~MARK); */
		...
		case F_ISFILE:
			...
			if(sh.readscript)
			{
				register wchar_t *cp = st.cmdadr;
				st.cmdadr = sh.readscript;
				sh_fail(cp,ksh_gettxt(_SGI_DMMX_e_exec,e_exec),EXEBAD);
			}
		default:
...
register int flag;
{
	...
	filecreate:
		created++;
		fd = ksh_creat(name,(flag&O_CREAT)?mode:0644);
	}
	else
	{
		fd = ksh_open(name,flag&(O_RDWR|O_RDONLY|O_WRONLY));
