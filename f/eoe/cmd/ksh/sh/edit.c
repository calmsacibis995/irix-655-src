...
static int compare();
    ...
    static wchar_t bellchr[] = L"\7";	/* bell char */
#   define tenex 1
#   ifdef tenex
	static wchar_t *overlay();
#   endif /* tenex */
#endif /* VSH || ESH */
...
int ed_window()
{
	...
	register wchar_t *cp = nam_strval(COLUMNS);
	if(cp)
	{
		n = wcstol(cp,(wchar_t **)0,10)-1;
		if(n > MAXWINDOW)
			n = MAXWINDOW;
	}
#ifdef TIOCGWINSZ
...
void ed_flush()
{
	...
	if(n<=0)
		return;
	ksh_write(fd,editb.e_outbase,(unsigned)n);
...
register wint_t i;
{
	...
	if(i==L'_')
		...
		macro[2] = 0L;
	if (is_walnum(i)&&(np=nam_search(macro,sh.alias_tree,N_NOSCOPE))&&(out=nam_strval(np)))
	{
#ifdef MULTIBYTE
		/* copy to buff in internal representation */
		wint_t c = out[LOOKAHEAD];
		out[LOOKAHEAD] = 0L;
		i = ed_internal(out,buff);
		out[LOOKAHEAD] = c;
#else
		wcsncpy((wchar_t*)buff,out,LOOKAHEAD);
	i = wcslen((wchar_t*)buff);
#endif /* MULTIBYTE */
		while(i-- > 0)
			ed_ungetchar(buff[i]);
		return(1);
...
int mode;
{
...
#ifdef MULTIBYTE
	{
		register wint_t c = *cur;
		register wchar_t *cp;
		/* adjust cur */
		cp = (wchar_t *)outbuff + *cur;
		c = *cp;
		*cp = 0L;
		*cur = ed_external((wchar_t*)outbuff,(wchar_t*)stakptr(0));
		*cp = c;
	...
	ap->argnxt.ap = 0;
	{
		...
		if(mode==L'\\')
			addstar = L'*';
#endif /* tenex */
		*((wchar_t *)stakptr(staktell()-WC_SZ)) = addstar;
		wc_stakfreeze(WC_SZ);
	}
	if(mode!=L'*')
		on_option(MARKDIR);
	{
		...
		st.intfn = savfn;
		/*  match? */
		if (*com==0L || (!istilde && narg <= 1 && eq(ap->argval,*com)))
		{
			rval = -1;
			goto done;
		}
		...
		*cur = (out-outbuff);
		...
		out = sh_copy((wchar_t *)stakptr(0),out);
		*eol = (out-outbuff);
	}
 done:
	stakset(staksav,0);
...
#   ifdef tenex
static wchar_t *overlay(str,newstr)
register wchar_t *str,*newstr;
{
...
ed_fulledit()
{
	...
	if(editb.e_hline == editb.e_hismax)
	{
		if(editb.e_eol<=0)
			return(BAD);
		editb.e_inbuf[editb.e_eol+1] = 0L;
	...
	cp = sh_copy(wc_e_runvi, editb.e_inbuf);
	cp = sh_copy(sh_itos(editb.e_hline), cp);
	editb.e_eol = cp - editb.e_inbuf;
	return(GOOD);
}
#endif	/* KSHELL */
...
retry:
	...
	{
		errno=0;
		if ((i = ksh_read(fildes,readin, nchar)) != -1)
			break;
                else if (errno == EINTR)
                {
#ifdef JOBS
...
register wint_t c;
{
	register wchar_t *dp = editb.e_outptr;
#ifdef MULTIBYTE
	register int d;
...
int cur;
{
	register wchar_t *sp = virt;
	register wchar_t *dp = phys;
	register wint_t c;
	wchar_t *curp = sp + cur;
