...
void	sh_freeup()
{
	...
	while(iop)
	{
		ksh_unlink(iop->ioname);
		if(iop->iolink)
			free((char *)iop->iolink);
		iop=iop->iolst;
	}
	st.iotemp = 0;
	if(st.staklist)
...
register int flg;
{
	...
	else if((t=item(NLFLG|MTFLG)) && sh.wdval==L'|')
	{
...
register int esym;
{
	...
	else
	{
		...
		while(1)
		{
			...
			r->regptr=sh.wdarg;
			if(sh.wdval==L')' || sh.wdval==L'|' || ( sh_lex()!=L')' && sh.wdval!=L'|' ))
				sh_syntax();
			if(sh.wdval==L'|')
				sh_lex();
			else
		...
		sh.wdset &= ~IN_CASE;
		r->regcom=sh_cmd(0L,NLFLG|MTFLG);
		if(sh.wdval==ECSYM)
...
int	flag;
{
	...
	switch(sh.wdval)
	{
		...
		case SELSYM:
		{
			...
			t->for_.forlst=0;
			chkword();
			t->for_.fornam=(wchar_t*) sh.wdarg->argval;
			if(skipnl()==INSYM)
			{
				chkword();
				t->for_.forlst=(struct comnod*) item(0);
				if(sh.wdval!=NL && sh.wdval!=L';')
					sh_syntax();
				if(sh.wdval==NL)
					sh_prompt(0);
				skipnl();
		...
		funct_5_2:
		{
			...
			if(flag)
				chkword();
			t->proc.procnam= (wchar_t*)sh.wdarg->argval;
			skipnl();
		...
		case 0L:
		{
			...
			retry:
				...
				if((sh.wdval==IPROC ||sh.wdval==OPROC))
				{
					...
					sh_copy(wc_e_nullstr, argp->argval);
					argno = -1;
					*argtail = argp;
					argtail = &(argp->argnxt.ap);
				...
				sh_lex();
#endif	/* DEVFD */
				if(argno==1 && !t->com.comset && sh.wdval== L'(')
				{
					/* SVR2 style function */
					sh_lex();
					if(sh.wdval == L')')
...
register int sym;
{
	if(sym&SYMFLG)
	{
		...
		p_str(ksh_gettxt(_SGI_DMMX_e_endoffile,e_endoffile),0L);
	else
	{
		if(sym&SYMREP)
			p_char(sym);
		if(sym==NL)
			p_str(ksh_gettxt(_SGI_DMMX_e_nlorsemi,e_nlorsemi),0L);
		else
			p_char(sym);
	}
	p_char(L'\'');
...
void	sh_syntax()
{
	...
	{
		p_str(ksh_gettxt(_SGI_DMMX_e_atline,e_atline),0L);
		p_num(sh.olineno,SP);
	}
	p_str(wc_e_colon,L'`');
	if(w)
	...
	else
	{
		...
		p_str(sh.wdarg->argval,L'\'');
	}
	if(cp == e_unexpected)
		p_str(ksh_gettxt(_SGI_DMMX_e_unexpected,e_unexpected),NL);
	else	p_str(ksh_gettxt(_SGI_DMMX_e_unmatched,e_unmatched),NL);
	exitset();
	sh_exit(SYNBAD);
}
/*
	...
	argp->argflag = 0;
	{
		...
		while(c= *sp++)
		{
			...
			else if(isexp(c))
			{
				...
				{
					if(c == L'{' && *sp == WCNULL && is_option(WORDEXP))
						sh_exit(CTXBAD);
					/* special case of L'[' */
					if(*sp || c!=L'[')
						flag |= EXP_FILE;
...
int argn;
{
	...
	dp = (struct dolnod*)stakalloc((unsigned)sizeof(struct dolnod) + EXTRA*sizeof(wchar_t*) + argn*sizeof(wchar_t*));
	cp = dp->dolarg+EXTRA;
	dp->doluse = argn;
	ap = ac->comarg;
	while(ap)
...
register int offset;
{
	if(st.exec_flag)
		return(st.cmdline);
	return(st.standin->flin-offset-((st.peekn)==NL));
}
