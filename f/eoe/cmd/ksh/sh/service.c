...
static wchar_t		*prune();
static wchar_t		*execs();
...
static wchar_t		*xecmsg;
static int		 xecrval;
static wchar_t		**xecenv;
static int		pruned;
...
wchar_t *path_pwd(flag)
int flag;
{
	register wchar_t *cp;
	register wchar_t *dfault = (wchar_t *)wc_e_dot;
	register int count = 0;
	extern WCMSG	wc_e_crondir;
	if(sh.pwd)
		return(sh.pwd);
	while(1) 
	{
		...
		if(cp && *cp==L'/' && test_inode(cp,wc_e_dot))
			break;
	}
	if(count>1)
...
@*/
{
	register wchar_t *path;
	register wchar_t *sp = sh.lastpath;
	if(wcschr(s,L'/'))
		return(WCNULLSTR);
	path = nam_fstrval(PATHNOD);
	if(path==WCNULL)
		path = (wchar_t *)wc_e_defpath;
	path = stakcopy(path);
 ...
 @*/
{
	...
	{
		if(is_option(RSHFLG))
			sh_fail(name, ksh_gettxt(_SGI_DMMX_e_restricted,e_restricted),ERROR);
	}
	else
	{
		if(path==WCNULL)
	...
	do
	{
		...
		if((n = ksh_open(path_relative((wchar_t *)stakptr(WC_OPATH)),O_RDONLY)) >= 0)
		{
...
@*/
{
	...
	{
		register wchar_t *vp = np->value.namval.cp;
		register int n = 1;
		register int nofree = nam_istype(np,N_FREE);
		nam_offtype(np,~NO_PRINT);
		if(vp==0L || (n=wcscmp(sp,vp))!=0)
			nam_putval(np,sp);
...
/*@
 ...
 @*/
{
	register wchar_t *start = (wchar_t *)name;
	while (*name)
		if ((*name++ == L'/') && *name)	/* don't trim trailing / */
			start = (wchar_t *)name;
	return (start);
}
...
@*/
{
	...
	if(flag)
	{
		...
		while((sh.lastpath=path_absolute(name))==0L && pruned)
			nam_ontype(tracknod,NO_ALIAS);
	}
	if(flag==0 || sh.lastpath==0L)
	{
		register wchar_t *path;
		int savestates;
...
@*/
{
	...
	do
	{
		...
		path=path_join(path,name);
#endif /* VPIX */
		f = canexecute((wchar_t *)stakptr(WC_OPATH));
	}
#ifdef VPIX
	...
	if(*((wchar_t *)stakptr(WC_OPATH))!=L'/')
		path_join(path_pwd(1),(wchar_t*)(wc_stakfreeze(WC_SZ)+WC_OPATH));
	return((wchar_t*)(wc_stakfreeze(WC_SZ)+WC_OPATH));
}
...
@*/
{
	...
	if(ksh_stat(path,&statb) < 0)
	{
		if(errno!=ENOENT) {
			xecmsg = ksh_gettxt(_SGI_DMMX_e_exec,e_exec);
			xecrval = EXEBAD;
		}
		return(-1);
	}
...
wchar_t *path_relative(file)
register const wchar_t *file;
/*@
	assume file!=NULL;
	return x satisfying x!=NULL;
@*/
{
	register wchar_t *pwd;
	register wchar_t *fp = (wchar_t*)file;
...
@*/
{
	register const wchar_t *path = wc_e_nullstr;
	register wchar_t **t = at;
	xecmsg=ksh_gettxt(_SGI_DMMX_e_found,e_found);
	xecrval = FNDBAD;
	...
	if(wcschr(t[0],L'/'))
	{
		...
		if(is_option(RSHFLG))
			sh_fail(t[0],ksh_gettxt(_SGI_DMMX_e_restricted,e_restricted),ERROR);
	}
	else {
		path=path_get(*t);
	}
 ...
 */
static wchar_t *prune(path,fullname)
register wchar_t *path;
...
@*/
{
	register wchar_t *p = path;
	register wchar_t *s;
	int n = 1; 
	const wchar_t *base;
	wchar_t *inpath = path;
	if(fullname==WCNULL  || *fullname != L'/' || *path==0L)
		return(path);
	...
	do
	{
		...
		if(*s != L'/' || (n=wcscmp((wchar_t *)stakptr(WC_OPATH),fullname))==0)
		{
...
@*/
{
	register wchar_t *p, *prefix;
	prefix=path_join(ap,t[0]);
	xecenv[0] =  (wchar_t *)stakptr(0);
	*((wchar_t *)stakptr(0)) = L'_';
	*((wchar_t *)stakptr(WC_SZ)) = L'=';
	p=(wchar_t *)stakptr(WC_OPATH);
	p_flush();
#ifdef VPIX
	if(dospath(ap))
	{
		...
		while(**suffix)
		{
			...
			if(canexecute(p)>=0)
			{
				...
				{
					case ENOENT:
						sh_fail(vp,ksh_gettxt(_SGI_DMMX_e_found,e_found),FNDBAD);
					default:
						sh_fail(vp,ksh_gettxt(_SGI_DMMX_e_exec,e_exec),EXEBAD);
				}
			}
		}
		t[0] = savet;
		*((wchar_t *)stakptr(suffix_offset)) = 0L;
	}
#endif /* VPIX */
	if(sh.trapnote&SIGSET)
		sh_exit(SIGFAIL);
/* 	p = path_relative(p); Bug #594441 */
	...
	fail:
			sh_fail(p, ksh_gettxt(_SGI_DMMX_e_open,e_open),ERROR);
	openok:
			close(n);
#endif /*SUID_EXEC */
			/* get name returns the interpreter name */
			if(get_shell(p, iname)<0)
				sh_fail(p, ksh_gettxt(_SGI_DMMX_e_exec,e_exec),EXEBAD);
			t--;
			t[0] = iname;
...
#ifdef ENAMETOOLONG
		case ENAMETOOLONG:
			xecmsg = ksh_gettxt(_SGI_DMMX_e_longname,e_longname);
			xecrval = ERROR;
			return(prefix);
#endif /* ENAMETOOLONG */
		case ENOMEM:
			sh_fail(p,ksh_gettxt(_SGI_DMMX_e_toobig,e_toobig),ERROR);

		case E2BIG:
			sh_fail(p,ksh_gettxt(_SGI_DMMX_e_arglist,e_arglist),ERROR);

		case ETXTBSY:
			sh_fail(p,ksh_gettxt(_SGI_DMMX_e_txtbsy,e_txtbsy),ERROR);
#ifdef ELIBACC
		case ELIBACC:
			sh_fail(p, ksh_gettxt(_SGI_DMMX_e_libacc,e_libacc),ERROR);

		case ELIBBAD:
			sh_fail(p, ksh_gettxt(_SGI_DMMX_e_libbad,e_libbad),ERROR);

		case ELIBSCN:
			sh_fail(p, ksh_gettxt(_SGI_DMMX_e_libscn,e_libscn),ERROR);

		case ELIBMAX:
			sh_fail(p, ksh_gettxt(_SGI_DMMX_e_libmax,e_libmax),ERROR);
#endif /* ELIBACC */
		 ...
		 */
		case EBADRQC:
			sh_fail(p, ksh_gettxt(_SGI_DMMX_e_badmagic,e_badmagic),ERROR);
			break;
#endif /* sgi */
...
@*/
{
	...
	if(--st.fn_depth==1 && mode==3)
		sh_fail(com[0],ksh_gettxt(_SGI_DMMX_e_recursive,e_recursive),ERROR);
...
@*/
{
	...
	return(1);
#else /* apollo */
	register wchar_t *cp = path;
	wchar_t buffer[PATH_MAX+1];
	register wchar_t *savecp;
	int depth = 0;
	wchar_t c;
	int n;
	while(*cp)
	{
		...
		c = *cp;
		*cp = 0L;
		n = ksh_readlink(path,buffer,PATH_MAX);
		*cp = c;
		if(n>0)
		{
			if(++depth > MAXDEPTH)
			...
			wcscpy(buffer+n,cp);
			if(*buffer==L'/')
				cp = wcscpy(path,buffer);
			else
			{
				/* check for path overflow */
				cp = savecp;
				if((wcslen(buffer)+(cp-path)) > PATH_MAX)
					return(0);
				wcscpy(cp,buffer);
			}
		}
