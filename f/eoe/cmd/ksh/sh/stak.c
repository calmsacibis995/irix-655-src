 ...
 */
wchar_t	*stakcopy(str)
const wchar_t *str;
{
	register Stak_t *sp = &_stak_cur;
	register wchar_t *cp = (wchar_t *)str;
	register int n;
	while(*cp++);
	n = round((cp-str)*WC_SZ,rndsize);
	increment(copy);
	if((sp->stakleft += ((sp->staktop-sp->stakbot)-n)) <=0)
		_stakgrow(n);
	cp = (wchar_t *)sp->stakbot;
	wcscpy(cp,str);
	sp->stakbot = sp->staktop = (char *)cp+n;
	return(cp);
}
