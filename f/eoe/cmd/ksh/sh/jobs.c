...
#   endif /* alliant */
	...
	while((job.mytgid=tcgetpgrp(JOBTTY)) != job.mypgid)
	{
		...
		if(ntry++ > MAXTRY)
		{
			p_str(ksh_gettxt(_SGI_DMMX_e_no_start,e_no_start),NL);
			return;
		}
	}
#  endif /* WAIT1ARG */
...
#	endif /* FIOPUSHLD */
		if(lflag==0)
			p_str(ksh_gettxt(_SGI_DMMX_e_newtty,e_newtty),NL);
		else
			job.linedisc = -1;
	}
#   else
...
int job_close()
{
	...
	if(beenhere++ == 0 && job.pwlist)
	{
		...
		if(count)
		{
			p_str(ksh_gettxt(_SGI_DMMX_e_terminate,e_terminate),NL);
			return(-1);
		}
		else if(sh.login_sh)
		{
			p_str(ksh_gettxt(_SGI_DMMX_e_running,e_running),NL);
			return(-1);
		}
	}
#   ifndef  WAIT1ARG
...
#	else
		if(ioctl(JOBTTY,TIOCSETD,&job.linedisc) !=0)
			return(0);
#	endif /* FIOPUSHLD */
		p_str(ksh_gettxt(_SGI_DMMX_e_oldtty,e_oldtty),NL);
	}
#   endif /* NTTYDISC */
#   ifdef CNSUSP
	if(job.suspend==CNSUSP)
...
wchar_t *jobs[];
{
	...
	else while(jobid = *jobs++)
	{
		job_string = jobid;
		if(*jobid==0L)
			sh_fail(job_string,ksh_gettxt(_SGI_DMMX_e_jobusage,e_jobusage),ERROR);
		if(*jobid == L'%')
			pw = job_bystring(jobid);
		else
		{
			int pid = wcstol(jobid,(wchar_t **)0,10);
			if(pid<0)
				jobid++;
			while(iswdigit(*jobid))
				jobid++;
			if(*jobid)
				sh_fail(job_string,ksh_gettxt(_SGI_DMMX_e_jobusage,e_jobusage),ERROR);
			if(!(pw = job_bypid(pid)))
...
register int flag;
{
	...
	do
	{
		...
		{
			msg = ksh_gettxt(_SGI_DMMX_e_done,e_done);
			n = px->p_exit;
		}
		else
			msg = ksh_gettxt(_SGI_DMMX_e_Running,e_Running);
		p_str(msg,0L);
		msize = wcslen(msg);
		if(n)
		{
			p_str(wc_e_nullstr,L'(');
			p_num((int)n,L')');
			msize += (3+(n>10)+(n>100));
		}
		if(px->p_flag&P_COREDUMP)
		{
			p_str(ksh_gettxt(_SGI_DMMX_e_coredump,e_coredump),0L);
			msize += wcslen(ksh_gettxt(_SGI_DMMX_e_coredump,e_coredump));
		}
		...
		if(flag&L_FLAG)
			...
			px = NULL;
		if(px==NULL)
			hist_list(pw->p_name,0L,L";");
		else
			p_str(wc_e_nlspace,0L);
	}
	while(px);
	px = pw;
...
register wchar_t *ajob;
{
	...
	c = *ajob;
	if(iswdigit(c))
		pw = job_byjid( wcstol(ajob,(wchar_t **)0,10));
	else if(c==L'+' || c==L'%')
		;
	else if(c==L'-')
	{
...
#	define stopsig	1
	...
	if(pw->p_flag&P_BYNUM)
	{
		if(pid==0 && job.jobcontrol)
			return(job_walk(job_kill,sig,(wchar_t**)0));
#ifdef SIGTSTP
...
#   endif /* _poll_ */
	...
	error:
		p_setout(ERRIO);
		p_str(ksh_gettxt(_SGI_DMMX_e_killcolon,e_killcolon),0L);
		if(pw && pw->p_flag&P_BYNUM)
			msg = ksh_gettxt(_SGI_DMMX_e_no_proc,e_no_proc);
		else
			msg = ksh_gettxt(_SGI_DMMX_e_no_job,e_no_job);
		p_str(job_string,0L);
		p_str(wc_e_colon,0L);
		if(errno == EPERM)
			msg = ksh_gettxt(_SGI_DMMX_e_access,e_access);
		p_str(msg,NL);
		r = 2;
	}
	return(r);
...
wchar_t *name;
{
	...
	for(;pw;pw=pw->p_nxtjob)
	{
		...
		{
			if(pz)
				sh_fail(name-1,ksh_gettxt(_SGI_DMMX_e_ambiguous,e_ambiguous),ERROR);
			pz = pw;
		}
	}
	return(pz);
...
#ifdef DEBUG
		p_num(getpid(),L':');
		p_str(L" job with pid",L' ');
		p_num(p,L' ');
		p_str(L"complete with status",L'=');
		p_num(w,L' ');
		p_str(L"jobid",L'=');
		p_num(pw->p_job,NL);
		p_flush();
			...
			if (WIFKILLED(wstat))
			{
...
#endif /* JOBS */
				{
					...
					if(pw->p_exit!=SIGINT &&
						pw->p_exit!=SIGPIPE)
					{
						register wchar_t *msg;
						if(pid!=p||!(st.states&PROMPT))
						{
							wchar_t n[20];
							sh_copy(sh_itos(p),n);
							p_prp(n);
							p_str(wc_e_nullstr,SP);
						}
						...
						p_str(msg,(p==0?0L:NL));
						if(p==0)
							p_str(ksh_gettxt(_SGI_DMMX_e_coredump,e_coredump),NL);
					}
					if(!intr)
						pw = job_byjid(jobid);
				}
...
static wint_t *job_sigmsg(sig)
int sig;
{
	static wchar_t *signo = L"Signal xxx";
#ifdef apollo
	/*
