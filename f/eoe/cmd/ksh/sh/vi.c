...
#   define isalph(v)	is_walnum(virtual[v])
#   define isblank(v)	__iswblank(virtual[v])
...
#define wide_cnt	editb.e_wide_cnt	/* double width (or more) extra count */
...
static wchar_t last_cmd = L'\0';		/* last command */
static char repeat_set;
static char nonewline;
static wchar_t *lastline;
static wchar_t paren_chars[] = L"([{)]}";   /* for % command */
...
register wchar_t *shbuf;			/* shell line buffer */
unsigned nchar;				/* number of chars to read */
{
	register wint_t c;			/* general variable */
	register int i;			/* general variable */
	register wint_t term_char;	/* read() termination character */
	wchar_t prompt[PRSIZE+2];		/* prompt */
	wchar_t Physical[2*MAXLINE];	/* physical image */
	wchar_t Ubuf[MAXLINE];	/* used for U command */
	wchar_t ubuf[MAXLINE];	/* used for u command */
	wchar_t Window[WINDOW+10];	/* window image */
	char cntl_char;			/* TRUE if control character present */
	static wint_t Globals[9];			/* local global variables */
#ifndef FIORDCHK
	...
	if( !is_option(VIRAW) )
	{
		...
		{
			return(ksh_read(fd, shbuf, nchar));
		}
...
#   endif /* FIORDCHK */
		...
		if( i == 0 )
		{
			...
			term_char = L'\033';
			shbuf[i--] = L'\0';	/* null terminate line */
		}
...
#else
	if(yankbuf==0L) {
		yankbuf = (wchar_t*)malloc(sizeof(wchar_t)*(MAXLINE));
	}
#endif
...
#else
	if(lastline==0L) {
		lastline = (wchar_t*)malloc(sizeof(wchar_t)*(MAXLINE));
	}
#endif
	if( last_cmd == L'\0' )
	{
...
#   endif /* !ECHOCTL */
		...
		if( term_char!=L'\033'  && (last_virt==INVALID
...
# endif /*ECHOCTL */
		...
		if( crallowed == YES )
		{
			...
			window[0] = L'\0';
			pr_prompt();
			if( term_char==L'\033' && virtual[last_virt]!=L'\033' )
				refresh(CONTROL);
			else
				refresh(INPUT);
		}
...
cntlmode()
{
	...
	while( c=getchar() )
	{
		...
		case cntl(L'V'):
		{
			register const wchar_t *p = &wc_e_version[5];
			save_v();
			del_line(BAD);
			while(c = *p++)
				append(c,APPEND);
		...
		case L'j':		/** get next command **/
		case L'+':		/** get next command **/
			curhline += repeat;
			if( curhline > histmax )
			{
				curhline = histmax;
		...
		case L'k':		/** get previous command **/
		case L'-':		/** get previous command **/
			if( curhline == histmax )
			{
				u_space = tmp_u_space;
				i = u_column;
			...
			save_v();
	newhist:
			oldlen = wcswidth(virtual,MAXLINE);
			hist_copy((wchar_t*)virtual, curhline, -1);
			newlen = wcswidth(virtual,MAXLINE);
			while(newlen < oldlen)
			{
				wcscat(virtual,L" ");
				++newlen;
...
#endif /* MULTIBYTE */
			if( (last_virt = genlen((wchar_t*)virtual) - 1) >= 0 )
				cur_virt = 0;
			else
				cur_virt = INVALID;
			break;
		...
		case L'u':		/** undo the last thing done **/
			restore_v();
			break;

		case L'U':		/** Undo everything **/
			save_v();
			if( virtual[0] == L'\0' )
				goto ringbell;
			else
			{
				int diff;
				if((diff = (wcswidth(virtual,MAXLINE) - wcswidth(U_space,MAXLINE))) >= 0)
					wide_cnt = diff;
		...
		case L'#':	/** insert(delete) # to (no)comment command **/
			if( cur_virt != INVALID )
			{
...
char mode;
{
	...
	if( nchars > 0 )
	{
		...
		if( mode != 'y' )
		{
			...
			/* calculate number of extra spaces needed
			 * for deleted chars that take more than one
			 * physical screen location.
			 */
			while(w<nchars)
			{
				switch(len=wcwidth(*(vp+w)))
				{
					case 0:
					case -1:
						break;
...
#       define delta end
	...
	virtual[last_virt--] = 0L;
	if(end==BAD)
		return(BAD);

	end = cur_virt;
	if( mode=='c' && end>begin && wcschr(L"wW", motion) )
	{
...
register int mode;
{
	...
	escape:
		...
		case cntl(L'W'):		/** delete back word **/
...
register wint_t motion;
{
	...
	case L'0':		/** First column **/
		tcur_virt = 0;
		break;

	case L'^':		/** First nonblank character **/
		tcur_virt = first_virt;
		while( isblank(tcur_virt) && tcur_virt < last_virt )
			++tcur_virt;
		break;
	...
	case L',':		/** reverse find old char **/
	case L';':		/** find old char **/
		switch(last_find)
		{
		case L't':
		case L'f':
	...
	case L't':		/** find up to new char forward **/
	case L'f':		/** find new char forward **/
		bound = last_virt;
		incr = 1;

	case L'T':		/** find up to new char backward **/
	case L'F':		/** find new char backward **/
		last_find = motion;
		if((findchar=getrchar())==L'\033')
			return(GOOD);
find_b:
		motion_arg = findchar;
        ...
        case L'%':
	{
		...
		while( tcur_virt <= last_virt
			&& wcschr(paren_chars,virtual[tcur_virt])==(wchar_t*)0)
				tcur_virt++;
		if(tcur_virt > last_virt )
			return(BAD);
		nextc = virtual[tcur_virt];
		count = wcschr(paren_chars,nextc)-paren_chars;
		if(count < 3)
		{
			incr = 1;
			bound = last_virt;
...
{
	/*** copy string s ***/
	register wchar_t *ptr = editb.e_outptr;
	while(*s)
		*ptr++ = *s++;
	editb.e_outptr = ptr;
	return;
...
#	define	v	regb
	...
	/*** erase trailing characters for wide chars if needed ***/
...
restore_v()
{
	...
	tmpcol = u_column;
	save_v();
	if((diff = (wcswidth(virtual,MAXLINE) - wcswidth(tmpspace,MAXLINE))) >= 0)
		wide_cnt = diff;
...
register wchar_t mode;
{
	...
	if( mode == L'/' || mode == L'?')
	{
		...
		getline(SEARCH);
		first_virt = 0;
		virtual[last_virt + 1] = L'\0';	/*** make null terminated ***/
		direction = mode==L'/' ? -1 : 1;
	}
	...
	if( cur_virt==0L ||  fold(mode)==L'N' )
	{
		/*** user wants repeat of last search ***/
		del_line(BAD);
		wcscpy( ((wchar_t*)virtual)+1, lsearch);
#ifdef MULTIBYTE
		*((char*)virtual) = '/';
		ed_internal((char*)virtual,virtual);
#endif /* MULTIBYTE */
...
#endif /* MULTIBYTE */
	location = hist_find(((wchar_t*)virtual)+1, curhline, 1, new_direction);
	wcsncpy(lsearch, ((wchar_t*)virtual)+1, SEARCHSIZE);
...
{
	register int i;
	register wchar_t *p = lastline;
	register int trepeat = repeat;
	static int lastmacro;
	genchar *savep;

	if(mode && (fold(lastmotion)==L'F' || fold(lastmotion)==L'T')) 
		lastmotion = L';';
...
addin:
...
#ifdef KSHELL
	case L'*':		/** do file name expansion in place **/
	case L'\\':		/** do file name completion in place **/
		if( cur_virt == INVALID )
			return(BAD);
	case L'=':		/** list file name expansions **/
		save_v();
		i = last_virt;
		++last_virt;
		virtual[last_virt] = 0L;
		if( ed_expand((wchar_t*)virtual, &cur_virt, &last_virt, c) )
		{
			last_virt = i;
			bell;
		}
...
#endif	/* KSHELL */
	case L'_':		/** append last argument of prev command **/
		save_v();
		{
			wchar_t tmpbuf[MAXLINE];
			if(repeat_set==0)
				repeat = -1;
			p = (wchar_t*)hist_word((wchar_t *)tmpbuf,repeat);
#ifndef KSHELL
			if(p==0)
			{
				bell;
	...
	case L'A':		/** append to end of line **/
		cur_virt = last_virt;
		sync_cursor();
	...
	case L'I':		/** insert at beginning of line **/
		cur_virt = first_virt;
		sync_cursor();
...
deleol:
	...
	case L'Y':		/** Yank to end of line **/
		c = L'$';
		goto yankeol;

	case L'y':		/** yank thru motion **/
		if( mode )
			c = lastmotion;
		else
			c = getcount(getchar());
yankeol:
		...
		else if( delmotion(c, L'y') == BAD )
		{
			return(BAD);
		}
		break;

	case L'x':		/** delete repeat chars forward - dl **/
		c = L'l';
		goto deleol;

	case L'X':		/** delete repeat chars backward - dh **/
		c = L'h';
		goto deleol;

	case L'~':		/** invert case and advance **/
		if( cur_virt != INVALID )
		{
			save_v();
			i = INVALID;
