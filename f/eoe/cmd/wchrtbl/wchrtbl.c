...
/*
 * If the keyword ENCODING is found in the source file,
 * the mbtowc() converter for that encoding will be used
 * to get the wc value, instead of the built-in one (which
 * only works for euc). Note: the ENCODING keyword has to
 * appear before any character codes.
 *
 * open_encoding() tries to open that converter based on the
 * encoding name. BtoVAL() uses that converter if the converter
 * was successfully opened. Otherwise, it uses the built-in
 * converter.
 */
static void     open_encoding ( unsigned char * enc );
static int	BtoVAL ( int );
...
char	encoding [ 256 ] = "";		/* mb encoding name */
...
/*   22    */ MSGNUM_DEFMSG ( _MSG_WCHRTBL_NO_ENCODING ),
/*   23    */ MSGNUM_DEFMSG ( _MSG_WCHRTBL_CANT_OPEN_MBTOWC ),
/*   24    */ MSGNUM_DEFMSG ( _MSG_WCHRTBL_MBTOWC_FAILED ),
};
...
char	**argv;
{
	...
	/* Check if we should use another iconvtab file.
	 * This may be needed since we might be in the process
	 * of creating a locale which is not yet available on the
	 * build system, and hence the mbwc_get_mbhandle function
	 * may not be able to return the correct pointer
	 *
	 * Note: this has to be done before setlocale() since
	 * that function is the first to use __iconv_tabname
	 */
	char * iconv_tabname;
	if ( iconv_tabname = getenv ( "_ICONVTAB_NAME" ) )
	    __iconv_tabname__ = iconv_tabname;
...
void check_posix()
{
    ...
    for ( cl = cln; cl->name != NULL; cl ++ )
    {
        ...
        for ( c = cl->posix_members; *c != NULL; c ++ )
            if ( ! ( ( ctype + 1 )[ *c ] & cl->num ) )
               pfmt( stderr,MM_WARNING, MSGNUM_DEFMSG ( _MSG_WCHRTBL_POSIX_WARN ), *c, cl->name );
    }
}
...
static ICONV_MBTOWC	mbtowc_f;
static void *		iconv_handle = NULL;

extern _iconv_mbhandle_t
__mbwc_get_mbhandle ( const char * loc, _iconv_mbhandle_t mbhandle );


static void
open_encoding ( unsigned char * enc )
{
    _iconv_mbhandle_t	mbhandle = NULL;

    mbhandle = __mbwc_get_mbhandle ( (const char *)enc, NULL );

    if ( !mbhandle || (void*)mbhandle == (void*)-1 )
	error ( msg[23], input, enc );	/* cannot open mbtowc converter */
    
    mbtowc_f = mbhandle->iconv_meths->iconv_mbtowc;
    iconv_handle = mbhandle->iconv_handle;
}

char * int2mbchar ( const unsigned int i, char * mb )
{
    int c = sizeof ( unsigned int ) -1;
    char * p = mb;
...
static int
BtoVAL ( const int mbint )
{
    if ( mbtowc_f )
    {
	static char	mbchar [ MB_LEN_MAX ];
	static wchar_t	wc;

	int2mbchar ( mbint, mbchar );

	if ( (*mbtowc_f) ( & wc, mbchar, MB_LEN_MAX, iconv_handle ) == -1 )
	    error ( msg[24], input, mbint ); /* mbtowc() failed */

	/* Remove codeset mask */
	return (int) ( ~EUCMASK & wc );
    }
    else
	/* Default to the built-in mbtowc converter */
	return ((mbint & 0x7f)|((mbint & 0x7f00)>>1)|((mbint & 0x7f0000)>>2));
