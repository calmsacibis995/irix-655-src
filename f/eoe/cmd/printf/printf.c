...
/* Internationalized to handle strings of multibyte chars.	*/
...
printbyte(char *fmt, char *str)
{
	...
	for (s = p  = str  ; *p != '\0' && quit == 0 ; p++) {
		if(!(I18N_SBCS_CODE || I18N_EUC_CODE) ) {
			if (dno_of_bytes == 0) {
				dno_of_bytes = mblen(p, MB_CUR_MAX);
				if (dno_of_bytes < 1) dno_of_bytes = 1 ;
				if (dno_of_bytes == 1) 
					is_mb = 0 ;
				else
					is_mb = 1 ;
	...
	if (str[0] == '\0')
		lval = 0;
		 ...
		 if (mbtowc(&wc, &str[1], MB_CUR_MAX) <= 0) {
			pfmt(stderr, MM_WARNING, PFMTTXT(_MSG_MMX_printfbadc), str);
			errcode++;
		 }
		 lval = wc ;
	} else {
		...
		if (*badnum != '\0') {
			if (lval) {
				...
				pfmt(stderr, MM_WARNING, PFMTTXT(_MSG_MMX_printfbads), str);
			}
			errcode++;
		} else if (errno == ERANGE) {
			pfmt(stderr, MM_WARNING, PFMTTXT(_MSG_MMX_printfoverf), str);
	    		setoserror(0);
			errcode++;
		}
	}
