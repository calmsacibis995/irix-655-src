#ident	"cmd/failover/foconfig.c: $Revision: 1.3 $"
...
main (int argc, char *argv[])
{
	...
	*/
	...
	while (fgets(input_line,sizeof(input_line),input_file) == input_line) {
		...
		*/
		...
		for (i=0, j=0; i<input_tokens && j<MAX_FO_PATHS; i++) {	/* skips tokenless lines */
			...
			if (*vector == '#') {
				if (i == 0 && (strcmp(vector,"#verbose") == 0)) {
					vflag++;
				}
		...
		*/
		...
		if (!k) {
			...
			printf ("%s: dry run: fgi_lun_vhdl(s) %d %d %d %d.\n",program_name,
				fgi.fgi_lun_vhdl[0],
				fgi.fgi_lun_vhdl[1],
				fgi.fgi_lun_vhdl[2],
				fgi.fgi_lun_vhdl[3]);
