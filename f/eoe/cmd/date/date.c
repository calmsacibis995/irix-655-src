...
#ident "$Header: /proj/irix6.5f/isms/eoe/cmd/date/RCS/date.c,v 1.20 1998/09/18 19:47:22 sherwood Exp $"
...
char	**argv;
{
	...
	if (argc > 1) {
		if (strcmp(argv[1], "-u") == 0) {
			...
			if( retval < 0 ) {
				pfmt(stderr, 
				  MM_ERROR, PFMTTXT(_MSG_DATE_CANNOT_ADJUST),
					strerror(errno));
				exit(1);
			}
			exit(0);
	...
	if (uflag && vflag) {
		...
		strftime(buf, sizeof(buf),
		   (const char *)GETTXT(_MSG_DATE_TIME_FMT),
			tp);
		} else {	/* !uflag */
...
#define badconv PFMTTXT(_MSG_DATE_BAD_CONVERSION)
...
time_t tv;
{
	...
	if (sp == 0) {
                pfmt(stderr,MM_ACTION,PFMTTXTCAT(_MSG_DATE_UDP_UNKNOWN_SERVICE));
		return (0);
	}
	...
	if (s < 0) {
		if (errno != EPROTONOSUPPORT)
                	pfmt(stderr,MM_ACTION,PFMTTXTCAT(_MSG_SOCKET_LF));
		goto bad;
	}
	...
	for (port = IPPORT_RESERVED - 1; port > IPPORT_RESERVED / 2; port--) {
		...
		if (errno != EADDRINUSE) {
			if (errno != EADDRNOTAVAIL)
                                pfmt(stderr,MM_ACTION,PFMTTXTCAT(_MSG_DATE_BIND));
			goto bad;
		}
	}
	if (port == IPPORT_RESERVED / 2) {
        	pfmt(stderr,MM_ACTION,PFMTTXTCAT(_MSG_DATE_ALL_PORTS_IN_USE));
		goto bad;
	}
	...
	if (connect(s, &dest, sizeof(struct sockaddr_in)) < 0) {
        	pfmt(stderr,MM_ACTION,PFMTTXTCAT(_MSG_DATE_CONNECT));
		goto bad;
	}
	if (send(s, (char *)&msg, sizeof (struct tsp), 0) < 0) {
		if (errno == ECONNREFUSED) {
        	...
        	pfmt(stderr,MM_ACTION,PFMTTXTCAT(_MSG_DATE_SEND));
		goto bad;
	}
	...
	for (;;) {
		...
		if (getsockopt(s, SOL_SOCKET, SO_ERROR,
			...
			if (errno == ECONNREFUSED)
				return (0);
                                pfmt(stderr,MM_ACTION,PFMTTXTCAT(_MSG_DATE_SEND_DELAYED_ERR));
			break;
		}
		...
		if (recvfrom(s, (char *)&msg, sizeof (struct tsp), 0,
			     &from, &length) < 0) {
			if (errno != ECONNREFUSED)
                                pfmt(stderr,MM_ACTION,PFMTTXTCAT(_MSG_DATE_RECVFROM));
			break;
		}
...
bad:
        pfmt(stderr,MM_ACTION,PFMTTXTCAT(_MSG_DATE_CANNOT_REACH_TIME_DAEMON));
	(void)close(s);
	return (0);
}
#endif
