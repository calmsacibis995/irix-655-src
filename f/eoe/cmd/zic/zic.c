...
static void	string_error P((const char * message));
...
error(void)
{
	/* called after pfmt(stderr, MM_NOSTD...) */
	/*
...
static void
string_error(const char * const string)
{
	/* print string to stderr. */

	pfmt(stderr, MM_NOSTD, 
		_SGI_MMMX_zic_file_line_err, 
...
associate P((void))
{
	...
	/*		(int (*)(const void * , const void *))rcomp); */
		for (i = 0; i < nrules - 1; ++i) {
			...
			pfmt(stderr, MM_NOSTD,
				 _SGI_MMMX_zic_rulemultifile_err,
				 filename, linenum);
			error();
			--errors;
		...
		if (zp->z_nrules == 0) {
			...
			*/
			if (strchr(zp->z_format, '%') != 0) {
				pfmt(stderr, MM_NOSTD, 
					_SGI_MMMX_zic_zoneruleless_err,
					filename, linenum);
				error();
			}
		}
...
infile(const char * name)
{
	...
	if (strcmp(name, "-") == 0) {
		...
		pfmt(stderr, MM_ERROR, _SGI_MMMX_zic_open_err,
			name, strerror(errno));
		(void) exit(EXIT_FAILURE);
	}
	wantcont = FALSE;
	...
	if (fp != stdin && fclose(fp)) {
		...
		pfmt(stderr, MM_ERROR, 
			_SGI_MMMX_zic_close_err, 
			filename, strerror(errno));
		(void) exit(EXIT_FAILURE);
	}
	if (wantcont) {
		pfmt(stderr, MM_NOSTD, 
			_SGI_MMMX_zic_linecont_err,
			filename, linenum);
		error();
	}
}
...
inzone(char ** const fields, const int nfields)
{
	...
	if (strcmp(fields[ZF_NAME], TZDEFAULT) == 0 && lcltime != NULL) {
		pfmt(stderr, MM_NOSTD, 
			_SGI_MMMX_zic_zonelopt_err,
			filename, linenum, TZDEFAULT);
		error();
		return FALSE;
	}
	if (strcmp(fields[ZF_NAME], TZDEFRULES) == 0 && psxrules != NULL) {
		pfmt(stderr, MM_NOSTD, 
			_SGI_MMMX_zic_rulepopt_err,
			filename, linenum, TZDEFRULES);
		error();
		return FALSE;
	}
	for (i = 0; i < nzones; ++i)
...
inzsub(char ** const fields, const int nfields, const int iscont)
{
	...
	if ((cp = strchr(fields[i_format], '%')) != 0) {
		if (*++cp != 's' || strchr(cp, '%') != 0) {
			pfmt(stderr, MM_NOSTD, 
				_SGI_MMMX_zic_abbrevformat_err,
				filename, linenum);
			error();
			return FALSE;
		}
	...
	*/
	if (timecnt > 1)
		(void) qsort((void *) attypes, (size_t) timecnt,
			(size_t) sizeof (*attypes), 
			(int (*)(const void * , const void *))atcomp);
	/*
	** Optimize.
	*/
	{
...
static int
addtype(const long gmtoff, const char * const abbr, 
        const int isdst, const int ttisstd, const int ttisgmt)
{
	register int	i, j;

	if (isdst != TRUE && isdst != FALSE) {
		string_error(_("internal error - addtype called with bad isdst"));
		(void) exit(EXIT_FAILURE);
	}
	if (ttisstd != TRUE && ttisstd != FALSE) {
		string_error(_("internal error - addtype called with bad ttisstd"));
		(void) exit(EXIT_FAILURE);
	}
	if (ttisgmt != TRUE && ttisgmt != FALSE) {
		string_error(_("internal error - addtype called with bad ttisgmt"));
		(void) exit(EXIT_FAILURE);
	}
	/*
...
leapadd(const time_t t, const int positive, const int rolling, int count)
{
	...
	for (i = 0; i < leapcnt; ++i)
		if (t <= trans[i]) {
			if (t == trans[i]) {
				pfmt(stderr, MM_NOSTD, 
					_SGI_MMMX_zic_repeatlsecond_err,
					filename, linenum);
				error();
				(void) exit(EXIT_FAILURE);
			}
...
yearistype(const int year, const char * const type)
{
	...
	if (result == (1 << 8))
		return FALSE;
	pfmt(stderr, MM_NOSTD, 
		_SGI_MMMX_zic_wildresult_err,
		filename, linenum);
	error();
	pfmt(stderr, MM_ERROR, 
		_SGI_MMMX_zic_command_err, 
		buf, result);
	for ( ; ; )
		(void) exit(EXIT_FAILURE);
}
...
newabbr(const char * const string)
{
	...
	if (charcnt + i > TZ_MAX_CHARS) {
		pfmt(stderr, MM_NOSTD, 
			_SGI_MMMX_zic_toomanyabbrev_err,
			filename, linenum);
		error();
		(void) exit(EXIT_FAILURE);
	}
...
eitol(const int i)
{
	...
	if ((i < 0 && l >= 0) || (i == 0 && l != 0) || (i > 0 && l <= 0)) {
		pfmt(stderr, MM_ERROR, 
			_SGI_MMMX_zic_sign_extend_err, i);
		(void) exit(EXIT_FAILURE);
	}
	return l;
}
