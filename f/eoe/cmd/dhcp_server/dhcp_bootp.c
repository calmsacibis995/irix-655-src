...
char	VERS[] = "SGI bootp/dhcp Server V3.1.0";
...
main(int argc, char *argv[])
{
    ...
    /* Before we do anything we check for a valid NetLS License. */
    /* Removing the licensing stuff */
	...
	if( (dhcp_server == mltaddr) || matchhost(inet_ntoa(ipn)) ) {
		...
		if ( (1 == dhcp_dnsupd_on) && (alt_naming_flag == 0) &&
		    ...
		    dhcp_dnsadd(0, req->dh_ipaddr, req->dh_hsname, 
				client_domain, req->dh_reqLease, dci_data.client_fqdn);
		...
		if (debug) {
		    ipn.s_addr = dhcp_ipaddr;
		    syslog(LOG_DEBUG,
			   "DHCPREQUEST: Init-Reboot from client (%s)",
			   inet_ntoa(ipn));
		}
		/* INIT-REBOOT State */
		mltaddr =
