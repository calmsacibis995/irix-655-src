	...
	bool k;
{
	...
	while ((c = getchar()) != '\n') {
		...
		while (length--)
			*gp++ = *mbp++;
		if (gp >= &globuf[GBSIZE - MB_CUR_MAX - 1])
...
magic:
		...
		if (rp >= &rhsbuf[RHSSIZE - MB_CUR_MAX]) {
toobig:
			*rp = 0;
			error(EXmreppatid, EXmreppat);
		}
...
oncemore:
		multic[i] = '\0';
		length = mbtowc(&wchar, multic, MB_LEN_MAX);
		if (length <= 0) {
			if (i < MB_CUR_MAX) {
				multic[i++] = *rp++;
				goto oncemore;
	...
	register unsigned char *sp, *l1, *l2;
{
	...
	while (l1 < l2) {
		length = mbtowc(&wchar, (char *)l1, MULTI_BYTE_MAX);
		if (length <= 0)
			length = 1;
		l1 += length;
		wchar = wfixcase(wchar);
...
error(":147", "Missing closing delimiter|Missing closing delimiter for regular expression");
	...
	for (;;) {
		...
		if (gp >= &genbuf[LBSIZE - MB_CUR_MAX - 2])
complex:
