...
va_list ap;
{
	unsigned char fcode;
	int prec;
	int scrwid,chlen,length,mask1,nbits,n;
	long int mask2, num;
	register unsigned char *bptr, *mbptr;
	unsigned char *ptr;
	...
	for (;;) {
		...
		switch (fcode = *fmt++) {
			...
			case 'c':
				buf[0] = va_arg(ap, int);
				ptr = bptr = &buf[0];
				chlen = mbtowc(&wc_fcode, bptr, MB_LEN_MAX);
				if(chlen <= 0) {
					chlen = 1;
				} else {
					if (wc_fcode != L'\0')
			...
			case 's':
				...
				if (prec < 0)
					prec = MAXINT;
				mbptr = bptr;
				chlen = mbtowc(&wc_fcode, mbptr, MB_LEN_MAX);
				if(chlen <= 0)
					chlen = 1;
				scrwid = scrwidth(wc_fcode);
				for (n=0; (wc_fcode != L'\0') && n+scrwid <= prec; n += scrwid) {
					mbptr += chlen;
					chlen = mbtowc(&wc_fcode, mbptr, MB_LEN_MAX);
					if(chlen <= 0)
						chlen = 1;
