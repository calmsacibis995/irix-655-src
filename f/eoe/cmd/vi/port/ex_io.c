...
getargs()
{
	...
	static unsigned char fpatbuf[256];	/* hence limit on :next +/pat */
	int length;
	...
	if (peekchar() == '+') {
		for (cp = fpatbuf;;) {
			...
			while (length--)
				*cp++ = *mbp++;
			/*
			 * assumption: for normal ascii chars (c<0200),
			 * length = 1 and c = multic[0].
			 */
			if (cp >= &fpatbuf[sizeof(fpatbuf)-MB_CUR_MAX])
	...
	for (;;) {
		...
		default:
			if (cp > &genbuf[LBSIZE - MB_CUR_MAX - 1])
flong:
