...
char	*av[];
{
	...
	(void)setlabel("UX:ctags");

	if (ac <= 1) {
usage:
		pfmt(stderr, MM_ACTION, PFMTTXT(_MSG_CTAGS_USAGE));
		exit(1);
	}
...
bool	f;		/* f == TRUE when function */
{
	...
	if ((np = (NODE *) malloc(sizeof (NODE))) == NULL) {
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_CTAGS_TOOMANYTAGS));
		put_entries(head);
		free_tree(head);
	...
	NODE *node,*cur_node;
{
	...
	if (dif == 0) {
		if (node->file == cur_node->file) {
			if (!wflag) {
				pfmt(stderr, MM_WARNING, PFMTTXT(_MSG_CTAGS_DUPENTRY),
				    node->file,lineno,node->entry);
				pfmt(stderr, MM_INFO, PFMTTXT(_MSG_CTAGS_ENTRYIGN));
			}
			return;
		}
		if (!cur_node->been_warned)
			if (!wflag)
			    pfmt(stderr, MM_WARNING, PFMTTXT(_MSG_CTAGS_DUPINFILES),
