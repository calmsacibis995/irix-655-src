	...
	char *name;
{
	...
	if (lstat64(name, &st) < 0) {
		errmsg(MM_WARNING, 0, MSGNUM_DEFMSG(_MSG_CANNOT_ACCESS),
		       name, strerror(errno));
                       /* ":5:Cannot access %s: %s\n"        */
		return 1;
	}
	...
	if (chmod(name, newmode(st.st_mode, name, &errmode)) == -1) {
		errmsg(MM_WARNING, 0, MSGNUM_DEFMSG(_MSG_CHMOD_FAILED_ON),
		       name, strerror(errno));
                       /* ":13:chmod() failed on %s: %s\n"   */
		return 1;
	}
	if (errmode)
		return 1;
	...
	mode_t  mode;
{
	...
	if (getcwd(savedir, 1024) == 0) { 
		errmsg(MM_WARNING, 255, MSGNUM_DEFMSG(_MSG_CANT_DET_CUR_DIR));
	...
	if (chdir(dir) < 0) {
		errmsg(MM_WARNING, 0, MSGNUM_DEFMSG(_MSG_FRMT_12),
		       dir, strerror(errno));
		      /* ":12:%s: %s\n" */
		return (1);
	}
	...
	(void) closedir(dirp);
	if (chdir(savedir) < 0) {
		errmsg(MM_WARNING, 255, MSGNUM_DEFMSG(_MSG_CANT_CHNGBK_TO),
...
*/
			...
			case '+':
				...
				if ( xcheck == 1 && (b & GROUP & EXEC) == (GROUP & EXEC)) {
					...
					if ( lcheck == 1 ) {
						errmsg(MM_ERROR, 3,
						MSGNUM_DEFMSG(_MSG_GROUP_LOCK_NOT_PERMIT));
						/* ":17:Group execution and locking not
						permitted together\n" */
					}
					...
					if ( (nm & GROUP & (LOCK | EXEC)) == LOCK ) {
						errmsg(MM_WARNING, 0,
						MSGNUM_DEFMSG(_MSG_GROUP_EXEC_NOT_PERMIT),
						file);
						/* ":18:Group execution not
				...
				if ( scheck == 1 && (b & GROUP & SETID) == (GROUP & SETID) ) {
					...
					if ( lcheck == 1 && (b & GROUP & EXEC) == (GROUP & EXEC) ) {
						errmsg(MM_ERROR, 4,
						MSGNUM_DEFMSG(_MSG_SET_GID_LOCK_NOT_PERMIT));
						/* ":19:Set-group-ID and locking
					...
					if ( (nm & GROUP & (LOCK | EXEC)) == LOCK ) {
						errmsg(MM_WARNING, 0,
						MSGNUM_DEFMSG(_MSG_SET_GID_NOT_PERMIT),file);
						/* ":20:Set-group-ID not
						permitted\n\ton %s, a lockable file\n" */
						*errmode = 1;
						return(om);
				...
				if ( scheck == 1 ) {
					...
					if ( ((nm | b) & m & EXEC & (USER | GROUP)) != (m & EXEC & (USER | GROUP)) ) {
						errmsg(MM_WARNING, 0,
						MSGNUM_DEFMSG(_MSG_SET_ID_EXEC_PERM_REQ_4),file);
						/* ":21:Execute permission required for set-ID\n\ton
						execution for %s\n" */
						*errmode = 1;
						return(om);
					}
				...
				if ( lcheck == 1 ) {
					...
					if ( (nm & GROUP & EXEC) == (GROUP & EXEC) ) {
						errmsg(MM_WARNING, 0,
						MSGNUM_DEFMSG(_MSG_LOCK_NOT_PERMIT_GRP),file);
						/* ":22:Locking not permitted\n\ton %s, a group
						executable file\n" */
						*errmode = 1;
						return(om);
					}
			...
			case '-':
				...
				if ( xcheck == 1 && scheck == 0
					...
					errmsg(MM_WARNING, 0,
					MSGNUM_DEFMSG(_MSG_SET_ID_DISABL),file);
					/* ":23:Corresponding set-ID also disabled\n\ton
					%s since set-ID requires execute permission\n" */
			...
			case '=':
				...
				if ( lcheck == 1 ) {
					...
					if ( (b & GROUP & EXEC) == (GROUP & EXEC) ) {
						errmsg(MM_ERROR, 3,
						MSGNUM_DEFMSG(_MSG_GROUP_LOCK_NOT_PERMIT));
						/* ":17:Group execution and locking not permitted
						together\n" */
					}
   ...
   the corresponding execution must be requested, too! */
				if (scheck == 1 
				  && (b & EXEC & (USER | GROUP))  
				    != (m & EXEC & (USER | GROUP)) ) {
					errmsg(MM_ERROR, 2,
					MSGNUM_DEFMSG(_MSG_SET_ID_EXEC_PERM_REQ));
					/* ":24:Execute permission required for
					set-ID on execution\n" */
				}
 ...
 */
	...
	if (*--msp) {
		errmsg(MM_ERROR, 5, MSGNUM_DEFMSG(_MSG_INVLD_MODE));
                      /* ":14:Invalid mode\n" */
	}
	return(nm);
}
	...
	mode_t mode;
{
	...
	for ( i = 0; (c = *msp) >= '0' && c <= '7'; msp++)
		i = (mode_t)((i << 3) + (c - '0'));
	if (*msp)
		errmsg(MM_ERROR, 6, MSGNUM_DEFMSG(_MSG_INVLD_MODE));
                      /* ":14:Invalid mode\n"  */
