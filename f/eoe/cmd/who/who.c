...
static const char
#ifdef i386
	using_init_base[] = MSGNUM_DEFMSG(_MSG_USING_INIT_BASE),
#endif
	badstat[] = MSGNUM_DEFMSG(_MSG_CANNOT_ACCESS),
	badopen[] = MSGNUM_DEFMSG(_MSG_CANNOT_OPEN),
	badread[] = MSGNUM_DEFMSG(_MSG_READ_ERROR);
...
int	initbase = 0;
	...
	*/
	...
	if (goerr > 0) {
		pfmt(stderr,MM_ACTION,PFMTTXT(_MSG_WHO_USAGE_MSG),program);
		pfmt(stderr,MM_NOSTD,PFMTTXT(_MSG_WHO_A_OPT));
		pfmt(stderr,MM_NOSTD,PFMTTXT(_MSG_WHO_B_OPT));
		pfmt(stderr,MM_NOSTD,PFMTTXT(_MSG_WHO_D_OPT));
		pfmt(stderr,MM_NOSTD,PFMTTXT(_MSG_WHO_H_OPT));
		pfmt(stderr,MM_NOSTD,PFMTTXT(_MSG_WHO_L_OPT));
		pfmt(stderr,MM_NOSTD,PFMTTXT(_MSG_WHO_N_OPT));
		pfmt(stderr,MM_NOSTD,PFMTTXT(_MSG_WHO_P_OPT));
		pfmt(stderr,MM_NOSTD,PFMTTXT(_MSG_WHO_Q_OPT));
		pfmt(stderr,MM_NOSTD,PFMTTXT(_MSG_WHO_R_OPT));
		pfmt(stderr,MM_NOSTD,PFMTTXT(_MSG_WHO_S_OPT),program);
		pfmt(stderr,MM_NOSTD,PFMTTXT(_MSG_WHO_TIME_OPT));
		pfmt(stderr,MM_NOSTD,PFMTTXT(_MSG_WHO_TTY_OPT));
		pfmt(stderr,MM_NOSTD,PFMTTXT(_MSG_WHO_U_OPT));
		exit(1);
	}
	...
	*/
	...
	if (!terse) {
		...
		PFMTTXT(_MSG_WHO_NAME_LINE_TIME));
	...
	*/
	if (qopt) pfmt(stdout, MM_NOSTD,PFMTTXT(_MSG_WHO_NO_OF_USERS), totlusrs);
	return(0);
}
dump()
{
		...
		if ((rc = stat(path, stbufp)) != -1) {
			...
			if (hr == 0 && min == 0) printf("   .  ");
				...
				if (hr < 24) pfmt(stdout, MM_NOSTD,
					PFMTTXT(_MSG_WHO_HR_FMT), hr, min);
				else pfmt(stdout, MM_NOSTD, PFMTTXT(_MSG_WHO_OLD));
			}
		}
		...
		*/
		if (utmpp->ut_type == DEAD_PROCESS) {
			pfmt(stdout, MM_NOSTD,
				PFMTTXT(_MSG_WHO_ID_TERM_EXIT_FMT),
			...
			*/
			...
			if (*iinit != '\0') {
 			...
 			pfmt(stdout, MM_NOSTD,PFMTTXT(_MSG_WHO_ID_FMT), utmpp->ut_id);
...
ck_file(char *name)
{
	...
	*/
	...
	if (rc) {
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_WHO_NOT_UTMP_FILE), name);
		exit(1);
	}
}
