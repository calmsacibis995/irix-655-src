 ...
 * |-----------------------------------------------------------|
 */
#ident	"$Header: /proj/irix6.5f/isms/eoe/cmd/strings/RCS/strings.c,v 1.18 1999/05/11 21:29:59 olson Exp $"
...
err_opt(char *c)
{
	...
	_sgi_nl_error(SGINL_NOSYSERR, cmd_label,
	    GETTXT(_MSG_MMX_illoption), wc);
	_sgi_nl_usage(SGINL_USAGE, cmd_label,
	    GETTXT(_MSG_MMX_usage_strings));
}
...
getch(void)
{
	...
	wc = getwchar();
	/* Illegal byte sequence error is ignored. */
	if (feof(stdin) || (ferror(stdin) && errno!=EILSEQ)) {
	    end = 1;
	    return -1;
	}
	foff = ftell(stdin);
