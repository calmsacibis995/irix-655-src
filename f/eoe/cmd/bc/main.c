     ...
     char *argv[];
{
  ...
  if(std_only && multi_name ) {
        fprintf(stderr,GETTXTCAT(_MSG_BC_EXCL_OPTIONS_WM));
        exit(1);
  }
  if(std_only && warn_not_std ) {
        fprintf(stderr,GETTXTCAT(_MSG_BC_EXCL_OPTIONS_WS));
        exit(1);
  }
...
open_new_file ()
{
  ...
  if (use_math && first_file)
	...
	{
	  fprintf (stderr, GETTXTCAT(_MSG_BC_MATHLIB_UNAVAIL));
	  exit (1);
	}
#else
  ...
  if (file_names != NULL)
      ...
      if (new_file != NULL)
	{
      ...
      fprintf (stderr, GETTXTCAT(_MSG_BC_FILE_NOT_FOUND), file_names->name);
      exit (1);
    }
...
use_quit (sig)
     int sig;
{
  printf (GETTXTCAT(_MSG_BC_INTRP));
  signal (SIGINT, use_quit);
}
