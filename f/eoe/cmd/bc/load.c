     ...
     char byte;
{
  ...
  if (seg >= BC_MAX_SEGS)
    {
      yyerror (GETTXTCAT(_MSG_BC_FUNC_TOO_BIG));
      return;
    }
  ...
  while (*str != 0)
	...
	if (load_const)
	...
	else
	  {
		...
		if (label_no > 65535L)
		  {  /* Better message? */
		    fprintf (stderr,GETTXTCAT(_MSG_BC_PROG_TOO_BIG));
		    exit(1);
		  }
		...
		clear_func (func);
#if DEBUG > 2
		printf (GETTXTCAT(_MSG_BC_LOAD_FUNC), func);
#endif
		/* get the parameters */
		while (*str++ != '.')
		  {
...
#if DEBUG > 2
			printf (GETTXTCAT(_MSG_BC_VAR_PARAM_NUM), ap_name);
#endif
			...
			ap_name = long_val (&str);
#if DEBUG > 2
			printf (GETTXTCAT(_MSG_BC_PARAM_NUM), ap_name);
#endif
			functions[(int)func].f_params = 
			  nextarg (functions[(int)func].f_params, ap_name,
				   FALSE);
		...
		while (*str != '[')
		  {
		    ...
		    ap_name = long_val (&str);
#if DEBUG > 2
		    printf (GETTXTCAT(_MSG_BC_AUTO_NUM), ap_name);
#endif
