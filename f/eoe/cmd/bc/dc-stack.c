...
#define Empty_Stack	fprintf(stderr, GETTXTCAT(_MSG_BC_EMPTY_STACK), progname)
	...
	int kscale DC_DECLEND
{
	...
	if (dc_stack->value.dc_type!=DC_NUMBER
		...
		fprintf(stderr, GETTXTCAT(_MSG_BC_NON_NUMERIC), progname);
		return;
	}
	(void)dc_pop(&b);
	(void)dc_pop(&a);
	...
	dc_data *result DC_DECLEND
{
	...
	if ( ! r ){
		...
		dc_show_id(stderr, regid, GETTXTCAT(_MSG_BC_DC_REGID));
		return DC_FAIL;
	}
	...
	dc_data *result DC_DECLEND
{
	...
	if (!r){
		fprintf(stderr, GETTXTCAT(_MSG_BC_STACK_REG), progname);
		dc_show_id(stderr, stackid, GETTXTCAT(_MSG_BC_DC_STACKID));
		return DC_FAIL;
	}
	if (r->value.dc_type!=DC_NUMBER && r->value.dc_type!=DC_STRING)
