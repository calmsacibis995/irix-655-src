	...
	dc_num *result DC_DECLEND
{
	...
	if (bc_divide(CastNum(a), CastNum(b), (bc_num *)result, kscale)){
		fprintf(stderr, GETTXTCAT(_MSG_BC_DIV_ZERO), progname);
	...
	dc_num *result DC_DECLEND
{
	...
	if (bc_modulo(CastNum(a), CastNum(b), (bc_num *)result, kscale)){
		fprintf(stderr, GETTXTCAT(_MSG_BC_REM_ZERO), progname);
		return DC_DOMAIN_ERROR;
	}
	return DC_SUCCESS;
}
	...
	dc_num *result DC_DECLEND
{
	...
	if (!bc_sqrt(&tmp, kscale)){
		fprintf(stderr, GETTXTCAT(_MSG_BC_SQROOT_NEG), progname);
		free_num(&tmp);
		return DC_DOMAIN_ERROR;
	}
	*((bc_num *)result) = tmp;
...
#endif
{
  ...
  fprintf (stderr, GETTXTCAT(_MSG_BC_RUNERROR), error_mesg);
}
...
#endif
{
  ...
  fprintf (stderr, GETTXTCAT(_MSG_BC_RUNWARN_FUNCS), error_mesg);
}
