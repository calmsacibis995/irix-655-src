     ...
     int depth;
{
  ...
  if (depth > 0)
    {
      rt_error (GETTXTCAT(_MSG_BC_STACK_ERROR));
      return FALSE;
    }
  return TRUE;
}
  ...
  if (var_name > 3)
      ...
      if (is_neg (ex_stack->s_num))
	{
	  switch (var_name)
	    {
	    case 0:
	      rt_warn (GETTXTCAT(_MSG_BC_IBASE_NEG));
	      temp = 2;
	      break;
	    case 1:
	      rt_warn (GETTXTCAT(_MSG_BC_OBASE_NEG));
	      temp = 2;
	      break;
	    case 2:
	      rt_warn (GETTXTCAT(_MSG_BC_SCALE_NEG));
	      temp = 0;
	      break;
#ifdef READLINE
	    case 3:
	...
	case 0:
	  if (temp < 2 && !toobig)
	    {
	      i_base = 2;
	      rt_warn (GETTXTCAT(_MSG_BC_IBASE_SMALL_SET2));
	    }
	  else
	    if (temp > 16 || toobig)
	      {
		i_base = 16;
		rt_warn (GETTXTCAT(_MSG_BC_IBASE_LARGE));
	      }
	    else
	      i_base = (int) temp;
	  break;
	...
	case 1:
	  if (temp < 2 && !toobig)
	    {
	      o_base = 2;
	      rt_warn (GETTXTCAT(_MSG_BC_OBASE_SMALL_SET2));
	    }
	  else
	    if (temp > BC_BASE_MAX || toobig)
	      {
		o_base = BC_BASE_MAX;
		rt_warn (GETTXTCAT(_MSG_BC_OBASE_LARGE), BC_BASE_MAX);
	      }
	    else
	      o_base = (int) temp;
	  break;
	...
	case 2:
	  ...
	  if (temp > BC_SCALE_MAX || toobig )
	    {
	      scale = BC_SCALE_MAX;
	      rt_warn (GETTXTCAT(_MSG_BC_SCALE_LARGE_SETD), BC_SCALE_MAX);
	    }
	  else
	    scale = (int) temp;
	  break;
	...
	case 3:
	  if (toobig)
	    {
	      temp = -1;
	      rt_warn (GETTXTCAT(_MSG_BC_HIST_LARGE));
	      unstifle_history ();
	    }
	  else
	    {
     ...
     int var_name;
{
  ...
  if (index < 0 || index > BC_DIM_MAX ||
    ...
    rt_error (GETTXTCAT(_MSG_BC_SUBS_OUTOF_BND), a_names[var_name]);
  else
    {
     ...
     int var_name;
{
  ...
  switch (var_name)
	...
	rt_warn (GETTXTCAT(_MSG_BC_INBASE_SMALL));
      break;
	...
	rt_warn (GETTXTCAT(_MSG_BC_OBASE_SMALL));
      break;
	...
	rt_warn (GETTXTCAT(_MSG_BC_SCALE_NEGATIVE));
      break;
	...
	{
	  n_history = -1;
	  rt_warn (GETTXTCAT(_MSG_BC_HIST_NEG));
	  unstifle_history ();
	}
#endif
     ...
     int var_name;
{
  ...
  switch (var_name)
	...
	rt_warn (GETTXTCAT(_MSG_BC_IBASE_BIG));
      break;
	...
	rt_warn (GETTXTCAT(_MSG_BC_OBASE_BIG));
      break;
	...
	rt_warn (GETTXTCAT(_MSG_BC_SCALE_TOOBIG));
      break;
	...
	{
	  n_history = -1;
	  rt_warn (GETTXTCAT(_MSG_BC_HIST_UNLIMITD));
	  unstifle_history ();
	}
#endif
     ...
     int func;
{
  ...
  while ((ch = byte(pc)) != ':')
      ...
      if (params != NULL)
	{
	  if ((ch == '0') && params->av_name > 0)
		...
		if (params->arg_is_var)
	      ...
	      {
		if (params->av_name < 0)
		  rt_error (GETTXTCAT(_MSG_BC_PARAMTYPE_MISMATCH),
	...
	{
	    rt_error (GETTXTCAT(_MSG_BC_PARAMUM_MISMATCH));
	    return;
	}
      params = params->next;
    }
  if (params != NULL) 
    rt_error (GETTXTCAT(_MSG_BC_PARAMNUM));
}
