...
opt_newline		: /* empty */
			| NEWLINE
			    { warn (GETTXTCAT(_MSG_BC_NO_NEWLINE)); }
			;
...
statement 		: Warranty
			...
			| expression
			    {
			      if ($1 & 2)
				warn (GETTXTCAT(_MSG_BC_COMP_EXPR));
			      if ($1 & 1)
				generate ("W");
			      else 
				generate ("p");
			...
			| Break
			    {
			      if (break_label == 0)
				yyerror (GETTXTCAT(_MSG_BC_BREAK_OUTSIDE));
			      else
				{
			...
			| Continue
			    {
			      warn (GETTXTCAT(_MSG_BC_CONT_STMT));
			      if (continue_label == 0)
				yyerror (GETTXTCAT(_MSG_BC_CONT_OUT));
			      else
				{
			...
			| For 
			  ...
			  '(' opt_expression ';'
			    {
			      if ($4 > 1)
				warn (GETTXTCAT(_MSG_BC_CMP_IN_FIRST));
			      $4 = next_label++;
			      if ($4 < 0)
			  ...
			  opt_expression ')'
			    {
			      if ($10 > 1)
				warn (GETTXTCAT(_MSG_BC_CMP_IN_THIRD));
			      if ($10 < 0)
			...
			| '{' statement_list '}'
			    { $$ = 0; }
			| Print
			    {  warn (GETTXTCAT(_MSG_BC_PRINT_STMT)); }
			  print_list
			;
...
opt_else		: /* nothing */
			| Else 
			    {
			      warn (GETTXTCAT(_MSG_BC_ELSE_IN_IF));
			      $1 = next_label++;
...
argument_list 		: expression
			    {
			      if ($1 > 1) warn (GETTXTCAT(_MSG_BC_COMP_ARG));
			...
			| argument_list ',' expression
			    {
			      if ($3 > 1) warn (GETTXTCAT(_MSG_BC_COMP_ARG));
			      $$ = nextarg ($1,0,FALSE);
			    }
			| argument_list ',' NAME '[' ']'
			    {
...
opt_expression 		: /* empty */
			    {
			      $$ = -1;
			      warn (GETTXTCAT(_MSG_BC_MISSING_EXPR));
			    }
			| expression
			;
return_expression	: /* empty */
			...
			| expression
			    {
			      if ($1 > 1)
				warn (GETTXTCAT(_MSG_BC_COMP_RET_EXPR));
			    }
			;
expression		:  named_expression ASSIGN_OP 
			  ...
			  expression
			    {
			      if ($4 > 1) warn(GETTXTCAT(_MSG_BC_COMP_ASSIGN));
			      if ($2 != '=')
				{
				  sprintf (genstr, "%c", $2);
				  generate (genstr);
			...
			| NOT expression
			    {
			      $$ = $2;
			      warn(GETTXTCAT(_MSG_BC_EXCL_OP));
			      generate ("!");
			    }
			...
			| Read '(' ')'
			    {
			      warn (GETTXTCAT(_MSG_BC_READ_FUNC));
			      generate ("cI"); $$ = 1;
			    }
			;
named_expression	: NAME
			...
			| NAME '[' expression ']'
			    {
			      if ($3 > 1) warn(GETTXTCAT(_MSG_BC_COMP_SUBSCRIPT));
			      $$ = lookup($1,ARRAY);
			    }
			| Ibase
			    { $$ = 0; }
			...
			| History
			    { $$ = 3;
			      warn (GETTXTCAT(_MSG_BC_HIST_VAR));
			    }
			| Last
			    { $$ = 4;
			      warn (GETTXTCAT(_MSG_BC_LAST_VAR));
			    }
			;
