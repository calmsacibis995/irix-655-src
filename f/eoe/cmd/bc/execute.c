     ...
     int sig;
{
  ...
  rt_error (GETTXTCAT(_MSG_BC_INTRP_EXECUTED));
}
...
execute ()
{
  ...
  while (pc.pc_addr < functions[pc.pc_func].f_code_size && !runtime_error)
	...
	printf (GETTXTCAT(_MSG_BC_FUNC_ADDR),pc.pc_func, pc.pc_addr, inst);
  
	if (temp == NULL) printf (GETTXTCAT(_MSG_BC_EMPTY_STCAK), inst);
	else
	  {
	    depth = 1;
	    while (temp != NULL)
	...
	if (!functions[new_func].f_defined)
	  {
	    rt_error (GETTXTCAT(_MSG_BC_FUNC_NOT_DEFINED), f_names[new_func]);
	    break;
	  }
	...
	fflush (stdout);
        ...
        {
		/* Processing to handle the '\' character properly ,as it can
                   come as the second byte of a multibyte character 
                */

	        /* Collect all the characters in a character array and pass it to
                   mbPrintprocess function to perform the wide char handling
                */

                while ((ch = byte(&pc)) != '"')
                        mbString[nIndex++] = ch;
             
                mbString[nIndex] = NULL ;

                mbPrintprocess(mbString);
	...
	if (pc.pc_func != 0)
	  {
	    ...
	    pc.pc_func = fpop ();
          ...
          {
	  rt_error (GETTXTCAT(_MSG_BC_RET_FROM_MAIN));
          }
	break;
	...
	case 'R':  /* Square Root function. */
	  if (!bc_sqrt (&ex_stack->s_num, scale))
          {
	    rt_error (GETTXTCAT(_MSG_BC_SQRT_NEGNUM));
          }
	  break;
	...
	if (check_stack(2))
	  {
		...
		init_num (&temp_num);
             ...
             {
	      rt_error (GETTXTCAT(_MSG_BC_DIVBY_ZERO));
             }
	  }
	break;
	...
	if (check_stack(2))
	  {
	    if (is_zero (ex_stack->s_num))
	      rt_error (GETTXTCAT(_MSG_BC_ZERO_MODULO));
	    else
	      {
	...
	if (check_stack(2))
	  {
	    ...
	    if (is_zero (ex_stack->s_next->s_num) && is_neg (ex_stack->s_num))
	      rt_error (GETTXTCAT(_MSG_BC_ZERO_DIVIDE));
	    pop();
	    pop();
	    push_num (temp_num);
	    init_num (&temp_num);
	...
	default  : /* error! */
	  rt_error (GETTXTCAT(_MSG_BC_BAD_INSTR), inst);
      }
    }
  ...
  if (interactive)
    {
      ...
      if (had_sigint)
	printf (GETTXTCAT(_MSG_BC_INTR_COMPL));
    }
}
...
/*     Purpose : Prints the valid control characters on stdout 		*/
/*     Parameters : Input is a multibyte string      			*/ 
/*     Return Values : Returns 0 if the print is successful 		*/
/*     otherwise returns 1						*/
/*     For newline character the 'out_col' variable is set to 0       	*/
/*     NOTES:								*/
/*	Follow the convention that one backspace character '\b'		*/
/*	moves one column (not one character).				*/


int mbPrintprocess(char *mbStr)
{
 
  size_t  szStatus, szLen;
  wchar_t *wPwcs, wCh;
  int     nWclen, nCount;

        /* Since the '\' can occur as second byte of a multibyte character so 
           perform wide character processing */

        szLen = strlen(mbStr);

        if( (wPwcs = (wchar_t *) malloc(sizeof(wchar_t ) * (szLen + 1))) == NULL )
        {
                perror(GETTXTCAT(_MSG_BC_MEM_PROBLEM));
                return 1;
        }

        szStatus = mbstowcs(wPwcs, mbStr, szLen+1);
        if ( szStatus  == (size_t)-1)
        {
                perror(GETTXTCAT(_MSG_BC_MBSTOWCS_FAILED));
                return 1;
        }
 	...
 	nWclen = wcslen(wPwcs);
        ...
        for( nCount = 0; nCount < nWclen; nCount++ )
        {
           wCh = wPwcs[nCount];
           ...
           else
           {
           	...
           	switch(wCh)
           	{
                	case L'a': putchar(007); break;
                	case L'f': putchar('\f'); break;
                	case L'n': {out_col=0; putchar('\n');} break;
                	case L'q': putchar('"'); break;
                	case L'r': putchar('\r'); break;
