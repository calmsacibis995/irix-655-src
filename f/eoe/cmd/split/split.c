...
#ident	"$Header: /proj/irix6.5f/isms/eoe/cmd/split/RCS/split.c,v 1.8 1998/09/18 19:47:22 sherwood Exp $"

/* I18n Change History : 02 Jul 1997                             */
/*      -b option is not changed since that is an essential      */
/*         feature of this command. So this program can split MB */
/*         characters into two different files, with some bytes  */
/*         the MB chars in one file and remaining bytes in next. */
...
char *argv[];
{
	...
	if(**argv == '-' && delimit == 0){
		...
		case 'a':
		  ...
		  if(suffix_length <= 0){
		      ...
		      pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SUFFIX_LENG_NEG));
		      exit(1);
		  }
		  break;
		...
		case 'l':
		  if(setbytes == 1){
		      pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_INCORRECT_USAGE));
		      pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_SPLIT_B_L_OPTIONS_TOGETHER));
		      exit(1);
		  }
		  setlines = 1;
		  if(argv[0][2] == '\0'){
		...
		case 'b':
		  ...
		  if(nbytes <= 0){
		      pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_INCORRECT_USAGE));
		      pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_NO_OF_BYTES_NEG));
		      exit(1);
		  }
	    ...
	    if(count <= 0){
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_INCORRECT_USAGE));
		pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_LINE_COUNT_NEG));
		exit(1);
	    }
	...
	if (statvfs64(head, &stbuf) < 0) {
	    pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_MSG_STRING), head, strerror(errno));
	    exit(1);
	}

	if (strlen(tail) > (stbuf.f_namemax-suffix_length) ) {
	    pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_MORE_CHARS_IN_OP_FILE),
	...
	pfmt(stderr, MM_ERROR, PFMTTXT(_MSG_CANNOT_ALLOC_MEM));
	exit(1);
    }
    base[0] = 1;
	  ...
	  for(i=0; i<count; i++)
		...
		if(f) { /* need a new output file */
			...
			pfmt(stderr, MM_ERROR,
			     PFMTTXT(_MSG_MORE_OP_FILES_NEEDED));
			exit(1);
		    }
...
{
    pfmt(stderr, MM_ERROR,  PFMTTXT(_MSG_INCORRECT_USAGE));
    pfmt(stderr, MM_ACTION, PFMTTXT(_MSG_SPLIT_USAGE));
    exit(1);
}
