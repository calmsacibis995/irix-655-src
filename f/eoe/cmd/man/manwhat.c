...
main(int argc, char * argv[])
{
	...
	*/
	...
	while ((o = getopt(argc, argv, "M:W")) != -1) {
		...
		case 'M':	/* man path */
			...
			break;
		case '?':
                        (void) fprintf(stderr, CATGETS(catd,_MSG_MANWHAT_USAGESTR));
			catclose(catd);
			exit(2);
		}
	}
	...
	*/
	...
	if(!tbuf) {
		perror(CATGETS(catd,_MSG_MANWHAT_BUFMEMORYERR));
		catclose(catd);
		exit(1);
	}
...
parsePaths(char * pathstr, char * patharray[])
{
	...
	if(count >= MANDIRCNT/2)
		fprintf(stderr, CATGETS(catd, _MSG_MANWHAT_DIR_CNT),
			MANDIRCNT/2);
	ps = NULL;
    }
    return(count);
...
awfit:
			 ...
			 if(!chknfmtexist) {
				if((chknfmtexist = chkexist(nroffcmd)) == -1) {
					...
					if((chknfmtexist = forkandexeclp(awfcmd)) != -1) {
						fprintf(stderr,CATGETS(catd, _MSG_MANWHAT_TEXT_FORMATTER));
						manfmtcmd = awfcmd;
						goto awfit;
					}
				}
    ...
    case NOTREGULARFILE:
    default:
	fprintf(stderr, CATGETS(catd, _MSG_MANWHAT_NOT_PRINTABLE), name);
	...
	if(skipit) {
	    if(!nodwb) {
		nodwb = 1;
		fprintf(stderr,CATGETS(catd, _MSG_MANWHAT_NOT_INSTALLED));
	    }
	    fprintf(stderr, CATGETS(catd, _MSG_MANWHAT_SKIPPING), name);
	    return;
	}
    }
...
filetype(char * file)
{
	...
	if(!fsize) { /* first manpage, and direct i/o not supported on
	    * this man page (probably over nfs) */
		...
		perror(CATGETS(catd,_MSG_MANWHAT_FILEBUFMEMORYERR));
                catclose(catd);
		exit(1);
	    }
	}
                ...
                else
                {
                    ...
                    */
                    ...
                    for (i=0; i<fbsz-1 && printable; ) {
                        if((mbtowc(&wc, &filebuf[i], MB_CUR_MAX)) == -1)
                        {
                            if (i<=(fbsz-MB_CUR_MAX))
                                printable=0;
                                break;
                        }
                        i+=mblen(&filebuf[i], MB_CUR_MAX);
...
getcatNAME(char *buf, int len)
{
    ...
    regex_t *sectionNumberPattern=NULL;
    char *strp, *inp, *endp;
    char last;
    unsigned int spaces;
	...
	if(!sectionNumberPattern)
		...
		/* section number is '(' [0-9l][0-9a-zA-Z+]* ')' */
                regcomp(sectionNumberPattern, "(\\([0-9lD][+0-9a-zA-Z]*\\))", REG_EXTENDED);
        }
	endp = &buf[len-1];
	...
	while (buf < endp) {
		...
		*/
		if (!gotname && !havesection) {
                        ...
                        sectionNumberStr[pmatch.rm_eo-pmatch.rm_so]='\0';
...
/*  Keeping this function as it is assuming no changes will be done in the
    tag <Title> ... */
...
gethtmlNAME(char *buf, int len)
{
    ...
    char titleStr[4096];
    char titleLine[4096];
    regex_t *bTitlePattern = 0;
    regex_t *eTitlePattern = 0;
    char *strp, *inp, *endp;
    char last;
    ...
    if(!bTitlePattern)
    {
	bTitlePattern = (regex_t *)malloc(sizeof(regex_t));
	regcomp(bTitlePattern, "(<[Tt][Ii][Tt][Ll][Ee]>)([^\n]*)", REG_EXTENDED);
    }

    if(!eTitlePattern)
    {
	eTitlePattern = (regex_t *)malloc(sizeof(regex_t));
	regcomp(eTitlePattern, "([^<]*)(</[Tt][Ii][Tt][Ll][Ee]>)", REG_EXTENDED);
    }
	...
	*inp = '\0';
	...
	if(!havehtmltitle){
	    if(!inhtmltitle){
		if(! regexec(bTitlePattern, in, 3 ,pmatch, 0)) {
		    strncpy(matchStr, in+pmatch[2].rm_so, pmatch[2].rm_eo-pmatch[2].rm_so);
		    matchStr[pmatch[2].rm_eo]='\0';
		    if(! regexec(eTitlePattern, matchStr, 3 ,pmatch, 0)) {
		        strncpy(titleLine, matchStr+pmatch[1].rm_so, pmatch[1].rm_eo-pmatch[1].rm_so);
		        titleLine[pmatch[1].rm_eo]='\0';
			havehtmltitle = 1;
		...
		if(! regexec(eTitlePattern, in, 3 ,pmatch, 0)) {
		    strncpy(titleLine, in+pmatch[1].rm_so, pmatch[1].rm_eo-pmatch[1].rm_so);
		    titleLine[pmatch[1].rm_eo]='\0';
		    if(titleLine[0] != NULL){
			strncat(titleStr, " ", 1);
