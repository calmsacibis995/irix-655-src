...
static sort_t *sortlist = (sort_t *)0;
int _dns_sock = -1;
int _dns_ndots = 1;
...
/*
** This routine compares an address with each of the addresses in the
** current sortlist and returns the current "addrlist" position according
** to "sortlist" ordered rules/statements if the address matches, and 0
** if it does not.
*/
struct addrlist *
dns_insortlist(struct addrlist *addrlist)
{
	sort_t *sp;
	struct addrlist *prior, *where;

	for (sp = sortlist; sp; sp = sp->next) {
		if ((addrlist->addr.s_addr & sp->netmask.s_addr) ==
		    (sp->addr.s_addr & sp->netmask.s_addr)) {
			nsd_logprintf(NSD_LOG_HIGH, "\tsortlist match: %s\n",
			    inet_ntoa(addrlist->addr));
			/*
			** first element found for "sortlist" specific
			** item/statement found.
			*/
			if(sp->sublist == 0) {
				sp->sublist = addrlist;
			}
			...
			/*
			** order/find position in current sublist.
			** need to start from alist due to single link list.
			** but also to stop at "nosort" since addresses not
			** "sortlist" relevant are put at the end of alist/aend
			** in dns_query_callback().
			*/
			for (prior = 0, where = alist;
			    where && (where != nosort);
			    prior = where, where = where->next)
			{
				/*
				** skip ordered sublists for previous "sortlist"
				** specific items/statements.
				*/
				if ((where->addr.s_addr & sp->netmask.s_addr) !=
				    (sp->addr.s_addr & sp->netmask.s_addr)) {
					continue;
				}

				 /*
				 ** find position in sublist.
				 */
				 if (where->addr.s_addr >=
				     addrlist->addr.s_addr) {
					/*
					** Don't forget to update "sublist" if
					** better choice than earlier one.
					*/
					if (where->addr.s_addr <
					    sp->sublist->addr.s_addr) {
						sp->sublist = where;
						break;
					}
				}
...
dns_search_parse(char *search)
{
	...
	if (! pp) {
		nsd_logprintf(NSD_LOG_CRIT,
		    "dns_search_parse: failed to split search list\n");
		return (domain_t *)0;
	}

	/*
	** Loop through domains, appending elements to the
	** domain list.
	*/
	end = (domain_t **)&list;
	for (i = 0; pp[i]; i++) {
...
dns_servers_parse(char *server)
{
	...
	if (! pp) {
		nsd_logprintf(NSD_LOG_CRIT,
		    "dns_servers_parse: failed to split nameserver list\n");
		return (server_t *)0;
	}

	for (i = 0, list = 0, end = &list; pp[i]; i++) {
		...
		for (p = pp[i]; *p && !isspace(*p); p++);
		*p = (char)0;
		he = gethostbyname_r(pp[i], &h, buf, sizeof(buf), &herr);
		if (! he) {
			nsd_logprintf(NSD_LOG_CRIT,
			  "dns_servers_parse: illegal nameserver address: %s\n",
			    pp[i]);
			free_servers(&list);
			free(sp);
			free(pp);
...
"Ignoring hostresorder keyword found in resolv.conf.  Resolve order is now controlled by nsswitch.conf.\n");
	...
	if (! servers) {
		...
		**
		** We want the address to be 0.0.0.0 and next to be NULL.
		*/
