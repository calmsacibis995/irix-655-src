	...
	for (e = ldap_first_entry(ld, result); e != NULL;
		...
		for (item = fmt->item; item != NULL; 
			...
			if (vals && vals[0]) {
				...
				if (*(vals[0]) && (nsd_append_result(*rq, 
					NS_SUCCESS, vals[0], strlen(vals[0]))) 
					!= NSD_OK) goto _sr_err;
...
int open_server(_lsrv_t_ptr sv, int eto) 
{
	...
	nsd_logprintf(3, "open_server %s, status: %d\n", sv->addr, sv->status);
		...
		case SRV_CONNECTING:
			...
			if (LDAP_MORE(sv->ldap)) {
				/* XXX is it possible that the server
				   ...
				   response to a bind? */
				nsd_logprintf(0, "ldap: MORE TO READ\n");
			}
			if (rc == 97) {
