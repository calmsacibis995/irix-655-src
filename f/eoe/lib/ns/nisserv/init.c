...
reject_err(nisserv_t *nd, uint32_t error)
{
	...
	*p++ = htonl(error);
	...
	if (nd->proto == IPPROTO_UDP) {
		sendfromto(nd->fd, _nisserv_buf, len, 0, &nd->local, &nd->sin,
		    sizeof(nd->sin), nd->lbl);
	} else {
		*save = htonl(0x80000000 + len - sizeof(*p));
		write(nd->fd, _nisserv_buf, len);
		nsd_callback_remove(nd->fd);
		close(nd->fd);
	}
	nisserv_data_clear(&nd);
	...
	*/
	...
	if (nd->proto == IPPROTO_TCP) {
		*save = htonl(0x80000000 + len - sizeof(*p));
		...
		nsd_logprintf(NSD_LOG_LOW, "\tsendto %s.%d\n",
		    inet_ntoa(nd->sin.sin_addr), nd->sin.sin_port);
		sendfromto(nd->fd, _nisserv_buf, len, 0, &nd->local,
		    &nd->sin, sizeof(nd->sin), nd->lbl);
	}
	nisserv_data_clear(&nd);
	nsd_file_clear(&rq);
...
nisserv_null(nsd_file_t **rqp, nisserv_t *nd, uint32_t *p)
{
	...
	sendfromto(nd->fd, _nisserv_buf, (char *)p - _nisserv_buf, 0,
	    &nd->local, &nd->sin, sizeof(nd->sin), nd->lbl);
	nisserv_data_clear(&nd);
	...
	if (dp &&
		...
		*p++ = htonl(TRUE);
		nsd_logprintf(NSD_LOG_LOW, "\treturning TRUE to %s.%d\n",
		    inet_ntoa(nd->sin.sin_addr), nd->sin.sin_port);
		sendfromto(nd->fd, _nisserv_buf, (char *)p - _nisserv_buf,
		    0, &nd->local, &nd->sin, sizeof(nd->sin), nd->lbl);
	} else if (nack) {
		p = (uint32_t *)_nisserv_buf;
		p += rpc_hdr(nd, p);
		*p++ = htonl(FALSE);
...
dispatch_tcp(nsd_file_t **rqp, int fd)
{
	nisserv_t *nd;
	char *q;
	uint32_t *p, *end = (uint32_t *)(_nisserv_buf + 1600);
	int len = 0, qlen = sizeof(_nisserv_buf);
	nsd_file_t *dp, *dp2;
	hash_file_t *hf;
	struct stat sbuf;
	kvpair kv;
	...
	*/
	...
	if (! nd) {
		nsd_logprintf(NSD_LOG_LOW,
		    "Cannot find request structure for file descriptor: %d\n",
		    fd);
		goto tcperror;
	}
	...
	/*
	** If we were already started we prepend the data to the incoming
	** buffer.
	*/
	q = _nisserv_buf;
	...
	*/
	if (nd->status < 10) {
		...
		if (len <= 0) {
			if ((len < 0) && (errno == EWOULDBLOCK)) {
				nsd_logprintf(NSD_LOG_LOW,
				    "\treturning NSD_CONTINUE\n");
				return NSD_CONTINUE;
			}
			nsd_logprintf(NSD_LOG_RESOURCE,
			    "dispatch_tcp: read failure: %s\n",
			    strerror(errno));
			goto tcperror;
		}
	}
	...
	/*
	** Parse message header.  If we did not read enough last time to
	** parse the whole header then we just return to where we left off.
	*/
	p = (uint32_t *)_nisserv_buf;
	...
	switch(nd->status) {
		...
		if (ntohl(*p++) != YPVERS) {
			nsd_logprintf(NSD_LOG_LOW,
			    "\treturning PROG_MISMATCH 2\n");
			return accept_err(nd, PROG_MISMATCH);
		}
		len -= sizeof(*p);
	    case 5:
		/*
		** We only support yp_all calls via tcp.
		*/
		if (len < sizeof(*p)) {
			nd->status = 5;
			goto needmore;
		}
		if (ntohl(*p++) != YPPROC_ALL) {
			nsd_logprintf(NSD_LOG_LOW,
			    "\treturning YP_BADOP: %d\n", *(p-1));
			return nisserv_err(nd, YP_BADOP);
		}
		nd->cmd = YPPROC_ALL;
		len -= sizeof(*p);
	    ...
	    case 7:
		if ((len < (2 * sizeof(*p))) ||
		    (len < (ntohl(p[1]) * sizeof(*p)))) {
			nd->status = 6;
			goto needmore;
		}
/* XXX - VERF, currently we just skip it */
		len -= ((2 + ntohl(p[1])) * sizeof(*p));
		p += (2 + ntohl(p[1]));
	    case 8:
		if (len < sizeof(*p)) {
			nd->status = 8;
		...
		memcpy(nd->domain.string, (char *)(p + 1), qlen);
		nd->domain.string[qlen] = (char)0;
		nd->domain.len = qlen;
		nd->domain.words = WORDS(qlen);
		nsd_logprintf(NSD_LOG_LOW, "\tdomain = %s\n",
		    nd->domain.string);
		...
		*/
		...
		p += nd->domain.words + 1;
		len -= (nd->domain.words + 1) * sizeof(*p);
	    case 9:
		if (len < sizeof(*p)) {
			nd->status = 9;
			goto needmore;
		...
		memcpy(nd->table.string, (char *)(p + 1), qlen);
		nd->table.string[qlen] = (char)0;
		nd->table.len = qlen;
		nd->table.words = WORDS(qlen);
		nsd_logprintf(NSD_LOG_LOW, "\ttable = %s\n", nd->table.string);

		p += nd->table.words + 1;
		len -= (nd->table.words + 1) * sizeof(*p);

		/*
		** We are no longer waiting for data on our descriptor so
		** we turn off the bit for the select so we will not be
		** woken up when waiting to be able to write.
		*/
		nsd_callback_remove(fd);
		nsd_callback_new(fd, dispatch_tcp, NSD_WRITE);
		...
		** needed, regain the lock on the hash file, then jump back
		** to where we were.
		*/
		...
		*/
		...
		if (! (dp =
		       nsd_file_byname(dp->f_data, nd->table.string, 
				       nd->table.len))) {
			nsd_logprintf(NSD_LOG_LOW, "\treturning YP_NOMAP\n");
			return nisserv_err(nd, YP_NOMAP);
		}
		...
		if (! (dp2 = nsd_file_byname(dp->f_callouts, ".nisserv", 8))) {
			if (! (dp2 = nsd_file_byname(dp->f_callouts,
			    ".ypserv", 7))) {
				nsd_logprintf(NSD_LOG_LOW,
				    "\treturning YP_NODOM\n");
				return nisserv_err(nd, YP_NODOM);
	...
	** Walk the tree looking for the callout directory.
	*/
	if (! __nsd_mounts || ! (dp = nsd_file_byname(__nsd_mounts->f_data,
	...
	if (! (dp =
	    nsd_file_byname(dp->f_data, nd->table.string, nd->table.len))) {
		nsd_logprintf(NSD_LOG_LOW, "\treturning YP_NOMAP\n");
		return nisserv_err(nd, YP_NOMAP);
	}
	...
	/*
	** Check the nis_secure argument to see if we can accept this request
	*/
	if (nsd_attr_fetch_bool(dp->f_attrs, "nis_secure", 0) &&
	    ntohs(nd->sin.sin_port) >= 1024) {
	    nsd_logprintf(NSD_LOG_LOW, "\tReturning YP_NOMAP\n");
	...
	*/
	...
	if ((! hf->map) || (hf->version < sbuf.st_mtime) 
	    || _Mdbm_invalid(hf->map)) {
		if (! hash_reopen(hf)) {
			nsd_logprintf(NSD_LOG_LOW, "\treturning YP_BADDB 3\n");
			return nisserv_err(nd, YP_BADDB);
		}
		hf->version = sbuf.st_mtime;
	}
...
init(char *map)
{
	...
	*/
	...
	setsockopt(_nisserv_udp, IPPROTO_IP, IP_RECVDSTADDR, (char *)&on,
	    sizeof(on));
