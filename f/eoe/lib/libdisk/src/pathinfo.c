...
findsize(char *path)
{
	...
	/* XXX:em it sure would be nice to get rid of the distinctions
	 * between xlv/xvm at the apps level and use common structures
	 * as well as common ioctls.
	 */
	if (is_xlv) {
		ret = ioctl(fd, DIOCGETVOL, (caddr_t)&xlv);
		close(fd);
		if (ret < 0)
			return (-1LL);
		return (long long)(xlv.subvol_size);
	} else if (is_xvm) {
		if (ioctl(fd, DIOCGETVOL, (caddr_t)&xvm) < 0) {
			return (-1LL);
		} else {
			return (long long)(xvm.subvol_size);
		}
	}
...
dev_to_rawpath(dev_t dev, char *destpath, int *destlen, struct stat64 *sbuf)
{
	...
	/* everything but XLV is in the hardware graph */
	if (major(dev) != XLV_MAJOR) {
	...
	/*
         * We're an XLV volume.  XLV is not part of the HWG, so we'll
