...
static int mnt_is_xvm_subvol(dev_t devt);
static int mnt_find_xvm_mount_conflicts(mnt_check_state_t *c, dev_t dev);
...
mnt_copy_ret_info(mnt_plist_return_t *part, mnt_plist_entry_t *item)
{
	...
	part->pathname = "";
	part->dev = NODEV;
	part->owner = MNT_OWNER_XVM;	/* must be xvm for now */
    } else {
		       ...
		       int cause)
{
    ...
    if (ptable && mnt_load_partition_table(ptable) == -1)
	return -1;
		...
		char *prefix)
{
    ...
    if (causebits & MNT_CAUSE_XVM_MNT) {
	strcpy(ostr, p);
	strcat(ostr, "device contains a mounted xvm subvolume\n");
	fprintf(fp, ostr, count++);
    }
}
...
mnt_string_cause(int cause)
{
    ...
    case MNT_CAUSE_XVM_MNT:
	return "xvm mounted volume";
    case MNT_CAUSE_XVM_PART:
	return "partition of an xvm-owned device";
    default:
	break;
    };
	...
	void                *ourdata)
{
	...
	xvmdisk = (inv->inv_class == INV_DISK &&
		   inv->inv_type == INV_SCSIDRIVE &&
		   (inv->inv_state & INV_XVMVOL));
	...
	for (which_part = 0; which_part < NPARTAB; which_part++) {
		...
		if (which_part == DSK_PART_VH) {
			...
			pentry->devptr->valid = 1;
			/* all partitions on xvm disks are owned by xvm */
			if (xvmdisk)
				tmp_deventry->owner = MNT_OWNER_XVM;
			continue;
		}
	...
	*p = '\0';
	...
	/* all partitions on xvm disks are owned by xvm */
	if (xvmdisk)
		pentry->devptr->owner = MNT_OWNER_XVM;
	...
	for (;;) {
		...
		/* 
		 * on xvm disks, all existing partitions except p1 are
		 * marked as owned by xvm so that they will always generate
		 * a conflict.  p1 is special cased because it can be used
		 * as a normal swap partition.
		 */

		if (xvmdisk && which_part != 1)
			pentry->devptr->owner = MNT_OWNER_XVM;
	}
	closedir(dp);
...
mnt_check_init(mnt_check_state_t **overlap_state)
{
    ...
    *overlap_state = new_mnt_check_state();
    ...
    /* note:  xvm-owned partitions are marked in diskscan */
...
mnt_ptable_check_mounts(mnt_check_state_t *check_state, mnt_partition_table_t *ptable_p)
{
	...
	/* first, check if the partition is owned by xvm */

	if (pentry->devptr->owner == MNT_OWNER_XVM) {
		cause = MNT_CAUSE_XVM_PART;
	} else {
		/* if not mounted and not owned by xlv/lv, then skip */
		cause = 0;
...
mnt_find_dev_mount_conflicts(mnt_check_state_t *check_state, dev_t dev)
{
    ...
    } else if (mnt_is_xvm_subvol(dev)) {
   	return mnt_find_xvm_mount_conflicts(check_state, dev);
    }
    ...
    /* 
     * If disk owned by xvm, register it and return
     */

    if (pentry->devptr->owner == MNT_OWNER_XVM) {
	mnt_register_partition(check_state, NULL, -1, 1, MNT_CAUSE_XVM_PART);
	count++;
	return count;
    }
 ...
 */
void
xvm_get_subvol_stripe(char *fs, int type, int *sunit, int *swidth)
{
	xvm_getdev_t getdev;
	char *rawfile;
	int fd;
...
/*
 * This is a wrapper for "xlv" and "xvm" to return the subvol stripe
 * information.
 */
void
get_subvol_stripe_wrapper(char *fs, int type, int *sunit, int *swidth)
{
	...
	/*
	 * Determine if this Volume belongs to XLV or XVM.
	 */
	if (stat64(fs, &sbuf) < 0) {
		return;
	}
	...
	if (major(dev) == XLV_MAJOR) {
		...
		if (major(dev) == XVM_MAJOR) {
			is_xvm = 1;
		} else {
		if (dev_to_drivername(dev, devname, &devnamelen))
			if (strncmp(devname, "xlv", 3) == 0)
				is_xlv = 1;
			else if (strncmp(devname, "xvm", 3) == 0)
				is_xvm = 1;
		}
	}
	if (is_xvm)
	 	xvm_get_subvol_stripe(fs, type, sunit, swidth);
	else if (is_xlv)
		xlv_get_subvol_stripe(fs, type, sunit, swidth);
}

/*
 * Check if dev is a mounted xvm device.
 */

static int
mnt_find_xvm_mount_conflicts(mnt_check_state_t *c, dev_t dev)
{
	int		mounts = 0, devlen;
	char		devname[128], *blockpath;
	dev_t		blkdev;
	FILE		*fp;
	struct mntent 	*mp;
	struct stat64	sbuf;
	mnt_plist_entry_t	*conflict;
	mnt_device_entry_t	*devent;

	/*
	 * Make sure dev is a block-special
	 */

	devlen = sizeof(devname);
	dev_to_devname(dev, devname, &devlen);
	blockpath = findblockpath(devname);

	stat64(blockpath, &sbuf);
	blkdev = sbuf.st_rdev;
	...
	while (mp = getmntent(fp)) {
		stat64(mp->mnt_fsname, &sbuf);

		if (blkdev != sbuf.st_rdev) {
			continue;
		}

		mnt_register_partition(c, NULL, -1, 1, MNT_CAUSE_XVM_MNT);
		mounts++;
	}
	...
	/*
	 * If not a mounted fs, see if dev is an xvm-owned partition
	 */

	for (devent = c->device_list; devent; devent = devent->next) {
		if (devent->part && devent->part->device == dev) {
			break;
		}
	}

	if (devent && devent->owner == MNT_OWNER_XVM) {
		mnt_register_partition(c, NULL, -1, 1, MNT_CAUSE_XVM_PART);
		mounts++;
	}
...
mnt_is_xvm_subvol(dev_t dev)
{
	...
	dev_to_drivername(dev, devname, &devnamelen);
	return (!strncmp(devname, "xvm", 3));
}
