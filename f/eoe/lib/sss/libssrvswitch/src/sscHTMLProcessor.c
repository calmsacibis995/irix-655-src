...
/* --------------------------------------------------------------------------- */
typedef void FPgetSSSData(char *url, sssSession *SSS);
...
/* ----------------------------- sscHTMLProcessor ---------------------------- */
void sscHTMLProcessor(sscErrorHandle hError, const char *url, streamHandle result)
{  char tmpbuffer[512];
   sssSession *SSS;
   int i;
   char *c = &tmpbuffer[0];
   ...
   { SSS->err = hError;
     ...
     getSSSDataByURL(c+strlen(SSS_ROOT),result,SSS);
     if(sscWereErrorsInZone(SSS->err)) 
      putStringFmt(sscGetErrorStream(SSS->err),szUrlWasProcessed,url);
     closeSSSSession(SSS);
   }
...
/* ---------------------------- getRGData ------------------------------------ */
static void getRGData(char *url, sssSession *SSS)
{  int  argc, i,  accept_rawcmd;
   ...
   { if(argc > 1) sscError(SSS->err,"Error in URL syntax in getRGData()");
     else putRGServerAttribute(url, SSS);
     return;
   }
   ...
   if((id = getReportSession(SSS)) != 0) srgGenerateReport(SSS->err,id,SSS->argc,SSS->argv,rawcmdstr,SSS->out);
   else sscError(SSS->err,"Can not getReportSession() in getRGData()");
}
/* --------------------------- getSSSTitle ----------------------------------- */
static void getSSSTitle(char *url, sssSession *SSS)
{ putStringFmt(SSS->out,"<B>%s.</B> Irix Release 1999",szSSCOficialName);
}
/* -------------------------- getSSSVersion ---------------------------------- */
static void getSSSVersion(char *url, sssSession *SSS)
{ 
...
/* ---------------------------- getStatus ------------------------------------ */
static void getStatus(char *url, sssSession *SSS)
{ hPlugInInfo p;
...
{  SSSSwitcher *p;
   ...
   { keylen = strlen(p->id);
     if(!strncasecmp(p->id,url,keylen)) 
     { p->getSSSData(url+keylen, SSS);
       return;
     }
   }
