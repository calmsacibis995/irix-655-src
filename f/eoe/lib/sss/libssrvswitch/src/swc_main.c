...
<H1>Not Found</H1><BODY TEXT=\"#000000\" BGCOLOR=\"#F9F9F9\">\
...
{ char *url,*wbuf,*param;
  int ssscalled,wbufmaxsize,wbufactsize;
  sscErrorHandle hErr;
  ...
  { sscHTMLProcessor(hErr,(const char *)url,hResult);
    if(sscWereErrors(hErr))
  ...
  ssscalled = sscHTMLPreProcessor(hErr,hInput,hResult,(const char*)param);
  ...
  ssrvexf.fp_ssrvSetIntegerByTcb(hTcb,SSRVSETI_HTTPRETCODE,0,HTTP_OK);                 /* Set HTTP return code */
  ssrvexf.fp_ssrvSetIntegerByTcb(hTcb,SSRVSETI_CONTENTLENGTH,0,sizeofStream(hResult)); /* Set "Content-length" value */
  if(ssscalled) ssrvexf.fp_ssrvSetIntegerByTcb(hTcb,SSRVSETI_NOCACHEFLG,0,1);          /* Set "No-Cache" flag */
  return 1;
}
/* -------------------------- swcContProc ------------------------------------ */
