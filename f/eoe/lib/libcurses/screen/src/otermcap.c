...
otgetent(char *bp, char *name)
{
	...
	*/
	if (strlen(bp) >= 1024)
	    (void) fprintf (stderr, CATGETS(catd, _MSG_CAPTOINFO_TENTRYTOOLONG),
	        otermname);
	return ret;
}
...
#define TERMNAMESIZE 16
	...
	while (*--p != ':')
		if (p<tbuf) {
			(void) fprintf (stderr, CATGETS(catd, _MSG_CAPTOINFO_BADTERMCAP_ENTRY),
			    otermname);
			return (0);
		}
	p++;
	...
	*q = 0;
	if (++hopcount > MAXHOP) {
		(void) fprintf (stderr, CATGETS(catd, _MSG_CAPTOINFO_INFINITE_LOOP),
		    otermname);
		return (0);
	}
	...
	if (l > TBUFSIZE) {
		(void) fprintf (stderr, CATGETS(catd, _MSG_CAPTOINFO_TNCHKTC_TOOLONG),
		    otermname);
