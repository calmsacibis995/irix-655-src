...
main(int argc, char **argv)
{
	...
	switch (i)
	    ...
	    case '?':
	    usage:
		(void) fprintf(stderr, CATGETS(catd, _MSG_TPUT_USAGE_HELP),
			progname);
    ...
    if (!term || !*term)
	{
	(void) fprintf(stderr,CATGETS(catd, _MSG_TPUT_NO_TERM_VALUE),
	    progname);
	exit(2);
	}
	...
	{
	case -2:
	    (void) fprintf(stderr,CATGETS(catd,	_MSG_TPUT_UNREADABLE_TERM),
			progname, term);
	    exit(3);
	case -1:
	    (void) fprintf(stderr,CATGETS(catd, _MSG_TPUT_NO_TERMINFO),
			progname);
	    exit(3);
	case 0:
	    (void) fprintf(stderr,CATGETS(catd, _MSG_TPUT_UNKNOWN_TERM),
			progname, term);
	    exit(3);
	}
...
outputcap(register char *cap, int argc, char **argv)
{
    ...
    (void) fprintf(stderr, CATGETS(catd, _MSG_TPUT_UNKNOWN_TERMCAP),
		progname, cap);
    exit (4);
    return(0);
}
     ...
     /* file = File to copy. */
{
    ...
    if (fd < 0)
	(void) fprintf (stderr, CATGETS(catd, _MSG_TPUT_INIT_FILE_ERR),
	    progname, file);
    else
	{
