/*	Copyright (c) 1990, 1991 UNIX System Laboratories, Inc.	*/
/*	Copyright (c) 1988 AT&T	*/
/*	  All Rights Reserved  	*/

/*	THIS IS UNPUBLISHED PROPRIETARY SOURCE CODE OF     	*/
/*	UNIX System Laboratories, Inc.                     	*/
/*	The copyright notice above does not evidence any   	*/
/*	actual or intended publication of such source code.	*/

#ident	"@(#)curses:screen/tic_hash.c	1.3"
/*********************************************************************
*                         COPYRIGHT NOTICE                           *
**********************************************************************
*        This software is copyright (C) 1982 by Pavel Curtis         *
*                                                                    *
*        Permission is granted to reproduce and distribute           *
*        this file by any means so long as no fee is charged         *
*        above a nominal handling fee and so long as this            *
*        notice is always included in the copies.                    *
*                                                                    *
*        Other rights are reserved except as explicitly granted      *
*        by written permission of the author.                        *
*                Pavel Curtis                                        *
*                Computer Science Dept.                              *
*                405 Upson Hall                                      *
*                Cornell University                                  *
*                Ithaca, NY 14853                                    *
*                                                                    *
*                Ph- (607) 256-4934                                  *
*                                                                    *
*                Pavel.Cornell@Udel-Relay   (ARPAnet)                *
*                decvax!cornell!pavel       (UUCPnet)                *
*********************************************************************/

/*
 *	comp_hash.c --- Routines to deal with the hashtable of capability
 *			names.
 *
 *  $Log: tic_hash.c,v $
 *  Revision 1.5  1998/09/18 19:47:22  sherwood
 *  SJIS feature for 6.5.2f
 *
 *  Revision 1.6  1998/09/16 02:31:53  rkm
 *  i18n message cleanup
 *
 *  Revision 1.5  1998/01/30 02:42:16  ktill
 *  merged HCL changes
 *
 *  Revision 1.4.18.1  1997/10/31 08:46:00  scm
 *  I18N changes for SJIS/BIG5 support.
 *
 * Revision 1.4.18.2  1997/10/04  19:21:59  rajkr
 * New Coding and Cataloguing Changes
 *
 * Revision 1.4  1993/09/09  00:02:58  igehy
 * Converted to 64-bit.
 *
 * Revision 1.3  1993/08/04  22:29:32  wtk
 * Fullwarn of libcurses
 *
 * Revision 1.2  1991/12/09  16:32:25  daveh
 * Add forward reference needed to avoid undefined symbol in link.
 *
 * Revision 2.1  82/10/25  14:45:34  pavel
 * Added Copyright Notice
 * 
 * Revision 2.0  82/10/24  15:16:34  pavel
 * Beta-one Test Release
 * 
 * Revision 1.3  82/08/23  22:29:33  pavel
 * The REAL Alpha-one Release Version
 * 
 * Revision 1.2  82/08/19  19:09:46  pavel
 * Alpha Test Release One
 * 
 * Revision 1.1  82/08/12  18:36:23  pavel
 * Initial revision
 * 
 *
 */

#include "curses_inc.h"
#include "compiler.h"
#include <locale.h>
#include <nl_types.h>
#include <msgs/uxeoe.h>
extern nl_catd catd;

static void make_nte(void);
static int hash_function(char *string);

/*
 *	make_hash_table()
 *
 *	Takes the entries in cap_table[] and hashes them into cap_hash_table[]
 *	by name.  There are Captabsize entries in cap_table[] and Hashtabsize
 *	slots in cap_hash_table[].
 *
 */

void
make_hash_table(void)
{
	int	i;
	int	hashvalue;
	int	collisions = 0;

	make_nte();
	for (i=0; i < Captabsize; i++)
	{
	    hashvalue = hash_function(cap_table[i].nte_name);       
	    DEBUG(9, "%d\n", hashvalue);

	    if (cap_hash_table[hashvalue] != (struct name_table_entry *) 0)
		collisions++;

	    cap_table[i].nte_link = cap_hash_table[hashvalue];
	    cap_hash_table[hashvalue] = &cap_table[i];
	}

	DEBUG(3, CATGETS(catd, _MSG_TIC_HASH_COLL), collisions);
	DEBUG(3, CATGETS(catd, _MSG_TIC_OO_ENTR), Captabsize);
}

/*
 * Make the name_table_entry from the capnames.c set of tables.
 */

static void
make_nte(void)
{
	register int i, n;
	extern char *boolnames[], *numnames[], *strnames[];

	n = 0;

	for (i=0; boolnames[i]; i++) {
		cap_table[n].nte_link = NULL;
		cap_table[n].nte_name = boolnames[i];
		cap_table[n].nte_type = BOOLEAN;
		cap_table[n].nte_index = (short) i;
		n++;
	}
	BoolCount = i;

	for (i=0; numnames[i]; i++) {
		cap_table[n].nte_link = NULL;
		cap_table[n].nte_name = numnames[i];
		cap_table[n].nte_type = NUMBER;
		cap_table[n].nte_index = (short) i;
		n++;
	}
	NumCount = i;

	for (i=0; strnames[i]; i++) {
		cap_table[n].nte_link = NULL;
		cap_table[n].nte_name = strnames[i];
		cap_table[n].nte_type = STRING;
		cap_table[n].nte_index = (short) i;
		n++;
	}
	StrCount = i;

	Captabsize = n;
}

/*
 *	int hash_function(string)
 *
 *	Computes the hashing function on the given string.
 *
 *	The current hash function is the sum of each consectutive pair
 *	of characters, taken as two-byte integers, mod Hashtabsize.
 *
 */

static int
hash_function(char *string)
{
	int	sum = 0;

	while (*string)
	{
	    sum += *string + (*(string + 1) << 8);
	    string++;
	}

	return (sum % Hashtabsize);
}



/*
 *	struct name_table_entry *
 *	find_entry(string)
 *
 *	Finds the entry for the given string in the hash table if present.
 *	Returns a pointer to the entry in the table or 0 if not found.
 *
 */

struct name_table_entry *
find_entry(char *string)
{
	int	hashvalue;
	struct name_table_entry	*ptr;

	hashvalue = hash_function(string);

	ptr = cap_hash_table[hashvalue];

	while (ptr != (struct name_table_entry *) 0  &&
			       	           strcmp(ptr->nte_name, string) != 0)
	    ptr = ptr->nte_link;

	return (ptr);
}
