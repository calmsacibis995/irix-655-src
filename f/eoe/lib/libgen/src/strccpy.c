...
strcadd(register char *pout, register const char *pin)
{
	...
	while (c = *pin) {
	    /* I18n note: At this point, pin always points to the
	     * first byte of a character (in multibyte locales)
	     */
		if (c == '\\')
		{
		        pin ++;
		...
		{
		    if ( ( n = mblen ( pin, MB_CUR_MAX ) ) < 0 )
			/* Invalid multibyte character, let's skip it */
			n = 1;
