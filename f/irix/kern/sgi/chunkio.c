...
static int 	nfs3_fstype;
extern struct vnodeops *nfs3_getvnodeops(void);

/*
 * This define is used in a couple of places to test for nfs3 vnodes.
 * The original version only used a bhv_lookup_unlocked which is 
 * inefficient and also causes deadlocks when doing xfs relocation
 * for cxfs.  Adding the test on vfs_fstype solves both of those
 * problems.  It is likely that this makes the second part of test
 * superfluous but I decided to be conservative and leave it in.
 */
#define VNODE_IS_NFS3(vp) \
	((vp)->v_vfsp->vfs_fstype == nfs3_fstype && \
	 vn_bhv_lookup_unlocked(VN_BHV_HEAD(vp), nfs3_getvnodeops()))
...
cinit(void)
{
	...
	/*
	 * Get the nfs3 file-system type in a way that is will not
	 * cause link problems if it is not there.
	 */
	for (i = 1; i < nfstype; i++) {
		if (vfssw[i].vsw_name && !strcmp(vfssw[i].vsw_name, FSID_NFS3))
			nfs3_fstype = i;
	}
