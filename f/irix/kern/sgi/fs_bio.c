...
static void bflushdev_hash(dhbuf_t *, bfidev_t *);
#ifdef DEBUG_BUFTRACE
static void binvaldev_hash(dhbuf_t *, bfidev_t *, inst_t *);
#else
static void binvaldev_hash(dhbuf_t *, bfidev_t *);
 ...
 *
 * This routine is probably not needed but is included for binary compatibility
 * since fflush_hash was not declared as static.
 * 
 */
void
bflush_hash(
	...
	dev_t	dev)
{
	...
	bfid.bfid_dev = dev;
	bfid.bfid_flags = 0;
	bzero(&bfid.bfid_r, sizeof(bfid.bfid_r));

	bflushdev_hash(bdp, &bfid);
}
	...
	bfidev_t *bfidp)
{
	buf_t	*bp;
	int	s;
	dev_t   dev = bfidp->bfid_dev;
	int     flags = bfidp->bfid_flags;
...
restart:
	...
	for (bp = bdp->bd_forw; bp != (buf_t *) bdp; bp = bp->bd_forw) {
		...
		if ((bp->b_edev == dev) &&
			...
			if ((flags & BFID_NOVNBUF) && (bp->b_vp != NULL)) {
				bfidp->bfid_r.bfidr_vskip++;
				continue;
			}

			mutex_spinunlock(&bdp->bd_lock, s);
			if (flags & BFID_TRYLOCK) {
			        if (!cpsema(&bp->b_lock)) { 
					bfidp->bfid_r.bfidr_lskip++;
					s = mutex_spinlock(&bdp->bd_lock);
			...
			if ((bp->b_edev == dev) &&
				...
				bfidp->bfid_r.bfidr_wrote++;
				notavail(bp);
				bp->b_flags |= (B_ASYNC|B_BFLUSH);
				bwrite(bp);
			} else {
				bfidp->bfid_r.bfidr_mchange++;
				vsema(&bp->b_lock);
			}
			s = mutex_spinlock(&bdp->bd_lock);
			if (bp->bd_hash != bdp) goto restart;
	...
	dev_t	dev)
{
	...
	bflushdev_hash(bdp, &bfid);
	bflushdev_hash(&nodev_chain, &bfid);
}

/*
 * For flushing write-behind blocks on a given device.  
 *
 * Provides more options and status reporting than bflush.
 */
void
bflush_dev(
	bfidev_t  *bfidp)
{
	...
	bzero(&bfidp->bfid_r, sizeof(bfidp->bfid_r));

	bdp = bdhash(bfidp->bfid_dev);
	bflushdev_hash(bdp, bfidp);
	bflushdev_hash(&nodev_chain, bfidp);
}
...
#define do_binvaldev_hash(bdp, bfidp)	\
	binvaldev_hash(bdp, bfidp)
#else
#define do_binvaldev_hash(bdp, bfidp)	\
	binvaldev_hash(bdp, bfidp, (inst_t *)__return_address)
#endif /* DEBUG_BUFTRACE */
	...
	)
{
	...
	bfid.bfid_flags = 0;
#ifdef DEBUG_BUFTRACE
	binvaldev_hash(bdp, &bfid, ra);
#else
	binvaldev_hash(bdp, &bfid);
#endif /* DEBUG_BUFTRACE */
}
	...
	dev_t	dev)
{
	...
	bdp = bdhash(dev);
	do_binvaldev_hash(bdp, &bfid);	   /* Invalidate hashed buffers */
	do_binvaldev_hash(&nodev_chain, &bfid); /* Invalidate unhashed buffers */
}
	...
	bfidev_t *bfidp)
{
	...
	bdp = bdhash(bfidp->bfid_dev);
	do_binvaldev_hash(bdp, bfidp);	   /* Invalidate hashed buffers */
	do_binvaldev_hash(&nodev_chain, bfidp); /* Invalidate unhashed buffers */
}
...
getrbuf_priv(
	int	sleep,
	struct zone *alt_zone)	/* Alternate zone for buffers */
{
	buf_t	*bp = kmem_zone_zalloc((alt_zone?alt_zone:buf_zone), sleep);
...
getrbuf_zone(
	...
	struct zone *alt_zone)	/* Alternate zone for buffers */
{
	return(getrbuf_priv(sleep, alt_zone));
}
...
getrbuf( int	sleep )
{
	...
	return(getrbuf_priv(sleep, buf_zone));
}
...
/*
 * void
 * freerbuf_priv(buf_t *, zone_t *)
 *      free up space allocated by getrbuf()
 *	Caller must supply correct zone if buffer was
 *	from a zone other than buf_zone.
 *
 * Calling/Exit State:
 *
 *      None
...
#endif
	kmem_zone_free((alt_zone?alt_zone:buf_zone), bp);
}

/*
 * void
 * freerbuf_zone(buf_t *, zone_t *)
 *      free up space allocated by getrbuf_zone()
 *	Caller must supply correct zone if buffer was
 *	from a zone other than buf_zone.
 *
 * Calling/Exit State:
