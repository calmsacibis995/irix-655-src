 ...
 */
void
kmem_zone_fill_max(register zone_t *zone, void *ptr, int fillsize, 
	int maxpages, int *rc)
{
	int	i, s, size, freesize;
	void	**zone_freep;
	...
	/*
	 *  Set zone_units_pp after we lock and decide to really do this. 
	 *	Up to this point, we have't changed anything in the zone
	 *	structure.
	 */
	s = mutex_psbitlock((__psunsigned_t *)zone_freep, 1);
	if ((zone->zone_total_pages + btoc(fillsize)) > maxpages) {
		*rc = -1;
		mutex_psbitunlock((__psunsigned_t *)zone_freep, 1, s);
		return;
	}
