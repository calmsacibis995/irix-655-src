...
#ifdef CELL_CAPABLE
extern void kthread_cell_init(kthread_t *); 
extern void kthread_cell_destroy(kthread_t *); 
extern int thread_remote_interrupt(kthread_t *, int *);
#endif
...
{
#ifdef CELL_CAPABLE
	kthread_cell_destroy(kt);
#endif
        kthread_afflink_unlink(kt);
 ...
 * thread_interrupt
 *
 * Send an interrupt to another thread.
 * Input:
 *	kt - thread to be interrupted
 *	spl - prev spl if kt is going to be unlocked
 *	alway_unlock  
 *		0 -> kt will be unlocked if return value is 1. If return
 *		     value is 0, kt will still be locked & was never unlocked
 *		     during processing of this function.
 *		1 -> unlock kt regardless of return value.
 *
 * Returns:
 *	0 - thread not put on runq
 *	1 - thread put an runq or guaranteed to be put on
 *	    the runq in the future.
 */ 
...
#pragma mips_frequency_hint NEVER
		for (;;) {
			ret = thread_remote_interrupt(kt, s);
			if (ret >= 0)
				break;
			*s = kt_lock(kt);
		}
