...
physmem_mldset_place(mldset_t* mldset)
{
	...
	for (i = 0, live_nodes = 0; i < numnodes; i++) {
	...
	if( base_mldlist_len > live_nodes &&
        ...
        /*
         * Determine effective mldlist to be used by
         * placement methods
         */
        ...
        {
                /*
                 * We create a new list, choosing one mld out of every
                 * CPUS_PER_NODE mlds in the base_mldlist, and then
                 * repeating for every cpu.
                 */
                if ((mldlist = pmolist_create(base_mldlist_len)) == NULL) {
                        errcode = PMOERR_NOMEM;
                        goto error1;
                }
                for (i = 0; i < base_mldlist_len; i += CPUS_PER_NODE) {
                        pmolist_insert(mldlist,
                                       pmolist_getelem(base_mldlist, i),
                                       pmo_ref_keep);
                }

                base_efflen = (base_mldlist_len + 1) / CPUS_PER_NODE;
                rep_factor = (base_efflen + live_nodes - 1) / live_nodes;
                mldlist_len = (base_efflen + rep_factor - 1) / rep_factor; 
                break;
        }
        default:
                errcode = PMOERR_EINVAL;
	...
	case TOPOLOGY_CUBE:
		...
		if (rafflist) {
			int x, y, k;
			int nfac = facs0thru7[cube_dim];
			int ncube = 1 << cube_dim;
			unsigned char* List = perms0thru7[cube_dim];
			int Cube[MAX_NODES];
			/*
	 ...
	 */
        ...
        /*
         * Final operations dependent on topology
         */
        switch(topology_type) {
        case TOPOLOGY_FREE:
        case TOPOLOGY_CUBE:
        ...
        case TOPOLOGY_PHYSNODES:
                /*
                 * We are done with these topologies already...
                 */
                ASSERT(base_mldlist == mldlist);
                break;
        case TOPOLOGY_CPUCLUSTER:
                for (i = 0; i < base_mldlist_len; i += CPUS_PER_NODE) {
                        int index;
                        ...
                        for (j = 1; j < CPUS_PER_NODE; j++) {
                                index = i + j;
                                if (index < base_mldlist_len) {
                                        physmem_mld_place_onnode(
                                                (mld_t*)pmolist_getelem(base_mldlist, index),
                                                node);
                                        ASSERT(mld_isplaced((mld_t*)pmolist_getelem(base_mldlist, index)));
                                }
                        }
                }
                ASSERT(base_mldlist != mldlist);
                pmolist_destroy(mldlist, pmo_ref_keep);
                break;
        default:
                cmn_err_tag(103,CE_PANIC, "INVALID TOPOLOGY AT END OF MLDSET_PLACE");
        }
  ...
  error2:
        if (topology_type == TOPOLOGY_CPUCLUSTER) {
                ASSERT(base_mldlist != mldlist);
                pmolist_destroy(mldlist, pmo_ref_keep);
        } else {
                ASSERT(base_mldlist == mldlist);
                       ...
                       size_t* mem_ask)
{
	int i;
        ...
        ASSERT(rafflist_len >= mldlist_len);
        ASSERT(mldlist);
        ASSERT(rafflist);
...
get_shortest_paths_from_node(cnodeid_t node)
{
	...
	/* REFERENCED */
	graph_error_t rv;
	vertex_hdl_t target_vertex, chase_list[2*MAX_NODES];
	...
	/*
	 * Sanity check: all nodes *should* have been visited 
	 */
	for (i=0;i<numnodes;i++) ASSERT( distance_metric[node][i] != MAX_DIST );
