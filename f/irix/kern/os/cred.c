...
#if defined(CELL_CAPABLE)
#define	CRSIZE	(sizeof(cred_t) - sizeof (cr1->cr_ref) - sizeof(cr1->cr_id) - \
		 sizeof(cr1->cr_groups))
	char *crc1 = (char *)(cr1) + sizeof (cr1->cr_ref);
	char *crc2 = (char *)(cr2) + sizeof (cr2->cr_ref);

	if (bcmp(crc1, crc2, CRSIZE))
		return (1);

	return (bcmp(cr1->cr_groups, cr2->cr_groups, sizeof(gid_t)*(ngroups_max)));
#else
	char *crc1 = (char *)(cr1) + sizeof (cr1->cr_ref);
	char *crc2 = (char *)(cr2) + sizeof (cr2->cr_ref);
