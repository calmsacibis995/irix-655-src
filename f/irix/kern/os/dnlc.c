...
dnlc_lookup(vnode_t *dp, register char *name, cred_t *cred, uint vn_flags)
{
	...
	return dnlc_lookup_full(dp, name, NULL, &fd, cred, vn_flags, NULL);
}
...
{
	return dnlc_lookup_full(dp, name, pnp, fd, cred, vn_flags, NULL);
}


bhv_desc_t *
dnlc_lookup_full(vnode_t *dp, char *name, struct pathname *pnp,
		 ncfastdata_t *fd, struct cred *cred, uint vn_flags,
		 uint64_t *ncptr)
{
	register u_int namlen;
	...
	/*
	 * Return name capability that we matched on (if we matched) to
         * the caller if requested.  This is needed for cfs name
         * caching.
	 */
	if (ncptr)
		*ncptr = ncp->vp_cap;
