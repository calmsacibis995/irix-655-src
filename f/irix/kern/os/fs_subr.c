...
/*
 * Define VN_RELE_NON_CELL to deal with VOP_COVER semantics issue for CELL
 * configurations.  See the comment in cmount (os/vfs.c) for an explanation.
 */
#if CELL_CAPABLE
#define VN_RELE_NON_CELL(cvp)  { if (!cell_enabled) VN_RELE(cvp); }
#else
#define VN_RELE_NON_CELL(cvp)  VN_RELE(cvp); 
#endif /* CELL_CAPABLE */
...
fs_cover(bhv_desc_t *bdp, struct mounta *uap, char *attrs, struct cred *cred)
{
	...
	/* Reject the mount if we we have a cover in progress,
	 * or there is someone using the covered node.
	 * If the vnode is a ready covered, it means that another
	 * mount finished after we did the lookup.
	 * If the to-be-covered node is a root, make sure we are 
	 * allowed to cover it.  Note that in the remount case, 
         * cvp is really the root vnode since lookup will traverse
	 * through the cover.
	 */
	if (cvp->v_flag & VMOUNTING || 
	    cvp->v_vfsmountedhere != NULL ||
	    !remount && ((cvp->v_flag & (VROOT | VROOTMOUNTOK)) == VROOT)) {
		VN_UNLOCK(cvp, s);
		VN_RELE_NON_CELL(cvp);
	...
	/*
         * Remount's original fucntion was in connection with the 
         * idea of mounting the root as read-only until fsck ran
	 * and only then switching to read-write mode.  With the
         * ascendancy of xfs and its elimination of fsck, it ceases
	 * have this function.  We instead are using it as part of
	 * a general facility fot updating mount which has its
	 * major function updating enterprise server control options
	 * and initiating server relocation.
         *
         * The old facility did not have a use for MS_RDONLY to be
	 * specified together with MS_REMOUNT.  It either was 
	 * switching from read-only to read-only, a useless no-op
	 * or from read-write to read-only which was invalid.
         *
         * To avoid adding a new bit to the general mount flags and
	 * attendant 6.5 compatibility issues, we are hijacking
	 * the combination of MS_REMOUNT and MS_RDONLY to represent
	 * the new mount-update function.  It does not make a 
         * file system read-only, but it doesn't make it read-write
	 * either.  It just leaves this stuff alone.
	 */
	if (remount) {
		/*
	...
	if (remount) {
		/* Note that cvp is pointing to the root, and that
		 * the covered node is already pointing to this 
		 * filesystem.
		 */
	...
	int ioflag)
{
	...
	if (!VNODE_CHECK_OPSFLAG(vp, VFS_OPSFL_FRLOCK2)) {
		panic("fs_frlock2 called from non-CXFS fs\n");
	}
	...
	if (cmd == F_CLNLK) {
		...
		if (cmd == F_CHKLKW)
			frcmd |= SLPFLCK;
		error = reclock2(vp, bfp, frcmd, flag, offset, cr, ioflag);
