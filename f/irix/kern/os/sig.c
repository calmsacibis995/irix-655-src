	...
	sigqueue_t *sqp)
{
	...
	if (sigbitismember(&stopdefault, sigbit, sig)
		 ...
		 * resumed now.
			...
			thread_interrupt(kt, &s, 1 /* Unlock KT */);
			goto out2;
		}
		goto out;
	}
	 ...
	 * it running.
	 */
	if (thread_interrupt(kt, &s, 0)) /* KT unlocked if return value non-zero */
		goto out2;
...
out:	
	if (n) {
		thread_interrupt_self(UT_TO_KT(ut));
	}
...
static int dumpvmap(vnode_t *, kvmap_t *, int, caddr_t, off_t);
