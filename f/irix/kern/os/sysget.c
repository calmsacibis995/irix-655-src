 ...
 *                                                                        *
 **************************************************************************/
#ident "$Id: sysget.c,v 1.33 1999/05/14 20:13:13 lord Exp $"
/*
...
sysget(struct sysgeta *uap, rval_t *rvp)
{
	...
	error = (*sap->func)(uap->name, buffer, &buflen, uap->flags, &cookie,
		&info);
	if (error) {
		goto done;
	}
		...
		sgt_info_t *info_p)
{
	...
	case SGT_READ:
		 ...
		 * where we left off.   We also check to make sure not
		 * to run off the end of the supplied buffer.
		 */
		...
		for (i = start; i >= 0 && i < end && flen > 0; i++) {
			if (pdaindr[i].CpuId == -1) {
				continue;
			}
		...
		sgt_info_t *info_p)
{
	...
	case SGT_READ:
		...
		if (flags & SGT_SUM) {
			...
			/* Allocate a temporary work buffer so we don't
			 * need multiply copyouts 
			 */

			klen = ap->size * (max - start);
			targ = kmem_zalloc(klen, KM_CACHEALIGN);
		}
		src = targ;
