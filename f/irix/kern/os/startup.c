...
devnm_to_dev(char *devnm)
{
	...
	dev_t hwdisk;
	int rc;
	extern dev_t xvm_devnm_to_dev(char *name);


	/*
	 * If devnm represents an xvm device, return the devt of the
	 * data subvolume block special.
	 */

	if ((hwdisk = xvm_devnm_to_dev(devnm)) != NODEV) {
		return hwdisk;
	}
