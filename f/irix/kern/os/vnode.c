...
/* 
 * Provides a similar function to vn_purge except that the caller
 * has a reference on the vnode and that the vnode must have been
 * evicted.
 */
void
vn_expunge(struct vnode *vp)
{
	register vfreelist_t *vfp;
	register int s;
	register int mask;
	...
	s = VN_LOCK(vp);
	ASSERT(vp->v_count == 1 && 
	       (vp->v_flag & (VSHARE | VEVICT)) == (VSHARE | VEVICT));
	...
	if (vp->v_fbhv != NULL) {
		...
		ASSERT(cache == VN_INACTIVE_CACHE ?
		       !(vp->v_flag & VINACTIVE_TEARDOWN) : 
		       (vp->v_flag & VINACTIVE_TEARDOWN));
	}

	s = VN_LOCK(vp);
	vp->v_flag = (vp->v_flag | VRECLM) & ~(VEVICT | VINACT);
	...
	if (vn_reclaim(vp, FSYNC_INVAL) != 0)
		panic("vn_expunge: cannot reclaim");
	...
	NESTED_VN_LOCK(vp);
	oldf = vp->v_flag; 
	vp->v_flag = oldf & ~(VINACT|VWAIT|VRECLM|VGONE|VEVICT);
	 ...
	 */
	if (oldf & VWAIT) {
	        vn_trace_entry(vp, "EXPUNGE_WAKEUP",
				(inst_t *)__return_address);
		sv_broadcast_bounded(vptosync(vp));
	}
}
...
out:
	 ...
	 *
        ...
        if (!error && !existing && *vpp) {
                if ((error = _MAC_VSETLABEL(*vpp, get_current_cred()->cr_mac)) ||
                    (error = _ACL_INHERIT(dvp, *vpp, vap))) {
