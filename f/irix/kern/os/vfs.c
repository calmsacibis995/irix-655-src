...
cmount(struct mounta *uap, rval_t *rvp, char *attrs, enum uio_seg seg)
{
	...
	if (vp->v_flag & VROOT && 
	    !((vp->v_flag & VROOTMOUNTOK) || (uap->flags & MS_REMOUNT))) {
		VN_RELE(vp);
	 ...
	 * Cover this vnode.  
         *
         * There is a basic problem in the interface to VOP_COVER as
         * released in 6.5.  The semantics are that the vnode reference
	 * on the vnode to be covered (or the root of the fs in the
	 * remount case) is donated to the cover operation by the
	 * caller.  In the case of a succesful mount (not a remount),
	 * this reference is retained to keep the covered vnode around.
	 * In all other cases, either failure or a successful remount,
	 * the reference is released as part of the VOP_COVER (by fs_cover),
	 * before returning to the caller.
	 *
	 * This interface poses problems in that the vnode in question
	 * may be deallocated upon return from fs_cover and thus may 
	 * not be touched.  In CELL_ARRAY configurations, the unlonking
	 * of the vnode behavior lock at the end of VOP_COVER can run 
	 * into this problem.
	 *
         * For 6.6 (if there is a 6.6), this problem should be fixed
	 * by changing the semantics of VOP_COVER so that it is the
	 * responsiblity of the caller to release his own reference in
	 * all cases.  For 6.5, the issue is more dificult.  Fixing this 
         * problem in a binary-compatible way does not seem possible if 
	 * there is any third-party code that actually calls VOP_COVER.  
         * Although this is unlikely, the possibility cannot be excluded.
         *
         * The best (least bad) solution that I can come up with is to
	 * make it the responsibility of the caller to release the
	 * reference in those cases in which it is unavoidable.  This
         * means that we continue to use the old strategy wherevr it 
	 * would not run into problems:
         *
         *     1) Non-CELL_CAPABLE systems
         *     2) CELL_CAPABLE systems not running cells.
	 *     3) Successful mounts (not remounts)
         *
         * In any other cases in which we have a failed mount or a
	 * successful remount on a CELL kernel where the VOP_COVER 
	 * is called from third-party code which is not going to
	 * release its reference, we have an incompatibility, but
	 * this is a trade of reference leak for a possible panic
	 * and/or memory corruption, which is a definite win.  The
	 * problem is inherent to the definition of VOP_COVER which
	 * is embedded (or may be embedded) in third-party kernel
	 * binaries.  We have to deal with it the best we can and
	 * hope that third-party code doesn't call VOP_COVER, which
	 * is not a bad bet.
	 */

	VOP_COVER(vp, uap, attrs, get_current_cred(), error);
#if CELL_CAPABLE
	if (cell_enabled && (error || (uap->flags & MS_REMOUNT)))
		VN_RELE(vp);
#endif
	return(error);
}
...
#ifdef CELL_CAPABLE
	{
		extern int	cfs_fstype;

		if (vfsp == NULL)
			vfsp = vfs_busydev(vp->v_rdev, cfs_fstype);
	}
#endif
