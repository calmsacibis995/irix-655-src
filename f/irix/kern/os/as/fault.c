...
#ifdef CELL_CAPABLE
				/*
				 * Notify FS that page dirty, this is used by
				 * cxfs to maintain data consistency between
				 * cells.
				 */
				notify_cxfs++;
#endif
...
#pragma mips_frequency_hint NEVER
			if (notify_cxfs) {
				VOP_VNODE_CHANGE(vp,VCHANGE_FLAGS_PAGE_DIRTY,0);
			}
		}
#endif
	}
...
#define MAX_RETRY_DELAY 32
 ...
 fastfind:
		...
		if (pfd) {
			...
			va.va_mask = AT_SIZE;
			VOP_GETATTR(vp, &va, ATTR_LAZY|ATTR_NOLOCK, get_current_cred(), rv);
		...
		auio.uio_pbuf = 0;
		ioflag = (rw & W_WRITE) ? IO_PAGE_DIRTY : 0;
		if (curuthread &&
		    (IS_THREADED(curuthread->ut_pproxy))) {
			ioflag |= IO_MTTHREAD;
		}
		VOP_READ(vp, &auio, ioflag, get_current_cred(),
			 &ut->ut_flid, error);
#ifdef DEBUG
	...
	if (ret  == USE_LARGE_PAGE) {
		...
		*cur_flt_infop = start_flt_infop  + (btoct(vaddr - lpage_start_addr));
		...
		if (ret == USE_BASE_PAGE)
			pmap_downgrade_to_base_page_size(pas, lpage_start_addr,
				 prp, desired_psize, PMAP_TLB_FLUSH);
		*npgs = btoct(NBPP);
	} else { /* Retry fault */
		/*
		 * As pfind was called with VM_ATTACH the pages will have
