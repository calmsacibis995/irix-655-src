...
STATIC int	reclock_new(struct vnode *, struct flock *, int, int, off_t,
				struct cred *);
STATIC int	reclock2_new(struct vnode *, struct flock *, int, int, off_t,
				struct cred *, int);
...
STATIC int	cleanlocks_new(struct vnode *, pid_t, sysid_t, int);
...
{
	/*
	 * Cluster systems must use the new flock, regardless of
	 * the systune value.
	 */
#ifdef CELL_CAPABLE
  	if (cell_enabled || use_old_flock == 0) {
#else
	if (use_old_flock == 0) {
#endif
...
{
        if (FLOCK_NEWER)
		(void)cleanlocks_new(vp, pid, sysid, 1);
	else
		(void)cleanlocks_old(vp, pid, sysid, 1);
}
	...
	sysid_t 	sysid)		/* The sysid for the process. */
{
	int	vn_changed;
        ...
        if (FLOCK_NEWER)
		vn_changed = cleanlocks_new(vp, pid, sysid, 0);
	else
		vn_changed = cleanlocks_old(vp, pid, sysid, 0);
	...
	int             cmd)            /* Lock command flags. */
{
	...
	wp->flw_req = *req;
	wp->flw_err = -1;
	initnsema(&wp->flw_sema, 0, "frlock");
 ...
 *
 * Using a semaphore rather than a sync-variable allows us to
 * drop the vnodeflLock, since we don't have to worry about
 * lost wakeups.  Plus, we need to drop the lock to avoid a deadlock
 * in the cluster case.
 *
...
reclockGet(
	...
	int		ioflag)		/* ioflag from IO path */
{
...
#if CELL_CAPABLE
	int		token_index = IO_TOKEN_SET(ioflag);
#endif
	...
	if (found == NULL) {
		 ...
		 */
		if (cmd & INOFLCK) {
			vrwlock_t vrwlck = (cmd & INOFLCK_READ) ?
				     VRWLOCK_READ : VRWLOCK_WRITE;
#if CELL_CAPABLE
			/* 
			 * For CELL_CAPABLE systems, we might need to
			 * release CFS tokens.  
			 */
			if (token_index != 0) {
				ASSERT(cell_enabled);
				ASSERT_ALWAYS(VNODE_CHECK_OPSFLAG(vp,VFS_OPSFL_FRLOCK2));
				FSC_NOTIFY_FRLOCK_WAIT_PREPARE(
					vp->v_fbhv,
					vrwlck, ioflag);
			} else 
#endif
				VOP_RWUNLOCK(vp, vrwlck);
		}
		if (psema(&waitp->flw_sema, PZERO+1) == -1) {
...
#if CELL_CAPABLE
			/* 
			 * If we released tokens above, get them
			 * back here.
			 */
			if (token_index != 0) {
				ASSERT(cell_enabled);
				ASSERT_ALWAYS(VNODE_CHECK_OPSFLAG(vp,VFS_OPSFL_FRLOCK2));
				FSC_NOTIFY_FRLOCK_WAIT_DONE(
					vp->v_fbhv,
					vrwlck, ioflag);
			} else 
#endif
				VOP_RWLOCK(vp, vrwlck);
		}
		vnodeflLock(vp);
...
reclock_new(
	...
	cred_t	*	cr)		/* User credentials. */
{
	return reclock2_new(vp, lckdat, cmd, flag, offset, cr, 0);
}
...
reclock2(
	...
	int		ioflag)		/* ioflag from IO path */
{
	return reclock2_new(vp, lckdat, cmd, flag, offset, cr, ioflag);
}
	...
	int		ioflag)		/* ioflag from IO path */
{
	...
	case F_RDLCK:
	case F_WRLCK:
	        retval = reclockGet(vp, lckdat, cmd, &locks_were_held, ioflag);
		break;
	case F_UNLCK:
	...
	cred_t *cr)
{
	...
	/*
	 * CXFS already has the DVN_SIZE_READ token, so make
	 * sure to tell getattr not to try and get it again
	 * by passing along ATTR_NOSIZETOK.
	 */
	VOP_GETATTR(vp, &vattr, ATTR_NOSIZETOK, cr, error);
	if (error)
		return error;
