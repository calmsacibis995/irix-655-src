...
static int iexec(char *fname, char **argp, char **envp);

/* ARGSUSED */
int
exec(struct execa *uap, rval_t *rvp)
{
	return (iexec(uap->fname, uap->argp, NULL));
}
...
{
	return (iexec(uap->fname, uap->argp, uap->envp)); 
}
...
static int
iexec(char *fname, char **argp, char **envp)
{
	int error = 0;
	vnode_t *vp;
	struct pathname pn;
