...
alloc_cpupda(int i)
{
	...
	if (i != master_procid ||
...
#ifndef IP19
		pfd = pagealloc_node(node, colorof(PDAPAGE), VM_VACOLOR);
#else /* IP19 */
		...
		pfd = pagealloc_node(node,
				     colorof(PDAPAGE)+i*(cachecolormask+1),
				     VM_VACOLOR);
#endif /* IP19 */
...
initmasterpda(register pfn_t fpage)
{
	...
	allset_cpumask._bits[0] = -1LL;
	allset_cpumask._bits[1] = -1LL;
#else
...
pda_actioninit(int i)
{
	...
	/* Now hang the "extra" entries from the pre-allocated ones.
	 * Currently these are all allocated on the master cpu.
	 */
	      ...
	      void *p0, void *p1, void *p2, void *p3)
{
	...
	ASSERT(who < maxcpus &&  pdaindr[who].CpuId != -1);
...
enter_panic_mode(void)
{
	...
	/* Enter one last UTRACE, then turn them off so traces generated
	 * while dumping don't fill the buffer
	 */
	UTRACE(RTMON_ALL, UTN('PANI','Cmde'), private.p_tstamp_mask, 0);
	private.p_tstamp_mask = 0;
}
