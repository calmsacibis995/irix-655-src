...
void _mraccess(mrlock_t *, inst_t *);
void idbg_mrlocks(__psint_t);
#endif
extern int mrcheckupd(mrlock_t *);
static void mr_idemote(mrlock_t *mrp);
...
#if defined(SEMAINFOP) || defined(SEMAMETER) || defined(SEMANAME)
int	semanamesavail=1;
#else
int	semanamesavail=0;
#endif
	...
	inst_t		*callpc)
{
	...
	interruptible = ((KT_ISUTHREAD(kt) || IS_MTHREAD(kt))
			 && ((pri & PMASK) > PZERO));
	if (interruptible)
		bla_isleep();
...
vsema(register sema_t *sp)
{
	...
	ASSERT_ALWAYS(sp->s_un.s_st.count < SHRT_MAX);
	count = ++sp->s_un.s_st.count;
	if (count <= 0) {
#ifdef SEMAMETER
		if (km) {
...
#define MR_IS_BHV(mrp)		((mrp)->mr_qbits&MR_BEHAVIOR)
...
#if defined(MRLOCK_CONSISTENCY_CHECK) || defined(DEBUG)
void
...
mrlock_init(mrlock_t *mrp, int lock_type, char *name, long sequence)
{
	...
	if (lock_type & MRLOCK_BEHAVIOR)
		mrp->mr_qbits |= MR_BEHAVIOR;
	if (lock_type & MRLOCK_DBLTRIPPABLE)
		mrp->mr_qbits |= MR_DBLTRIPPABLE;
	if (lock_type & MRLOCK_DIVEST_PRI_INHERIT)
		mrp->mr_qbits |= MR_DIVEST_PRI_INHERIT;
...
/*
 * void mrlock(mrlock_t *mrp, int type, int pri);
 *
 * type == MR_ACCESS or MR_UPDATE
 * flags == unused
 */
/*ARGSUSED*/
void
...
#ifdef SEMAMETER
	if (type == MR_ACCESS)
		_mraccess(mrp, (inst_t *)__return_address);
	else
...
mrremoveholder(mrlock_t *mrp, kthread_t *kt)
{
	...
	if (!mrip) {
		#pragma mips_frequency_hint NEVER
		if (mrp->mr_qbits & MR_DIVEST_PRI_INHERIT) {
			return;
		} else {
			cmn_err(CE_PANIC, "Unlocking an unknown mrlock!\n");
		}
	}
	if (mrp->mr_qbits & MR_DBLTRIPPABLE) {
		#pragma mips_frequency_hint NEVER
...
/*
 * mrdivest disassociates the calling thread from the
 * the mrlock held information for any locks it currently
 * holds that have the X flag set.  Called from the cell
 * message path for certain messages that need to hold locks
 * across multiple messages (causing the lock to be unlocked
 * by a different thread than locked it).
 */
void
mrdivest(mrlock_t *mrmatch)
{
	mri_t *mrip;
	mria_t *mriap;
	kthread_t *kt = private.p_curkthread;
	...
	{
		mrp = mrip->mri_mrlock;
		if (mrp && (mrmatch == NULL || mrmatch == mrp)) {
			ASSERT_ALWAYS(mrp->mr_qbits & MR_DIVEST_PRI_INHERIT);
			s = mr_lockq(mrp);
	...
	for (mriap = kt->k_mria.mria_next; mriap; mriap = mriap->mria_next) {
			...
			mrip < &mriap->mria_mril[MRI_MAXLINKS]; mrip++)
		{
			mrp = mrip->mri_mrlock;
			if (mrp && (mrmatch == NULL || mrmatch == mrp)) {
				ASSERT_ALWAYS(mrp->mr_qbits & 
					MR_DIVEST_PRI_INHERIT);
				s = mr_lockq(mrp);
				mrremoveholder(mrp, kt);
				mr_unlockq(mrp,s);
			}
...
mrlock_hpri_waiter(mrlock_t *mrp)
{
	...
	/* ASSERT(mr_islocked(mrp)); crashes idbg mrlock command */
	for (qp = pq->pq_pq; qp < &pq->pq_pq[NSCHEDCLASS]; qp++)
		if (*qp)
			return(*qp);
	CERT_DEBUG(pq->pq_pri == (-PX_PRIO_MAX)-1);
...
#endif
{
	...
	ASSERT(mr_islocked(mrp));
	ASSERT(private.p_curkthread);
	ASSERT(!(flags & ~(KT_WUPD|KT_LTWAIT)));
...
#ifdef CELL_CAPABLE
			/*
			 * Ignore the barrier semantic on BHV locks if the current
			 * thread already holds BHV read locks. NOTE that when this thread 
			 * drops ALL BHV locks, it will be delayed until the waiter
			 * for this lock acquires it. This will prevent starvation.
			 */
			if (MR_IS_BHV(mrp) && !(mrp->mr_lbits & MR_UPD) &&
					bla_test_defer_barrier(mrp)) {
				(void)mrcheckupd(mrp);
...
#ifdef CELL_CAPABLE
		if ((flags & KT_WUPD) && MR_IS_BHV(mrp))
...
#endif
			kt_fclr(kt, (flags & KT_WUPD) ? KT_WUPD : KT_WACC);
	...
	if (mrp->mr_pcnt) {
		...
		if (km) {
			...
			if (kt->k_flags & KT_WUPD) {
				km->mr_updv++;
				ASSERT(km->mr_updlocks == km->mr_updunlocks);
				km->mr_updlocks++;
...
wakeacc:
	...
	mrp->mr_pcnt -= nacc;
	mrp->mr_lbits += nacc * (MR_ACCINC-MR_WAITINC);
	mr_unlockq(mrp, s);
...
/*ARGSUSED*/
void
_mraccess(mrlock_t *mrp, inst_t *retaddr)
{
...
#ifdef CELL_CAPABLE
	if (MR_IS_BHV(mrp))
		bla_push(mrp, BLA_ACCESS, retaddr);
#endif
...
#ifdef BLALOG
	if (MR_IS_BHV(mrp))
		bla_push(mrp, BLA_UPDATE, retaddr);
#endif

	if (km) {
#if 0
		/* XXX appears to be a bogus assertion */
		ASSERT(km->mr_updlocks == km->mr_updunlocks);
#endif
...
#ifdef CELL_CAPABLE
		if (MR_IS_BHV(mrp))
			bla_push(mrp, BLA_TRY|BLA_ACCESS, __return_address);
#endif
		return 1;
	}
...
#ifdef BLALOG
		if (MR_IS_BHV(mrp))
			bla_push(mrp, BLA_TRY|BLA_UPDATE, __return_address);
#endif
		return 1;
	}
...
#ifdef CELL_CAPABLE
	ASSERT(!MR_IS_BHV(mrp) || (mrp->mr_lbits & MR_UPD));
	if (MR_IS_BHV(mrp) && bhv_try_deferred_ucallout(mrp))
		return;
#endif
	if (km) {
		if (mrp->mr_lbits & MR_UPD) {
...
#ifdef CELL_CAPABLE
	if (MR_IS_BHV(mrp))
		bla_push(mrp, BLA_ACCESS, __return_address);
#endif
...
mraccessf(mrlock_t *mrp, int flags)
{
	...
	if (mrp->mr_lbits & (MR_UPD|MR_ACCMAX|MR_WAIT)) {
		...
		flags = (flags & PLTWAIT) ? KT_LTWAIT : 0;
#ifdef SEMAMETER
		mrpsema(mrp, s, flags, km);
#else
		mrpsema(mrp, s, flags);
#endif
		/* mpsema unlocks mrp */
...
#if defined(CELL_CAPABLE) && !defined(SEMAMETER)
	if (MR_IS_BHV(mrp))
		bla_push(mrp, BLA_ACCESS, __return_address);
#endif
}
...
#if defined(BLALOG) && !defined(SEMAMETER)
	if (MR_IS_BHV(mrp))
		bla_push(mrp, BLA_UPDATE, __return_address);
#endif
}
...
#ifdef CELL_CAPABLE
	if (MR_IS_BHV(mrp)) {
		ASSERT( !(mrp->mr_lbits & MR_ACC));	/* function used ONLY to release WRITE locks */
#ifndef SEMAMETER
		if (bhv_try_deferred_ucallout(mrp))
			return;
#endif
#ifdef BLALOG
		bla_pop(mrp);
...
#ifdef CELL_CAPABLE
	if (MR_IS_BHV(mrp) && bla_do_deferred_barrier())
		bla_prevent_starvation(-1);
#endif
}
...
#ifdef BLALOG
	if (MR_IS_BHV(mrp))
		bla_pop(mrp);		/* ZZZ ?? need a bla_demote?? */
#endif
