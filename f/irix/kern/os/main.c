...
#endif
		vhost_lp_init(void), procinit(void), ptpool_init(void);
extern void	bhv_global_lp_init(void);
extern void	init_all_devices(void);
extern void	p0init(void), p0exit(void);
extern void 	xbox_sysctlr_init(void);
#if CELL_CAPABLE
extern void	tsv_init(void);
extern void	cms_config_init(void);
extern void	bhv_global_dp_init(void);
#endif /* CELL_CAPABLE */
...
static void (*postintrinit_tbl[])(void) = {
	...
	uma_init,
#endif
	vhost_lp_init,		/* global state services */
 ...
 * CELL initialization.
 *
 * initializations not requiring any cellular service, including cellid
 * can be done here
 */
extern void 	bla_init(void);

static void (*cell_init_tbl[])(void) = {
	bhv_global_dp_init,	/* behavior code init */
	bla_init,
	vhost_init,
	cell_array_preinit,
	cms_config_init,
	tsv_init,		/* Thread supervisor init */
	0
};
#endif /* CELL_CAPABLE */
...
p0(void)
{
	...
	vfs_mounthwgfs();			/* Mount the hwgfs */
