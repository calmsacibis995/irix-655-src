...
int invent_to_irix5(void *, int , xlate_info_t *);
...
extern int sgicell(struct syssgia *uap, rval_t *rvp);
		...
		do {
			...
			for ( ; --sz >= 0 ; pt++, vaddr += NBPC) {
				pg_setfetchop(pt);
                                ...
                                /*
                                 * Migration for a fetchop page is disabled
                                 * when it is locked, due to the rawcnt
                                 * increment.
                                 */
				...
				if (pg_isvalid(pt)) {
					/* 
					 * this case will occur if the same process or
					 * multiple processes within a share group
					 * make the syssgi call, we want to return
					 * error mainly for debuggin potential problems
					 * in the field since the fetchop lib will 
					 * ignor this error in the cases it knows
					 * what it's doing
					 */
...
#endif
		  rc = fo_scsi_device_switch(p_vhdl, &s_vhdl);
#if DEBUG
		  printf("syssgi: SGI_FO_SWITCH s_vhdl = %d\n", s_vhdl);
#endif
		  if (rc == FO_SWITCH_SUCCESS || rc == FO_SWITCH_ONEPATHONLY) {	/* copyout if s_vhdl returned */
