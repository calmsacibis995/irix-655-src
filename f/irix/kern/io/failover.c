...
#ident "io/failover.c: $Revision: 1.26 $"

/*
 * XXX - still need to mirror VH in dksc.c : dksc_disk_part_hwg_update()
*/
...
static void fo_remove_lun(vertex_hdl_t p_lun_vhdl);
static int  fo_path_add (struct scsi_fo_instance *foi, vertex_hdl_t lun_vhdl, int is_viable_primary, int keep_partitions);
...
fo_init()
{
  ...
  for (i = 0, foc = fo_candidates; foc->scsi_match_str1; ++i, ++foc) {
    ...
    if (!fo_candidate_generic && strcmp (foc->scsi_match_str1, GENERIC_MATCH_1) == 0
	...
	*/
	for (i=0; i<MAX_FO_PATHS; i++) {
		...
		if (!found_match && foi->foi_path[i].foi_path_status != FOI_PATHSTAT_INVALID) {
			...
			*/
			pinv = fo_hwgraph_inventory_get (foi->foi_path[i].foi_path_vhdl);
			if (pinv) {
				pinv->inv_state &= ~(INV_ALTERNATE|INV_FAILED|INV_PRIMARY);
			}
			/*
			** Clear DK_FAILOVER within dksc.
			*/
			if (dk_clr_flags (lun_vhdl[j],DK_FAILOVER)) {
				DBG (2,"Unable to clear failover flag in dk struct.\n");
			}
			/*
	...
	for (i=0; i<paths; i++) {
		...
		if (!found_match) {
			...
			*/
			...
			error = fo_path_add (foi, lun_vhdl[i], is_viable_primary, fgi->fgi_keep_partitions);
...
static int
fo_path_add (struct scsi_fo_instance *foi, vertex_hdl_t lun_vhdl, int is_viable_primary, int keep_partitions)
{
	int i, error;
	inventory_t *pinv;
	...
	for (i=0; ; ++i) {
		...
		if (i == MAX_FO_PATHS) {
			...
			cmn_err(CE_WARN, "%s: Alternate path count exceeded %d. "
					 "SCSI host ID conflict possible\n",
		...
		if (foi->foi_path[i].foi_path_status == FOI_PATHSTAT_INVALID) {
			...
			if (foi->foi_primary == FOI_NO_PRIMARY) {
				if (is_viable_primary) {
					...
					/*
					** Update inventory to flag as primary.
					*/
					if (foi->foi_foc == fo_candidate_generic) {
				...
				/*
				** Update inventory to flag as alternate.
				** Clear the DK_FAILOVER flag.
				*/
				pinv = fo_hwgraph_inventory_get (lun_vhdl);
				if (pinv) {
					pinv->inv_state &= ~(INV_FAILED|INV_PRIMARY);
					pinv->inv_state |= INV_ALTERNATE;
				}
				if (dk_set_flags (lun_vhdl,DK_FAILOVER)) {
					DBG (2,"Unable to set failover flag in dk struct.\n");
				}
				if (!keep_partitions) fo_remove_lun (lun_vhdl);
			}
			break;
		}
	}
...
fo_scsi_device_update(u_char *inq, vertex_hdl_t lun_vhdl)
{
   ...
   * struct, then we've previously encountered this device. We would
   * normally bail out here, unless no viable primary path has yet
   * been found.
   */
  foi = SLI_FO_INFO(lun_info);
  if (foi) {
  ...
  if (foc->scsi_foi) {
	...
	for (i = 0; ; ++i) {
	  ...
	  if (foi->foi_path[i].foi_path_status == FOI_PATHSTAT_INVALID) {
	    ...
	    SLI_FO_INFO(lun_info) = foi;
	    if (foi->foi_primary == FOI_NO_PRIMARY)
	      foi->foi_primary = is_viable_primary ? i : FOI_NO_PRIMARY;
    ...
    foi->foi_foc = foc;
    foi->foi_grp_name = foi_name;
    foi->foi_primary = is_viable_primary ? 0 : FOI_NO_PRIMARY;
     ...
     * List is empty
     */
    DBG(10, "fo_scsi_device_update(%d,%d,%d) -- %s %s - NEW (1st)\n",
	    ctlr, targ, lun,
	    path_name,
	    (*foc->scsi_sprintf_func)(foi_name));
    foi = foc->scsi_foi = kern_calloc(1, sizeof(struct scsi_fo_instance));
    foi->foi_foc = foc;
    foi->foi_grp_name = foi_name;
    foi->foi_primary = is_viable_primary ? 0 : FOI_NO_PRIMARY;
    foi->foi_path[0].foi_path_vhdl = lun_vhdl;
...
fo_scsi_device_switch(vertex_hdl_t p_vhdl, vertex_hdl_t *s_vhdl)
{
   ...
   */
  rc = hwgraph_traverse(p_vhdl, "../"EDGE_LBL_SCSI, &tmp_vhdl);
  if (rc == GRAPH_SUCCESS) {
  ...
  rc = hwgraph_traverse(p_vhdl, "../"EDGE_LBL_BLOCK, &tmp_vhdl);
  if (rc == GRAPH_SUCCESS) {
    hwgraph_vertex_unref(tmp_vhdl);
    DBG(10, "fo_scsi_device_switch: %d is a hwg-char/block vertex\n", p_vhdl);
    goto switch_1;
  }
  rc = hwgraph_traverse(p_vhdl, "../"EDGE_LBL_CHAR, &tmp_vhdl);
  if (rc == GRAPH_SUCCESS) {
    hwgraph_vertex_unref(tmp_vhdl);
    DBG(10, "fo_scsi_device_switch: %d is a hwg-char/block vertex\n", p_vhdl);
    goto switch_1;
 ...
 switch_1:
  ...
  rc = hwgraph_vertex_name_get(p_vhdl, path_name, MAXDEVNAME);
  if (rc != GRAPH_SUCCESS) {
   ...
   * degenerate case.
   */
  for (i = 0, j = 0; i < MAX_FO_PATHS; ++i) {
    if (foi->foi_path[i].foi_path_status != FOI_PATHSTAT_INVALID)
      ++j;
  }
  if (j == 1 && foi->foi_primary != FOI_NO_PRIMARY) {
    *s_vhdl = p_vhdl;
  ...
  if (foi->foi_primary == FOI_NO_PRIMARY || foi->foi_path[foi->foi_primary].foi_path_vhdl == p_lun_vhdl) {
    ...
    /*
     * Find a unfailed secondary path and use as next primary. If all
     * paths already tried, return FO_SWITCH_PATHS_EXHAUSTED. Before
     * returning, mark all paths as virgin untried.
     */
  switch_retry:
    if (foi->foi_primary != FOI_NO_PRIMARY) {
	foi->foi_path[foi->foi_primary].foi_path_status = FOI_PATHSTAT_FAILED;
    }
    else
	foi->foi_primary = 0;
    for (i = foi->foi_primary; ;) {
      if (foi->foi_path[i].foi_path_status == FOI_PATHSTAT_VALID) {
	foi->foi_primary = i;
	break;
      }
	...
	i = 0;
      if (i == foi->foi_primary) {
	/*
	 * No valid paths found or all other paths have already
	 * failed. Make all failed paths valid and set the primary to
	 * first valid path found.
	 */
	rc = FO_SWITCH_PATHS_EXHAUSTED;
	foi->foi_primary = FOI_NO_PRIMARY;
	for (i = 0; i < MAX_FO_PATHS; ++i) {
	  if (foi->foi_path[i].foi_path_status == FOI_PATHSTAT_FAILED) {
	    foi->foi_path[i].foi_path_status = FOI_PATHSTAT_VALID;
	    if (foi->foi_primary == FOI_NO_PRIMARY)
	      foi->foi_primary = i;
	  }
    ...
    /*
     * Call the device specific switch routine for the new path. If
     * switch fails, try switching to next valid path.
     */
    if (foc->scsi_switch_func) {
      if ((*foc->scsi_switch_func)(p_lun_vhdl, s_lun_vhdl)) {
	goto switch_retry;
      }
    }
  }
  ...
  rc = hwgraph_vertex_name_get(s_lun_vhdl, s_base_name, MAXDEVNAME);
  ASSERT(rc == GRAPH_SUCCESS);
  ...
  /*
   * Clear the DK_FAILOVER flag in the dksoftc structure for the device.
   * To stop incoming requests from being returned with EIO.
   ...
   *
   * Note: this is only performed for "generic" devices.
   */
  if (p_lun_vhdl != s_lun_vhdl
   && foi->foi_foc == fo_candidate_generic) {
	inventory_t *pinv;
	if (dk_clr_flags (s_lun_vhdl,DK_FAILOVER)) {
		DBG (2,"Unable to set failover flag in dk struct.\n");
	}
	pinv = fo_hwgraph_inventory_get (s_lun_vhdl);
	if (pinv) {
		pinv->inv_state &= ~(INV_ALTERNATE|INV_FAILED);
		pinv->inv_state |= INV_PRIMARY;
	}
  ...
  /*
   * Set the DK_FAILOVER flag in the dksoftc structure for the device.
   * To cause the queued requests to be dequeued and returned with
   * EIO.  This should result in a retry of those requests down the
   * new path.  Also results in new incoming requests for the device
   * being rejected with EIO.
   *
   * Afterwards adjust the hwgraph inventory state for the device.
   */
	if (dk_set_flags (p_lun_vhdl,DK_FAILOVER)) {
		DBG (2,"Unable to set failover flag in dk struct.\n");
	}
	pinv = fo_hwgraph_inventory_get (p_lun_vhdl);
	if (pinv) {
		pinv->inv_state &= ~INV_PRIMARY;
		pinv->inv_state |= (INV_ALTERNATE | INV_FAILED);
	}
...
static void
fo_remove_lun(vertex_hdl_t p_lun_vhdl)
{
  vertex_hdl_t p_disk_vhdl;
  int          rc;
#if 0
  vertex_hdl_t to;
  u_char       part;
  ...
  /*
   * If disk vertices exist, remove them.
   */
  rc = hwgraph_traverse(p_lun_vhdl, EDGE_LBL_DISK, &p_disk_vhdl);
  if (rc == GRAPH_SUCCESS) {
    dk_namespace_remove(p_disk_vhdl);
#if 0
    /* OBSOLETE jeremy apr99 */
    for (part = 0; part < DK_MAX_PART; ++part) {
      if (scsi_part_vertex_exist(p_disk_vhdl, part, &to)) {
	 dksc_part_vertex_remove (p_disk_vhdl, part);
      }
    }
#endif
