...
#ident "io/dksc.c $Revision: 3.301 $"
...
copyerr:
	...
	case DIOCGETVH:
	{
		...
		mutex_lock(dksema, PRIBIO);
		rval = gencopy(&dk->dk_vh, arg, sizeof(dk->dk_vh), 1);
		mutex_unlock(dksema);
...
dk_print_sense(struct dksoftc *dk, struct scsi_request *sp, dev_t dev)
{
	...
	if (add_sense < SC_NUMADDSENSE) {
		/*
		 * Don't report 'recovered' errors on defect list; it means
		 *   the drive doesn't support the requested format, and is
		 *   responding with it's default format.
		 * Don't report 'drive not ready' if the command issued was
		 *   a 'test unit ready'.
		 */
		if ((add_sense == SC_DEFECT_ERR && sense_key == SC_ERR_RECOVERED) ||
		    (add_sense == SC_NOTREADY && sp->sr_command[0] == 0))
			key_msg = NULL;
		else if (add_sense == 0)
			add_msg = NULL;
		else
		 ...
		 char *port, char *lun, char *part)
{
	...
	if (node && port) {
		...
		sprintf(node, "%x", nodenum);
		portnum = SDI_PORT(disk_info);
		sprintf(port, "p%x", portnum);
	}
	if (lun) {
		/*
		 * Find the lun number
...
dk_user_printable_sense(struct dksoftc *dk, register scsi_request_t *sp, int mute)
{
	...
	else if(key==SC_ERR_RECOVERED && addsense==0x1c)	/* defect list not found;
		* see this when we ask for logical block format, but drive doesn't support
		* it, for example.  We don't want to print that; fx will just fall back
		* on alternate format.  See bug #554444.  IBM  DDRS-39130W is one of those
		* drives */
		rc = 0;
