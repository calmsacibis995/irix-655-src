...
void	idbg_traplog(__psint_t);
void do_traplogs(void (*)(pda_t *), int);
void dumptraplog(pda_t *);
#endif /* TRAPLOG_DEBUG */
...
#if R10000 && (EVEREST || IP28 || IP30 || IP32)
...
struct idbgcommfunc idbgBasicBuiltIn[] = {
    ...
    "traplog",   VD idbg_traplog,
        INTEGER_ARGS,   "Dump current CPU traplog",
#endif /* TRAPLOG_DEBUG */
...
idbg_kthread(__psint_t x)
{
	...
	qprintf(" preempted %d \n", kt->k_preempt);
	CELL_ONLY(qprintf(" k_bla @0x%x, in_gbla %d, starvearg 0x%x,"
			" deferred_barrier %d\n", 
			kt->k_blap, kt->k_blap->kb_in_gbla,
			kt->k_blap->kb_starvearg, kt->k_blap->kb_deferred_barrier));
...
char	*tab_bflags[] = {
	...
	"b_priv_addr",	/* 0x0002000000000000 */
	"0x8000000000000000",
			/* 0x8000000000000000 */
	0
};
...
idbg_bhv(__psint_t x)
{
	...
	if (x < 0) {
		...
		do {
			bhv_identity_t	*bip = BHV_IDENTITY(bdp);
			void		*offset;
			qprintf(" ops %s+0x%x"
				" id %d pos %d"
				" vobj 0x%x pdata 0x%x next 0x%x\n",
                                fetch_kname(bdp->bd_ops, &offset), offset,
				bip->bi_id, bip->bi_position,
...
idbg_vnbhv(__psint_t x)
{
	...
	if (x < 0 ) {
		...
		do {
			...
			if (bdp->bd_ops == &spec_cs_vnodeops) {
...
idbg_prlsnode(struct lsnode *lsp)
{
	...
	qprintf("  dev 0x%x(%d/%d, %d)", lsp->ls_dev,
					 major(lsp->ls_dev),
					 emajor(lsp->ls_dev),
					 minor(lsp->ls_dev));
	...
	qprintf("  opencnt %d gen %d ops 0x%x fsid %d flags ",
					lsp->ls_opencnt, lsp->ls_gen,
					lsp->ls2cs_ops, lsp->ls_fsid);
...
#ifdef	SPECFS_DEBUG
	qprintf("  line# %d cycle 0x%x.%x",
				lsp->ls_line,
				lsp->ls_cycle.tv_sec, lsp->ls_cycle.tv_nsec);
#endif
	qprintf(
...
idbg_prcsnode(struct csnode *csp)
{
	...
	qprintf("  dev 0x%x(%d/%d, %d) cvp 0x%x{%d}\n",
						csp->cs_dev,
						major(csp->cs_dev),
						emajor(csp->cs_dev),
						minor(csp->cs_dev),
	...
	qprintf("  opencnt %d mapcnt %d gen %d ",
			csp->cs_opencnt, csp->cs_mapcnt, csp->cs_gen);

	qprintf("flags %s%s%s%s%s%s",
	...
	if (p = idbg_findfssw_vfs(vfsp, &bdp)) {
		...
		if ((vfsops_t *)BHV_OPS(bdp) == &efs_vfsops) {
			...
			for (ip = bhvtom(bdp)->m_inodes; ip; ip = ip->i_mnext)
				prvn(itov(ip), all);
#ifdef CELL_CAPABLE
		} else if ((vfsops_t *)BHV_OPS(bdp) == &cfs_vfsops) {
			extern prcfsvn(bhv_desc_t *, int);
...
prvfs(register struct vfs *vfsp, int summary)
{
	...
	CELL_ONLY(qprintf("\n"));
	CELL_ONLY(qprintf("  vfs_cell %d vfsbh_mrlock 0x%x",
				vfsp->vfs_cell, &vfsp->vfs_bh.bh_lockp)); 
	qprintf("\n");
	...
	if (i = idbg_findfssw_vfs(vfsp, &bdp)) {
		...
		if (bdp && (vfsops_t *)BHV_OPS(bdp) == &efs_vfsops) {
			if (bhvtom(bdp) == NULL) {
				qprintf("  No EFS superblock!\n");
				return;
			}
...
idbg_mrlock(mrlock_t *mrp)
{
	...
	qprintf("mrlock: 0x%x [%s], lbits 0x%llx qbits 0x%llx (TYPE:%s%s%s%s%s%s%s):",
		...
		mrp->mr_qbits & MR_DIVEST_PRI_INHERIT ? "divest" : "",
	...
	for (i = 0; i < NSCHEDCLASS; i++) {
		...
		for (n=0, kt = mrp->mr_waiters.pq_pq[i]; kt; kt = kt->k_flink) {
			if (n++) qprintf("           ");
			qprintf("kthread 0x%x %s,  mpri %d, bpri %d, kpri %d\n",
				kt, (kt->k_flags&KT_WUPD) ? "update" : "access",
				kt->k_mrqpri, kt->k_basepri, kt->k_pri);
	...
	for (i = 0; i < NSCHEDCLASS; i++) {
		...
		for (n=0, mrip = mrp->mr_holders.pq_pq[i]; mrip; mrip = mrip->mri_flink) {
			...
			qprintf("kthread 0x%x mpri %d, bpri %d, kpri %d\n",
				kt, mrip->mri_pri, kt->k_basepri, kt->k_pri);
...
#if LARGE_CPU_COUNT
	qprintf(
	"  schedflags 0x%x action 0x%x cpumask [0x%x 0x%x]\n",
		pd->p_schedflags, pd->p_actionlist.todolist,
		pd->p_cpumask._bits[0], pd->p_cpumask._bits[1]);
...
{
	qprintf("user area dump for proc 0x%x (pid %d), physaddr 0x%x\n",
		pp, pp->p_pid);
	...
	for (i = 0; i < maxcpus; i++) {
		if (pda = pdaindr[i].pda) {
			...
			if (kt = pda->p_curkthread) {
				qprintf("  on cpu 0x%x pri %d\n",kt,kt->k_pri);
			}
			if (pda->p_nextthread && kt != pda->p_nextthread) {
				kt = pda->p_nextthread;
				qprintf("  next 0x%x pri %d\n",kt,kt->k_pri);
			}
			kt = pda->p_cpu.c_threads;
			if (kt) do {
				qprintf("     kthread 0x%x pri %d %s\n",
					kt, kt->k_pri,
					kt->k_flags & KT_HOLD
						? "affinity" : "");
				kt = kt->k_rflink;
...
doxthread:
		...
		btrace((uint*)pc, (uint*)sp, (uint*)0, 60, (uint*)UT_VEC, kt, 0);
		return;
	    } else if (zzubt || uthreadp_is_valid((uthread_t *)x[1])) {
		ut = (uthread_t *)x[1];
		p = UT_TO_PROC(ut);
	    } else {
		p = (proc_t *)x[1];
...
*/
void
idbg_traplog(__psint_t x)
{
	qprintf("TRAPLOG:\n");
	do_traplogs(dumptraplog, x);
}

void
do_traplogs(void (*func)(pda_t *), int which)
{
	if (which >= 0 && which < maxcpus) {
		if (pdaindr[which].CpuId != -1) {
			(*func)(pdaindr[which].pda);
...
#endif 
		if (which < -1)
			traplog_dumpcnt = -which;
		(*func)((pda_t *)PDAADDR);
	}
}

void
dumptraplog_entry( long long *entry)
{
	switch (entry[7]) {
	case 1:
		qprintf("TLBrefl");
	...
	case 7:				/* TFP only */
		...
		break;
	default:
		qprintf("CODE 0x%x:", entry[7]);
	}

	qprintf("(%x) EPC 0x%x BVADR 0x%x RA 0x%x SP 0x%x CTHREAD 0x%x CAUSE %R\n",
		(long long)entry&0x0ffff, entry[0], entry[1],
		entry[4], entry[6], entry[5], entry[2], cause_desc);
}
 ...
 */
void
dumptraplog(pda_t *pd)
{
	int i, nentries, dumpcnt;
	long long *traplog_ptr,*traplog_bufstart,*start,*traplog_bufend;

	qprintf("pda 0x%x (0x%x) id %d ", pd, pd->p_flags, pd->p_cpuid);

	if ((long long)pd & (NBPP-1)) {
		qprintf("Illegal pda pointer 0x%x\n", pd);
		return;
	}
	/* Need to convert the pda (private) address in the traplog to
	 * an address relative to the passed in pda pointer.
	 */
	traplog_ptr = (long long *)((char*)pd +
		((*(long long*)((char*)pd+(TRAPLOG_PTR&(NBPP-1))))&(NBPP-1)));
	traplog_bufstart =
		(long long *)((char*)pd + (TRAPLOG_BUFSTART & (NBPP-1)));

	traplog_bufend =
		(long long *)((char*)pd + (TRAPLOG_BUFEND & (NBPP-1)));

	qprintf("current pointer 0x%x start 0x%x\n", traplog_ptr,
		traplog_bufstart );
	dumpcnt = nentries = traplog_dumpcnt;
	qprintf("will print %d most recent entries \n", nentries);

	start =(long long *)((char *)traplog_ptr - nentries*TRAPLOG_ENTRYSIZE);
	if (start < (long long *)traplog_bufstart) {

		nentries = ((char*)traplog_ptr - (char*)traplog_bufstart)/
				TRAPLOG_ENTRYSIZE;

		start = (long long *)((char *)traplog_bufend -
				(dumpcnt-nentries)*TRAPLOG_ENTRYSIZE);

		/* Dump entries before wrap-around point
		 * (i.e. at end of traplog buffer)
		 */

		for (i=0; i<(dumpcnt-nentries); i++) {
			dumptraplog_entry( start );
			start += 8;
		}

		/* Setup to dump entries after wrap-around point
		 * (i.e. at start address of traplog buffer).
		 */

		start = (long long *)traplog_bufstart;
	}
