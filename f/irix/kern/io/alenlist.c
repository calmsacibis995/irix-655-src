...
/*
 * Many alenlist interfaces accept both an alenlist and and cursor as
 * parameters.  This macro is used by those interfaces to insure that
 * there is one -- and exactly one -- alenlist specified by those two
 * parameters.  If only one of those parameters is non-NULL, this macro
 * derives the other parameter.  If neither parameter is specified, this
 * macros returns ALENLIST_FAILIURE to whoever called the interface.
 */
#define NORMALIZE_CURSOR_AND_ALENLIST_CHECK(List, Cursor, Check)		\
	if (List) {								\
		if (Cursor) {							\
			if ((Check) && (Cursor)->al_alenlist != (List)) {	\
				/* Mismatch between List and Cursor */		\
				return(ALENLIST_FAILURE);			\
			}							\
		} else {							\
			/* Use List's internal cursor */			\
			(Cursor) = &(List)->al_cursor;				\
		}								\
	} else {								\
		if (Cursor) {							\
			/* Derive List from Cursor */				\
			(List) = (Cursor)->al_alenlist;				\
		} else {							\
			/* No list specified */					\
			return(ALENLIST_FAILURE);				\
		}								\
...
/*
 * Used only for initialization when it's ok to have the cursor
 * and alenlist mismatch
 */
#define NORMALIZE_CURSOR_AND_ALENLIST_INIT(List, Cursor) \
	NORMALIZE_CURSOR_AND_ALENLIST_CHECK(List, Cursor, 0)
	
/* 
 * Generally we want to ensure that there is no mismatch between the 
 * cursor alenlist and the supplied one.
 */
#define NORMALIZE_CURSOR_AND_ALENLIST(List, Cursor) \
	NORMALIZE_CURSOR_AND_ALENLIST_CHECK(List, Cursor, 1)
			...
			unsigned flags)
{
	...
	NORMALIZE_CURSOR_AND_ALENLIST(alenlist, cursorp);
...
alenlist_cursor_init(alenlist_t alenlist, size_t offset, alenlist_cursor_t cursorp)
{
	...
	NORMALIZE_CURSOR_AND_ALENLIST_INIT(alenlist, cursorp);
...
{
	ASSERT_ALWAYS(cursorp_out); /* Must specify a destination */

	NORMALIZE_CURSOR_AND_ALENLIST(alenlist, cursorp_in);
...
/*
 * Return the Address/Length List associated with the specified cursor.
 */
alenlist_t
alenlist_cursor_to_alenlist(alenlist_cursor_t cursorp)
{
	if (cursorp)
		return(cursorp->al_alenlist);
	else
		return(NULL);
}
		...
		unsigned flags)
{
	...
	if (maxlength) {
		...
		/*
		 * If AL_UNALIGNED_OK bit is not set in flag and if
		 * we've requested a power-of-2 for maxlength, then force 
		 * addr + maxlength to land on the specified power-of-2
		 * boundary within the addr + maxlength address range.
		 */
		if (((maxlength & maxlen1) == 0) && /* power of 2 */
		    (!(flags & AL_UNALIGNED_OK))) {
				maxlength -= 
