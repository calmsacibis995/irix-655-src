...

{
	...
	for (targ = 0 ; targ < SCSI_MAXTARG ; targ++) {
		char		str[3];
		vertex_hdl_t	vhdl_3;
...

{
	...
	ASSERT(blk_vhdl != GRAPH_VERTEX_NONE);
	ASSERT(char_vhdl != GRAPH_VERTEX_NONE);	
...
scsi_disk_update(	vertex_hdl_t lun_vhdl, int type) 
{
	...
	dk_base_namespace_add(disk_vhdl, B_FALSE);
