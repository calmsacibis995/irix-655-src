...
#define VN_BHV_LOCK_MINE(bhp)		BHV_LOCK_MINE(bhp)
...
{       								\
	VN_BHV_READ_LOCK(&(cvp)->v_bh);       				\
        rv = _VOP_(vop_cover, cvp)((cvp)->v_fbhv, uap, attrs, cr); 	\
	VN_BHV_READ_UNLOCK(&(cvp)->v_bh);       				\
}
...
#define	VNODE_POSITION_OPLOCK	 (VNODE_POSITION_BASE+8)  /* oplock hooks      */
...
#define	VNODE_POSITION_CFS_DC	 (VNODE_POSITION_BASE+4)  /* CFS client	       */
#define	VNODE_POSITION_CFS_DS	 (VNODE_POSITION_BASE+3)  /* CFS server	       */
