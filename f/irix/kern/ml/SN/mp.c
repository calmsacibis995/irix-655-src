...
allowboot(void)
{
	...
	*/
	timeout = BOOTTIMEOUT * (num_cpus < 20 ? num_cpus : 20);
	cpucount = num_cpus - 1;
...
cont_nmi_dump(void)
{
	 ...
	 * Add a 4 sec delay so that the other cpus can save their
	 * register contents. (4 sec was determined empirically on a 256
	 * cpu system - 2.3sec was max observed)
	 */
	us_delay(4000000);
	nmi_eframes_save();
