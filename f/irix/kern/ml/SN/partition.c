...
#pragma mips_frequency_hint NEVER
		if (is_specified(arg_clusterid))
			p->p_clusterid = (uchar_t)atoi(arg_clusterid);
		else
 ...
 */
{
...
#ifdef	CELL_CAPABLE
	if (cell_enabled && is_specified(arg_clusterid))
    ...
    timeout_pri(part_heartbeat, 0, partition_heartbeat_ticks, hb_pri);
...
void
part_set_clusterid(clusterid_t id)
{
	partid_t pid;
#ifdef DEBUG
	CELL_NOT(panic("part_set_clusterid called by non-cell"));
#endif
	for (pid = 0; pid < MAX_PARTITIONS; pid++) {
		if (pIdle != partitions[pid].p_state) {
			partitions[pid].p_clusterid = id;
		}
	}
}
