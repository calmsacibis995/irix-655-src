...
static void queue_push(queue_t *q, router_elt_t *elt)
{
	...
	if (q->head == q->tail)
		db_log_printf((IP27LOG_INFO,
			       "WARNING: Router router queue limit %d "
			       " exceeded!\n",
			       MAX_RTR_BREADTH));
}
			     ...
			     vertex_hdl_t	routerv)
{
	...
	queue_t		router_vertex_q;
	queue_t		visited_router_vertex_q;
	nodepda_t	*npda;
	...
	queue_init(&router_vertex_q,Q_TYPE_HWG_RTR);
	/* Initialize the visited queue of router elements */
	queue_init(&visited_router_vertex_q,Q_TYPE_HWG_RTR);
	...
	queue_push(&router_vertex_q,&next_router);

	while (!queue_empty(&router_vertex_q)) {
		candidate_npda 		= NULL;
		/* Look at the first router in the bfs router queue */
		queue_pop(&router_vertex_q,&next_router);
		/* Check if we have already looked at the links of this 
		 * router
		 */
		visited_router = queue_find(&visited_router_vertex_q,
		...
		for(i = 0 ; i < MAX_ROUTER_PORTS ; i++) {
			...
			if ((cnodeid = nodevertex_to_cnodeid(port_vhdl))
				...
				if (rev_port = router_link_get(port_vhdl,next_router.u.k_elt.vhdl)) {
					...
					queue_push(&router_vertex_q,&new_router);
		 ...
		 * this router.
		 */
		queue_push(&visited_router_vertex_q,&next_router);
