...
kl_force_scecc(paddr_t paddr, md_error_params_t *uap, rval_t *rvp)
{
	...
	s = spl7();		/* no intrs ensure we dont lose cache line */
	/*
	 * set the second double word to 0. This is because the
	 * cache operations accept and return only one doubleword, while
	 * the ecc is computed for a quadword on the secondary cache.
	 */

	*(volatile __uint64_t *)(k0addr) = 0xdeadbeef;
	ecc_cacheop_get(CACH_SD + C_HWBINV, k0addr, &cop);

	*(volatile __uint64_t *)(k0addr);
	uncached();		/* run uncached so line is not replaced */
	...
	cmn_err(CE_WARN, "kl_force_scache_decc: force done");
	return 0;
}
