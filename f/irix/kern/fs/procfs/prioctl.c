...
#define CTX     ctx
	...
	default:
        ...
        case PIOCTHREAD:        /* issue ioctl on specified thread. */
        {
	        uthread_t	*utp;
                ...
                pioc_ctx_t      ctx2;
		prthreadctl_t   *ptp = (prthreadctl_t *) pcbuf;
                int             dir;
		int             set;
		...
		if (cmaddr == NULL) {
			...
			break;
				...
				irix5_to_prthreadctl, target_abi, 1)) {
			error = EFAULT;
			break;
		}
		...
		error = prioctl_prepare(&ctx2, pnp, ptp->pt_data, flag);
		if (error)
			break;
	...
	case PIOCWSTOP:		/* wait for process thread(s) to stop */
	{
		...
		if (error)
			goto fallout; /* prunlock already done */
	...
	{
		prrun_t *prrun = prioctl_prrun(&ctx, cmaddr);
                prsetrun_t prsr;
		int ss;
		proc_proxy_t	*px = &p->p_proxy;
		...
		if (error == 0 && (prrun->pr_flags & PRSTEP))
		        error = uthread_apply(px, UT_ID_NULL,
					      prchkrun_step, prrun);
		if (error == 0)
	...
	case PIOCSEXIT:		/* set syscall exit bit mask */
	{
		...
		if (cmd == PIOCSENTRY) {
			if (emptymask)
				emptymask =
					prisempty(&p->p_trmasks->pr_exitmask);
			prassignset(&p->p_trmasks->pr_entrymask, &prmask);
		} else {
			if (emptymask)
				emptymask =
					prisempty(&p->p_trmasks->pr_entrymask);
	...
	case PIOCRFORK:		/* reset inherit-on-fork flag */
		...
		error = prioctl_setflags(p, dbgflags);
                break;
		...
		error = prioctl_resetflags(p, dbgflags);
                break;
	...
	case PIOCACTION:	/* get signal action structures */
	{
		...
		for (sig = 1; sig < NSIG; sig++) {
			...
			if (priface_sigaction_copyout(
						(caddr_t)&sigact, cmaddr)) {
				error = EFAULT;
				break;
			}
			cmaddr += bufsize;
	...
	case PIOCENEVCTRS:
	{
		...
		enable_evctr_args = kmem_alloc(sizeof(hwperf_profevctrarg_t), KM_SLEEP);
	...
	case PIOCENEVCTRSEX:
	{
		...
		enable_evctr_args = kmem_alloc(sizeof(hwperf_profevctrarg_t), KM_SLEEP);
		enable_evctr_aux = kmem_alloc(sizeof(hwperf_profevctraux_t), KM_SLEEP);

		if (copyin(arg, (void *)&args_ex,
	...
	case PIOCSETEVCTRL:
	{
		...
		if (copyin(arg, evctr_args, sizeof(hwperf_profevctrarg_t))) {
			error = EFAULT;
			*rvalp = -1;
		}
		else {
	...
	case PIOCGETSN0REFCNTRS:
	{
		...
		for (i = 0; i < npgs; i++, vaddr += NBPREFCNTP, outbuf++) {
			...
			if (pfn) {
				temp_buf.paddr = ctob(pfn) + refcntpage_offset(vaddr);
                                temp_buf.cnodeid = PFN_TO_CNODEID(pfn);
                                temp_buf.page_size = pgsz;
				migr_refcnt_read(&temp_buf);
			} else {
				temp_buf.paddr = 0;
                                temp_buf.cnodeid = CNODEID_NONE;
                                temp_buf.page_size = 0;
				bzero(&temp_buf.refcnt_set, sizeof(sn0_refcnt_set_t));
			}	

			if (copyout(&temp_buf, outbuf, sizeof(sn0_refcnt_buf_t))) {
				error = EFAULT;
				break;
			}
		}
		...
		for (i = 0; i < npgs; i++, vaddr += NBPREFCNTP, outbuf++) {
			...
			if (pfn) {
				temp_buf.paddr = ctob(pfn) + refcntpage_offset(vaddr);
                                temp_buf.cnodeid = PFN_TO_CNODEID(pfn);
                                temp_buf.page_size = pgsz;
				migr_refcnt_read_extended(&temp_buf);
			} else {
				temp_buf.paddr = 0;
                                temp_buf.cnodeid = CNODEID_NONE;
                                temp_buf.page_size = 0;
