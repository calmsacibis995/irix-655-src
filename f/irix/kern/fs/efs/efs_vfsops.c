 ...
 */
static int
devvptoefs(struct vnode *devvp, struct vnode **clonevpp,
	   struct buf **bpp, struct efs **fsp, struct cred *cr)
{
	int error;
	int	retval;
	dev_t dev;
	...
	*clonevpp = devvp;
	VOP_OPEN(devvp, clonevpp, FREAD, cr, error);
	...
	if (devvp == *clonevpp) {
		VN_RELE(devvp);		/* no clone - release the hold */
	} else {
		devvp = *clonevpp;	/* device has cloned - reset devvp */
	}
...
efs_statdevvp(struct statvfs *sp, struct vnode *devvp)
{
	...
	int unused;
        struct buf *bp;
        struct efs *fs;
	struct vnode *clonevp;

	if (error = devvptoefs(devvp, &clonevp, &bp, &fs, get_current_cred()))
		return error;
	...
	brelse(bp);
	VOP_CLOSE(clonevp, FREAD, L_TRUE, get_current_cred(), unused);
        return error;
}
