...
rw3vp_nobds:
	...
	if (error == EINTR &&
	    rw == UIO_WRITE &&
	    ((ioflag & IO_SYNC) || (ioflag & IO_DSYNC))) {
		uio->uio_resid = resid;
	...
	stable_how *stab_comm, int frombio, struct buf *bp)
{
	...
	if (cp->c_flags == COMMIT_UNSTABLE && b_unstablecnt > (nbuf * 5 / 8)) {
		mutex_enter(&cp->c_mutex);
		if ((cp->c_flags & COMMIT_UNSTABLE) && cp->c_count > rtoblksz(rp) * (nbuf / 4))
...
nfs3_inactive(bhv_desc_t *bdp, cred_t *cr)
{
	 ...
	 */
	{
		...
		mi = RTOMI(rp);
		if ((mi->mi_flags & MIF_SILENT) && (vp != mi->mi_rootvp)) {
			error = rlock_nohang(rp);
			if (error) {
				return error;
			}
...
#pragma mips_frequency_hint NEVER
	...
	if (lbolt - cp->c_flushtime > COMMIT_FLUSH_INTERVAL)
		nfs3_background_commit(rp, cp);
