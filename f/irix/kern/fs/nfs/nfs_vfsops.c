...
nfs_init(struct vfssw *vswp, int fstype)
{
	...
	extern mutex_t chtablesem;
	extern sv_t chtablewait;
	extern mutex_t xchtablesem;
	extern sv_t xchtablewait;
	...
	sv_init(&chtablewait, SV_DEFAULT, "chtabwt");
	mutex_init(&xchtablesem, MUTEX_DEFAULT, "xchtbsem");
	sv_init(&xchtablewait, SV_DEFAULT, "xchtabwt");
...
retry:
	milock_mp(mi);
	for (rp = mi->mi_rnodes; rp != NULL; rp = rp->r_mnext)
		if (bcmp( fid->fid_data, rtofh(rp), fid->fid_len ) == 0 ) {
			vmap_t vmap;
			vp = rtov(rp);
			if (!vp) {
			...
			if (vp = vn_get(vp, &vmap, 0)) {
				/*
				 * We need to make sure that the file has not been removed
				 * from the server.
				 * Check the attributes by calling nfs_getattr with a NULL
				 * vattr pointer.  We don't need the attributes to be
				 * returned here.
				 * This makes nfs_vget more like nfs_lookup.  nfs_lookup
				 * validates the attributes of the directory prior to going
				 * to the dnlc.
				 */
				error = nfs_getattr(rtobhv(rp), (vattr_t *)NULL, 0, sys_cred);
				if (error) {
					VN_RELE(vp);
					vp = NULL;
				}
	...
	miunlock_mp(mi);
	/*
	 * If we get here, we either could not locate the rnode or we found the
	 * rnode but were unable to get the vnode.  Just make a new rnode since
	 * we know that the fid is really the file handle.
