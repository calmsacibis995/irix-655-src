	...
	int flags)
{
	...
	*(ti->ti_txconsptr) = 0;
	...
	ti->ti_geninfo->recv_jumbo_rcb.RCB_max_len = ti->ti_if.if_mtu + 18;
 ...
 */
static int
eg_fill_buffs (struct eg_info *ti, struct mbuf **pool, struct recv_bd *ring, int prod, int n, int mod)
{
	...
	for (i = 0; i < n; i++) {
	        if ((m = m_vget(M_DONTWAIT, sizeof (struct egrxbuf),
				MT_DATA)) == NULL) {
		...
		ring[prod].BD_len = EGRDATASZ + ETHER_HDRLEN;
		 ...
		 */
		__vm_dcache_inval(mtod(m, caddr_t), sizeof(struct egrxbuf));
#else
		/*
		...
		CACHE_WB(mtod(m, caddr_t), sizeof (struct egrxbuf));
#endif
...
eg_fill_std(struct eg_info *ti)
{
	...
	ti->ti_rxstd_prod = eg_fill_buffs (ti, ti->ti_rxm_std, ti->ti_rxring_std, prod, n, RECV_STANDARD_RING_ENTRIES);
...
eg_fill_jmb(struct eg_info *ti)
{
	...
	ti->ti_rxjumbo_prod = eg_fill_buffs (ti, ti->ti_rxm_jumbo, ti->ti_rxring_jumbo, prod, n, RECV_JUMBO_RING_ENTRIES);
...
eg_recv(struct eg_info *ti, int rxrtn_prod, struct ifqueue *qp)
{
	...
	for (i = cons; i != rxrtn_prod; i = INCRING(i, RECV_RETURN_RING_ENTRIES)){
		 ...
		 */
		xdki_dcache_validate(mtod(m, void *), sizeof(struct egrxbuf));
#endif
		GOODMP(m);
		GOODMT(m->m_type);
...
eg_dumpti(struct eg_info *ti)
{
	...
	qprintf("evprodptr 0x%x\n",ti->ti_evprodptr);
        ...
        qprintf("rxring_std 0x%x rxring_rtn 0x%x rxstd_prod %u rxstd_cons %u rxm_std 0x%x\n",
                ti->ti_rxring_std, ti->ti_rxring_rtn,
		ti->ti_rxstd_prod, ti->ti_rxstd_cons, ti->ti_rxm_std);
