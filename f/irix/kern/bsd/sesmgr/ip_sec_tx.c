	...
	mac_label *     dlbl)
{
	...
	doi = ifattrs->ifs_doi;
	switch ( idiom = (int)ifattrs->ifs_idiom ) {
	...
	case IDIOM_BSO_RX:
		if ( dlbl->ml_msen_type == MSEN_TCSEC_LABEL ) {
			m = ip_xmt_bso( dlbl, ifattrs->ifs_authority_max );
		}
		break;
	...
	int *                error)
{
	...
	idiom = (int)ifattrs->ifs_idiom;
	if (! dacful(idiom) ) 		
		duid = -2;  	/* "nobody" for sat_bsdipc_range */

	if ( ifattrs->ifs_label_max == 0 
	||   ! mac_inrange( ifattrs->ifs_label_min, dlbl, ifattrs->ifs_label_max ) ) {
	...
	if ((idiom = (int)ifattrs->ifs_idiom) == IDIOM_BSO_RX 
	&&  mac_equ( dlbl, ifattrs->ifs_label_max ) ) 
