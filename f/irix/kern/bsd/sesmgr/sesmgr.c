...
restart:
	...
	while (m && uio->uio_resid > 0 && error == 0) {
		...
		if (!do_oob && samp_count == 0) {
			...
			/*
			 * First we copy the minimum header size.  Once
			...
			if (samp_hdr_offset >= T6SAMP_MIN_HEADER) {
				...
				if (samp_hdr_offset == samp_len) {
					...
					if (error = samp_get_attrs(so, src, &samp_hdr)) {
						if (pr->pr_flags & PR_ATOMIC) {
							if (sblock(&so->so_rcv, NETEVENT_SOUP, so, 1))
								return error;
							soattrs->sm_samp_cnt = 0;
							sbdroprecord(&so->so_rcv);
							goto restart;
	...
	if ((flags & MSG_PEEK) == 0) {
		...
		zwindow = (!(pr->pr_flags & PR_ATOMIC) && error == 0 && orig_resid == uio->uio_resid);
...
/* ARGSUSED */
int
irix5_to_t6ifreq (enum xlate_mode a, void * b, int c, xlate_info_t * d)
{
	return 0;
}
