...
/*ARGSUSED*/
int
sesmgr_socket_copy_attrs(struct socket *so_src, struct socket *so_dst,
			 struct ipsec *ipsec)
{
...
#ifdef DEBUG
	sesmgr_socket_check_attrs(so_src);
#endif
	...
	dst_attrs->sm_ip_flags &= ~IPSOPT_IPOUT;
	dst_attrs->sm_soacl = NULL;

	/* Note:  the only time that ipsec should be null is
	 * when we are connection over uds. 
	 */
	if (ipsec != NULL) {
		dst_attrs->sm_rcvuid = ipsec->sm_rcvuid;
		dst_attrs->sm_ip_lbl = ipsec->sm_ip_lbl;
		dst_attrs->sm_ip_uid = ipsec->sm_ip_uid;
		dst_attrs->sm_ip_ifp = ipsec->sm_ip_ifp;
	}
...
#ifdef DEBUG
	sesmgr_socket_check_attrs(so_dst);
#endif
	return 0;
