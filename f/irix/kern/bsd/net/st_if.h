...
/* $Revision: 1.1 $    $Date: 1999/06/18 23:36:33 $ */
 ...
 * ULP Port Flags
 */
#define ST_DIRECT_PORT  (1<<0)
#define ST_IOCTL_PORT	(1<<1)
#define ST_APPEND_PORT	(1<<2)
...
typedef struct st_io_s {
	struct inaddrpair iap;
        ...
        sthdr_t           sth;
	int		  num_falias;
	struct in_addr	  falias[STP_MAX_ALIAS];
} st_io_t;
...
#define MAX_SPRAY 16
	...
	int32_t mx_base;	/* Smallest Mx supported by the NIC */
	int32_t mx_num;		/* Number of Mx's supported by the NIC */
	lock_t  mx_lock;	/* Mutex for mx_table */
	void   *mx_table;	/* Bit vector of Mx's currently in use */
...
#define MAX_SLOT_VC 4
	...
	int32_t	(*if_st_set_port)(struct ifnet *, int32_t *,st_port_t *, uint32_t);
	...
	int32_t (*if_st_get_key)(struct ifnet *, int32_t, uint32_t *);
} st_ifnet_t;
...
#undef ST_DEAD_CODE
#ifdef ST_DEAD_CODE /* this appears to be dead code, but keep it for now */
