...
char		       *opt_title	= "Origin2000 System Activity";
...
void usage(void)
{
    ...
    perr("          [-t \"title\"] [-N cpus] [-D fctr] [-gdmnG]\n");
    perr("Options:\n");
...
main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "D:GN:b:d:c:f:gmnt:u:")) >= 0)
	switch (c) {
	case 'D':
	...
	if (m && graph_started) {
	    ...
	    if ((p = load_graph(opt_gfx,
				opt_decay,
				period)) == 0) {
		static int load_graph_warn;

		if (load_graph_warn == 0) {
		    warning("Could not obtain load graph\n");
		    load_graph_warn = 1;
		}
	    } else if (cmd_graph_data(m, p,
				      ncpus * bardim,
				      ncpus) < 0)
		drop();
	}
