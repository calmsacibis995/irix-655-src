...
halt_counting(void) {
  int re;
    ...
    if(!(counts_enabled)) {
      ...
      {
        long long c0=0, c1=0;
        gen_stop = PFX_read_counters_pid(EventType0,&c0,EventType1,&c1,pidc,1);
...
Forkit(int remap_stdout,int argc, char **argv) 
{
    ...
    if (prog_args == NULL) {
      fprintf(output_stream,"Malloc of prog_args failed: %s", strerror(errno));
      exit(1);
    }
    ...
    *arg_ptr++ = NULL;
    ...
    /* fork child */
    if (((int)(pidc = fork())) < 0) {
      fprintf(output_stream,"failed to fork child: %s", strerror(errno));
      exit(1);
    }
	...
	if ((fd = open(program_input, O_RDONLY)) < 0) {
	  fprintf(output_stream,"Unable to open %s", program_input);
	  exit(1);
	}
      }
	...
	if ((fd = creat(program_output, 0666)) < 0) {
	  fprintf(output_stream,"unable to create %s", program_output);
	  exit(1);
	}
...
main(int argc,char *argv[])
{
 ...
 fprintf(output_stream,"WARNING: Multiplexing events to project totals--inaccuracy possible.\n");
	    ...
	    {
	      if (argc == 1) {
		fprintf(output_stream,"option '%s' needs an argument.",argv[0]);
		exit(1);
	      } else {
		events = atoi(argv[1]);
		if(events < HWPERF_CNT1BASE) {
...
/* set up signal handlers */
  ...
  if(counts_enabled) {
    if(using_signals) {
      gen_stop = PFX_read_counters_pid( EventType0, &mcount0, EventType1, &mcount1, pid, 0);
    } else {
      gen_stop = PFX_read_counters_pid( EventType0, &mcount0, EventType1, &mcount1, pid, 1);
#ifdef _BDB
      { int i;
	for(i=0;i<31;i++) {
