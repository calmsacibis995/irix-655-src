...
/* 
 * make sure backward compatible (char to long change); we may need to 
 * expand this when we start to support more 255 cpus
 */
...
display_item(inventory_t *pinvent, void *ptr)
{
	...
	case INV_DISK:
		...
		case INV_SCSIDRIVE:
			...
			if (pinvent->inv_state & INV_XVMVOL)
				printf (" (XVM disk)");
