#ident  "$Header: /proj/irix6.5f/isms/irix/cmd/icrash/RCS/struct.c,v 1.1 1999/05/25 19:52:33 tjm Exp $"
...
print_kthread(kaddr_t kthread, k_ptr_t ktp, int flags, FILE *ofp)
{
	...
	fprintf(ofp, "%16llx %4llu  %16llx  ", 
		kthread, 
		KL_UINT(ktp, "kthread", "k_type"),
		KL_UINT(ktp, "kthread", "k_id"));

	if (kl_kaddr(ktp, "kthread", "k_wchan")) {
		if (PTRSZ64) {
			fprintf(ofp, "%16llx ", kl_kaddr(ktp, "kthread", "k_wchan"));
		} 
		else {
			fprintf(ofp, "%8llx ", kl_kaddr(ktp, "kthread", "k_wchan"));
		}
	} 
	else {
		if (PTRSZ64) {
	...
	if (flags & C_NEXT) {
		if (IS_STHREAD(ktp)) {
			...
			kl_get_struct(kthread, STHREAD_S_SIZE, stp, "sthread_s");
			fprintf(ofp, "\n");
			indent_it(flags, ofp);
			sthread_s_banner(ofp, BANNER|SMINOR);
			indent_it(flags, ofp);
			print_sthread_s(kthread, stp, flags, ofp);
			kl_free_block(stp);
		}
			...
			kl_get_struct(kthread, UTHREAD_S_SIZE, utp, "uthread_s");
			fprintf(ofp, "\n");
			indent_it(flags, ofp);
			...
			xtp = kl_alloc_block(XTHREAD_S_SIZE, K_TEMP);
			kl_get_struct(kthread, XTHREAD_S_SIZE, xtp, "xthread_s");
			fprintf(ofp, "\n");
			indent_it(flags, ofp);
			xthread_s_banner(ofp, BANNER|SMINOR);
			indent_it(flags, ofp);
			print_xthread_s(kthread, xtp, flags, ofp);
			kl_free_block(xtp);
		}
		fprintf(ofp, "\n");
	}
