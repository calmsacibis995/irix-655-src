#ident  "$Header: /proj/irix6.5f/isms/irix/cmd/icrash/cmds/RCS/cmd_search.c,v 1.17 1999/05/25 19:52:33 tjm Exp $"
	...
	kaddr_t start_addr, k_uint_t length, int flags, FILE *ofp)
{
	...
	buf = (unsigned char *)alloc_block((IO_NBPC + MAX_SEARCH_SIZE), K_TEMP);
	...
	while (1) {
		...
		while (i < s) {
			 ...
			 * outta' here.
			 */
			if (((vaddr_mode == 0) && (addr > (K_K0BASE + K_K0SIZE))) ||
					((vaddr_mode == 1) && (addr > (K_K1BASE + K_K1SIZE)))) {
				addr = K_K0BASE;
			...
			if (pattern_match(pattern, mask, &buf[i], nbytes)) {
				...
				if (is_kseg2(j + i) || is_kseg1(j + i)) {
					fprintf(ofp, "0x%llx\n", j + i);
				} 
				else if (IS_SMALLMEM(j + i)) {
					if (IS_PHYSICAL(j + i)) {
						fprintf(ofp, "0x%llx\n", 
							(j + i) | K_K0BASE);
					} 
...
search_cmd(command_t cmd)
{
	...
	if (IS_PHYSICAL(start_addr)) {
		...
		if (length <= 0 || length >= ((K_SYSSEGSZ * K_PAGESZ) - 
				   (start_addr - K_K2BASE))) {
			length = (K_SYSSEGSZ * K_PAGESZ) - 
				(start_addr - K_K2BASE);
		}
	}
