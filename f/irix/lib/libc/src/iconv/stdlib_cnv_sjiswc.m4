...
define(`GET_VALUE',`
	...
	if ('RETURN_VAR` <= 0x7f) { /* ASCII */
		...
		goto cont$1;
	} if ('RETURN_VAR` >= 0xa0 && 'RETURN_VAR` <= 0xdf) {
		...
		c2 = (unsigned int) 'RETURN_VAR`;
	        ...
	        /* Check for illegal SJIS codepoint */
		if ( c1 > 0xFC || c2 == 0x7F || c2 < 0x40 || c2 > 0xFC )
		    goto illegal_recovery;

		if ( 0xF0 <= c1 && c1 <= 0xFC && 0x40 <= c2 && c2 <= 0xFC ) {
	            /* User-defined SJIS */
		    /* Bring it down to a SJIS value that will yield a valid JIS code */
		    c1 -= 0x6F;
		    state$1 = P10;
		} else
	            /* JIS 208 */
