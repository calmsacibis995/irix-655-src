...
/* ======== __mbwc_get_mbhandle  ========================================== */
/* PURPOSE:
 *	Get an iconv_mbhandle structure containing pointers to the mbwc
 *	functions, from a locale or encoding name.
 *	If mbhandle is not NULL, it will set the it to the passed handle.
 * RETURNS:
 * 	The opened iconv_mbhandle.
 */

_iconv_mbhandle_t
__mbwc_get_mbhandle (
    const char		* loc,	/* Source locale name or encoding */
    _iconv_mbhandle_t	mbhandle
) {
    void	    * p_sp;
    char	    * wcenc = "irix-wc";
    ...
    if ( magic_resvals ) {
         ...
         if ( *magic_resvals != ICONV_MAGIC_RESVALS ) return 0;
    ...
    if ( mbhandle )
	__mbwc_close( mbhandle );

    return __mbwc_open( p_sp, s_sk, mbhandle );

} /* end __mbwc_get_mbhandle */
...
int	__mbwc_setmbstate(
    ...
    if ( __mbwc_get_mbhandle ( loc, __libc_mbhandle ) )
	return 0;
    else
	return -1;
