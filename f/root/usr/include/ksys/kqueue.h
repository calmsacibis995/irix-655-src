...
#ident "$Id: kqueue.h,v 1.8 1999/05/14 20:13:13 lord Exp $"

#ifndef _MASTER_C
/* Do not include stddef.h from master.c it is not guaranteed to
 * be there.
 */
#include <stddef.h>
#endif
...
/*
 * Removes first element from kqueue. If the queue is empty it will
 * return 0 else returns 1. Wrote this function so that the 
 * we don't have to export the internals of mpkqueuehead_t
 */
static __inline int
mpkqueue_remove_first(mpkqueuehead_t *queue, kqueue_t **element)
{
	int s = mpkqueue_lock(queue);
	if (kqueue_isempty(&queue->head)) {
		mpkqueue_unlock(queue, s);
		return 0;
	}
	*element = kqueue_first(&queue->head);
	kqueue_remove(*element); 
	mpkqueue_unlock(queue, s);
	return 1;
}
