...
static __inline int
set_member_max(cell_set_t *set)
{
	cell_t	index;

	for (index = CELL_SET_SIZE - 1; index >= 0; index--)
		if (set_is_member(set, index))
			return index;
	return -1;
}
...
static __inline void
set_add_member_atomic(cell_set_t *set, cell_t cell)
{
	atomicSetUlong(set, (cell_set_t)1 << cell);
}

static __inline void
set_del_member_atomic(cell_set_t *set, cell_t cell)
{
	atomicClearUlong(set, (cell_set_t)1 << cell);
}

static __inline void
set_union_atomic(cell_set_t *set0, cell_set_t *set1)
{
	atomicSetUlong(set0, *set1);
}

#define	FORCELLSET(cell, set)				\
	for (cell = 0; cell < CELL_SET_SIZE; cell++)	\
		if (set_is_member(set, cell))
