...
typedef struct k_bla {
	...
	uchar_t		kb_deferred_barrier; /* deferred barrier pending */
...
static __inline void kbla_never(void) {}
#pragma mips_frequency_hint NEVER kbla_never
...
#define KBLA_SAVE(kt) 						\
	if (cell_enabled) {					\
	    kbla_never();					\
	    if (private.p_blaptr) { 				\
		(kt)->k_blap->kb_lockpp = private.p_blaptr; 	\
		KBLA_VERIFY(kt);				\
	    }							\
	}
...
#define KBLA_RESUME(kt) 					\
	if (cell_enabled) {					\
	    kbla_never();					\
		private.p_blaptr = (kt)->k_blap->kb_lockpp;	\
		KBLA_VERIFY(kt);				\
	}
...
#define KBLA_EMPTY(kt)                                          \
	if (cell_enabled) {					\
	    kbla_never();					\
		private.p_blaptr = (kt)->k_blap->kb_lockp	\
	}
#else
#define KBLA_EMPTY(kt)						\
        ASSERT(!cell_enabled || (private.p_blaptr == (kt)->k_blap->kb_lockp));
#endif
#else  /* CELL_CAPABLE */
#define KBLA_SAVE(kt)
#define KBLA_CSAVE(kt)
...
#ifdef CELL_CAPABLE
	k_bla_t		*k_blap;	/* BLA ptr for remembering behavior locks held*/
	service_t	k_remote_svc;	/* Where this thread has gone */
	void		*k_mesg_stack;	/* Stack of above if callbacks occur */
	cell_set_t	k_route;	/* Callback cell routing */
	void		*k_route_stack;	/* Stack of above, in case of nesting */
	struct callback_stash * k_callback_stash; /* used by callback functions. */
#elif CELL_PREPARE
