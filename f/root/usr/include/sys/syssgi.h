...
/* SGI_CELL suboptions - ADD TO THE END ONLY!!! - DON'T CHANGE ORDER */
enum {
	SGI_IS_OS_CELLULAR,	/* OS cellular or not */
	SGI_CELL_PID_TO_CELLID,	/* cell a proc is running on */
	SGI_CELL_OBJ_EVICT,	/* evict an object */
	SGI_ADMIN_SHUTDOWN,	/* shutdown services and leave membership */	
	SGI_SEND_TEST_MESG,	/* Send a test message to a given cell*/
	SGI_FAIL_CELL,		/* Cell commits suicdie */	
	SGI_MEMBERSHIP_STATS_X,	/* Defunct - cell membership statistics */
	SGI_MESG_STATS_X,	/* Defunct - inter cell message statistice */
	SGI_CELL_MEMBERSHIP_X, 	/* Defunct - current cell membership */
	SGI_CREATE_WP_TEST,     /* wp test hook */
	SGI_START_MEMBERSHIP_DAEMON,	/* Start TCP transport */
	SGI_TEST_TCP_MESSAGES,	/* Test out of band messages */
	SGI_MEASURE_MESG_PERF,  /* Measure message performance */
	SGI_TEST_PAGE_LENDING,	/* test page lending/borrowing */
	SGI_TEST_PAGE_LENDING_RETURN,	/* test page lending/borrowing */
	SGI_TEST_MESG_ROUTE,	/* test mesg routing/recovery */
	SGI_TEST_MESG_CALLBACK,	/* test mesg callback/recovery */
	SGI_SIMULATE_FAILURE,	/* simulate a cell failure */
	SGI_INJECT_SIMFAIL,	/* inject simulated failure during recovery */
	SGI_INJECT_LDR_FAILURE,	/* inject leader failure into recovery */
	SGI_LDR_CONFIG,		/* configure leader cell */
	SGI_INJECT_FAILURE,	/* inject real failure during recovery */
	SGI_START_TCP_TRANSPORT,/* obsolete, can be reused */
	SGI_SET_MEMBERSHIP_CONFIG, /* Set membership configuraton information */
	SGI_START_TCP_HEART_BEAT, /* Stop and start TCP heart beat */
	SGI_KORE_TRIP,		/* set failure point for relocation recovery */
	SGI_SET_CLUSTER_CONFIG,	/* set cellular cluster config */
	SGI_TEST_MESG_REPLY_ID_CANCEL,
	SGI_TEST_MESG_FREEZE,	/* freeze channel to a cell */
	SGI_TEST_MESG_THAW_INBOUND,	/* thaw channel for inbound mesg'ing */
	SGI_CMS_CONFIG_ID,	/* set configuration id */
	SGI_CMS_INCLUDE_CELL,	/* include a cell in membership */
	SGI_CMS_EXCLUDE_CELL,	/* exclude a cell from membership */
	SGI_CMS_HOST_ID,	/* set host id for an interface */
	SGI_CELL_END
};
