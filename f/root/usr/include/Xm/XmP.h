 ...
 * Motif Release 1.2.4
*/ 
/*   $RCSfile: XmP.h,v $ $Revision: 0.13 $ $Date: 1995/06/28 17:08:05 $ */
/*
...
#  include <Xm/SgiP.h>		/* for SGI widget code */
#endif /* __sgi */

#ifdef __sgi /* SGI upgrade for 64-bits */
#ifdef XM_LIBRARY_BUILD /* SGI upgrade for 64-bits */

/*
 * strlen() -- Xm strings are never more than 32 bits, so this definition
 *	does not hurt.  It does prevent "implicit conversion from "unsigned
 *	long" to ..." warnings.
 *
 * sizeof() -- Ditto for "sizeof".
 *
 * XtMalloc() -- We have no need to XtMalloc more than 32 bits worth.
 *
 * Doing this sort of thing is a really ugly kludge to get rid of a lot
 * of compiler warnings in just this one place.  The real problem is that
 * XtMalloc & friends are busted -- they should take size_t sizes, justg as
 * underlying routines they wrap do.  Once that happens, these defines
 * should be removed.  Also those in the Mrm and uil directories.
 */
#  define strlen(s) ((unsigned int)strlen(s))
#  define sizeof(s) ((unsigned int)sizeof(s))
#  define XtMalloc(s) (XtMalloc((Cardinal)(s)))
#  define XtCalloc(n,s) (XtCalloc((n),(Cardinal)(s)))
#  define XtRealloc(p,n) (XtRealloc((p),(Cardinal)(n)))

/*
 * Motif has a number of implicit conversions that risk losing data.
 * Probably the most troublesome are long-to-int type conversions, because
 * in a 64 bit (but not a 32-bit) environment the upper 32 bits are thrown
 * away.  This creates a bug that will work OK on a little-endian machine,
 * but fail on a MIPS machine.  We have really found such a bug in libMrm,
 * and there is other code where it would be very difficult to check by
 * hand whether that could ever occur.  Therefore, we make a ASSERT to
 * catch such stuff.
 *
 * This assertion will pass all good data.  It will fail to catch some bad
 * data.  Probably close enough for catching porting mistakes, though.
 */
#  if (_MIPS_SZLONG == 64)
#    include <assert.h>
#    define SGI_ASSERT_NO_TRUNC(val) assert( (((val) & 0xFFFFFFFF00000000) == 0) || (((val) & 0xFFFFFFFF00000000) == 0xFFFFFFFF00000000) )
#  else
#    define SGI_ASSERT_NO_TRUNC(val)
#  endif /* _MIPS_SZLONG */

#endif /* XM_LIBRARY_BUILD */
#endif /* __sgi */
...
/*#define SGImode 0 lets debug the switch */
       extern int darkScheme;
       extern int SGImode;
       extern int SGImodeInit;
...
#ifdef __sgi
/*
 * It was a blunder that these are Xm* instead of Sg*.
 * Probably should fix this one of these days.
 */
  typedef void (*XmRenderShadowsProc)();
  typedef void (*XmRenderIndicatorProc)();
#endif /* __sgi */
 ...
 * Probably should fix this one of these days.
 */
  typedef void (*XmRenderShadowsProc)(Widget,Display*,Drawable,
     shaderptr,Position,Position,Dimension,Dimension,
     int*,int,unsigned int);
  typedef void (*XmRenderIndicatorProc)(Widget,Display*,Drawable,
     shaderptr,Position,Position,Dimension,Dimension,
     int*);
#endif /* __sgi */
...
enum{	XmGEO_EXPAND,			XmGEO_CENTER,
	XmGEO_PACK
#ifdef __sgi /* add new spacing constant for right-justified buttons */
        , SgGEO_PACK_RIGHT
#endif /* __sgi */
	} ;
 ...
 * Probably should fix this one of these days.
 */
#  define XmInheritShadowRenderer ((XmRenderShadowsProc)_XtInherit)
#  define XmInheritIndicatorProc ((XmRenderIndicatorProc)_XtInherit)
#  define XmInheritRenderer ((XtWidgetProc) _XtInherit)
#endif /* __sgi */
...
# define XmGetPartOffset(r, offset) \
	...
	(*(offset))[(r)->resource_offset >> XmOFFSETBITS];
       ...
       /*
        * The macro should not end with a semi-colon,
	* and it should be in parentheses.
	*/
# define XmGetPartOffset(r, offset) \
       ( ((r)->resource_offset & XmOFFSETMASK) + \
	(*(offset))[(r)->resource_offset >> XmOFFSETBITS] )
...
/* let subclasses inherit state array */

#define XmInheritStateArray (visual_state_array)NULL
#endif /* __sgi */
