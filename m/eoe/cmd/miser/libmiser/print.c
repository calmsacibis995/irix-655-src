/*
 * eoe/cmd/miser/libmiser/print.c
 *      Library functions that print miser data.
 */
...
miser_print_job_desc(bid_t jid)
{
	...
	if (miser_get_prpsinfo(jid, &pinfo) != 0) {
		...
		if (pwptr != NULL)
			/* Print username and process command name */
                	printf("%-8s %-15s", pwptr->pw_name, pinfo.pr_fname);
        }
...
miser_print_job_sched(miser_seg_t *seg)
{
	...
	printf("%3d %5.0b %4d:%.2d.%.2d %17s %17s %3d %3d %1s%1s\n",
		seg->ms_resources.mr_cpus,
		(float) seg->ms_resources.mr_memory,
		seg->ms_rtime / 3600,
 ...
 */
void
miser_print_block(int count, miser_resources_t *first, miser_resources_t *last,
                        int start)
{
        ...
        while (first != last) {
                ...
                strftime(buf, 1024, "%D %T", tmptr);

                if (current->mr_cpus == first->mr_cpus &&
                        current->mr_memory == first->mr_memory && !previous) {
                ...
                if (!previous || first->mr_cpus != 0 && first->mr_memory != 0)
                        printf(" %s %5d %7.0b\n", buf, first->mr_cpus,
