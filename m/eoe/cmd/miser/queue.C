...
miser_queue::space_reset(int interval)
{
	...
	convert_to_localtime(i, i + duration,
				tspace.begin(), tspace.end());
...
system_queue_updater(quanta_t length, quanta_t offset)
	: tmp(length),
		begin(system_pool->space_available.begin()
...
qitt(miser_queue& q, cyclic_iterator<space> i)
{
	...
	return q.last_accessed_time
		...
		+ (access_offset > iterator_offset ? 2 * duration : 0);
...
long
queue_find_block(miser_queue* q, quanta_t start, quanta_t end, miser_seg_t& seg)
{
	...
	cyclic_iterator<space> block = find_seq(qtti(*q, start), qtti(*q, end),
		bind2nd(dominates(), seg.ms_resources), length);
			...
			miser_qseg_t* erb )
{
	if ((void*)erb < start_request || (void*)erb > end_request)
        ...
        if (!from_queue) return G_error;
        
        miser_queue* to_queue = find_queue(to, W_OK);

        if (!to_queue) return G_error;

        if (from_queue == to_queue) return MISER_ERR_QUEUE_ID_BAD;
		...
		space::iterator sbegin, space::iterator send)
{
	...
	if (endtm.tm_isdst < 0 || starttm.tm_isdst < 0) {
		merror("Information on daylight savings is not available,"
			"therefore, only using timezone difference.");
		return 0;
	}
...
queue_create(queue_definition* spec, queuelist *newlist)
{
	...
	PRINT_VERBOSE(("      system: %s", miser_qstr(system_id)));

	if (spec->queue == system_id) {
		PRINT_VERBOSE(("Create system queue"));
	}
...
_qdef_alloc(miser_qhdr_t *qhdr)
{
	...
	if (time_quantum == 0) {
		...
		if (time_quantum <= 0) {
			merror("invalid time quantum");
			exit(1);
		}

		if (sysmp(MP_MISER_SETRESOURCE, MPTS_MISER_QUANTUM,
							&time_quantum)) {
			merror("failed to set quantum, %s", strerror(errno));
			exit(1);
		}
	}
...
_qdef_callb(void *qdef)
{
	...
	if (error != MISER_ERR_OK) {
		merror("miser: Failed to create queue \"%s\": %s", 
	...
	for (i = 0; i < mj->mj_count; i++) {
                if (queue_find_block(q,get_start_time(mj->mj_segments[i]),
                        mj->mj_segments[i].ms_etime,
                                mj->mj_segments[i]) == -1) {
                        merror("Could not schedule job %d.", mj->mj_bid);
                        exit(1);
                }
...
reschedule_job(bid_t& bid)
{
	...
	STRING_PRINT("[reschedule_job] Start");
	...
	if (sysmp(MP_MISER_GETRESOURCE, MPTS_MISER_JOB, &mgr) == -1) {
		merror("Could not get job %d: %s", bid, strerror(errno));
                exit(1);
        }
	...
	for_each(mj->mj_segments, mj->mj_segments + mj->mj_count,
        ...
        if (!q) {
                merror("Could not reschedule job %d "
...
repack_reschedule_job(jobschedule::iterator& job)
{
	...
	STRING_PRINT("[repack_reschedule_job] Start");
...
error_t
miser_reset(char* fname, queuelist*& new_queues)
{
