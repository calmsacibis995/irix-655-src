/*
 * eoe/cmd/miser/job.C
 *	Miser job management functions.
 */
...
/*
 * job_deschedule
 * 	Called when a job terminates. It will call the jobs policy if it 
 * terminates before its previous end time setting. Thus taking advantage 
 * of the repack policy's ability to reschedule jobs in the queue
 */
error_t
job_deschedule(bid_t bid)
{
	...
	jobschedule::iterator j
		=  find(job_schedule_queue.begin(), job_schedule_queue.end(),
			bid);
	     ...
	     miser_seg_t* eseg, miser_time_t *end_time, int ca)
{
        ...
        if (!q || !q->policy) return G_error;

        STRING_PRINT("[job_schedule] Found the Queue");

	if (ca && ((void *)eseg < start_request || (void *)eseg > end_request))
