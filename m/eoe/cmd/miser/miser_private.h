/*
 * eoe/cmd/miser/miser_private.h
 *	Miser private function prototypes.
 */
...
#ifndef __MISER_PRIVATE_HEADER__
#define __MISER_PRIVATE_HEADER__
...
#define PRINT_VERBOSE(a) \
		if (G_verbose && !G_syslog) \
			merror a
struct miser_queue;
void queue_update(miser_queue* queue);


struct miser_space : miser_resources_t {
        miser_space() {
		mr_memory = 0;
		mr_cpus = 0;
	}
...
struct miser_queue {
	...
	void init()
	{
		...
		queue_update(this);
		acl = 0;
		policy = definition ? find_policy_manager(definition->policy)
				    : 0;
	}
...
error_t		job_schedule(id_type_t, bid_t, miser_seg_t*,
				miser_seg_t*, quanta_t*, int ca = 1);
...
error_t		miser_reset(char* filename, queuelist*&);
