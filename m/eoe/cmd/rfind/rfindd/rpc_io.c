...
extern hadsigpipe;		/* if sigpipe sets, must be client vanished */
...
void rpcsetbuf() {
	...
	stdoutbuf.io_rfh.bufp = stdout->_base;
	...
	/* adjust for _findbuf.c: iop->_base = buf + PUSHBACK; */
	stderrbuf.io_rfh.bufp = stderr->_base;
...
void rpcflush (struct io *bp) {
	...
	if (len > RPCBUFSIZ) {
		if (bp == &stderrbuf)
			...
			error ("internal buffer overflow botch", "");
	}
	bp->io_rfh.sz = len;
