/*
 * char *getfenv (inod_t *p, fenv_t ft) 
 *
...
void putfenv (inod_t *p, fenv_t ft, char *val) {
	...
	size_t flen;				/* length of entire fenv */
	char *oldval;				/* previous value of "ft" */
	size_t oldvlen, newvlen;		/* length of old and new values of "ft" */
	size_t oldhlen, newhlen;		/* length old/new "ft" header+nul */
	int growth;				/* how many more bytes needed in fenv */
