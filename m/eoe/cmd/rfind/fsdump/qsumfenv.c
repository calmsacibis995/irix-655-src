...
int fchanged (inod_t *, char *, char *);
char *quicksum (char *);
int havethetime();		/* non-zero if time remains in allowed quantum */
 ...
 * the next by stashing it in the fenv of inode 2, as the
 * value FV_QFLAG.  This overloading is safe because inode 2
 ...
 * state that was stashed in inode 2, and if we managed
...
extern void initqsumflag();		/* set qsumflag from FV_QFLAG in inode 2 */
extern void dumpsumflag();		/* if (havethetime()) toggle ino 2's FV_QFLAG */
...
void qsumfenvtreewalk (ino_t dirino) {
	inod_t *p, *q;
	dent_t *ep;
	index sz;
	int i;
...
void initqsumflag(){
	char *flagp = getfenv (PINO(&mh,2), FV_QFLAG);
