...
mcast_out:
		...
		if (ltaddr == NULL || lbaddr == NULL) {
			syslog(LOG_ERR, "%s: cannot allocate local netbuf: %s",
			       nconf->nc_netid, t_errlist[t_errno]);
			exit(1);
		}
		...
		memcpy(ltaddr->addr.buf, nb->buf, ltaddr->addr.len = nb->len);
		free(nb);
		...
		l_xprt = (SVCXPRT *)svc_tli_create(lfd, nconf, lbaddr,0,0);
		if (l_xprt == 0) {
			syslog(LOG_ERR, "%s: could not create"
...
#else
{
	...
	opterr = 0;
	while ((t = getopt(argc, argv, "a:Abdf:mvw")) != EOF) {
		switch (t) {
		case 'a': {
			char *cp;
			struct oknet n;
	...
	if (argerr) {
		...
		fprintf(stderr,
"usage: rpcbind [-vmAbw] [-a mask,match | -a match]\n");
		exit(1);
	}
}
