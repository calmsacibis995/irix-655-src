#ident "$Id: fsr_xfs.c,v 1.4 1999/05/28 22:22:59 cwf Exp $"
	...
	xfs_bstat_t	*statp)
{
	...
	/* Currently do not support extended attribute files */
	if ((fcntl(fd, F_FSGETXATTRA, &fsx)) < 0) {
		fsrprintf("failed to get ex-attrs: %s\n", fname);
		return(-1);
	}
	if (fsx.fsx_nextents > 0) {
		if (vflag)
			fsrprintf("File has ext attrs: %s: not supported\n",
			           fname);
		return(-1);
	}
