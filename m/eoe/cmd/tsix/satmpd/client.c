...
purge_token_cache (struct tserver *ts, u_int generation)
{
	...
	atomic_store ((atomic_var_t *) &ts->generation, generation);
}
...
receive_init_request (init_request *req, init_reply *rep)
{
	...
	if (ts != NULL)
	{
		...
		if (req->generation < atomic_load ((atomic_var_t *) &ts->generation))
		{
			decref (ts);
...
receive_init_reply (init_reply *rep)
{
	...
	{
		/* cache exists: check its generation number */
		if (rep->generation < atomic_load ((atomic_var_t *) &ts->generation))
		{
			decref (ts);
			return (SATMP_INVALID_GENERATION);
		}
...
get_client_generation (u_int host, u_int *generation)
{
	...
	if (generation != NULL)
		*generation = atomic_load ((atomic_var_t *) &ts->generation);
	decref (ts);
