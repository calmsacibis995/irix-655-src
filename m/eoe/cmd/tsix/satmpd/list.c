...
print_tserver (void *ptr, void *args)
{
	...
	debug_print ("print_tserver (%u, %u)\n", ts->hostid, ts->generation);
		    ...
		    u_int *seqno, u_short attrid)
{
	...
	av->token = (u_int) atomic_fetch_and_increment ((atomic_var_t *) seqno);
	av->attrid = attrid;
