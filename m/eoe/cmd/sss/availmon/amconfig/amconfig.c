...
int  SetConfigIntVal (const char *szName, int Value )
{
    ...
    {
      int  tickduration, daysupdate ;
      
      if ( Value ) 
      { /* Get Start Parameters */
        Res = GetConfigIntVal ( szAmcTickerInt, &tickduration ); 
        if ( Res < 0 ) 
             return -1;
        
        Res   = GetConfigIntVal ( szAmcStatusInt, &daysupdate );
        if ( Res < 0 )
             return -1;
      }
      ...
      /* Shutdown amtickerd */
      snprintf( systemcmd, sizeof(systemcmd), 
                "/usr/etc/eventmond -a off 2>/dev/null");
      system  ( systemcmd );
      ...
      if ( Value ) 
      {
        ...
        {
          snprintf ( systemcmd, sizeof(systemcmd), 
	            "/usr/etc/eventmond -a on -j %d -e %d 2>/dev/null",
 	            (tickduration > 0 ? tickduration : 300), daysupdate * 24);
          system(systemcmd);
        }   
      }
    } 
    ...
    { /* Status Interval have been changed */
      int  tickerd, tickduration;
       
      Res = GetConfigIntVal ( szAmcTickerd, &tickerd );      
      if ( Res < 0 ) 
           return -1;
      ...
      if ( tickerd ) 
      {
        if ( Value > 0 ) 
        {
           snprintf( systemcmd, sizeof(systemcmd), 
                     "/usr/etc/eventmond -a on -j %d -e %d  2>/dev/null",
		     (tickduration > 0 ? tickduration:300), Value * 24 );
           system(systemcmd);
        }
      } else {
        fprintf(stderr, 
        "Warning: availmon status report will be sent only when"
