...
{  ReportSession_t *session;
   ...
   { memset(session,0,sizeof(ReportSession_t));
     session->plugin = p;
     pthread_mutex_lock(&rgSessionLock);
     session->link = busySessionHandles;
     busySessionHandles = session;
     pthread_mutex_unlock(&rgSessionLock);
   }
...
/* -------------------------- srgGenerateReport ------------------------------ */
void srgGenerateReport(sscErrorHandle hError, srgSessionID hSession, int argc, char* argv[],char *rawCommandString, streamHandle result)
{  int retcode;
   ReportSession_t *r = hSession;
   if(checkSessionID(r))
   { lockPlugin(r->plugin);
     ...
     unlockPlugin(r->plugin);
     if(retcode != RGPERR_SUCCESS)
     { sscError(hError,"Invalid RG plugin return code,  must be RGPERR_SUCCESS");
       sscError(hError,"RG plugin is not up-to-date!!!");
     }
   }
