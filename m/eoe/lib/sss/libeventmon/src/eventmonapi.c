...
/* ------------------------- declare_something ------------------------------- */
static int declare_something(char cmd)
{ struct sockaddr_un su_addr;
  char buf[16];
  int s,l;
  if((s = socket(AF_UNIX, SOCK_DGRAM, 0)) < 0) return 0;
  prepare_cmdbuffer(buf,cmd); /* prepare command buffer */
  l = sendto(s,buf,4,0,(struct sockaddr*)&su_addr,make_sockaddr(&su_addr,szEventMonInfoSockPath)); close(s);
  return (l == 4);
}
/* --------------------------- evmapi_sleep ---------------------------------- */
...
{ return declare_something(EVMCMD_TEST);
}
/*---------------------- emapiDeclareDaemonUnload ---------------------------- */
int EVMONAPI emapiDeclareDaemonUnload()
{ return (!evmapi_is_priv() || !declare_something(EVMCMD_TEST)) ? 0 : declare_something(EVMCMD_STOP);
}
/* ------------------ emapiDeclareDaemonReloadConfig ------------------------- */
int EVMONAPI emapiDeclareDaemonReloadConfig()
{ return (!evmapi_is_priv() || !declare_something(EVMCMD_TEST)) ? 0 : declare_something(EVMCMD_RFILT);
}
/* ------------------------ emapiSendEvent------------------------------------ */
...
{ char buf[EVMONAPI_MAXEVENTSIZE+128],rcvbuf[16],tmpSockName[256],hostNameEmpty[2];
  ...
  if(!eventbuffer || !eventbuffer[0] || !declare_something(EVMCMD_TEST)) return 0;
...
{ printf("emapiIsDaemonInstalled() = %d\n",emapiIsDaemonInstalled());
  ...
  printf("emapiSendEvent(...) = %d\n",emapiSendEvent(NULL,time(NULL),254,"Hello \"Peace of mind\""));
}
----------------------------------------------------------------------------------- */
