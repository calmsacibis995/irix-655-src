...
struct dplace_info {
  ...
  struct
  { int number; node_mapping_t *maps; } node_maps;
  struct { int count; link_t *links; } thread_link;
...
do_limits(void){
  ...
  /*  if (d->verbose) fprintf(stderr,"stack: 0x%010llx to 0x%010llx\n",
		       d->stack_min,d->stack_max);*/
  ...
  /*if (d->verbose) fprintf(stderr,"data:  0x%010llx to 0x%010llx\n",
		       d->data_min,d->data_max);*/
} 
...
do_args(void){
  ...
  if( 0 == strcmp("on",c) ) d->migration_enabled = 1;
  else if ( 0 == strcmp("off",c) ) d->migration_enabled = 0;
  else {
    int n;
    if( 1 != sscanf(c,"%d",&n)){
      fprintf(stderr,"dplace migration parameter error.\n");
  ...
  if( NULL == c ){
    ...
    if( d->inherit == 0 && dplace_pid != getppid() ) d->disable = 1;
    if( d->inherit ){
  ...
  if( d->disable && NULL != (c=getenv("_RLD_LIST"))){
    char *new = (char*) strchr(c,':');
    if( NULL != new ){
      sprintf(rld_list,"_RLD_LIST=%s",new+1);
      putenv(rld_list);
    }
  }
  if ( getenv("__DPLACE_MANDATORY_") ){
    if( d->verbose ) fprintf(stderr,"Mandatory placement.\n");
		       d->rq_mode = RQMODE_MANDATORY;
  }
  if ( getenv("__DPLACE_MUSTRUN_") ){
    if( d->verbose ) fprintf(stderr,"Mustrunning threads.\n");
		       d->mustrun = 1;
  }
}
...
attach_pid_to_mld(int pid,pmo_handle_t mld,int thread){
  if( !d->mustrun )
    ...
    { 
      ...
      struct stat sinfo;
      sprintf(path,"%s/cpu/%c", dev_to_devname(node , devname, &length),
	      "ab"[cpu]);
      if( stat(path, &sinfo) ) cpu ^= 1;
      if( d->verbose ){
	length = sizeof(devname);
	fprintf(stderr,"Attaching thread %d to %s/cpu/%c\n",
		thread,dev_to_devname(node , devname, &length),
		"ab"[cpu]);
      }
    }
...
__dplace_sproc_child (void)
{
  ...
  if( d->verbose )spin_lock(&d->is_running);
  if(d->verbose)fprintf(stderr,"there are now %d threads\n",i);  
  if( d->disable ) return;
  if( i >= d->thread_max ){
    d->thread_max *= 2;
    d->thread_pid = (int*)__dplace_realloc_check(d->thread_pid, 
						 d->thread_max*sizeof(int));
    bzero((void*)&d->thread_pid[d->thread_max/2],(d->thread_max/2)*sizeof(int));
  }
  ...
  if( i <= d->nthreads && d->thread_link.links[i-1].valid ){
	...
	fprintf(stderr,
      ...
      attach_pid_to_mld_and_cpu(pid,
				d->mldlist[d->thread_link.links[i-1].node 
					  + d->mld_offset],
				d->thread_link.links[i-1].cpu );
    } else {
      if(d->verbose)
	fprintf(stderr,
		"Attaching process [%d] to memory [%d]\n", pid,
 ...
 if( d->verbose )release_lock(&d->is_running); 
}
...
do_memory(void){
  ...
  if ( __dplace_arena == (void * ) 0 ){
    ...
    if ( NULL == mapped ){
      perror("mmap of MAP_FILE failed");
      die();
    }
    __dplace_arena = (void *) mapped;
    ac = acreate(__dplace_arena,npages*getpagesize(),MEM_NOAUTOGROW,0,0);
    if ( NULL == ac ){
      perror("Arena creation failed.\n");
      die();
    }
    d = (struct dplace_info *)__dplace_malloc_check(sizeof( struct dplace_info ));
    d->memory_fd = fd;
  } else
    {
      fd = d->memory_fd;
...
set_dpagesize(int n){
  if( 0 == d->dpagesize_set ){
    ...
    fprintf(stderr,"placement error on line %d:",d->line_count+1);
    fprintf(stderr,"Global data_pagesize has already been set to %lldk.\n",
	    d->dpagesize>>10);
    fprintf(stderr,"Aborting.\n");
    die();
  }
...
set_spagesize(int n){
  if( 0 == d->spagesize_set ){
    ...
    fprintf(stderr,"placement error on line %d:",d->line_count+1);
    fprintf(stderr,"Global stack_pagesize has already been set to %lldk.\n",
	    d->spagesize>>10);
    fprintf(stderr,"Aborting.\n");
    die();
  }
...
set_tpagesize(int n){
  if( 0 == d->tpagesize_set ){
    ...
    fprintf(stderr,"placement error on line %d:",d->line_count+1);
    fprintf(stderr,"Global text_pagesize has already been set to %lldk.\n",
	    d->tpagesize>>10);
    fprintf(stderr,"Aborting.\n");
    die();
  }
...
set_affinity(struct list f_list){
  ...
  if( 0 == d->affinity_set ) {
    ...
    while(p){
      if(d->verbose) fprintf(stderr,"raff[%d] = %s\n", d->affinity_set, p->name);
...
set_nodes(int n){
  ...
  if (d->verbose) fprintf(stderr,"Setting memories = %d\n",n);
}
...
set_threads(int n){
  ...
  if( d->nthreads != 0 ){
    ...
    d->thread_link.links = (link_t *)__dplace_calloc_check(n,sizeof(link_t));
  ...
  if( n >= d->thread_max ){
    d->thread_pid = (int*)__dplace_realloc_check(d->thread_pid, n*sizeof(int));
    bzero((void*)&d->thread_pid[d->thread_max],(n-d->thread_max)*sizeof(int));
    d->thread_max = n;
  }
  if (d->verbose) fprintf(stderr,"Setting threads = %d\n",n);
}
...
setup_first_thread_and_policy(void)
{
  ...
  if(d->verbose) fprintf(stderr,"Setting up policies and initial thread.\n");
  if( d->threads_set && 1 == d->thread_link.links[0].valid ){
	...
	fprintf(stderr,
      ...
      attach_pid_to_mld_and_cpu(pid,
				d->mldlist[d->thread_link.links[0].node 
					  + d->mld_offset],
				d->thread_link.links[0].cpu );
    } else {
      if(d->verbose)
  ...
  } else policy_set.placement_policy_args = (void*)1;
  if (d->verbose) {
    if(d->migration_enabled)fprintf(stderr,"Migration is on and level is %d%%.\n",
				    100 - (int)d->migration);  
    else fprintf(stderr,"Migration is off.\n");
  }
  migr_policy_args_init(&migr);
  if( d->migration_enabled ) migr.migr_base_enabled = SETON;
  else migr.migr_base_enabled = SETOFF;
  ...
  if(d->verbose)fprintf(stderr,"Data placement policy is %s.\n",
		     policy_set.placement_policy_name);		     
  if(d->verbose)fprintf(stderr,"Creating data PM.\n");
  policy_set.page_size = d->dpagesize;
  if (d->verbose) fprintf(stderr,"Data pagesize is %lldk.\n",d->dpagesize>>10);
  pm = pm_create(&policy_set);
  checkerr(pm);
  if(d->verbose)fprintf(stderr,"Setting data PM.\n");
  checkerr( pm_setdefault(pm,MEM_DATA) );
  if(d->verbose)fprintf(stderr,"Creating stack PM.\n");
  if (d->verbose) fprintf(stderr,"Stack pagesize is %lldk.\n",d->spagesize>>10);
  ...
  if(d->verbose)fprintf(stderr,"Stack placement policy is %s.\n",
		     policy_set.placement_policy_name);	
  pm = pm_create(&policy_set);
  checkerr(pm);
  if(d->verbose)fprintf(stderr,"Setting stack PM.\n");
  checkerr( pm_setdefault(pm,MEM_STACK) );

  if( pgsz != d->tpagesize || strcmp(d->text_policy,"PlacementDefault")){
    if(d->verbose)fprintf(stderr,"Creating text PM.\n");
    policy_set.page_size = d->tpagesize;
    pm = pm_create(&policy_set);
    checkerr(pm);
    if (d->verbose) fprintf(stderr,"Text pagesize is %lldk.\n",d->tpagesize>>10);
    if( d->mldset_handle ){
    policy_set.placement_policy_name = d->text_policy;
    policy_set.placement_policy_args = (void*)d->mldset_handle;
  } else policy_set.placement_policy_args = (void*)1;
  if(d->verbose)fprintf(stderr,"Text placement policy is %s.\n",
		     policy_set.placement_policy_name);	
    if(d->verbose)fprintf(stderr,"Setting text PM.\n");
...
add_range(__uint64_t min,__uint64_t max, int node, int size ){
  ...
  if( !d->initial_policies )setup_first_thread_and_policy();
  /* we can't attach the range unless the boundary of the heap (or
     stack) overlaps the address range, so we will queue stuff up and
     do it by spying on brk() and sbrk() We can do the stack (or
     anonymous) now since we already touched the first (lowest
     address) page of the stack */
  edge = (long long)_SBRK(0);
  if ( min > d->stack_min ||  max <= edge ){ 
    /* We have an address that is not in the unallocated heap
       so do it now! */
    ...
    if (d->verbose) fprintf(stderr,"Creating PM with PlacementFixed.\n");
    pm = pm_create(&policy_set);
    checkerr(pm);
    if(d->verbose) fprintf(stderr,"Attaching PM with %s.\n", 
			policy_set.placement_policy_name);
    checkerr(pm_attach(pm, (void*)min, (size_t)(max-min)));

    /* if this is data, it has already been touched so we need to migrate */
    if( max <= edge && min >=d->data_min ) migrate_range(min, (max<=edge)?max:edge, node );
...
add_link(int thread, int node ){
  ...
  if( d->thread_pid[thread] ){/* thread already exists */
    ...
    if(d->verbose)fprintf(stderr,"Attaching process [%d] to memory [%d]\n", pid,
		       node );
...
static void
distribute_threads(int distribution, int value,
		   int t0, int t1, int dt, int tmode,
		   int m0, int m1, int dm, int mmode)
{
  int nthreads,nnodes;
 ...
 if( dt > 0 ) nthreads = (t1+1 - t0)/dt;
 else nthreads = (t0+1 - t1)/(-dt);
 if( dm > 0 ) nnodes = (m1+1 -m0)/dm;
 else nnodes = (m0+1 -m1)/(-dm);
 ...
 if( distribution == block ){
   ...
   if(d->verbose) fprintf(stderr,"block distribution: %d threads per memory\n", value);
   for(i=0;i < nthreads;i++) {
	 ...
	 (dt <  0 && thread <  t1) || (dm < 0 && mem <  m1) ){
       ...
       add_link( thread , mem);
       if(d->verbose) fprintf(stderr,"thread %d will run on memory %d\n",
			      thread, mem);
     }
   }
   ...
   if(d->verbose) fprintf(stderr,"cyclic distribution by %d\n", value);
...
move_pid(int pid, int node ){
  ...
  for(i=0;i<d->nthreads;i++)if( pid == d->thread_pid[i] ) break;
  if( d->nthreads == i ){
...
show_layout(void){
  ...
  if(d->topology == cube ) fprintf(stderr," cube");
  else if( d->topology == cluster ) fprintf(stderr," cluster");
  else if( d->topology == none ) fprintf(stderr," none");
  else if( d->topology == physical ) fprintf(stderr," physical");
  if( d->affinity == Close ){
    fprintf(stderr," with d->affinity to ");
    for (i=0;i<d->affinity_set;i++)fprintf(stderr,"%s ",d->device[i]);
  } else if( d->affinity == None ) fprintf(stderr," with no d->affinity");
  fprintf(stderr,"\n");
...
place_nodes(void){
  ...
  if(d->verbose) fprintf(stderr,"Creating %d MLDS.\n",n_mld);
  ...
  for (i = 0; i < n_mld ; i++) {
    ...
    if(d->verbose) fprintf(stderr,"Created MLD [%d]\n", d->mldlist[i]);
    checkerr(d->mldlist[i]);
  }


  if(d->verbose) fprintf(stderr,"Setting up mldset\n");
        
  if(d->verbose) fprintf(stderr,"Creating the mldset\n");
  ...
  if( d->topology == physical && d->affinity_set < d->nnodes ){
    fprintf(stderr,"Error on line %d.\n",d->line_count+1);
    fprintf(stderr,"The affinity count (%d) must be at least",d->affinity_set);
    fprintf(stderr," as large as the number of memories (%d).\n",d->nnodes);
    fprintf(stderr,"Aborting.\n");
    die();
  }
	
  if(d->verbose) fprintf(stderr,"Placing the mldset\n");
...
dplace_line(char *line){
  ...
  n = strlen(line);
  if('\n' == line[n-1] ) sprintf(buffer,"%s",line);
...
__init_dplace(void){
  ...
  if (d->verbose) fprintf(stderr,"there are now %d threads\n",(int)d->tcount);
  d->line_count = 0;
  if(d->inputfile)dplace_file(d->inputfile);
  d->line_count = 0;
  if( !d->initial_policies )setup_first_thread_and_policy();
...
void *SBRK(ssize_t size)
{
  ...
  if ( __dplace_arena
       && (long long)newaddr >= (long long) _edata ) 
