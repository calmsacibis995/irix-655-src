...
dns_query_callback(nsd_file_t **rqp, int fd)
{
	...
	while (--ancount >= 0 && cp < eom) {
		...
		switch (type) {
			...
			*/
			...
			if (tmp = dns_insortlist(addr)) {
				nsd_logprintf(NSD_LOG_MAX,
				  "\t\tfound address record: %s - prepending\n",
				    inet_ntoa(addr->addr));
				if (! alist) {
					aend = &addr->next;
				}
				...
				nsd_logprintf(NSD_LOG_MAX,
				   "\t\tfound address record: %s - appending\n",
				    inet_ntoa(addr->addr));
				addr->next = 0;
				/*
				** Need to remember first address encountered
				** and not concerned by any "sortlist"
				** statements.
				*/
				if (*aend == 0) {
					nosort=addr;
	...
	*/
	cp = (uchar *)result;
	for (addr = alist; addr; addr = alist) {
		if (cp != (uchar *)result) {
			*cp++ = ' ';
		}
		alist = addr->next;
