...
static int fprom_wait(fprom_t *f, fprom_off_t offset, uchar_t byte)
{
    ...
    if (f->swizzle)
	nstatus = SN00_DATA_SWIZZLE(0x20);
    else
	nstatus = 0x20;
 ...
 *
 *   This call should be used before attempting to program the flash PROM.
 *   It should also be used following the failure of any other PROM call.
 *   If fprom_reset itself fails, the PROM is unavailable (fatal).
 *
 *   Returns FPROM_ERROR_code.
 */

int
fprom_probe_common(fprom_t *f, int *manu_code, int *dev_code)
{
...
fprom_probe(fprom_t *f, int *manu_code, int *dev_code)
{
    ...
    r1 = fprom_probe_common(f, manu_code, dev_code);
	...
	f->swizzle = 1;
	r2 = fprom_probe_common(f, manu_code, dev_code);
 ...
 * fprom_flash_sectors
 *
 *   Flashes any combination of 64K PROM sectors simultaneously.
...
int fprom_flash_sectors(fprom_t *f, int sector_mask)
{
	...
	if (sector_mask & 1 << i) {
	    ...
	    switch (f->dev) {
	    case FPROM_DEV_HUB:
		cmd(f, i * FPROM_SECTOR_SIZE, 0x30);
		break;
	    case FPROM_DEV_IO6_P0:
		if (i < 15)
		    cmd(f, i * FPROM_SECTOR_SIZE, 0x30);
	    ...
	    case FPROM_DEV_IO6_P1:
		if (i < 15)
		    cmd(f, i * (FPROM_SECTOR_SIZE / 2), 0x30);
		else {
		    cmd(f, 0xf0000 / 2, 0x30);
		    cmd(f, 0xf8000 / 2, 0x30);
		    cmd(f, 0xfa000 / 2, 0x30);
		    cmd(f, 0xfc000 / 2, 0x30);
		}
		break ;
	    }
    ...
    if ((r = fprom_wait(f, any * FPROM_SECTOR_SIZE, (uchar_t) 0xff)) < 0)
	return r;
