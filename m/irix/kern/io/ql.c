...
ql_set_defaults(pHA_INFO ha)
{
	...
	if (dev_admin = device_admin_info_get(ha->ctlr_vhdl, "ql_request_queue_depth")) 
	{
		chip_info->ql_request_queue_depth = atoi(dev_admin);
		if ((chip_info->ql_request_queue_depth * sizeof(queue_entry)) < NBPP) 
		{
			cmn_err(CE_WARN,"tried to set ql_request_queue_depth too low\n");
			chip_info->ql_request_queue_depth = NBPP / sizeof(queue_entry);
			cmn_err(CE_WARN,"set to %d instead\n",ha->chip_info->ql_request_queue_depth);
		}
	...
	else 
	{
        ...
        if (dev_admin = device_admin_info_get(ha->ctlr_vhdl, "ql_response_queue_depth")) 
	{
                chip_info->ql_response_queue_depth = atoi(dev_admin);
                if((chip_info->ql_response_queue_depth * sizeof(queue_entry)) < NBPP) 
		{
                   cmn_err(CE_WARN,"tried to set ql_response_queue_depth too low\n");
                   chip_info->ql_response_queue_depth = NBPP / sizeof(queue_entry);
                   cmn_err(CE_WARN,"set to %d instead\n",chip_info->ql_response_queue_depth);
                }
