 ...
 */
#ident "$Revision: 1.46 $ $Author: liang $"
...
typedef struct {
	...
	unsigned int	kp_curtimer;	/* currently active timer */
} ktimerpkg_t;
...
extern void ktimer_init(struct kthread *, unsigned int);
extern void ktimer_switch(struct kthread *, unsigned int);
extern uint64_t ktimer_readticks(const struct kthread *, unsigned int);
extern void ktimer_read(const struct kthread *, unsigned int, struct timespec *);
extern long ktimer_slptime(const struct kthread *);
#if defined(CELL_IRIX)
extern uint64_t ktimer_accum(struct kthread *, unsigned int, uint64_t);
#endif
