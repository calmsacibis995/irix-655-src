...
#define QL_MAX_PORTS		256	/* max ports in fabric */
...
/*
**	Data segment descriptor for 64 bit addressing.
*/
typedef struct {
	__psint_t	base;
	uint32_t	count;
...
#define MIN_COOKIE	(1)
#define MAX_COOKIE	(0777)

#define MAX_TARG_VHDL	(017777777)
typedef union {
	struct {
		int32_t	target_vhdl:22,
			value:10;
	} field;
	uint32_t data;
} REQ_COOKIE;
...
typedef struct {
	...
	uint8_t  portName[8];                    /* FC Node Mane for this adapter must be unique on the loop */
...
/*
**	Firmware Initialization Control Block. This is 32 bit word swapped.
**  First 32 bytes are same as nvram_risc_param structure but bytes
**  are swapped ready for DMA.
*/

typedef struct {
/* 6*/	uint16_t maxIOCBperPort;
	uint16_t	maxFrameSize;

/* 3*/	uint8_t	fwOption1;	/* bits 15-8 */
	uint8_t	fwOption0;	/* bits 7-0 */
	uint8_t	reserved0;
/* 0*/	uint8_t	version;
...
/*17*/	uint8_t  loginTimeout;
	uint8_t  inquiry_data;
	uint8_t  reserved1; 	/* part of hardID field */
	uint8_t  hardID;   
...
/*26*/	uint16_t	responseQ_depth;    /* Reseponse Queue size */
	uint16_t	requestQ_depth;     /* Request Queue size */

/*22*/	uint16_t	response_in;		/* Response Queue Pointer (Offset) */
/*20*/	uint16_t	request_out;		/* Request Queue Pointer (Offset) */

/*2C*/	uint32_t   requestQ_base_addrhigh; /* higher part of 64 bit Request Queue Physical address */
/*28*/	uint32_t   requestQ_base_addrlow;  /* lower part of 64 bit Request Queue Physical address */

/*34*/	uint32_t   responseQ_base_addrhigh; /* higher part of 64 bit Response Queue Physical address */
/*30*/	uint32_t   responseQ_base_addrlow;  /* lower part of 64 bit Response Queue Physical address */

/*3e*/	uint16_t	reserved4;
/*3c*/	uint16_t	lun_timeout;
/*3b*/	uint8_t		immediate_notify_resource_count;
/*3a*/	uint8_t		command_resource_count;
/*38*/	uint16_t	lun_enables;		

/*44*/	uint32_t	reserved5a;
/*43*/	uint8_t		interrupt_delay_timer;
/*42*/	uint8_t		response_accumulation_timer;
...
struct qlfc_local_lun_info {
	...
	uint32_t		qli_dev_flags;	/* lun device flags */
	int			qli_lun;	/* this should be obvious! */
};
...
#define TM_QUEUE_FULL		0x080	/* device reported queue full */
...
#define	DEMON_MAX_EVENTS	(QL_FABRIC_ID_HIGH+1+32)
...
enum demon_messages {
	...
	DEMON_MSG_LINK_UP,
	DEMON_MSG_SCN,
	DEMON_MSG_SYSTEM_ERROR,
	DEMON_MSG_ERROR_RECOVERY,
	DEMON_MSG_SET_TARGET_AWOL,
	DEMON_MSG_TARGET_AWOL,
	DEMON_MSG_ABORT_DEVICE
};
...
struct qlfc_controller {
	...
	*/
	...
	char		flags_ec[8];
	uint32_t	flags;			/* general flags for this adapter */
	...
	*/
	...
	char		dmamap_ec[8];
	pciio_dmamap_t	response_dmamap;	/* response dma map */
	pciio_dmamap_t	request_dmamap;		/* request dma map */
...
#define	CFLG_SCN_ACTIVE			0x02000000
...
#define ET_COMMAND              0x11
#define ET_CONTINUATION         0x02
#else
...
#define EF_BAD_ENTRY_ORDER          0x10
#define EF_ERROR_MASK   (EF_BUSY | EF_BAD_HEADER | EF_BAD_PAYLOAD | EF_BAD_ENTRY_ORDER)
...
typedef struct {
    ...
    uint32_t     base_high_32_0;
    uint32_t     base_low_32_0;

    uint32_t     base_low_32_1;
    uint32_t     count_0;

    uint32_t     count_1;
    uint32_t     base_high_32_1;
#else
...
*/
#define CF_RESERVED_0               0x0001
#define CF_HEAD_TAG                 0x0002
#define CF_ORDERED_TAG              0x0004
#define CF_SIMPLE_TAG               0x0008
#define CF_RESERVED_4               0x0010
#define CF_READ                     0x0020
#define CF_WRITE                    0x0040
#define CF_RESERVED_7       	    0x0080
#define CF_RESERVED_8       	    0x0100
#define CF_RESERVED_9       	    0x0200
#define CF_RESERVED_10       	    0x0400
#define CF_RESERVED_11       	    0x0800
#define CF_RESERVED_12       	    0x1000
#define CF_RESERVED_13       	    0x2000
#define CF_RESERVED_14       	    0x4000
#define CF_RESERVED_15       	    0x8000
...
typedef struct {
    ...
    uint32_t     base_low_32_2;
    uint32_t     count_1;

    uint32_t     count_2;
    uint32_t     base_high_32_2;

    uint32_t     base_high_32_3;
    uint32_t     base_low_32_3;

    uint32_t     base_low_32_4;
    uint32_t     count_3;

    uint32_t     count_4;
    uint32_t     base_high_32_4;
...
#define HCCR_CMD_SINGLE_STEP		0x4000	/* single step the RISC */
...
#define HCCR_CMD_WRITE_BREAK_ENABLES	0x8000	/* write to breakpoint enable bits */
...
#define DRAIN_TIME			(20)			/* forward progress timer, in seconds */
#define AWOL_TIMEOUT			(5)			/* how long between awol retries */
#define MAILBOX_COMMAND_TIMEOUT		(95)			/* Clariion raid can take a LONG time */
...
#endif
void			qlfc_intr(QLFC_CONTROLLER *);
void			qlfc_init(void);
...
static void			fill_sg_elem(QLFC_CONTROLLER *, data_seg *, alenaddr_t, size_t, int);
static void			qlfc_swap16 (uint8_t *, int);
static void			qlfc_swap32 (uint8_t *, int);
static void			qlfc_swap64(uint8_t *, int);
...
#ifdef DEBUG
#ifdef ISP1040
static void			dump_registers(QLFC_CONTROLLER *, CONTROLLER_REGS *);
static void			dump_parameters(vertex_hdl_t);
static void			dump_cmd_dma(QLFC_CONTROLLER *);
#endif
...
#else
	uint32_t	sns_response_buffer_addr_l;
	uint16_t	reserved1;
	uint16_t	sns_response_length;

	uint16_t	reserved2;
	uint16_t	sns_subcommand_length;
	uint32_t	sns_response_buffer_addr_u;

	uint32_t	reserved4;
	uint16_t	sns_response_limit;	/* in uint32_t units */
	uint16_t	sns_subcommand;
#endif
} sns_cmd_hdr_t;
...
typedef struct {
	...
	******************************************************/
	...
	#else
		sns_cmd_hdr_t	gan_cmd_hdr;
		uint32_t	gan_fill;			/* not part of spec */
		uint32_t	gan_port_id;			/* bits 23-0 */
	#endif
	...
	*****************************************************/
	...
	#else
		sns_cmd_hdr_t	gpn_cmd_hdr;
		uint32_t	gpn_fill;	/* not part of spec */
		uint32_t	gpn_port_id;	/* bits 23-0 */
	#endif
	...
	*****************************************************/
	...
	#else
		sns_cmd_hdr_t	gnn1_cmd_hdr;
		uint32_t	gnn1_fill;	/* not part of spec */
		uint32_t	gnn1_port_id;	/* bits 23-0 */
	#endif
...
#define PORT_NODE_NAME_LIST_ENTRIES	128

struct	port_node_name_list {
	struct {
		uint16_t	node_name [4];
		uint16_t	loop_id;
	} entry[PORT_NODE_NAME_LIST_ENTRIES];
};
