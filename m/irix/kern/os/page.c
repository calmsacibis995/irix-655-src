...
kmem_contig_free(void *vaddr, size_t size)
{
	...
	if (IS_KSEG2(vaddr)) {
		...
		for ( i = 0; i < npgs; i++, pd++) {
			pfd = pdetopfdat(pd);
			pfd->pf_flags &= ~(P_DUMP | P_BULKDATA);
#if R4000 || R10000
	...
	register size_t	size)
{
	...
	if ((pfd = pagealloc_size_node(private.p_nodeid, cachekey, flags, size)) != NULL) {
         ...
         * we do a radial search up to the max possible radius.
         */
	 ...
	 * try all nodes.
         */
	if (CNODEMASK_NOTEQ(get_effective_nodemask(curthreadp), boot_cnodemask)) {
        	if (physmem_select_masked_neighbor_node(private.p_nodeid,
                                         	physmem_max_radius,
					 	boot_cnodemask,
                                         	&neighbor,
        ...
        /*
         * The radial search above avoided borrowing memory from
         * nodes with high memory pressure.
         * The following loops just goes searching for any available
         * memory in the system.
         */
        
	for (node = 0; node < numnodes; node++) {
		if ((pfd = pagealloc_size_node(node, cachekey, flags, size)) != NULL) {
                        return pfd;
                }
        }
	...
	register size_t	size)
{
	...
	while ((pfd = pagealloc_size(cachekey, flags, size)) != NULL) {
		...
		/* Don't know how to validate proxy pfdats, so skip them. */
		if ((pfd->pf_flags & P_PROXY) == 0) {
			pfn = pfdattopfn(pfd);
...
page_zero(pfd_t *dpfd, int dcolor, uint pgoffset, uint cnt)
{
	...
	if (cnt1 >= BTE_LEN_MINSIZE) {	/* only use bte if big enough */
		bte_pbzero(pfdattophys(dpfd) + pgoffset, cnt1);
		cnt -= cnt1;
	}
	else
		cnt1 = 0;
