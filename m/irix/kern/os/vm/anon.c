...
/*
 * List of unreferenced, dirty, shared anonymous pages.
 */

int	psanonmem;		/* # pages on psanonlist     */
plist_t	psanonlist;		/* list of shared anon pages */
lock_t  sanon_list_lock;	/* lock for above list       */


/*
 * This macros removes the given pfdat from the sanon list.  For the
...
#define psremove(pfd)					\
	do {						\
		nested_spinlock(&sanon_list_lock);	\
							\
		if (pfd->pf_flags & P_SQUEUE)		\
			_psremove(pfd);			\
							\
		nested_spinunlock(&sanon_list_lock);	\
	} while (0)
...
init_anon(void)
{
	...
	psanonlist.pf_next = psanonlist.pf_prev = (pfd_t *)&psanonlist;
	spinlock_init(&sanon_list_lock, "sanon");
...
free_covered_page(anon_t *pap, struct pfdat *covered_pfd)
{
	...
	if (covered_pfd->pf_use == 0) {
		 ...
		 * off the psanon list and put it on the regular
 ...
 * gone sanon at one point and may now be sitting on the sanon list.  Finally,
 ...
 * for this node (which may either on the free list or the sanon list) or
...
anon_unlink(anon_t *ap, void *preempt_cookie)
{
	 ...
	 * list or the sanon list.  There might also be some pages in
		...
		*sp = scache_lookup(&ap->a_scache, lpn);
		...
		if (pfd || *sp) {
			...
			if (pfd) {
				...
				/*
				 * If the page is on the sanon list, take
 ...
 * memory.  If it's P_SQUEUE, take it off the sanon list and free it.
...
psanonfree(void *unused_arg, pfd_t *pfd)
{
	...
	if (pfd->pf_flags & P_SQUEUE) {
		...
		/*
		 * Now take it off the sanon list.  It should still be
 ...
 * This list is headed by psanonlist usiong the pf_prev/next pointers.
 ...
 * from the psanonlist and are logically allocated to vhand by incrementing
 ...
 * that are on the psanon list at all.  Yes, this is a kludge.  Maybe we'll 
...
pgetsanon(register int npgs)
{
	...
	/*
	 * Pull the first npgs pages off the front of psanonlist.  These are
	 * the oldest ones.
	 */

	while (npgs > 0 && psanonlist.pf_next != (pfd_t *)(&psanonlist)) {

		locktoken = mp_mutex_spinlock(&sanon_list_lock);
		pfd = psanonlist.pf_next;

		/*
		 * Look for a suitable page.
		 */

		while (pfd != (pfd_t *)(&psanonlist) && 
		...
		if (pfd == (pfd_t *)(&psanonlist)) {
			mp_mutex_spinunlock(&sanon_list_lock, locktoken);
			break;
		}
...
pagefreesanon(register pfd_t *pfd, int dirty)
{
	 ...
	 * goes away) whether the page needs psanonlist swapping or not.  
	 ...
	 * psanonlist if it hasn't already been swapped.  This makes sure
	...
	if (pfd->pf_use == 1 && (pfd->pf_flags & P_SWAP) == 0) {
		...
		nested_spinlock(&sanon_list_lock);
		psanonmem++;
		pfd->pf_next = (pfd_t *)&psanonlist;
		prev = pfd->pf_prev = psanonlist.pf_prev;
		psanonlist.pf_prev = prev->pf_next = pfd;
		nested_pfdat_unlock(pfd);
		nested_spinunlock(&sanon_list_lock);
		release_tree_lock(ap->a_lock, locktoken);
