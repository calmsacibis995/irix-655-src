...
{
	extern plist_t	psanonlist;	/* list of shared anon pages */
	extern int	psanonmem;
	pfd_t *pfd;
	int i;

	qprintf("sanon list @0x%x psanonmem %d\n",
			&psanonlist, psanonmem);
	for (pfd = psanonlist.pf_next, i = 0; pfd != (pfd_t *)&psanonlist;
					pfd = pfd->pf_next, i++) {
...
idbg_print_pgdata(cnodeid_t node, int colors)
{
	...
	qprintf(" freemem 0x%x future_freemem 0x%x empty_mem 0x%x total_mem 0x%x\n",
		pgdata->node_freemem,
	...
	for(i=0; i<NUM_PAGE_SIZES; ++i) {
		qprintf("  %8d 0x%16x 0x%16x 0x%03x %5d 0x%16x\n",
			...
			pgdata->pg_freelst->phead[i],
			pgdata->pg_freelst->pheadend[i],
			pgdata->pheadmask[i],
			pgdata->pheadshift[i],
			pgdata->pheadrotor[i]);
	}
