...
PRIFACE_prgetptimer(proc_t *p, PRIFACE_timespec_t *ptime)
{
	...
	/* Should not happen, since PIOCGETPTIMER does not operate
	 * on zombie procs.
	 */
	if (!ut)
		return;

	for (i = 0; i < MAX_PROCTIMER; i++, ptime++) {
		ktimer_read(UT_TO_KT(ut), i, &t);
		ptime->tv_sec = t.tv_sec;
		ptime->tv_nsec = t.tv_nsec;
	}
}
