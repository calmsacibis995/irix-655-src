...
fifo_inactive(bhv_desc_t *bdp, struct cred *crp)
{
	...
	/*
	 * If we have a realvp then sync the vnode, remove the fifo node and
	 * release the real vnode reference count. In the other case we only
	 * have to remove the fifo node from fifo node list.
	 */
	if (fnp->fn_realvp) {
		(void) fifo_fsync(bdp, FSYNC_WAIT, crp, (off_t)0, (off_t)-1);
		fiforemove(fnp);
