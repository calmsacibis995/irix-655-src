...
struct chtab xchtable[MAXCLIENTS];

mutex_t	chtablesem;		/* client handle table lock */
sv_t	chtablewait;		/* client handle table wait event */
mutex_t xchtablesem;		/* xattr client handle table lock */
sv_t	xchtablewait;		/* xattr client handle table wait event */
	    ...
	    mutex_t *sem, sv_t *sv)
{
	...
	for (;;) {
		...
		if (mi->mi_vers == NFS3_VERSION) 
			...
			CLSTAT.nclgets++;
		first_addr = table[0].ch_client ? 
				clnt_getaddr(table[0].ch_client) : INADDR_ANY;
		for (ch = table; ch < &table[MAXCLIENTS]; ch++) { 
			if (ch->ch_inuse) {
				if (allsame && ch->ch_client != NULL && 
				    clnt_getaddr(ch->ch_client) != first_addr) {
						allsame = FALSE;
				}
				if (!matchvers && ch->ch_prog == mi->mi_prog &&
				    ch->ch_vers == mi->mi_vers)
					matchvers = TRUE;
			} else {
				if (allsame && ch == &table[MAXCLIENTS-1] && 
				    mi->mi_addr.sin_addr.s_addr == first_addr) {
					/*
					 * Don't let RPCs to the same 
					 * server tie up table.
					 */
					lastunused = NULL;
					break;
		 ...
		 */
		if (!matchvers && lastunused) {
			lastunused->ch_inuse = TRUE;
			mutex_unlock(sem);
			ASSERT(lastunused->ch_client != NULL);
			CLNT_DESTROY(lastunused->ch_client);
		...
		sv_wait(sv, PRIBIO, sem, 0);
	}
}

/*
 * Called after mi is maked stale to wake up all users of the mount point.
 */
void
cl_abort_all(struct mntinfo *mi, struct chtab *table, mutex_t *sem)
{
	struct chtab *ch;
...
{
	cl_abort_all(mi, chtable, &chtablesem);
	cl_abort_all(mi, xchtable, &xchtablesem);
}
...
{
	return(clgetcommon(mi, cred, xchtable, &xchtablesem, &xchtablewait));
}

CLIENT *
clget(struct mntinfo *mi, struct cred *cred)
{
	return(clgetcommon(mi, cred, chtable, &chtablesem, &chtablewait));
}

static void
clfreecommon(CLIENT *cl, struct chtab *table, mutex_t *sem, sv_t *sv)
{
	struct chtab *ch;
...
xclfree(CLIENT *cl)
{
	clfreecommon(cl, xchtable, &xchtablesem, &xchtablewait);
}

void
clfree(CLIENT *cl)
{
	clfreecommon(cl, chtable, &chtablesem, &chtablewait);
}
	...
	do {
		...
		case RPC_SUCCESS:
			...
			if (mi->mi_flags & MIF_NO_SERVER)
				atomicClearInt(&mi->mi_flags, MIF_NO_SERVER);
			break;
		...
		default:
			...
			if (!(mi->mi_flags & MIF_NO_SERVER))
				atomicSetInt(&mi->mi_flags, MIF_NO_SERVER);
