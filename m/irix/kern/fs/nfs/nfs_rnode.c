...
retry:
	for (rp = *rh; ; rp = rp->r_next) {
		...
		/*
		 * If rp has a null vnode pointer, there must be another
		 * process executing in makenfsnode, below.  This competing
		 * process has inserted rp onto rh and released the rnode
		 * monitor.  We must sleep until the other process finishes
		 * allocating a vnode for rp.
		 */
		vp = rtov(rp);
		if (vp == 0) {
		...
		if (vp->v_vfsp == vfsp && !bcmp(rtofh(rp), fh, sizeof *fh)) {
	...
	/*
	 * Leave the monitor before trying to allocate or get rp's vnode,
