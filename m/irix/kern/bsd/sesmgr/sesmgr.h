 ...
 */
int sesmgr_ip_options(struct mbuf *, struct ifnet *, struct ipsec **);
int sesmgr_add_options(struct ifnet *, struct mbuf *, struct ipsec *,
		       struct sockaddr_in *, int *);
int sesmgr_ip_check(struct socket *, struct ipsec *);

#define _SESMGR_IP_OPTIONS(a,b,c)  ((sesmgr_enabled)? \
					sesmgr_ip_options(a,b,c): 0)
#define _SESMGR_ADD_OPTIONS(a,b,c,d,e)   ((sesmgr_enabled)? \
					sesmgr_add_options(a,b,c,d,e): 0)
#define _SESMGR_IP_CHECK(a,b)	    ((sesmgr_enabled)? \
					sesmgr_ip_check(a,b): 0)
