...
static int	_s;
static uint	_reg;
#define	ENET_READ(reg)	(_s = spl7(), hio->piocfg, _reg = (reg), splx(_s), _reg)
	...
	int flags)
{
	...
	ei->ei_sick = 0;
		...
		struct mbuf *m;
                ...
                rd->r_rbcnt = MAX_RPKT;
		if (!dma_copy) {
                	rd->r_rbufptr = IS_KSEG2(newrb->ebh.ebh_pad) ?
				K2_TO_PHYS(newrb->ebh.ebh_pad) :
				K0_TO_PHYS(newrb->ebh.ebh_pad);
		}
                rd->r_mbuf = m;
...
ec_reset(struct etherif *eif)
{
	...
	if (ei->ei_rnum) {
		...
		for (i = 0; i < ei->ei_rnum; i++) {
			...
			if (!dma_copy) {
                		rd->r_rbufptr = IS_KSEG2(rb->ebh.ebh_pad) ?
					K2_TO_PHYS(rb->ebh.ebh_pad) :
					K0_TO_PHYS(rb->ebh.ebh_pad);
			}
			rd->r_eor = 0;
...
next_pkt:
	...
	rd = (rd_desc *)PHYS_TO_K1(rd_tail->r_nrdesc);
		...
		if (!dma_copy) {
                	_rd.r_rbufptr = IS_KSEG2(newrb->ebh.ebh_pad) ?
				K2_TO_PHYS(newrb->ebh.ebh_pad) :
				K0_TO_PHYS(newrb->ebh.ebh_pad);
		}
		_rd.r_mbuf = m;
	...
	if (!ei->ei_sgi_edlc) {
		...
		collisions = (hio->seeq_coll_xmit[0] & 0xff) +
			((hio->seeq_coll_xmit[1] & 0x07) << 8);
		ei->ei_if.if_collisions += collisions;
		...
		if (n == ei->ei_tmbuf_tail) {
			...
			switch (ei->ei_c_detect) {
			  ...
			  case 2:
				...
				if ( no_carrier )
					no_carrier = no_carrier && !collisions;
				break;
			  default:
			...
			if (no_carrier) {
				...
				if (ei->ei_carrier_was_up) {
					...
					ei->ei_oerrors +=
					    ei->ei_opackets - ei->ei_carr_opkts;
					ei->ei_carr_opkts = ei->ei_opackets;
				}
...
#ifdef DEBUG	/* dwong */
	_ecintr_is_last[ei->ei_unit] = 1;
	_ecintr_xd_cur0[ei->ei_unit] = _ecintr_xd_cur1[ei->ei_unit];
	_ecintr_xd_prev0[ei->ei_unit] = _ecintr_xd_prev1[ei->ei_unit];
	_ecintr_xd_tact0[ei->ei_unit] = _ecintr_xd_tact1[ei->ei_unit];
#endif	/* DEBUG */
	...
	struct mbuf *m)			/* send this chain */
{
	...
	int mlen, mcnt, xstat;
	struct ether_header *eh;
	xd_desc *xd_chain, *xd_build, *xd_build2, *xd_cur;
	xd_desc *xd_prev;
	caddr_t cp;
	register struct ec_info *ei;
	EHIO hio;
...
#ifdef DEBUG	/* dwong */
	_xmit_xd_cur1[ei->ei_unit] = (__psunsigned_t)xd_cur;
	_xmit_xd_prev1[ei->ei_unit] = (__psunsigned_t)xd_prev;
	_xmit_xd_tact1[ei->ei_unit] = (__psunsigned_t)xd_chain;
#endif	/* DEBUG */
	while (xd_chain != xd_cur && xd_chain != xd_prev) {
...
append_packet:
	...
	for (n = m; n; n = n->m_next) {
		...
		if (!non_dma_mem) {
			...
			q = p + n->m_len - 1;
			p = IS_KSEG2(p) ?
				(caddr_t)K2_TO_PHYS(p) : (caddr_t)K0_TO_PHYS(p);
			q = IS_KSEG2(q) ?
				(caddr_t)K2_TO_PHYS(q) : (caddr_t)K0_TO_PHYS(q);
	...
	*(struct etheraddr *)eh->ether_shost = *esrc;
	...
	for (;;) {
		...
		_xd.x_xbufptr = IS_KSEG2(cp) ? K2_TO_PHYS(cp) : K0_TO_PHYS(cp);
		...
		if (hpc_pnum(cp) != hpc_pnum(cp2)) { /* does buf span pages? */
			...
			_xd2.x_xbufptr = IS_KSEG2(cp2) ?
				K2_TO_PHYS(cp2) & ~HPC_POFFMASK :
				K0_TO_PHYS(cp2) & ~HPC_POFFMASK;
	...
	/*
	 * starting DMA for the chain involves a race condition.  If
	 * dma is proceeding on the last buffer of the existing chain,
	 * setting x_eox to zero will have no effect.  In this case,
	 * we let the interrupt handler resume dma with the new addition.
	 */
	xstat = hio->trstat;
	if (!(xstat & HPC_STTRDMA)) {
		...
		/*
		 * previously stopped in the middle of a packet or this is
		 * the first time we start transmit DMA
		 */
		if (xd_cur->x_inuse) {
			while (!xd_cur->x_eoxp)
				xd_cur = (xd_desc *)PHYS_TO_K1(xd_cur->x_nxdesc);
