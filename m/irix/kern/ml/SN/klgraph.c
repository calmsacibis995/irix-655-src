			 ...
			 cnodeid_t cnode, nasid_t nasid)
{
	...
	for (port = 1; port <= MAX_ROUTER_PORTS; port++) {
		...
		if (NASID_TO_COMPACT_NODEID(router->rou_port[port].port_nasid) 
		    == INVALID_CNODEID) {
			cmn_err(CE_WARN, "nasid %d not present",
		...
		if (rc != GRAPH_SUCCESS) {
			...
			if (KL_CONFIG_DUPLICATE_BOARD(dest_brd)) {
				cmn_err(CE_NOTE, "brd %x is duplicate skip", dest_brd);
				continue;
			}
#endif /*  (CELL_IRIX) && (LOGICAL_CELLS) */
...
#if defined (CELL_IRIX) && defined (LOGICAL_CELLS)
		if (NASID_TO_COMPACT_NODEID(hub->hub_port.port_nasid) == INVALID_CNODEID){
			cmn_err(CE_WARN, "nasid %d not present",
